...
```a``` (*append*) 명령은 이후 입력된 내용 (```a``` 명령과 마침표 문자 사이의 모든 항목) 부터 마침표 문자 이전까지 입력한 라인의 텍스트를 파일(버퍼)에 추가한 후, ```현재텍스트``` (점 또는 *dot* 이라고도 함)로 설정합니다. 
...
```a``` 명령은 현재텍스트 (점, dot) 다음에 텍스트를 추가합니다. ```i``` (*insert*)명령은 ```a``` 명령과 비슷하지만 현재텍스트 앞에 텍스트를 추가합니다.
...
또한, 현재텍스트의 텍스트를 변경(대체) 하는 ```c``` (*change*) 명령과 삭제하는 ```d``` (*delete**) 명령도 있습니다. 아래에서 별도로 설명합니다.
...
the method in the madness that is the Emacs command structure.
```
아래에서 설명할 ```w``` (*write**) 명령을 제외하고, ```p``` 명령을 포함한 대부분의 명령들은 자신이 처리한 텍스트에 ```점 (현재텍스트) 을 설정``` 합니다. 따라서, ```a``` 명령과 ```i``` 명령은 현재텍스트를 새롭게 입력한 텍스트로 설정하고 ```p``` 명령은 출력한 텍스트를 현재텍스트로 설정하는 방식입니다. 마찬가지로 ```모든 명령 (w 제외) 은 기본적으로 현재텍스트에만 영향``` 을 미칩니다.

> [!NOTE]
[ed 와 다르게 일부 명령 (예: ```g```) 은 기본적으로 전체 파일 범위로 작동됩니다.]

현재텍스트를 사용자가 임의로 설정할 수 있기전까진 상황이 그다지 흥미롭지 않을 것입니다. 현재텍스트는 파일의 일부 라인을 명시적인 라인 주소로 지정하면 설정됩니다. 예를 들어, (주소) 1 은 파일의 첫 번째 라인을 현재텍스트로 설정합니다.
...
.
```
두 번째로 사용한 ```c``` 명령은 현재텍스트를 별도로 설정할 필요가 없습니다. 왜냐하면, 이전 ```p``` 명령으로 인해 현재텍스트를 1번 라인 (주소) 으로 설정했기 때문입니다. 

첫 번째 라인을 ```d``` (*delete*) 명령으로 완전히 삭제하는 것 또한 쉽습니다. 이후 ```1p``` 명령으로 인해 현재텍스트는 다시 1번 라인으로 재설정됩니다. 
...
```/text/``` 텍스트 검색 명령은 ```현재텍스트 다음부터 패턴의 텍스트가 처음으로 나타나는 위치를 검색한 후, 현재텍스트로 설정``` 합니다. 

> [!NOTE]
[ed 는 패턴 텍스트가 포함된 첫 번째 라인 주소와 일치합니다.] 

만약, 현재텍스트 다음부터 패턴 텍스트를 찾을 수 없는 경우 검색 작업은 파일 시작 부분부터 다시 시작해 현재텍스트 위치까지 계속 순환 검색됩니다.

```
/Emacs/p
Emacs
```
타이포그래피론 제대로 표현하기 어렵지만, 이 예제에선 ```"Emacs" 뒤의 개행 문자나 공백 문자는 현재텍스트에 포함되지 않습니다!!``` 따라서, ```p``` 명령으로 출력한 텍스트는 정확한 "Emacs" 문자열까지만 입니다. 
...
이것은 13번 라인 시작부터 15번 라인 끝까지를 선택한 후 현재텍스트로 설정합니다. 쉼표 문자에 대한 정의는 ```쉼표 문자 왼쪽 주소를 시작 (13번 라인 시작) 으로 오른쪽 주소의 끝 (15번 라인 끝) 까지를 선택한다``` 는 것입니다.

다음과 같은 몇 가지 특별한 주소 문자는 유용합니다. 

```.``` (마침표) 주소 문자는 현재텍스트인 점 (*dot*) 을 나타내고, ```0``` (0번 라인) 주소 문자는 파일의 시작 부분 앞의 ```null``` 문자를 선택하고, ```$``` 주소 문자는 (파일의 마지막 라인이 아님) 파일의 끝부분 다음의 
...
지금까지 사용한 주소는 모두 **절대 주소** 입니다. 즉, 파일내의 특정 위치를 직접 나타냅니다. sam은 현재텍스트 위치에 따라 달라지는 **상대 주소** 를 가지고 있으며 이미 한 가지 형식을 사용했습니다. 

"Emacs" 를 검색할 때, 현재텍스트의 위치에 따라 검색 결과가 달라집니다. 만약, 현재텍스트 이전의 첫 번째 검색을 원한다면 과연 어떻게 처리해야 할까요? 

검색 패턴 앞에 상대 주소인 빼기 (```-```) 문자를 추가하면 현재텍스트를 기준으로 역방향으로 검색합니다.
...
이 검색 명령은 파일의 처음부터 "Emacs" 패턴 문자열을 검색한 후, 첫 번째 검색 일치 항목을 현재텍스트로 선택합니다. 이것은 ```0 번 라인으로 이동한 다음, 순방향(+)으로 "Emacs" 패턴을 검색합니다``` 로 읽으면 됩니다. 
