# Introduction to Sed
sed 사용법이나 파일을 자동으로 수정하는 특수한 편집기 또는 파일을 변경하는 프로그램을 사용하려면 `sed` 를 사용해야 합니다.

유닉스 도구 상자(toolbox) 중에는 핵심적인 프로그램이 몇 가지 있습니다. 이 프로그램들은 간단한 작업에도 사용하기 쉬우면서 복잡한 작업까지 수행할 수 있는 풍부한 명령어 세트를 갖추고 있습니다. 프로그램의 복잡한 잠재력 때문에 간단한 측면을 제대로 활용하지 못하는 일이 없기를 바랍니다. 저는 간단한 개념부터 시작해서 나중에는 고급 주제까지 소개하겠습니다.

이 글을 처음 작성했을 때(1994년), sed 버전 대부분은 스크립트 내에 주석을 사용할 수 없었습니다. 참고로, ‘#’ 문자로 시작하는 줄은 주석입니다. 최신 버전의 sed는 줄 끝 주석도 지원합니다.

이를 이해할 수 있는 한 가지 방법은, 오래된 “클래식” 버전이 GNU, FreeBSD 및 Solaris 의 “sed” 기반이란 점입니다. 제가 어떤 환경에서 작업했는지 이해하는 데 도움이 되도록 [Sun/Oracle 의 sed(1) 매뉴얼 페이지](https://docs.oracle.com/cd/E26502_01/html/E29030/sed-1b.html#scrolltoc)를 잠시 소개합니다.


# The Awful Truth about sed
Sed는 궁극의 스트림 편집기(**s**tream **ed**itor)입니다. 이상하게 들린다면 파이프를 통해 흐르는 개울을 상상해 보시기 바랍니다. 음, 파이프 안에 있으면 개울을 볼 수 없습니다. 무언가 “흐른다” 는 비유를 시도한 대가입니다. 문학 작품을 원하시면 제임스 조이스를 읽으시기 바랍니다.

어쨌든 sed는 놀라운 유틸리티입니다. 안타깝게도 대부분의 사람들은 “sed의 진정한 힘” 을 제대로 배우지 못합니다. sed 자체는 매우 단순하지만 문서화는 엉망입니다. Solaris의 sed 온라인 매뉴얼 페이지는 전체 다섯 페이지인데 그 중 두 페이지는 사용 중에 발생 가능한 34가지 오류에 대해 설명하고 있습니다. 프로그램 설명과 동일한 분량을 오류 설명에 할애하는 이런 프로그램의 학습 곡선은 매우 가파릅니다.

하지만, 걱정하지 않아도 됩니다! sed를 이해하지 못하는 건 여러분 탓이 아닙니다. 제가 sed를 완전하게 설명해 드리겠습니다. 다만, 제가 배웠던 순서대로 기능을 설명해야 합니다. 저도 한 번에 모든 걸 배운 건 아닙니다. 여러분도 그럴 필요는 없습니다.


# The essential command: s for substitution
Sed에는 여러 가지 명령어가 있지만 대부분의 사용자는 대체 명령어 `s` 만 배웁니다. 대체 명령어는 정규 표현식과 일치하는 부분을 새로운 값(문자열)으로 바꿉니다. 간단한 예로 “old” 파일의 “day” 를 ‘new’ 파일의 “night” 로 바꾸는 작업이 있습니다:

```
sed s/day/night/ <old >new
```

또 다른 방법으로 (유닉스 초보자를 위한),
...
예제에서 인자에 따옴표(인용)를 붙이지 않은 이유는 이 예제는 필요가 없기 때문입니다. 이전 [따옴표 튜토리얼](https://grymoire.com/Unix/Quote.html)을 읽었다면 왜 따옴표가 필요 없는지 이해할 수 있습니다. 하지만, 저는 “따옴표를 사용할 것을 강력히 권장” 합니다. 만약 명령어에 특수 문자가 포함된 경우 따옴표는 "필수" 입니다. 무언가 확실치 않을 때의 좋은 습관으로나 앞으로의 “모범 사례” 를 강조하기 위해 예제에 따옴표를 사용할 것입니다. 

이전 예제에 강한 따옴표(단일 따옴표)를 사용하면 다음과 같습니다:

```
sed 's/day/night/' <old >new
```

sed 편집기는 "정확히 지시한대로 변경한다!" 는 점을 강조합니다. 따라서, 다음과 같이 실행했다면
...
또 다른 중요한 개념은 "sed는 줄 단위로 작동한다!!" 는 점입니다. 다음과 같은 입력 파일을 가정합니다:
...
각 줄 내의 “one” 을 한 번씩 “ONE” 으로 변경했음을 유의합니다. 첫 번째 줄에는 “one” 이 두 번 등장했지만 첫 번째 위치만 변경됩니다. 이것이 바로 "기본 동작" 방식입니다. 이와 다른 동작을 원한다면 사용 가능한 옵션 중 일부를 사용해야 합니다. 이는 나중에 다시 설명하겠습니다.
...
검색 패턴(Search Pattern)은 왼쪽에 위치하고 대체 문자열(Replacement string)은 오른쪽에 위치합니다.

[인용](https://grymoire.com/Unix/Quote.html)과 [정규 표현식](https://grymoire.com/Unix/Regular.html)에 대해선 이미 다뤘습니다. 그것만으로 sed 대체 명령어 학습에 필요한 90%를 차지합니다. 다시 말해, 가장 빈번하게 사용되는 “sed 기능의 90%를 이미 다룰 줄 안다” 는 뜻입니다. 앞으로 sed 전문가가 되려면 알아야 할 몇 가지 세부 사항이 좀 더 있습니다. (이제 겨우 1장을 마쳤습니다. 앞으로 63개 장이 더 남아 있습니다. :-) 

아, 그리고 이 페이지를 북마크하는 게 좋을 겁니다.... 혹시나 끝까지 못 할 경우를 대비해서 입니다..


# The slash as a delimiter
`s` 뒤의 문자는 “구분자” (delimiter)입니다. 관례적으로 슬래시(`/`) 문자를 사용하는데 이는 ed, more, vi 도 이 기호를 사용하기 때문입니다. 그러나, sed는 자신이 원하는 어떤 문자든 사용 가능합니다!! 예를 들어, 슬래시가 포함된 경로명(예: `/usr/local/bin` 을 `/common/bin` 으로 변경)을 바꾸려면 다음과 같이 슬래시를 백슬래시(`\`)로 인용해야 합니다:

```
sed 's/\/usr\/local\/bin/\/common\/bin/' <old >new
```

헉... 어떤 사람은 이것을 “피켓 펜스” (Picket Fence)라고 부르는데 보기에 흉합니다. “구분자” 로 슬래시 대신 밑줄 문자를 사용하면 좀 더 읽기 쉽습니다:
...
마음에 드는 걸로 골라서 사용합니다. 찾을 문자열에 포함되지 않으면 뭐든지 상관없습니다. 그리고 “구분자는 세 개가 필요하다!” 는 걸 기억해야 합니다. “Unterminated `s' command” 오류가 발생하면 구분자 중 하나가 빠졌기 때문입니다.


# Using & as the matched string
때로는 패턴을 검색한 후 패턴 주변이나 근처에 괄호 같은 문자를 추가할 때가 있습니다. 고정 문자열을 찾는 경우는 쉽게 처리할 수 있습니다:

```
sed 's/abc/(abc)/' <old >new
```

하지만, 정확히 무엇을 찾을지 모른다면 이런 방법은 통하지 않습니다. 찾을 문자열을 정확히 모르는데 어떻게 그 문자열을 대체 문자열에 넣을 수 있을까요?

해결책에는 특수 문자 “&” 가 필요합니다. 이것은 검색으로 발견된 “패턴 자체를 의미” 합니다.

```
sed 's/[a-z]*/(&)/' <old >new
```

대체 문자열에서 “&” 는 원하는 만큼 사용할 수 있습니다. 따라서, 패턴을 중복할 수 있습니다. 

다음은 줄 내의 첫 번째 숫자를 중복 출력합니다:
...
이 예제는 설명이 약간 필요합니다. sed는 첫 번째 문자열을 찾을 때 최대한 탐욕적으로 일치시킵니다. 이에 대해서는 나중에 다시 설명하겠습니다. 이런 탐욕적인 일치를 원하지 않는다면(즉, 일치를 제한하려면) 검색 일치 조건에 “제약을 설정” 해야 합니다.

'[0-9]*' 패턴의 첫 번째 일치 항목은 라인의 첫 번째 문자입니다. "0개 이상의 숫자와 일치" 하기 때문입니다. 따라서, 입력 값이 “abc 123” 이면 출력값은 변경되지 않습니다. (하지만, 첫 글자 앞에 공백이 추가된다는 점만 제외하면...) 

숫자를 복제하는 좀 더 나은 방법은 정확히 숫자만 일치하도록 패턴을 제한하는 것입니다:
...
문자열 “abc” 는 정규 표현식과 일치하지 않기 때문에 변경되지 않습니다. 출력에서 “abc” 를 제거하려면 확장 정규 표현식으로 줄의 나머지 부분을 일치시키고 “\(”, “\)” 및 “\1” 을 사용해서 정규 표현식의 일부를 명시적으로 제외해야 합니다. 이는 다음에 설명할 주제입니다.


# Extended Regular Expressions
간단히 덧붙이면, 위의 스크립트를 작성하는 다른 방법이 있습니다. 

- ``[0-9]*` 패턴은 0개 이상의 숫자를 일치시킵니다. 
- `[0-9][0-9]*` 패턴은 최소한 하나 이상의 숫자를 일치시킵니다. 

또 다른 방법은 확장 정규 표현식의 “+” 메타 문자를 사용하는 것입니다. “확장 정규 표현식” 에서 “+” 는 특수 문자로 처리되며 “[0-9]+” 패턴으로 사용합니다. 확장 정규 표현식은 좀 더 강력하지만 “+” 를 일반 문자로 취급하는 sed 스크립트에서 작동하지 않습니다. 따라서, 명령줄 옵션으로 확장 정규식 사용을 명시적으로 활성화해야 합니다.

GNU sed는 “-r” 명령줄 옵션으로 이 기능을 활성화합니다. 따라서, 위의 내용은 다음과 같이 작성할 수 있습니다.
...
Mac OS X와 FreeBSD는 -r 대신 -E 옵션을 사용합니다. 확장 정규 표현식에 대한 자세한 내용은 정규 표현식과 -r 명령줄 인수 설명 부분을 참조합니다.


# Using \1 to keep part of the pattern
정규 표현식에 관한 저의 이전 튜토리얼에서 “\(”,  “\)” 및 “\1” 의 [사용법](https://grymoire.com/Unix/Regular.html)을 설명한 바 있습니다. 복습하면 이스케이프된 괄호(즉, 앞에 백슬래시가 붙은 괄호)는 정규 표현식으로 일치시킨 “문자열의 일부를 기억” 합니다. 이를 활용하면 정규 표현식으로 일치시킨 문자열의 일부를 제한할 수 있습니다. “\1” 은 첫 번째로 기억된 패턴이고 “\2” 는 두 번째로 기억된 패턴입니다. Sed는 “최대 아홉 개의 기억된 패턴을 지원” 합니다.

줄 내의 첫 번째 단어는 유지하고 나머지 부분을 삭제하려면 중요한 부분을 괄호로 표시(mark)합니다:

```
sed 's/\([a-z]*\).*/\1/'
```

이 부분에 대해 좀 더 자세히 설명하겠습니다. 정규 표현식은 “탐욕적” 이며 가능한 한 많이 일치시키려고 시도합니다. 따라서, “`[a-z]*`” 패턴은 0개 이상의 소문자를 일치시키며 가능한 한 많은 문자를 일치시키려 합니다. “`.*`” 패턴은 첫 번째 일치 이후의 0개 이상의 문자를 일치시킵니다. 첫 번째 표현식에서 연속된 소문자를 모두 잡아내기 때문에 두 번째 표현식은 그 외의 모든 것을 일치시킵니다. 
...
“abcd” 를 출력하고 나머지(숫자)는 삭제합니다.

두 단어의 위치를 서로 바꾸려면 두 개의 패턴을 기억시키고 배치 순서를 바꿉니다:
...
> [!NOTE]
> 두 개의 기억된 패턴 사이의 공백에 유의합니다. 이는 두 단어가 발견되도록 제한하려는 용도로 사용합니다. 

그러나, 단일 단어나 글자가 아예 없는 행이면 아무런 효과가 없습니다. 단어가 “최소한 하나의 글자를 포함” 하도록 요구하려면 다음과 같이 사용합니다:

```
sed 's/\([a-z][a-z]*\) \([a-z][a-z]*\)/\2 \1/'
```

또는 확장 정규 표현식을 사용하면 다음과 같습니다:
...
또한, “\1” 은 반드시 대체 문자열(오른쪽)에서 사용할 필요는 없습니다. 검색 패턴(왼쪽)에도 사용할 수 있습니다. 따라서, 중복된 단어를 제거하려면 다음과 같습니다:

```
sed 's/\([a-z]*\) \1/\1/'
```

중복된 단어를 감지하려면 다음과 같습니다.
...
```
이를 필터로 사용하면 중복된 단어가 포함된 줄을 출력합니다.

기억된 부분 문자열의 숫자 값은 최대 아홉 개(“\1” 부터 “\9”)까지 지정할 수 있습니다. 따라서, 줄 내의 첫 세 글자를 반전시키려면 다음과 같이 사용합니다.
...
# Sed Pattern Flags
대체 명령 마지막 구분자 뒤에 "플래그" 를 사용합니다. 이전 대체 명령어 끝에 ‘p' 를 사용한 것을 눈치채셨을 겁니다. 또한, ’-n' 옵션도 함께 사용했습니다. 

먼저 'p' 와 다른 패턴 플래그에 대해 설명하겠습니다. 이 “플래그는 일치하는 패턴이 발견되었을 때 수행할 작업” 을 지정합니다. 각각에 대해 설명하겠습니다.


## /g - Global replacement
대부분의 유닉스 유틸리티는 파일을 처리할 때 한 줄씩 읽어서 처리합니다. sed도 기본적으로 마찬가지입니다. 또한, 특정 단어를 변경하도록 지시하면 해당 단어의 “첫 번째 발생 위치만 변경” 합니다. 줄 내의 첫 번째 단어가 아닌 모든 단어를 변경할 때도 있습니다. 예를 들어, 한 줄 내의 특정 단어를 괄호로 묶어 보겠습니다. 이때 “won't” 같은 단어를 일치시키지 못하는 “`[A-Za-z]*`” 패턴 대신, 공백을 제외한 모든 것을 일치시키는 “`[^ ]*`” 패턴을 사용합니다. “*” 문자는 “0개 이상을 의미” 하므로 이 패턴은 모든 것과 일치합니다. 

> [!NOTE]
> Solaris 버전의 sed(이 글을 작성하는 시점 기준)는 이 패턴에 “출력 행이 너무 길다” 는 오류 메시지를 발생시키거나 심지어 무한 루프에 빠질 수 있습니다. 저는 이를 버그로 간주해서 Sun에 보고했습니다. 

sed에서 “g” 플래그를 사용할 때는 “널 문자열 일치” 를 피해야 합니다. 이 문제를 해결할 패턴은 “`[^ ][^ ]*`” 입니다. 다음은 첫 번째 단어에 괄호를 추가합니다:

```
sed 's/[^ ]*/(&)/' <old >new
```

공백 문자열 일치를 피하면서 모든 단어에 변경 사항을 적용하려면 마지막 구분자 뒤에 “g” 프래그를 추가합니다:
...
## Is sed recursive?
sed는 입력 데이터에서 발견된 패턴에만 작동합니다. 즉, 입력 행을 읽고 지정한 패턴과 일치하면 수정된 출력을 생성하며, 입력 행의 나머지 부분을 계속 스캔합니다. “s” 명령어는 새로 생성한 출력을 다시 스캔하지 않습니다. 따라서, 다음과 같은 표현을 걱정할 필요는 없습니다:

```
sed 's/loop/loop the loop/g' <old >new
```

이는 무한 루프를 일으키지 않습니다!! 두 번째 “s” 명령어가 실행되면 이전 명령어의 결과를 수정하지 않습니다. 이후에 여러 개의 명령어를 실행하는 방법을 보여 드리겠습니다.


## /1, /2, etc. Specifying which occurrence
기본적으로 플래그가 없으면 첫 번째 일치 항목을 변경합니다. “g” 플래그를 사용하면 모든 일치 항목을 변경합니다. 줄 내에서 첫 번째가 아닌 특정 위치의 패턴을 수정하려면 패턴을 “\(” 와 “\)” 로 표시하고 “\1” 을 사용해서 첫 번째 패턴을 변경하지 않고 그대로 유지해야 합니다. 

다음 예제는 줄 내의 첫 번째 단어는 유지하고 두 번째 단어를 삭제합니다:

```
sed 's/\([a-zA-Z]*\) \([a-zA-Z]*\) /\1 /' <old >new
```

으악. 좀 더 쉬운 방법이 있습니다. 대체 명령어 뒤에 숫자 플래그를 추가해서 특정 위치의 패턴만 일치시킬 수 있습니다.

```
sed 's/[a-zA-Z]* //2' <old >new
```

또한, 숫자와 g (글로벌)플래그를 결합할 수 있습니다. 예를 들어, 첫 번째 단어는 그대로 두고 두 번째, 세 번째 단어 등을 “DELETED” 문자열로 변경하려면 “/2g” 를 사용합니다:

```
sed 's/[a-zA-Z]* /DELETED /2g' <old >new
```

Mac OS는 숫자와 g 플래그를 결합하면 제대로 작동하지 않는다고 들었습니다. 아마도 FreeBSD 버전의 sed도 이와 마찬가지일 것입니다.

`/2` 와 `\2` 를 혼동하지 않아야 합니다. 
...
> [!NOTE]
> 참고로 이 버그는 현재는 수정됐을 겁니다. 이는 숫자 플래그와 “g” 플래그가 동일한 버그를 가졌기 때문입니다. 
...
하지만, 이 글을 작성할 당시엔 이런 방법이 통하지 않았습니다. “`[^:][^:]*`” 로 우회하는 방법도 소용이 없습니다. 이는 존재하지 않는 비밀번호를 일치시키지 못할 뿐만 아니라 오히려 세 번째 필드의 사용자 ID를 삭제하기 때문입니다! 대신 다음과 같이 보기 흉한 괄호를 사용해서 처리해야 합니다:

```
sed 's/^\([^:]*\):[^:]:/\1::/'  </etc/passwd >/etc/password.new
```

첫 번째 패턴에 “:” 문자를 추가해서 빈 패턴과 일치하지 않도록 처리할 수 있습니다:

```
sed 's/[^:]*:/:/2'  </etc/passwd >/etc/password.new
```

숫자 플래그는 한 자릿수로만 제한되지 않습니다. 1에서  512까지 어떤 숫자든 사용 가능합니다. 예를 들어, 각 줄의 80번째 문자 뒤에 콜론을 추가하려면 다음과 같이 입력합니다:

```
sed 's/./&:/80' <file >new
```

또는 80개의 점(dot)을 사용해서 좀 더 어려운 방법으로 처리할 수도 있습니다:
...
## /p - print
sed는 기본적으로 모든 줄을 출력합니다. 대체 명령을 수행하면 기존 텍스트 대신 새로운 텍스트를 출력합니다. sed에 선택적 옵션 “sed -n” 을 사용하면 기본적으로 새로운 줄을 출력하지 않습니다. 이 옵션과 다른 옵션은 나중에 다시 다루겠습니다. 

“-n” 옵션을 사용할 때 “p” 플래그는 수정된 줄만 출력합니다. 다음은 sed로 grep의 기능을 복제하는 방법 중 하나입니다:

```
sed -n 's/pattern/&/p' <file
```

좀 더 간단한 버전은 나중에 다시 설명합니다.


## Write to a file with /w filename
세 번째 구분자 뒤에 지정할 수 있는 플래그가 하나 더 남아 있습니다. 이를 통해 수정된 데이터를 받을 파일을 지정할 수 있습니다. 예를 들어, 짝수 숫자로 시작하고 그 뒤에 공백이 오는 모든 줄을 “even 파일” 에 기록하려면 다음과 같이 처리합니다:

```
sed -n 's/^[0-9]*[02468] /&/w even' <file 
```

이 예제는 입력 파일을 수정하지 않으므로 사실 출력 파일이 필요하지 않습니다. “w” 와 파일 이름 사이에는 반드시 하나의 공백이 있어야 합니다!

또한, 하나의 sed 인스턴스로 열 개 파일까지 동시에 열 수 있습니다. 이를 통해 데이터 스트림을 별도의 파일로 분할해서 저장할 수 있습니다. 앞서 설명한 예제와 후술할 다중 치환 명령을 결합하면 첫 번째 숫자의 마지막 자릿수에 따라 파일을 열 개로 분할할 수 있습니다. 이런 방법으로 오류 또는 디버깅 정보를 별도 파일에 기록할 수 있습니다.


## /I - Ignore Case
GNU 버전의 sed는 “/I” 패턴 플래그를 추가했습니다.

이 플래그는 패턴을 일치시킬 때 대소문자를 구분하지 않도록 처리합니다. 따라서, “abc, aBc, ABC, AbC” 모두 일치합니다:
...
> [!NOTE]
> “/I”  와 “p” (print) 명령어 사이에 공백을 넣은 것은 여기서 사용한 “p” 는 패턴 매칭 과정의 수식어(플래그)가 아니라 패턴 매칭 이후 실행할 “명령어임을 강조” 하기 위함입니다.
...
다음으로 sed 옵션과 sed를 호출하는 다양한 방법에 대해 설명하겠습니다.



# Arguments and invocation of sed
지금까지 대체 명령어 하나만 사용했습니다. 만약 두 가지 작업이 필요하고 매뉴얼을 별도로 읽지 않으려면 여러 개의 sed 명령어를 파이프로 연결합니다:

```
sed 's/BEGIN/begin/' <old | sed 's/END/end/' >new
```

이 방법은 하나의 프로세스 대신 두 개의 프로세스를 사용합니다. sed 전문가(guru)는 하나의 프로세스로 해결할 수 있을 때 두 개의 프로세스를 절대로 사용하지 않습니다.
...
이전 예제는 “-e”  옵션이 필요하지 않았습니다. 왜냐하면 sed는 항상 “하나의 명령어가 있다!” 는 것을 잘 알고 있기 때문입니다. sed에서 하나의 인수만 주어지면 반드시 명령어이며 표준 입력에서 읽은 데이터를 편집합니다.

(GNU 버전의) 긴 옵션은 다음과 같습니다.
...
원한다면 명령줄에 파일을 지정할 수 있습니다. sed에서 옵션으로 시작하지 않는 인수가 하나 이상 있을 경우 반드시 “파일명” 이라야 합니다. 

다음 예제는 세 개의 파일에서 “#” 으로 시작하지 않는 줄의 라인 수를 셉니다.
...
- `sed` 의 substitute 명령어는 “#” 으로 시작하는 모든 줄을 빈 줄로 바꿉니다. 
- `grep` 은 빈 줄을 걸러내는(삭제하기) 용도로 사용합니다. 
- `wc` 는 남은 줄 수를 셉니다. 

sed는 grep이 불필요하게 만드는 처리 방법이 많이 있습니다. 그리고 “grep -c” 는 “wc -l” 명령을 대체할 수 있습니다. grep의 기능을 어떻게 복제할 수 있는지는 나중에 설명하겠습니다.
...
PATTERN 이 file 에 없다면 cat 처럼 동작합니다: 즉, 아무 것도 변경하지 않고 그대로 출력합니다. 만약 PATTERN 이 파일에 포함되면 해당 줄을 두 번 출력합니다. 따라서, “-n” 옵션을 추가하면 grep 프로그램처럼 동작합니다:
...
## Using 'sed /pattern/'
sed는 명령어 앞에 "주소" 를 지정해서 특정 줄을 검사하거나 수정할지 지정(제한)할 수 있습니다. 지금은 가장 간단한 “/PATTERN/” 패턴 주소만 설명하겠습니다. 이는 패턴과 일치하는 줄에만 명령어가 적용됩니다. 간단히 말해서 “/p” 플래그와 함께 사용하면 일치하는 줄을 두 번 출력합니다:

```
sed '/PATTERN/p' file
```

물론 PATTERN 부분에 어떤 정규 표현식도 사용 가능합니다.

하지만, 명령어(예: 출력용 “p”)를 포함하지 않으면 오류가 발생한다는 점에 유의해야 합니다. 즉, 다음과 같이 입력하면
...
또한, 꼭 그럴 필요는 없지만 “패턴과 명령어 뒤에 공백을 넣는 것을 권장” 합니다. 이렇게 하면 패턴 매칭을 수정하는 플래그와 패턴 일치 후 실행할 명령어를 구분하는 데 도움이 됩니다. 따라서, 다음과 같은 스타일로 작성할 것을 권장합니다:
...
grep의 기능을 복제하려면 “-n” (출력 안 함)옵션과 “/p” 출력 플래그를 함께 사용해야 합니다:
...
## sed -f scriptname
사용할 sed 명령어가 많다면, 이를 별도의 파일로 저장해서 사용합니다.

```
sed -f sedscript <old >new
```

sedscript 파일은 다음과 같습니다:
...
한 파일에 여러 명령어가 존재할 경우 각 명령어는 반드시 별도의 단독 줄에 위치해야 합니다.
...
또한, sed를 직접 실행하는 스크립트 작성 방법은 이후에 설명합니다.


## sed in shell scripts
쉘에서 입력할 명령어가 많아서 한 줄에 깔끔하게 들어가지 않을 경우 백슬래시 문자를 사용해서 줄을 나눠서 사용할 수 있습니다:
...
C 셸에서 여러 줄에 걸친 sed 스크립트를 사용할 수 있지만 “인용문이 여러 줄에 걸쳐 계속된다” 는 것을 C 셸에 알려야 합니다. 이를 위해 각 줄 끝에 백슬래시를 배치합니다:
...
Bourne 셸은 인용문에 여러 줄을 포함할 수 있으므로 좀 더 쉽게 처리할 수 있습니다:
...
## sed -V
“-V” 옵션은 현재 사용 중인 sed 버전을 출력합니다. 명령어의 긴 옵션은 다음과 같습니다.
...
## sed -h
“-h” 옵션은 sed 명령어의 요약본을 출력합니다. 명령어의 긴 옵션은 다음과 같습니다.
...
sed를 실행하는 또 다른 방법은 “인터프리터 스크립트” 로 사용하는 것입니다. 우선 다음과 같은 내용을 포함하는 파일을 생성합니다:
...
이 스크립트가 “CapVowel” 파일에 저장되어 있고 실행 가능한 상태라면 다음과 같이 간단한 명령어로 사용할 수 있습니다:
...
## Comments
sed에서 “주석” (comments 또는 remark)은 첫 번째 비공백 문자가 “#” 인 줄입니다. 많은 시스템에서 sed는 단 하나의 주석만 허용하며 반드시 스크립트의 첫 번째 줄이라야 합니다. Sun 시스템(이 글을 작성한 1988년 기준)은 스크립트 내 어디서나 여러 줄의 주석을 배치할 수 있습니다. 현대판 sed도 이 기능을 지원합니다. 

또한, 첫 번째 줄이 “#n” 으로 시작하면 “-n” 옵션과 동일한 효과를 냅니다: 즉, 기본적으로 “출력을 비활성화” 합니다. 

하지만, sed 인터프리터 스크립트는 이 작업을 수행할 수 없습니다. 왜냐하면, 첫 번째 줄은 반드시 “#!/bin/sed -f” 로 시작하기 때문입니다. 제 기억에 “#!/bin/sed -nf” 는 오류를 발생시켰습니다. 이 글을 작성했을 때(2008년)는 제대로 작동했습니다. “#!/bin/sed -fn” 은 작동하지 않습니다. sed는 스크립트 파일명을 “n” 으로 인식하기 때문입니다. 그러나,
...
sed를 호출하는 셸 스크립트에 단어(매개 변수)를 전달하는 것은 UNIX [쉘 인용 메커니즘에 대한 튜토리얼](https://grymoire.com/Unix/Quote.html)을 기억한다면 처리하기 쉽습니다. 복습하자면 인용을 켜고 끄려면 작은따옴표를 사용합니다. sed로 grep 기능을 모방하는 간단한 셸 스크립트는 다음과 같습니다:
...
그러나, 이 스크립트에는 미묘한 문제가 있습니다. 만약, 인자에 공백이 있다면 다음과 같은 구문 오류를 발생시킵니다.

```
sed: -e expression #1, char 4: unterminated `s' command
```

좀 더 나은 버전은 이런 일을 방지할 수 있습니다:
...
위의 내용이 sedgrep 파일에 저장되면 다음과 같이 실행합니다.

```
sedgrep '[A-Z][A-Z]' <file
```

이렇게 처리하면 sed는 grep 프로그램처럼 작동합니다.


## Using sed in a shell here-is document
sed를 사용해서 사용자에게 매개 변수를 입력하도록 요청한 후 입력한 매개 변수로 채워진 파일을 생성할 수 있습니다. 우선 가짜 값이 포함된 파일을 생성한 다음 sed로 가짜 값을 변경할 수 있습니다. 좀 더 간단한 방법은 쉘의 “here is” document 기능을 사용하는 것으로 셸 “스크립트의 일부 텍스트를 표준 입력처럼 사용” 합니다:
...
이는 매우 인위적인 예제임을 인정합니다. 사실 “Here is” document는 sed를 사용하지 않고도 값을 평가할 수 있습니다. 따라서, 다음 예제도 위와 동일한 작업을 수행합니다:
...
그러나, “here is” document와 sed를 결합하면 일부 복잡한 경우엔 유용할 수 있습니다. 참고로,

```
sed 's/XYZ/'$value'/' <<EOF
```

여기서 사용자가 “a b c” 처럼 공백이 포함된 답을 입력하면 구문 오류가 발생합니다. 좀 더 나은 방법은 입력 값을 이중 따옴표로 묶는 것입니다:
...
따옴표에 관한 제 [튜토리얼](https://grymoire.com/Unix/Quote.html)에서 이 내용에 대해 이미 다뤘습니다.


## Multiple commands and order of execution
sed 명령어를 깊이 알아갈수록 명령어는 복잡해지고 실행 순서가 혼란스러울 수 있습니다. 사실 아주 간단합니다. 

- 각 줄을 읽습니다. 
- 사용자가 지정한 순서대로 각 명령어는 입력 줄에 작업을 수행할 기회를 얻습니다. 
- 작업이 모두 완료된 후, 다음 명령은 동일한 줄(이전 명령에 의해 수정되었을 수 있음)에 작업할 기회를 얻습니다. 

무언가 궁금한 점이 있다면 작은 예제부터 만들어 보는 것이 결과를 파악하는 가장 좋은 방법입니다. 복잡한 명령이 작동하지 않으면 좀 더 단순하게 만듭니다. 복잡한 스크립트가 제대로 작동하지 않는다면 두 개의 작은 스크립트로 나눈 후 파이프로 서로 연결해서 처리합니다.



# Addresses and Ranges of Text
지금까지 단 하나의 명령어만 배웠을 뿐인데도 sed의 강력한 기능을 확인할 수 있습니다. 하지만, 이 명령으로 단순히 grep과 대체 작업을 수행했을 뿐입니다. 즉, 대체 명령어는 특정 행을 고려하지 않고 모든 행을 개별적으로 처리합니다. 

특정 행으로 작업을 제한할 수 있는 기능은 유용합니다. 이때 유용한 제한 조건은 다음과 같습니다:
...
sed는 이 모든 것과 그 이상도 처리할 수 있습니다. sed의 모든 명령어는 위의 목록처럼 “단일 주소” 나 “주소 범위” 같은 “제한 조건” 을 앞에 붙일 수 있습니다. 즉, 제한 조건이나 주소는 다음과 같이 명령어 앞에 위치합니다:
...
가장 간단한 제한은 줄 번호입니다. 3번째 줄의 첫 번째 숫자를 삭제하려면 명령어 앞에 “3” (행 번호)을 추가합니다:
...
## Patterns
vi 나 more 같은 유닉스 유틸리티는 정규 표현식 검색에 슬래시 문자를 사용합니다. sed도 동일한 규칙을 따르며 표현식은 슬래시로 종료해야 합니다. 예를 들어, 모든 줄에서 “#” 으로 시작하는 첫 번째 숫자를 삭제하려면 다음과 같이 사용합니다:

```
sed '/^#/ s/[0-9][0-9]*//'
```

“/expression/” 뒤에 공백을 하나 추가해서 가독성을 높였습니다. 이는 꼭 필요한 건 아니지만 없다면 명령어를 이해하기 어려울 수 있습니다. sed는 정규 표현식을 지정할 때 몇 가지 추가적인 옵션을 제공합니다. 하지만, 나중에 다시 설명하겠습니다. 

만약, 검색 패턴이 백슬래시 문자로 시작하면 그 다음 문자는 "구분자" 로 인식합니다!! 따라서, 슬래시 대신 쉼표를 구분자로 검색하려면 다음과 같이 사용합니다:

```
sed '\,^#, s/[0-9][0-9]*//'
```

이 기능의 장점은 “슬래시 문자도 검색할 수 있다!” 는 것입니다. 예를 들어, “/usr/local/bin” 문자열을 검색해서 “/common/all/bin” 으로 변경하려면 다음과 같이 백슬래시를 사용해서 슬래시를 모두 이스케이프 처리해야 합니다:

```
sed '/\/usr\/local\/bin/ s/\/usr\/local/\/common\/all/'
```

정규식으로 검색할 때 슬래시 대신 밑줄 문자를 사용한다면 좀 더 이해하기 쉬울 수 있습니다. 다음 예제는 “검색 명령과 대체 명령 모두 밑줄 구분자를 사용” 합니다:

```
sed '\_/usr/local/bin_ s_/usr/local_/common/all_'
```

이런 것이 "sed 스크립트는 난해하다!" 는 평을 얻는 이유를 정확히 보여줍니다. 일부러 좀 더 까다롭게 만들어 “g” 로 시작하는 모든 줄을 찾고 줄 내의 각 “g” 를 “s” 로 바꾸는 다음과 같은 예제를 보여줄 수 있습니다:
...
여기서 배울 점이 있습니다: sed 스크립트에는 “주석을 아낌없이 사용” 해야 합니다. 다른 (구형) 운영체제에서 이 스크립트를 실행하려면 주석을 제거할 수도 있지만 이제 그 정도 일은 쉽게 처리할 수 있는 sed 스크립트를 기술을 잘 알게 됐습니다! 

여하튼!! 주석은 좋은 것입니다. 작성할 당시엔 스크립트를 완벽하게 이해했을지 몰라도 6개월 이후엔 마치 모뎀 잡음처럼 보일 수 있습니다. 이런 비유가 이해가 잘 안 된다면 8개월 된 아기가 컴퓨터 키보드를 두드리는 모습을 상상하시기 바랍니다.


## Ranges by line number
두 개의 줄 번호 사이에 쉼표를 삽입해서 주소 범위로 지정합니다. 첫 100줄로 대체 작업을 제한하려면 다음과 같이 사용합니다:

```
sed '1,100 s/A/a/'
```

파일 내의 줄 수를 정확히 안다면 끝 줄의 숫자를 명시적으로 지정해서 파일 나머지 부분도 대체합니다. 이 경우, wc 명령어로 파일에 532줄이 있음을 확인했다고 가정합니다:

```
sed '101,532 s/A/a/'
```

좀 더 쉬운 방법은 특수 문자 “$” 를 사용하는 것으로 이는 “파일 내의 마지막 줄을 의미” 합니다.
...
## Ranges by patterns
두 개의 정규식을 범위로 지정합니다. “#” 이 주석 시작을 의미한다면 특정 키워드를 검색한 후 두 번째 키워드가 나타날 때까지의 모든 주석을 제거합니다. 이 경우 사용할 두 개의 키워드는 “start” 와 “stop” 입니다.
...
“start” 패턴이 발견되면 “start” 가 “포함된 행부터 시작한다” 는 점을 잘 알아야 합니다. 이는 스위치를 켭니다. 이 스위치는 행 단위로 작동합니다. 즉, 다음 행이 읽혀지고 대체 명령이 수행됩니다. “stop” 이 포함되면 이 스위치는 꺼집니다. 이 스위치는 단어 단위가 아니라 “행 단위로 작동” 합니다.

줄 번호와 정규 표현식을 결합할 수 있습니다. 다음 예제는 파일에서 처음으로 “start” 를 찾을 때까지의 주석을 제거합니다:

```
sed -e '1,/start/ s/#.*//'
```

다음 예제는 두 키워드 사이의 모든 주석을 제거합니다:

```
sed -e '1,/start/ s/#.*//' -e '/stop/,$ s/#.*//'
```

이 예제는 “/start/,/stop/” 범위와 중복되는 범위를 가집니다. 이는 두 범위 모두 “키워드가 포함된 줄까지 처리 대상으로 인식” 하기 때문입니다. 지정된 패턴이 포함된 줄까지(해당 줄 제외) 명령을 제한하는 방법은 나중에 설명하겠습니다. 이것은 패턴 범위 내에서 패턴 제외 작업에 해당하지만 먼저 몇 가지 기본 원칙을 다뤄야 합니다.

다양한 명령어를 논의하기 전에 일부 명령어는 “라인 범위로 적용할 수 없다!” 는 점에 대해 설명해야 합니다. 해당 명령어를 언급할 때 다시 알려 드리겠습니다. 다음 섹션에 세 가지 명령어를 설명할 예정인데 그 중 하나는 라인 범위로 적용할 수 없습니다.
...
이는 head 프로그램과 기능이 유사합니다. 메일 메시지의 헤더(첫 번째 빈 줄까지의 모든 내용)를 잘라내려면 다음과 같이 사용합니다:
...
세 번째는 줄 끝 바로 앞에 있는 모든 공백과 탭 문자를 제거하기 위해 추가합니다:

```
sed -e 's/#.*//' -e 's/[ ^I]*$//' -e '/^$/ d' 
```

여기서 문자 “^I” 는 “CTRL-I” 또는 “탭 문자” 입니다. 탭을 명시적으로 직접 입력해야 합니다. 위에서 작업 처리 순서는 타당한 이유로 정해진 순서임을 유의합니다. 주석은 줄 중간에서 시작할 수 있으며 그 앞에 공백 문자가 있을 수도 있습니다. 따라서, 주석 먼저 제거하면 주석 앞의 공백 문자가 남아 있을 수 있습니다. 두 번째 명령어는 모든 후행 공백을 제거해서 현재 공백인 줄을 빈 줄로 변환합니다. 마지막 명령어는 빈 줄을 삭제합니다. 이 세 명령어로 주석, 탭 또는 공백만 포함된 모든 줄을 제거할 수 있습니다.

다음은 sed가 한 줄에 작동할 때 사용하는 “패턴 공간” (pattern space)을 보여줍니다. sed가 실제로 처리하는 작업은 다음과 같습니다:
...
모든 줄을 복제합니다. 따라서, 빈 줄을 두 배로 늘릴려면 다음과 같이  사용합니다:
...
변경을 제한할 수 있는 간단한 명령이 하나 더 있습니다. 바로 “q” 명령입니다. 
...
11 번째 줄에 도달하는 즉시 종료합니다. 이 명령은 특정 조건이 충족된 후 작업을 즉시 중단할 때 유용합니다.

“q” 명령은 “주소 범위를 지정하지 않는 유일한 명령” 입니다. 따라서, 다음 명령은
...
이렇게까지 설명할 가치가 있을까요? 긴 설명 끝에 나온 해결책이 그저 중괄호로  일치시키는 것이라니... 음, 한 가지 복잡한 점이 있습니다. 각 sed 명령어는 반드시 별도의 줄에서 시작하므로 중괄호와 중첩된 sed 명령어는 “각기 다른 줄에 위치” 해야 합니다.
...
중괄호는 중첩할 수 있으므로 주소 범위를 결합할 수 있습니다. 다음은 이전과 동일한 작업을 수행하되 변경 작업을 처음 100줄로 “제한” 합니다:
...
중괄호 앞에 “!” (반전)를 배치합니다. 이것은 “주소 제한을 반전” 시켜서 두 예약어 사이에 있는 줄을 "제외한" 모든 줄의 주석을 제거합니다:
...
“패턴 범위에 대체 작업을 수행할 수 있다” 는 점을 기억하실 겁니다. 예를 들어, 시작/종료 패턴 사이의 “old” 를 “new” 로 변경하는 방식말입니다:
...
아래에 보다시피 이렇게 처리하면 코드가 좀 더 명확해지고 수정하기도 쉬워집니다.

“begin” 이란 단어가 나오는 곳을 변경하지 않으려면 해당 줄을 건너뛰도록 새로운 조건을 추가합니다:
...
그러나, “end” 가 포함된 줄을 건너뛰는 작업은 좀 더 까다롭습니다. “begin” 에서 사용한 것과 동일한 방법을 사용하면 sed 엔진이 범위를 종료하는 “end” 를 제대로 인식하지 못합니다. 엔진은 “end” 도 건너뛰기 때문입니다. 해결책은 “end” 가 없는 모든 줄에 다음과 같이 대체 작업을 수행하는 것입니다.
...
대체 명령어에서 “파일에 쓸 수 있다” 는 점을 기억하실 겁니다. 다시 한 번 짝수(뒤에 공백이 따라오는)로 시작하는 줄만 파일에 쓰는 예제를 보여드리겠습니다:

```
sed -n 's/^[0-9]*[02468] /&/w even' <file 
```

대체 명령어의 대체 문자열에 “&” 를 사용해서 해당 줄이 변경되지 않도록 처리했습니다. 좀 더 간단한 예로 ‘w’ 명령어를 사용하는 방법이 있으며 이는 대체 명령어의 “w” 플래그와 동일한 구문을 가집니다:

```
sed -n '/^[0-9]*[02468]/ w even' <file
```

명령어 뒤에는 반드시 공백 하나만 따라야 합니다. 그 외의 모든 것은 파일 이름의 일부로 간주됩니다. “w” 명령어는 대체 명령의 “w” 플래그와 동일한 제한 사항을 가집니다: sed는 최대 10개의 파일만 열 수 있습니다.
...
파일 끝에 “end” 파일의 내용을 추가합니다. (주소: “$”) 

다음은 “INCLUDE” 라는 단어가 있는 줄 뒤에 파일을 읽어서 삽입합니다:

```
sed '/INCLUDE/ r file' <in >out
```

다음은 중괄호를 사용해서 “INCLUDE” 가 있는 줄을 삭제합니다:
...
삭제 명령어 “d” 와 파일 읽기 명령어 “r” 의 “배치 순서는 중요” 합니다. 이 순서를 바꾼다면 제대로 작동하지 않습니다. 이 기능이 제대로 작동하지 않는 두 가지 미묘한 동작이 있습니다. 첫 번째는 “r” 명령어는 파일을 “출력 스트림에 기록한다” 는 점입니다. 따라서, 파일은 패턴 공간에 삽입되지 않으므로 어떤 명령어도 이를 수정할 수 없습니다. 즉, 삭제 명령어는 파일에서 읽은 데이터에 영향을 미치지 않습니다.

또 다른 미묘한 점은 “d” 명령어는 “패턴 공간의 현재 데이터를 삭제한다” 는 것입니다. 모든 데이터가 삭제되다면 다른 작업을 시도하지 않는 것이 당연합니다. 따라서, 중괄호 안에서 실행된 “d” 명령어는 이후 모든 작업을 중단시킵니다. 

예를 들어, 다음의 대체 명령어는 절대로 실행되지 않습니다:
...
앞서 제시한 예제는 C 언어 전처리기의 단순화된 버전입니다. 포함되는 파일은 미리 정해진 이름을 가집니다. sed가 고정된 파일 이름 대신 변수(예: “\1”)를 허용한다면 유용합니다. 아쉽게도 sed에는 이런 기능이 없습니다. 

이런 한계를 해결하려면 즉석에서 sed 명령어를 생성하거나 셸 인용을 사용해서 변수를 sed 스크립트로 전달하는 방법이 있습니다. 예를 들어, cpp처럼 파일을 포함하는 명령어를 만들어 파일 이름을 스크립트 인수로 전달하려면 다음과 같은 스크립트를 사용합니다:
...
자세히 설명해 드리겠습니다. 만약, 다음과 같은 내용을 포함한 파일이 있다면
...
# The '#' Comment Command
sed를 깊이 파고들수록 주석은 명령어를 이해하기 쉽게 해줍니다. 구버전의 sed는 주석으로 한 줄만 허용하며 반드시 첫 번째 줄이어야 합니다. SunOS(및 GNU sed)는 여러 개의 주석을 허용하며 주석은 반드시 첫 번째 줄일 필요도 없습니다. 마지막 예제는 다음과 같이 작성할 수 있습니다:
...
Sed는 출력 스트림에 새로운 줄을 추가할 수 있는 세 가지 명령어가 있습니다. 전체 줄이 추가되기 때문에 새로운 줄은 이를 강조하기 위해 단독 줄로 표시됩니다. 옵션은 없으며 전체 줄이 사용되며 “반드시 단독 줄” 이어야 합니다. 

많은 UNIX 유틸리티에 익숙하다면 sed도 유사한 규칙을 사용할 것을 예상할 수 있습니다: 이전 줄에서 “\” 문자로 끝내면 줄은 계속 이어집니다. 이 명령의 구문은 ‘r’ 및 “w” 명령어처럼 까다롭습니다.
...
원한다면 셸 스크립트에서 두 줄을 제거해서 한줄로 작성할 수 있습니다:
...
저는 “첫 번째 형식을 선호” 합니다. 새로운 명령어를 좀 더 쉽게 추가할 수 있고 사용 의도가 명확하기 때문입니다. 또한, “\” 뒤에는 공백이 있어선 안 됩니다!!



## Insert a line with 'i'
“i” 명령어로 패턴 이나 주소 앞에 새로운 줄을 삽입합니다:
...
앞서 설명한 것 같이 “d” 명령어 뒤에 ‘a’ 명령어를 실행하면 작동하지 않습니다!! “d” 명령어는 작업을 종료하기 때문입니다. 

세 가지 명령을 중괄호로 묶어서 결합할 수 있습니다:
...
Sed는 모든 명령에서 “선행 탭과 공백을 무시” 합니다. 그러나, “a”, “c” 또는 ‘i’ 명령 뒤에 오는 텍스트 시작 부분에 포함된 공백 문자는 무시될 수도 있고 무시되지 않을 수도 있습니다. SunOS는 두 가지 “기능” 모두 사용합니다. Berkeley(및 Linux) 스타일의 sed는 /usr/bin 에 있으며 AT&T 버전(System V)은 /usr/5bin/ 에 있습니다.

자세히 설명하면 /usr/bin/sed 명령어는 공백을 유지하는 반면 /usr/5bin/sed 명령어는 선행 공백을 제거합니다. 

선행 공백을 유지하고 현재 사용 중인 sed 버전에 상관없이 처리하려면 추가된 줄의 첫 번째 문자를 “\” 를 입력합니다:
...
세 명령어 모두 한 줄 이상 추가할 수 있습니다. 연속된 각 줄 끝에 “\” 를 붙입니다.
...
패턴 공간에 대해서는 이전에 한 번 언급한 바 있습니다. 대부분의 명령어는 패턴 공간에서 작동하며 후속 명령어는 마지막 수정 결과에 적용됩니다. 앞서 언급한 세 가지 명령어는 파일 읽기 명령과 마찬가지로 “패턴 공간을 우회해서 새로운 줄을 출력 스트림에 추가” 합니다.



## Address ranges and the above commands
이전에 일부 명령어는 줄 범위를 지정할 수 있지만 다른 명령어는 그렇지 않다고 경고한 것을 기억하실 겁니다. 정확히 말하면 “a”, “i”, “r”, ‘q’ 명령어는 “1,100” 이나 “/begin/,/end/” 같은 범위를 지정할 수 없습니다!! 문서에선 read 명령은 범위를 지정할 수 있다고 나와 있지만 제가 시도했을 때는 오류가 발생했습니다. “c” 변경 명령어는 이를 허용하며 여러 줄을 하나의 줄로 변경합니다:
...
이런 작업을 수행하려면 중괄호를 사용해서 처리할 수 있습니다. 이를 통해 모든 줄에 작업을 수행할 수 있습니다:
