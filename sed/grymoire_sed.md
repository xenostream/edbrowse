...
숫자 플래그는 한 자릿수로 제한되지 않습니다. 1부터 512까지 어떤 숫자든 사용 가능합니다. 각 줄의 80번째 문자 뒤에 콜론을 추가하려면 다음과 같이 입력합니다:
...
세 번째 구분자 뒤에 추가로 지정할 수 있는 플래그가 하나 더 있습니다. 이를 통해 수정된 데이터를 받을 파일을 지정합니다. 
...
또한, sed 한 인스턴스로 열 개 파일을 동시에 열 수 있습니다. 이를 통해 데이터 스트림을 별도의 파일로 분할합니다. 앞서 설명한 예제와 후술할 다중 치환 명령을 결합하면, 첫 번째 숫자의 마지막 자릿수에 따라 파일을 열 개로 분할합니다. 이 방법을 사용해서 오류 또는 디버깅 정보를 별도 파일에 기록할 수도 있습니다.
...
이전에는 대체 명령어를 하나만 사용했습니다. 두 가지 변경 작업이 필요하고 매뉴얼을 읽고 싶지 않다면 여러 sed 명령어를 파이프로 연결합니다:
...
원한다면 명령줄에서 파일을 지정합니다. sed에 옵션으로 시작하지 않는 인수가 하나 이상 있을 경우, 반드시 파일명이어야 합니다. 
...
sed는 grep을 불필요하게 만드는 더 많은 명령어가 있습니다. 그리고 grep -c 는 wc -l 을 대체합니다. grep의 일부 기능을 어떻게 복제할 수 있는지는 나중에 설명하겠습니다.
...
## Using 'sed /pattern/'
Sed는 명령어 앞에 주소를 지정해서 어떤 줄을 검사하거나 수정할지 지정합니다. 지금은 가장 간단한 버전인 /PATTERN/ 주소에 대해서만 설명하겠습니다. 이 옵션을 사용하면 패턴과 일치하는 줄에만 주소 뒤의 명령어가 적용됩니다. 간단히 말해, /p 플래그와 함께 사용하면 일치하는 줄이 두 번 출력됩니다:
...
명령어가 많아서 한 줄에 깔끔하게 들어가지 않을 경우, 백슬래시를 사용해서 줄을 나누어 사용합니다:
...
이 스크립트가 “CapVowel” 이란 이름의 파일에 저장되어 실행 가능하면, 간단한 명령어로 사용합니다:
...
이렇게 하면 sed가 grep 명령어처럼 작동합니다.



## Using sed in a shell here-is document
sed를 사용해서 사용자에게 매개 변수를 입력하도록 요청한 후 해당 매개 변수로 채워진 파일을 생성합니다. 먼저 가짜 값이 포함된 파일을 생성한 다음 sed로 그 가짜 값을 변경할 수도 있습니다. 
...
이것은 인위적인 예제임을 인정합니다. “Here 문서” 는 sed를 사용하지 않고도 값을 평가합니다. 다음 예제도 동일한 작업을 수행합니다:
...
그러나, “here 문서” 와 sed를 결합하는 것은 일부 복잡한 경우에 유용합니다.
...
단 하나의 명령어만 배웠을 뿐인데도 sed의 강력한 기능을 확인합니다. 하지만, 이 명령어는 단순히 grep과 대체 작업을 수행할 뿐입니다. 즉, 대체 명령어는 주변 행을 고려하지 않고 각 행을 개별적으로 처리합니다. 
...
Sed는 이 모든 것과 그 이상을 처리합니다. sed의 모든 명령어는 위의 예제들처럼 주소, 범위 또는 제한 조건을 앞에 붙일 수 있습니다. 제한 조건이나 주소는 명령어 바로 앞에 위치합니다:
...
이 기능의 주요 장점은 슬래시 문자 검색입니다. 예를 들어, “/usr/local/bin” 문자열을 검색해서 “/common/all/bin” 으로 변경하고 싶을 때, 백슬래시를 사용해서 슬래시를 이스케이프 처리합니다:
...
줄 번호 사이에 쉼표를 삽입해서 번호 범위를 지정합니다. 첫 100줄로 치환을 제한하려면 다음과 같이 사용합니다:

```
sed '1,100 s/A/a/'
```

파일의 줄 수를 정확히 알고 있다면, 그 숫자를 명시적으로 지정해서 파일의 나머지 부분도 치환합니다. 이 경우, wc 명령어를 사용해서 파일에 532줄이 있음을 확인했다고 가정해 보겠습니다:
...
줄 번호와 정규 표현식을 결합합니다. 다음 예제는 파일 시작부터 키워드 “start” 를 찾을 때까지 주석을 제거합니다:
...
# Delete with d
범위 사용은 혼란스러울 수 있으므로 새로운 스크립트를 시도할 때는 실험이 필요합니다. 
...
파일 길이를 알고 있다면 tail 명령어의 기능을 복제합니다. wc 는 줄 수를 세고, expr 은 줄 수에서 10을 뺄 수 있습니다. 파일의 마지막 10줄을 확인하는 Bourne 셸 스크립트는 다음과 같을 수 있습니다:
