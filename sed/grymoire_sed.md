...
Sed는 궁극의 스트림 편집기입니다. 이상하게 들린다면 파이프를 통해 흐르는 개울을 상상해 보시기 바랍니다. 음, 파이프 안에 있으면 개울을 볼 수 없습니다. 무언가 “흐른다” 는 비유를 시도한 대가입니다. 문학 작품을 원하시면 제임스 조이스를 읽으시기 바랍니다.

어쨌든 sed는 놀라운 유틸리티입니다. 안타깝게도 대부분의 사람들은 “sed의 진정한 힘” 을 제대로 배우지 못합니다. sed 자체는 매우 단순하지만 문서화가 엉망입니다. Solaris의 sed 온라인 매뉴얼 페이지는 전체 다섯 페이지인데 그 중 두 페이지는 사용 중에 발생 가능한 34가지 오류에 대해 설명하고 있습니다. 프로그램 설명과 동일한 분량을 오류 설명에 할애하는 이런 프로그램의 학습 곡선은 매우 가파릅니다.
...
Sed에는 여러 가지 명령어가 있지만 사용자 대부분은 대체 명령어 `s` 만 배웁니다. 대체 명령어는 정규 표현식과 일치하는 부분을 새로운 값(문자열)으로 바꿉니다. 
...
예제에서 인자에 따옴표(인용)를 붙이지 않은 이유는 이 예제는 필요가 없기 때문입니다. 이전 따옴표 튜토리얼을 읽었다면 왜 따옴표가 필요 없는지 이해할 수 있습니다. 하지만, 저는 “따옴표를 사용할 것을 강력히 권장” 합니다. 만약 명령어에 특수 문자가 포함된 경우 따옴표는 "필수" 입니다. 무언가 확실치 않을 때의 좋은 습관으로나 앞으로의 “모범 사례” 를 강조하기 위해 예제에 따옴표를 사용할 것입니다. 
...
sed 편집기는 "정확히 지시대로만 변경한다!" 는 점을 강조합니다. 따라서, 다음과 같이 실행했다면
...
검색 패턴(Search Pattern)은 왼쪽에 있고, 대체 문자열(Replacement string)은 오른쪽에 위치합니다.

인용문과 정규 표현식에 대해선 이미 다뤘습니다. 그것만으로 sed 대체 명령어 학습에 필요한 90%를 차지합니다. 다시 말해, 가장 빈번하게 사용되는 sed 기능의 90%를 이미 다룰 줄 안다는 뜻입니다. 앞으로 sed 전문가가 되려면 알아야 할 몇 가지 세부 사항이 좀 더 있습니다. (이제 겨우 1장을 마쳤습니다. 앞으로 63개 장이 더 남아 있습니다. :-) 

아, 그리고 이 페이지를 북마크하는 게 좋을 겁니다.... 혹시나 끝까지 못 할 경우를 대비해서 입니다..


# The slash as a delimiter
`s` 뒤의 문자는 “구분자” 입니다. 관례적으로 슬래시(`/`) 문자를 사용하는데 이는 ed, more, vi 도 이 기호를 사용하기 때문입니다. 그러나, sed는 자신이 원하는 어떤 문자든 사용 가능합니다!! 

예를 들어, 슬래시가 포함된 경로명(예: `/usr/local/bin` 을 `/common/bin` 으로 변경)을 바꾸려면 다음과 같이 슬래시를 백슬래시(`\`)로 인용해야 합니다:

```
sed 's/\/usr\/local\/bin/\/common\/bin/' <old >new
```

헉... 어떤 사람은 이것을 “피켓 펜스” 라고 부르는데 보기에도 흉합니다. “구분자” 로 슬래시 대신 밑줄 문자를 사용하면 좀 더 읽기 쉽습니다:
...
마음에 드는 걸로 하나 골라서 사용합니다. 찾을 문자열에 포함되지 않으면 뭐든지 상관없습니다. 그리고 “구분자는 세 개가 필요하다!” 는 걸 기억해야 합니다. “Unterminated `s' command” 오류가 발생하면 구분자 중 하나가 빠졌기 때문입니다.
...
해결책은 특수 문자 “&” 가 필요합니다. 이것은 검색으로 발견된 “패턴 자체” 를 의미합니다.
...
이 예제는 설명이 약간 필요합니다. sed는 첫 번째 문자열을 찾을 때 최대한 탐욕적으로 일치시킵니다. 이에 대해서는 나중에 다시 설명하겠습니다. 이런 탐욕적인 일치를 원하지 않는다면(즉, 일치를 제한하려면) 일치 조건에 제약을 설정해야 합니다.

'[0-9]*' 패턴의 첫 번째 일치 항목은 첫 번째 문자입니다. "0개 이상의 숫자와 일치" 하기 때문입니다. 따라서, 입력 값이 “abc 123” 이라면 출력값은 변경되지 않습니다. (하지만, 첫 글자 앞에 공백이 추가된다는 점만 제외하면...) 
...
이 경우 문자열 “abc” 는 정규 표현식과 일치하지 않기 때문에 변경되지 않습니다. 출력에서 “abc” 를 제거하려면 확장 정규 표현식으로 줄의 나머지 부분을 일치시키고 “\(”, “\)” 및 “\1” 을 사용해서 정규 표현식의 일부를 명시적으로 제외해야 합니다. 이는 다음에 설명할 주제입니다.
...
또 다른 방법은 확장 정규 표현식의 “+” 메타 문자를 사용하는 것입니다. “확장 정규 표현식” 에서 “+” 는 특수 문자로 처리되며 “[0-9]+” 패턴으로 사용합니다. 

확장 정규 표현식은 좀 더 강력하지만 “+” 를 일반 문자로 취급하는 sed 스크립트에서 작동하지 않습니다. 따라서, 명령줄 옵션으로 확장 정규식 사용을 명시적으로 활성화해야 합니다.
...
정규 표현식에 관한 저의 이전 튜토리얼에서 “\(”,  “\)” 및 “\1” 의 사용법을 설명한 바 있습니다. 복습하면 이스케이프된 괄호(즉, 앞에 백슬래시가 붙은 괄호)는 정규 표현식으로 일치시킨 “문자열의 일부를 기억” 합니다. 이를 활용하면 정규 표현식으로 일치시킨 문자열의 일부를 제한할 수 있습니다. “\1” 은 첫 번째로 기억된 패턴이고, “\2” 는 두 번째로 기억된 패턴입니다. Sed는 최대 아홉 개의 기억된 패턴을 지원합니다.
...
두 단어의 위치를 서로 바꾸려면, 두 가지 패턴을 기억시키고 배치 순서를 바꿉니다:

```
echo "abc 123" | sed 's/\([a-z]*\) \([a-z]*\)/\2 \1/'
```

두 개의 기억된 패턴 사이의 공백에 유의합니다. 이는 두 단어가 발견되도록 제한하려는 용도로 사용합니다. 

그러나, 단일 단어나 글자가 아예 없는 행이면 아무런 효과가 없습니다. 단어가 “최소한” 하나의 글자를 포함하도록 요구하려면 다음과 같이 사용합니다:
...
또한, “\1” 은 반드시 대체 문자열(오른쪽)에서 사용할 필요는 없습니다. 검색 패턴(왼쪽)에도 사용할 수 있습니다. 

따라서, 중복된 단어를 제거하려면 다음과 같습니다:
...
기억된 문자열의 숫자 값은 최대 아홉 개(“\1” 부터 “\9”)까지 지정할 수 있습니다. 

따라서, 줄 내의 첫 세 글자를 반전시키려면 다음과 같이 사용합니다.
...
대부분의 유닉스 유틸리티는 파일을 처리할 때 한 줄씩 읽어서 처리합니다. sed도 기본적으로 마찬가지입니다. 또한, 특정 단어를 변경하도록 지시하면 해당 단어의 “첫 번째 발생 위치만 변경” 합니다. 줄 내의 첫 번째 단어가 아닌 모든 단어를 변경할 때도 있습니다. 

예를 들어, 한 줄 내의 특정 단어를 괄호로 묶어 보겠습니다. 이때 “won't” 같은 단어를 일치시키지 못하는 “[A-Za-z]*” 패턴 대신, 공백을 제외한 모든 것을 일치시키는 “[^ ]*” 패턴을 사용합니다. “*” 문자는 0개 이상을 의미하므로 이 패턴은 모든 것에 일치합니다. 

> [!NOTE]
> 현재 버전의 Solaris sed(이 글을 작성하는 시점 기준)는 이 패턴에 “출력 행이 너무 길다” 는 오류 메시지를 발생시키거나 심지어 무한 루프에 빠질 수 있습니다. 저는 이를 버그로 간주해서 Sun에 보고했습니다. 

sed에서 “g” 플래그를 사용할 때는 공백 문자열 일치를 피해야 합니다. 이 문제를 해결할 패턴은 “[^ ][^ ]*” 입니다. 다음은 첫 번째 단어에 괄호를 추가합니다:
