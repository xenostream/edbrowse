...
하지만, 이 글을 작성할 당시엔 이런 방법이 통하지 않았습니다. “[^:][^:]*” 로 우회하는 방법도 소용이 없습니다. 이는 존재하지 않는 비밀번호를 일치시키지 못할 뿐만 아니라, 오히려 세 번째 필드의 사용자 ID를 삭제하기 때문입니다! 대신 다음과 같이 보기에 흉한 괄호를 사용해서 처리해야 합니다:
...
세 번째 구분자 뒤에 지정할 수 있는 플래그가 하나 더 남아 있습니다. 이를 통해 수정된 데이터를 받을 파일을 지정할 수 있습니다. 

예를 들어, 짝수 숫자로 시작하고 그 뒤에 공백이 오는 모든 줄을 “even 파일” 에 기록하려면 다음과 같이 처리합니다:

```
sed -n 's/^[0-9]*[02468] /&/w even' <file 
```

이 예제는 입력 파일이 수정하지 않으므로 사실 출력 파일이 필요하지 않습니다. “w” 와 파일 이름 사이에는 반드시 하나의 공백이 있어야 합니다!
...
“/I”  와 “p” (print) 명령어 사이에 공백을 넣은 것은 여기서 사용한 “p” 는 패턴 매칭 과정의 수식어(플래그)가 아니라 패턴 매칭 이후 실행할 명령어임을 강조하기 위함입니다.
...
이 방법은 하나의 프로세스 대신 두 개의 프로세스를 사용합니다. sed 전문가는 하나의 프로세스로 해결할 수 있을 때 두 개의 프로세스를 절대로 사용하지 않습니다.
...
이전 예제는 “-e”  옵션이 필요하지 않았습니다. sed는 항상 “하나의 명령어가 있다” 는 것을 잘 알고 있기 때문입니다. sed에서 하나의 인수만 주어지면 그것은 반드시 명령어이며 표준 입력에서 읽은 데이터를 편집합니다.
...
sed는 grep을 불필요하게 만드는 좀 더 많은 처리 방법이 있습니다. 그리고 “grep -c” 는 “wc -l” 명령을 대체할 수 있습니다. grep의 일부 기능을 어떻게 복제할 수 있는지는 나중에 설명하겠습니다.
...
PATTERN 이 file 에 없다면 cat 처럼 동작합니다: 즉, 아무 것도 변경하지 않고 그대로 출력합니다. PATTERN 이 파일에 있다면 이를 포함한 줄을 두 번 출력합니다. 
...
## Using 'sed /pattern/'
sed는 명령어 앞에 "주소" 를 지정해서 특정 줄을 검사하거나 수정할지 지정할 수 있습니다. 지금은 가장 간단한 “/PATTERN/” 패턴 주소만 설명하겠습니다. 이는 패턴과 일치하는 줄에만 명령어가 적용됩니다. 간단히 말해서 “/p” 플래그와 함께 사용하면 일치하는 줄을 두 번 출력합니다:
...
또한, sed를 직접 실행하는 스크립트 작성 방법은 다음을 참고합니다.


## sed in shell scripts
쉘에서 입력할 명령어가 많아서 한 줄에 깔끔하게 들어가지 않을 경우, 백슬래시 문자를 사용해서 줄을 나누어 사용합니다:
...
C 셸에서 여러 줄에 걸친 큰 sed 스크립트를 사용할 수 있지만, 인용문이 여러 줄에 걸쳐 계속된다는 것을 C 셸에 알려야 합니다. 이를 위해 각 줄 끝에 백슬래시를 배치합니다:
...
sed를 실행하는 또 다른 방법은 인터프리터 스크립트로 사용하는 것입니다. 

우선 다음과 같은 내용을 포함하는 파일을 생성합니다:
...
이 스크립트가 “CapVowel” 이란 파일에 저장되고 실행 가능한 상태라면, 다음과 같이 간단한 명령어 형태로 사용할 수 있습니다:
...
sed를 호출하는 셸 스크립트에 단어(매개 변수)를 전달하는 것은 UNIX 쉘 인용 메커니즘에 대한 튜토리얼을 기억한다면 처리하기 쉽습니다. 복습하면, 인용을 켜고 끄려면 작은따옴표를 사용합니다. sed로 grep 기능을 모방하는 간단한 셸 스크립트는 다음과 같습니다:
...
sed를 사용해서 사용자에게 매개 변수를 입력하도록 요청한 후, 입력한 매개 변수로 채워진 파일을 생성할 수 있습니다. 우선 가짜 값이 포함된 파일을 생성한 다음 sed로 가짜 값을 변경할 수 있습니다. 

좀 더 간단한 방법은 쉘의 “here document” 기능을 사용하는 것으로 셸 “스크립트의 일부 텍스트를 표준 입력처럼 사용” 합니다:
...
이것은 매우 인위적인 예제임을 인정합니다. 사실 “Here document” 는 sed를 전혀 사용하지 않고도 값을 평가할 수 있습니다. 

따라서, 다음 예제도 위와 동일한 작업을 수행합니다:
...
그러나, “here document” 와 sed를 결합하면 일부 복잡한 경우에 유용할 수 있습니다.
...
여기서 사용자가 “a b c” 처럼 공백이 포함된 답을 입력하면 구문 오류가 발생합니다. 좀 더 나은 방법은 입력 값을 따옴표로 묶는 것입니다:
...
따옴표에 관한 제 튜토리얼에서 이 내용에 대해 이미 다뤘습니다.
...
sed는 이 모든 것과 그 이상도 처리할 수 있습니다. sed의 모든 명령어는 위의 목록처럼 단일 주소나 주소 범위 같은 제한 조건을 앞에 붙일 수 있습니다. 즉, 제한 조건이나 주소는 다음과 같이 명령어 앞에 위치합니다:
...
## Patterns
vi나 more 같은 유닉스 유틸리티는 정규 표현식 검색에서 슬래시 문자를 사용합니다. sed도 동일한 규칙을 따르며, 표현식은 슬래시로 종료해야 합니다. 

예를 들어, 모든 줄에서 “#” 으로 시작하는 첫 번째 숫자를 삭제하려면 다음과 같이 사용합니다:
...
만약, 검색 패턴이 백슬래시 문자로 시작하면 그 다음 문자는 "구분자" 로 인식합니다!! 

따라서, 슬래시 대신 쉼표를 구분자로 사용하려면 다음과 같이 사용합니다:

```
sed '\,^#, s/[0-9][0-9]*//'
```

이 기능의 장점은 “슬래시 문자까지 검색할 수 있다” 는 것입니다. 예를 들어, “/usr/local/bin” 문자열을 검색해서 “/common/all/bin” 으로 변경하려면 다음과 같이 백슬래시를 사용해서 슬래시를 이스케이프 처리해야 합니다:

```
sed '/\/usr\/local\/bin/ s/\/usr\/local/\/common\/all/'
```

정규식으로 검색할 때 슬래시 대신 밑줄 문자를 사용한다면 좀 더 이해하기 쉬울 수 있습니다. 다음 예제는 검색 명령과 대체 명령 모두에서 밑줄 구분자를 사용합니다:

```
sed '\_/usr/local/bin_ s_/usr/local_/common/all_'
```

이것이 바로 "sed 스크립트는 난해하다!" 는 평을 얻는 이유를 정확히 보여줍니다. 일부러 좀 더 까다롭게 만들어 “g” 로 시작하는 모든 줄을 찾고 그 줄 내의 각 “g” 를 “s” 로 바꾸는 다음과 같은 예제를 보여줄 수도 있습니다:

```
sed '/^g/s/g/s/g'
```

이제 대체 명령어 뒤에 공백을 추가하고 밑줄 구분자를 사용한다면 가독성이 크게 향상됩니다:
...
“$” 기호는 cat, vi, ed 같은 유틸리티도 “마지막” 을 의미하는 관례 중 하나입니다. 

“cat” 명령어로 여러 파일을 편집할 경우 줄 번호는 누적됩니다. 즉,
...
## Ranges by patterns
범위로 두 개의 정규식을 지정합니다. “#” 이 주석 시작을 의미한다면 특정 키워드를 검색한 후 두 번째 키워드가 나타날 때까지의 모든 주석을 제거합니다. 이 경우 사용할 두 개의 키워드는 “start” 와 “stop” 입니다.
...
“start” 패턴이 발견되면 “start” 가 포함된 행부터 시작한다는 점을 잘 알아야 합니다. 이것은 스위치를 켭니다. 이 스위치는 행 단위로 작동합니다. 즉, 다음 행이 읽히고 대체 명령이 수행됩니다. “stop” 이 포함되면 이 스위치는 꺼집니다. 이 스위치는 단어 단위가 아니라 행 단위로 작동합니다.
...
이 예제는 두 키워드 사이의 모든 주석을 제거합니다:

```
sed -e '1,/start/ s/#.*//' -e '/stop/,$ s/#.*//'
```

이 예제는 “/start/,/stop/” 범위와 중복되는 범위를 가집니다. 이는 두 범위 모두 키워드가 포함된 줄까지 처리 대상으로 인식하기 때문입니다. 지정된 패턴이 포함된 줄까지(해당 줄 제외) 명령을 제한하는 방법은 나중에 설명하겠습니다. 이것은 패턴 범위 내에서 패턴 제외 작업에 해당하지만, 먼저 몇 가지 기본 원칙을 다뤄야 합니다.

다양한 명령어를 논의하기 전에, 일부 명령어는 “범위로 적용할 수 없다” 는 점에 대해 설명해야 합니다. 해당 명령어를 언급할 때 다시 알려 드리겠습니다. 다음 섹션에는 세 가지 명령어를 설명할 예정인데, 그 중 하나는 범위로 적용할 수 없습니다.
