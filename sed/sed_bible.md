...
이 예제는 sed 편집기의 `s` (substitute)명령어를 사용합니다. `s` 명령어는 슬래시 사이에 지정한 첫 번째 텍스트 문자열 패턴을 두 번째 텍스트 문자열로 대체합니다. 이 예제는 test 단어가 big test 로 대체됩니다.

이 예제를 실행하면 결과가 즉시 표시됩니다. 이것이 바로 sed 편집기의 강력함입니다. 대화형 편집기 중 일부가 시작되는 데 걸리는 시간과 거의 동일한 시간에 데이터에 여러 번의 편집을 수행할 수 있습니다.
...
sed는 데이터를 즉시 실행하고 반환합니다. 데이터의 각 줄을 처리할 때마다 결과가 곧바로 표시됩니다. sed 편집기가 전체 파일을 처리하기 전에 결과를 확인할 수 있습니다.

sed 편집기는 "텍스트 파일의 데이터를 수정하지 않는다!" 는 것을 기억합니다. 수정된 텍스트만 STDOUT 으로 출력할 뿐입니다. 

원본 텍스트 파일을 확인해 보면 여전히 원본 데이터가 포함되어 있습니다:
...
닫는 따옴표가 나타난 줄에서 명령어를 반드시 완료해야 합니다. bash 셸은 닫는 따옴표를 감지한 후 명령어를 처리합니다.

sed 명령어는 실행되면 파일의 각 데이터 줄에 지정한 명령어를 차례대로 적용합니다.


#### Reading editor commands from a file
마지막으로, 처리할 sed 명령어가 많다면 별도의 파일에 저장하는 것이 좋습니다. 

sed 명령어에 `-f` 옵션을 사용해서 지정한 파일을 실행합니다:
...
이 경우 각 명령어 뒤에 세미콜론을 붙이지 않습니다. sed 편집기는 각 줄이 별도의 명령어를 포함한다는 것을 인식합니다. 명령줄에서 명령어를 입력한 것과 마찬가지로, sed 편집기는 지정한 파일에서 명령어를 읽어 데이터 파일의 각 줄에 적용합니다.
...
sed 편집기를 성공적으로 사용하기 위한 핵심은 텍스트 편집을 맞춤 설정할 수 있는 명령어와 사용 형식을 숙지하는 것입니다. 이번 섹션은 sed 편집기를 스크립트에 통합할 수 있는 기본 명령어와 기능 몇 가지에 대해 설명합니다.



## Introducing more substitution options
`s` 명령어로 줄의 텍스트를 새로운 텍스트로 대체하는 방법을 이미 살펴 봤습니다. 대체 명령어는 작업을 좀 더 편리하게 만드는 몇 가지 추가적인 옵션이 있습니다.
...
- `g` 모든 발생 위치에 새로운 텍스트를 치환합니다.
- `p` 원본 줄의 내용을 출력합니다.
- `w` 파일 치환 결과를 파일에 기록합니다.

첫 번째 플래그는 sed 편집기가 새로운 텍스트로 치환할 일치 패턴의 발생 위치를 지정합니다:
...
플래그를 `2` 로 지정해서 sed 편집기는 각 줄 내에서 두 번째로 나타나는 패턴만 대체합니다. `g` 대체 플래그를 사용하면 모든 발생을 대체합니다:
...
`-n` 옵션은 sed 편집기의 기본 출력 기능을 억제합니다. `p` 플래그는 수정된 모든 줄을 출력합니다. 따라서, 두 가지를 함께 사용하면 대체 명령으로 수정된 줄만 출력합니다.
...
일반적으로 sed 편집기는 STDOUT 에 출력하지만, 패턴을 포함한 행만 지정한 파일에 저장합니다.


### Replacing characters
때로는 대체 문자열 패턴에 사용하기 어려운 문자를 만나기도 합니다. 리눅스 세계에서 흔한 예로는 슬래시(`/`) 문자가 있습니다. 따라서, 파일 경로명을 대체하는 작업은 까다로울 수 있습니다. 
...
## Using addresses
기본적으로 sed 편집기의 명령어는 데이터의 모든 줄에 적용됩니다. 특정 줄이나 줄 그룹에 명령어를 한정적으로 적용하려면 주소 지정 방식을 사용해야 합니다.
...
두 형태 모두 주소를 지정할 때 동일한 형식을 사용합니다:
...
다음 섹션은 sed 편집기에서 두 가지 주소 지정 기법을 모두 사용하는 방법을 보여줍니다.
