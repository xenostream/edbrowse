# Introducing sed 
지금까지 셸 스크립트를 사용하는 가장 흔한 용도 중 하나는 텍스트 파일 작업입니다. 로그 파일 검토, 설정 파일 읽기, 데이터 처리 등 셸 스크립트는 텍스트 파일에 포함된 모든 유형의 데이터를 조작하는 단조로운 작업을 자동화하는 데 도움이 될 수 있습니다.

그러나, 셸 스크립트 명령만 사용해서 텍스트 파일 내용을 조작하려면 다소 불편할 수도 있습니다. 셸 스크립트로 어떤 형태의 데이터 조작을 수행한다면 리눅스에서 사용할 수 있는 `sed` 도구에 익숙해져야 합니다. 이 도구는 모든 데이터 처리 작업을 크게 단순화할 수 있습니다.


<br><br>
## Manipulating Text
리눅스 환경에서 사용할 수 있는 다양한 편집기 프로그램으로 텍스트 파일을 편집하는 방법이 있습니다. 이런 편집기를 사용하면 간단한 명령어나 마우스 클릭만으로 텍스트 파일에 포함된 텍스트를 쉽게 조작할 수 있습니다.

하지만, 때로는 대화형 텍스트 편집기를 실행하지 않고 즉석에서 텍스트 파일 내의 텍스트를 조작할 때가 있습니다. 이럴 때 텍스트를 자동으로 서식을 지정하거나 삽입, 수정, 삭제할 수 있는 간단한 명령줄 편집기가 있다면 유용합니다.

리눅스 시스템은 바로 이를 위한 일반적인 도구를 제공합니다. 이번 섹션은 리눅스 세계에서 가장 널리 사용되는 명령줄 편집기인 `sed` 에 대해 설명합니다.


### Getting to know the sed editor
sed 편집기는 일반적인 대화형 텍스트 편집기와 달리 “스트림 편집기” (**S**tream **ED**itor)라고 불립니다. vim 같은 대화형 텍스트 편집기는 키보드나 명령을 사용해서 데이터의 텍스트를 삽입, 삭제 또는 교체합니다. 스트림 편집기는 데이터를 처리하기 전에 미리 제공한 “규칙 세트를 기반으로 데이터 스트림을 편집” 합니다.

sed 편집기는 명령줄에서 직접 입력하거나 명령 파일에 저장한 명령으로 데이터 스트림의 데이터를 조작할 수 있습니다. 
...
4. 새로 변경된 데이터를 STDOUT 으로 출력합니다.

스트림 편집기는 읽어 온 데이터 한 줄에 모든 명령어 매칭을 완료한 후 다음 데이터 줄을 읽고 이 과정을 반복합니다. 스트림 내의 모든 데이터 줄을 처리한 후 스트림 편집기는 종료합니다.

이처럼 `명령이 줄 단위로 순차적으로 적용` 되기 때문에 sed 편집기는 데이터 스트림을 한 번만 통과하면서 편집을 수행합니다. 이로 인해 sed 편집기는 대화형 편집기 보다 훨씬 빠르며 파일 내의 데이터를 즉석에서 신속하게 변경할 수 있습니다.
...
`options` 매개 변수로 sed의 동작을 사용자 정의할 수 있으며 다음과 같은 옵션이 포함됩니다.

| 옵션 | 설명 |
| --- | --- |
| `-e` | 실행할 명령어에 스크립트에서 지정한 명령어를 추가합니다 |
| `-f` | 실행할 명령어에 파일에서 지정한 명령어를 추가합니다 |
| `-n` | 각 명령에 대한 출력을 생성하지 않고 print 명령을 기다립니다 |

`script` 매개 변수는 스트림 데이터에 적용할 단일 명령을 지정합니다. 하나 이상의 명령이 필요한 경우 명령줄에서 지정하려면 `-e` 옵션을 사용하거나 명령을 파일에 저장한 후 `-f` 옵션을 사용해야 합니다.
...
기본적으로 sed 편집기는 지정한 “명령어를 STDIN 입력 스트림에 적용” 합니다. 이를 통해 데이터를 sed 편집기로 파이프해서 처리할 수 있습니다. 
...
예제는 sed 편집기의 `s` (substitute)명령어를 사용합니다. `s` 명령어는 슬래시 사이에 지정한 첫 번째 텍스트 문자열 패턴을 두 번째 텍스트 문자열로 대체합니다. 예제는 test 단어를 big test 로 대체합니다.

예제를 실행하면 결과는 즉시 표시됩니다. 이것이 바로 “sed 편집기의 강력함” 입니다. 대화형 편집기 중 일부가 시작되는 데 걸리는 시간과 거의 동일한 시간에 데이터에 여러 번의 편집을 수행할 수 있습니다.

물론 예제는 간단한 데이터 한 줄만 편집했습니다. 전체 데이터 파일을 편집하더라도 빠른 결과를 얻을 수 있습니다:
...
sed는 즉시 실행하고 결과 데이터를 반환합니다. 데이터의 각 줄을 처리할 때마다 결과는 곧바로 표시됩니다. sed 편집기가 전체 파일을 처리하기도 전에 결과를 확인할 수 있습니다.

sed 편집기는 "파일 데이터를 수정하지 않는다!!" 는 것을 항상 기억해야 합니다. 수정된 텍스트만 STDOUT 으로 출력할 뿐입니다. 원본 파일을 확인하면 여전히 원본 데이터가 그대로 포함되어 있습니다:
...
명령줄에서 두 개 이상의 sed 명령어를 실행하려면 `-e` 옵션을 사용합니다:
...
두 명령어 모두 파일 내의 각 데이터 행에 적용됩니다. 명령어는 세미콜론으로 구분하며 명령어 끝과 세미콜론 사이에 공백은 없어야 합니다.

명령을 구분할 때 세미콜론 대신 bash 셸의 보조 프롬프트를 사용할 수 있습니다. 단일 따옴표(`'`)를 입력하면 sed 프로그램 스크립트(sed 편집기 명령어 목록)가 열리고 닫는 따옴표(`'`)를 입력할 때까지 bash 셀은 추가 명령을 계속 요청합니다:
...
닫는 따옴표가 나타난 줄에서 명령어를 완료해야 합니다. bash 셸은 닫는 따옴표를 감지한 후 명령어를 처리합니다.

sed 명령어는 파일의 각 데이터 줄에 지정한 명령어를 차례대로 적용합니다.


#### Reading editor commands from a file
마지막으로 처리할 sed 명령어가 많다면 별도의 파일에 저장하는 것이 좋습니다. 

sed는 `-f` 옵션을 사용해서 지정한 파일로 실행합니다:
...
이 경우 각 명령어 뒤에 세미콜론을 붙이지 않아도 됩니다. sed 편집기는 "각 줄이 별도의 명령어를 포함한다!" 는 것을 인식합니다. 명령줄에서 명령어를 입력한 것과 마찬가지로 sed 편집기는 지정한 파일에서 명령어를 읽어 데이터 파일의 각 줄에 적용합니다.

> [!TIP]
> sed 편집기 스크립트 파일은 bash 셸 스크립트 파일과 혼동하기 쉽습니다. 이런 혼란을 방지하려면 sed 스크립트 파일의 확장자를 `.sed` 로 지정합니다.
...
sed 편집기를 성공적으로 사용하기 위한 핵심 사항은 텍스트 편집 작업을 맞춤 설정할 수 있는 “명령어와 사용 형식을 숙지하는 것” 입니다. 이번 섹션은 sed를 스크립트에 통합할 수 있는 기본적인 명령어와 기능 몇 가지에 대해 설명합니다.


## Introducing more substitution options
`s` 대체 명령어로 줄의 텍스트를 새로운 텍스트로 대체하는 방법은 이미 살펴 봤습니다. 대체 명령어는 작업을 좀 더 편리하게 만드는 몇 가지 추가적인 옵션이 있습니다.


### Substituting flags
대체 명령어가 텍스트 문자열에서 일치하는 패턴을 대체하는 방식에는 주의할 점이 있습니다.

다음 예제로 어떤 일이 발생하는지 살펴보겠습니다:
...
대체(substitute) 명령어는 여러 줄의 텍스트를 제대로 대체했지만 기본적으로 각 줄 내에서 "첫 번째 발생 위치만 대체" 합니다. 

줄 내의 다른 발생 위치도 substitute 명령어로 대체하려면 “플래그를 사용” 해야 합니다. 대체 플래그는 대체 문자열 뒤에 설정합니다:

```
s/pattern/replacement/flags
```

네 가지 유형의 대체 플래그를 사용할 수 있습니다:

| 플래그 | 설명 |
| --- | --- |
| `숫자` | 새로운 텍스트로 치환할 패턴의 발생 위치를 지정합니다 |
| `g` | 모든 발생 위치에 새로운 텍스트를 대체합니다 |
| `p` | 원본 줄의 내용을 출력합니다 |
| `w` | 대체 결과를 파일에 기록합니다 |

첫 번째 숫자 플래그는 sed 편집기가 새로운 텍스트로 대체할 일치 “패턴의 발생 위치를 지정” 합니다:
...
대체 플래그를 `2` 로 지정하면 sed 편집기는 각 줄 내에서 두 번째로 나타나는 패턴만 대체합니다. `g` 대체 플래그를 사용하면 모든 발생 위치를 대체합니다:
...
`p` 플래그는 일치하는 패턴을 포함한 줄을 출력합니다. 주로 `-n` sed 옵션과 함께 사용합니다:
...
`-n` 옵션은 sed 편집기의 “기본 출력 기능을 억제” 합니다. `p` 플래그는 수정한 모든 줄을 출력합니다. 따라서, 두 가지를 함께 사용하면 “대체 명령으로 수정한 줄만 출력” 할 수 있습니다.

`w` 플래그는 동일한 출력을 생성하지만 지정한 파일에 출력 결과를 저장합니다:
...
일반적으로 sed 편집기는 STDOUT (표준 출력)에 출력하지만 패턴을 포함한 행만 지정한 파일에 저장합니다.


### Replacing characters
때로는 대체 문자열에 사용하기 어려운 문자를 만나기도 합니다. 리눅스 세계에서 가장 흔한 예로는 슬래시(`/`) 문자가 있습니다. 따라서, 파일 경로명을 대체하는 작업은 까다로울 수 있습니다. 

예를 들어, /etc/passwd 파일에서 bash 셸을 C 셸로 대체하려면 다음과 같이 처리해야 합니다:

```
$ sed 's/\/bin\/bash/\/bin\/csh/' /etc/passwd
```

슬래시(`/`)는 구분자로 사용되기 때문에 패턴 텍스트에 슬래시가 나타나면 이를 이스케이프하기 위해 백슬래시를 사용해야 합니다. 이는 종종 혼란과 실수를 초래합니다.

이런 문제를 해결하기 위해 sed 편집기는 “대체 명령의 구분자를 다른 문자로 선택할 수 있도록 허용” 합니다:

```
$ sed 's!/bin/bash!/bin/csh!' /etc/passwd
```

예제는 구분자로 느낌표를 사용해서 경로 이름을 훨씬 더 쉽게 읽고 이해할 수 있도록 만듭니다.


## Using addresses
“기본적으로 sed 편집기의 명령어는 모든 줄에 적용” 됩니다. 특정 줄이나 줄 범위에만 명령어를 한정적으로 적용하려면 주소 지정 방식을 사용해야 합니다.

sed 편집기는 두 가지 형태의 주소 지정 방식이 있습니다:

- 숫자로 구성된 단일 줄이나 범위
- 특정 줄만 걸러내는 텍스트 패턴

두 형태 모두 주소를 지정할 때는 다음과 동일한 형식을 사용합니다:

```
[address]command
```

특정 주소에 여러 명령을 그룹화해서 처리할 수 있습니다:
...
sed 편집기는 명령 그룹으로 지정한 각각의 명령을 지정한 주소와 일치하는 행에만 적용합니다. 

다음 섹션은 sed 편집기에서 두 가지 주소 지정 방식을 사용하는 방법을 보여줍니다.


### Addressing the numeric line
숫자 기반 행(라인) 주소 지정 방식을 사용할 때는 텍스트 스트림 내의 행 위치를 참조합니다. 

sed 편집기는 텍스트 스트림의 첫 번째 행을 1번 행으로 지정하고 이후 각 행에 순차적으로 번호를 누적하면서 부여합니다. 명령에 지정할 주소는 “단일 행 번호” 나 “시작 행 번호, 쉼표, 종료 행 번호” 로 지정한 행 범위일 수 있습니다. 

다음은 sed 명령이 적용될 단일 행 번호를 지정한 예제입니다:
...
sed 편집기는 지정한 주소인 두 번째 줄의 텍스트만 수정합니다.

다음은 주소 범위를 사용하는 또 다른 예제입니다:
...
텍스트 내의 특정 지점부터 시작해서 텍스트 끝까지 이어지는 줄 그룹에 명령을 적용하려면 특수 주소인 달러 기호를 사용할 수 있습니다:
...
텍스트에 포함된 데이터 줄 수는 미리 알 수 없기 때문에 달러 주소 기호는 유용합니다.


### Using text pattern filters
명령어가 적용될 줄을 제한하는 또 다른 방법은 조금 복잡합니다. sed 편집기는 명령어를 적용할 줄을 필터링하는 텍스트 패턴을 지정할 수 있습니다.
...
지정한 패턴은 슬래시로 묶어야 합니다. sed 편집기는 지정한 텍스트 패턴이 포함된 줄에만 명령을 적용합니다.

예를 들어, Samantha 사용자의 기본 셸을 변경하려면 다음과 같은 sed 명령을 사용합니다:
...
위의 명령어는 일치하는 텍스트 패턴이 포함된 줄에만 적용됩니다. 

위의 사용자 ID 예제처럼 특정 값을 필터링할 때 고정된 텍스트 패턴을 사용하는 것이 유용할 수 있지만 이를 통해 수행할 수 있는 작업은 어느 정도 한계가 있습니다. sed 편집기는 텍스트 패턴에 "정규 표현식" 을 사용해서 상당히 복잡한 패턴을 생성할 수 있습니다.

정규 표현식을 사용하면 모든 종류의 데이터에 대응할 수 있는 “고급 텍스트 패턴 일치” 를 만들 수 있습니다. 와일드카드, 특수 문자 및 고정된 텍스트 문자를 결합해서 거의 모든 텍스트에 대응할 수 있는 간결한 패턴을 생성할 수 있습니다.

정규 표현식은 셸 스크립트 프로그래밍에서 “가장 어려운 부분 중 하나” 이며 이후 장에서 자세히 다룹니다.


## Grouping commands
개별 명령줄에서 두 개 이상의 명령을 수행할 경우 “중괄호로 명령을 그룹화” 합니다. sed 편집기는 지정한 주소에 나열한 각각의 명령을 모두 처리합니다:
...
두 명령어 모두 해당 주소에만 처리됩니다. 물론 그룹 명령어 앞에는 주소 범위를 지정할 수 있습니다:
...
sed 편집기는 주소 범위에 속하는 모든 줄에 그룹 내의 모든 명령을 적용합니다.


## Deleting lines
sed 편집기에서 사용할 수 있는 명령은 텍스트 대체 명령만 있는 것이 아닙니다. 텍스트 스트림에서 특정 줄을 삭제할 경우 delete 명령을 사용할 수 있습니다.

삭제 명령어(`d`)는 이름 그대로 작동합니다. 주소 지정 방식에 일치하는 텍스트 줄을 삭제합니다. 하지만, 삭제 명령을 사용할 때는 주의할 점이 있습니다. 주소 지정 방식을 잊으면 스트림 내의 모든 줄을 삭제합니다:
...
delete 명령어는 특정 주소와 함께 사용할 때 유용합니다. 이를 통해 데이터 스트림 내에서 특정 텍스트 줄만 삭제할 수 있으며 다음과 같이 줄 번호를 지정하거나:
...
특수한 파일 끝 문자(EOL) 기호를 사용할 수 있습니다:
...
sed 편집기의 패턴 일치 기능은 삭제 명령에도 그대로 적용됩니다:
...
sed 편집기는 지정한 패턴과 일치하는 텍스트가 포함된 줄만 제거합니다.

> [!NOTE]
> sed 편집기는 "원본 파일을 건드리지 않는다!" 는 점을 항상 기억해야 합니다. 삭제한 줄은 sed 편집기의 출력에서 사라질 뿐입니다. 원본 파일에는 여전히 “삭제된” 줄이 그대로 포함되어 있습니다.

텍스트 패턴 두 개를 사용해서 범위 주소를 삭제할 수도 있지만 이 경우도 주의해야 합니다.
...
또한, sed 편집기는 데이터 스트림에서 시작 패턴을 감지할 때마다 삭제 기능이 “활성화” 되므로 주의해야 합니다. 이는 예상치 못한 결과를 초래할 수도 있습니다:
...
“1” 이 포함된 다섯 번째 행에서 삭제 명령을 다시 트리거하면서 데이터 스트림의 나머지 행을 모두 삭제했습니다. 이는 중지 패턴이 인식되지 않았기 때문입니다.
...
첫 번째 패턴 일치 시 삭제 기능이 “활성화” 되나 종료 패턴 일치를 찾지 못하므로 전체 데이터 스트림은 삭제됩니다.


## Inserting and appending text
예상할 수 있듯이 다른 편집기와 마찬가지로 sed 편집기도 데이터 스트림에 텍스트를 삽입하거나 추가할 수 있습니다. 두 가지 동작의 차이점은 혼란스러울 수 있습니다:

- 삽입 명령(`i`)은 줄 앞에 새 줄을 추가합니다.
- 추가 명령(`a`)은 줄 뒤에 새 줄을 추가합니다.

이 두 가지 명령이 혼란스러운 점은 사용 형식 때문입니다. 단일 명령줄에 이 명령을 사용할 수 없습니다!! 삽입하거나 추가할 줄은 별도의 줄에 단독으로 지정해야 합니다. 
...
새 줄에 입력한 텍스트는 sed 편집기에서 사용한 명령에 따라 다른 위치에 추가됩니다. insert 명령은 “데이터 스트림 앞 줄에 추가된다” 는 점을 기억해야 합니다:
...
append 명령어은 데이터 스트림 뒷 줄에 추가합니다:
...
명령줄에서 사용할 때 새로운 데이터 줄을 입력하라는 보조 프롬프트가 표시됩니다. 이 줄에서 명령을 완료해야 합니다. 종료 단일 따옴표를 입력하면 bash 셸은 명령을 완료하고 처리합니다:
...
데이터 스트림의 앞 줄이나 뒷 줄에 텍스트를 추가하는 방법도 효과적이지만 데이터 스트림 내부에 어떻게 추가할까요?

데이터 스트림 내부에 삽입하거나 추가하려면 sed 편집기에게 데이터가 표시될 위치를 알려주는 주소 지정 방식을 사용해야 합니다. 이 명령은 단일 행 주소만 지정할 수 있습니다! 즉, 숫자 행 번호나 텍스트 패턴을 일치시킬 수 있지만 주소 범위는 사용할 수 없습니다!! 이는 매우 논리적인데 행 범위가 아닌 단일 행 앞이나 뒤에만 삽입하거나 추가할 수 있기 때문입니다.

다음은 데이터 스트림의 3행 앞에 행을 삽입하는 예제입니다:
...
데이터 스트림의 3번째 줄 뒤에 줄을 추가하는 예제는 다음과 같습니다:
...
이 명령어는 insert 명령어와 동일한 프로세스를 사용합니다. 단지 줄을 지정한 줄 번호 뒤에 배치할 뿐입니다.

여러 줄로 구성된 데이터 스트림이 있고 데이터 스트림 끝 부분에 줄을 추가하려면 달러 기호(`$`)를 사용합니다. 이 기호는 데이터의 마지막 줄을 의미합니다:
...
데이터 스트림의 시작 부분에 줄을 추가하는 경우도 이와 동일한 원리가 적용됩니다. 즉, 첫 번째 줄 앞에 줄을 삽입하면 됩니다.

하나 이상의 텍스트 줄을 삽입하거나 추가하려면 삽입하거나 추가하려는 마지막 줄에 도달할 때까지 각 줄 끝에 백슬래시를 추가해야 합니다:
...
이 경우 지정한 두 줄 모두 데이터 스트림에 추가됩니다.



## Changing lines
change 명령어는 데이터 스트림의 내용을 변경할 수 있습니다.

이 명령어는 insert 및 append 명령어와 동일한 방식으로 작동하며 변경할 줄은 sed 명령어의 나머지 부분과 별도로 지정해야 합니다:
...
이 예제에서 sed 편집기는 3번째 줄의 텍스트를 변경합니다. 주소 부분에 텍스트 패턴을 사용할 수 있습니다:
...
변경 명령어는 데이터 스트림 내에서 “일치하는 모든 텍스트 줄을 변경” 합니다.
...
변경 명령어로 주소 범위를 사용할 수 있지만 처리 결과는 예상과 다를 수 있습니다:
...
sed 편집기는 텍스트가 포함된 두 줄 모두를 변경하는 대신 단일 텍스트 줄로 두 줄을 변경합니다.



## Transforming characters
변환 명령어(`y`)는 단일 문자에 적용하는 유일한 sed 편집기 명령어입니다.
...
transform 명령어는 inchars 값과 outchars 값 간의 “일대일 매핑” 을 수행합니다. inchars 의 첫 번째 문자는 outchars 의 첫 번째 문자로 변환됩니다. inchars 의 두 번째 문자는 outchars 의 두 번째 문자로 변환됩니다. 이러한 매핑 작업은 지정한 문자 전체 길이에 걸쳐 계속됩니다. inchars 와 outchars 의 길이가 같지 않으면 sed 편집기는 오류 메시지를 생성합니다.
...
출력 결과에서 알 수 있듯이 inchars 패턴에 지정한 각 문자 인스턴스는 outchars 패턴의 동일한 위치의 문자로 대체됩니다.

또한, transform 명령어는 “전역 명령어” 입니다. 즉, 발생 횟수와 관계 없이 발견되는 모든 문자에 자동으로 변환을 수행합니다:
...
sed 편집기는 일치하는 문자 1 의 인스턴스 모두를 변환했습니다.

특정 발생 위치에 대한 변환을 제한할 수 없습니다.



## Printing revisited
“추가 대체 옵션 소개” 섹션에서 sed 편집기로 변경한 줄을 표시하기 위해 대체 명령어와 함께 `p`플래그를 사용하는 방법을 설명했습니다. 

또한 데이터 스트림에서 정보를 출력하는 데 사용할 수 있는 세 가지 출력 명령어가 있습니다:

- 텍스트 줄을 출력하는 `p` 명령어
- 줄 번호를 출력하는 등호(`=`) 명령어
- 줄을 나열하는 `l` (소문자 L) 명령어

다음 섹션은 sed 편집기에서 이 세 가지 출력 명령어에 대해 살펴 보겠습니다.


### Printing lines
대체 명령어의 `p` 플래그와 마찬가지로, `p` 명령어는 sed 편집기 출력에 한 줄을 출력합니다. 이 명령어 자체로는 그다지 흥미롭지 않습니다:
...
이 명령어는 이미 존재하는 텍스트를 중복 출력할 뿐입니다.

print 명령어의 가장 일반적인 사용 용도는 텍스트 패턴과 일치하는 텍스트를 포함하는 줄만 출력할 때입니다:
...
명령줄에 `-n` 옵션을 사용하면 다른 모든 줄의 출력은 생략하고 일치하는 텍스트 패턴이 포함된 줄만 출력할 수 있습니다. 이는 데이터 스트림에서 일부 줄만 빠르게 출력하는 방법으로 활용할 수 있습니다:
...
print 명령어는 substitution 이나 change 명령어로 라인을 변경하기 전에 확인 용도로 사용할 수 있습니다. 즉, 변경 전의 라인을 표시하는 스크립트를 만들 수 있습니다:
...
이 sed 편집기 명령어는 숫자 3이 포함된 줄을 검색해서 두 가지 명령을 실행합니다. 먼저 `p` 명령으로 해당 줄의 원본 버전을 출력하고, 이어서 `s` 명령으로 텍스트를 대체하며 `p` 플래그를 함께 사용해서 결과 텍스트를 출력합니다. 출력 결과는 원본 줄 텍스트와 새로운 줄 텍스트 모두 표시됩니다.
...
sed 편집기는 줄 앞에 줄 번호를 출력합니다. 데이터 스트림에서 특정 텍스트 패턴을 검색할 때 등호 명령어는 유용합니다:
...
`-n` 옵션을 사용하면 sed 편집기가 일치하는 텍스트 패턴을 포함하는 줄의 줄 번호와 텍스트 모두 표시하도록 처리할 수 있습니다.


### Listing lines
list 명령어(`l`)는 데이터 스트림 내의 텍스트와 비표시 문자 모두 출력할 수 있습니다. 모든 비표시 문자는 백슬래시(`\`)로 시작하는 8진수 값 또는 일반적인 비표시 문자에 대한 표준 C 스타일 명명법(예: 탭 문자의 경우 `\t`)을 사용해서 표시됩니다:
...
탭 문자 위치는 `\t` 표기법으로 표시됩니다. 줄 끝의 달러 기호는 줄바꿈 문자를 의미합니다. 이스케이프 문자가 포함된 데이터 스트림이 있는 경우, list 명령어는 필요한 경우 8진 코드를 사용해서 이를 표시합니다:
...
대체 명령어는 파일 작업을 가능하게 하는 플래그가 포함되어 있습니다. 텍스트를 대체하지 않고도 이를 수행할 수 있는 일반적인 sed 편집기 명령어가 있습니다.


### Writing to a file
`w` 명령어는 파일을 저장하는 데 사용됩니다. `w` 명령어의 형식은 다음과 같습니다:

```
[address]w filename
```

파일명은 상대 경로나 절대 경로로 지정할 수 있지만, 어느 경우든 sed 편집기를 실행하는 사용자는 해당 파일에 대한 쓰기 권한이 있어야 합니다. 주소는 sed에서 사용하는 모든 유형의 주소 지정 방식(예: 단일 행 번호, 텍스트 패턴, 행 번호 범위 또는 텍스트 패턴 범위)을 사용할 수 있습니다.

다음은 데이터 스트림의 처음 두 줄을 파일에 저장하는 예제입니다:
...
물론, STDOUT 에 줄을 표시하지 않으려면 sed에 `-n` 옵션을 사용할 수 있습니다.
...
sed 명령줄에서 데이터 스트림에 데이터를 삽입하거나 추가하는 방법은 이미 살펴봤습니다. read 명령어(`r`)를 사용하면 별도의 파일에 포함된 데이터를 삽입할 수 있습니다.

다음은 read 명령어의 사용 형식입니다:
...
하지만, read 명령어는 주소 범위를 사용할 수 없습니다! 단일 행 번호 또는 텍스트 패턴 주소만 지정할 수 있습니다. 

sed 편집기는 주소 뒤에 파일의 텍스트를 삽입합니다.
...
sed 편집기는 데이터 파일의 모든 라인을 데이터 스트림에 삽입합니다. 패턴 주소를 사용할 때도 이와 동일한 기법이 적용됩니다:
...
데이터 스트림 끝에 텍스트를 추가하려면 달러 기호 주소 기호를 사용합니다:
...
read 명령어의 멋진 활용법 중 하나는 delete 명령어와 함께 사용해서 파일 내의 자리 표시자를 다른 파일의 데이터로 대체하는 것입니다. 

예를 들어, 다음과 같은 형식의 양식이 파일에 저장되어 있다고 가정해 보겠습니다:
...
양식화된 편지는 사람 목록 대신 일반적인 자리 표시자 LIST 를 사용합니다. 자리 표시자 뒤에 사람 목록을 삽입하려면 read 명령을 사용합니다. 그러나, 이 경우도 출력 결과에 자리 표시자 텍스트가 그대로 남아 있습니다. 이를 제거하려면 delete 명령을 사용합니다. 결과는 다음과 같습니다:
...
## Summary
셸 스크립트는 자체적으로 많은 작업을 수행할 수 있지만, 셸 스크립트만으로 데이터를 조작하는 것은 종종 어려울 수 있습니다. 리눅스는 텍스트 데이터 처리를 돕기 위해 편리한 유틸리티를 제공합니다. sed 편집기는 데이터를 읽으면서 실시간으로 빠르게 처리할 수 있는 스트림 편집기입니다. sed 편집기에 데이터에 적용할 편집 명령어 목록을 제공해야 합니다.

sed 프로그램 사용의 핵심은 정규 표현식 활용법을 정확히 아는 것입니다. 정규 표현식은 텍스트 파일에서 데이터를 추출하거나 조작하기 위한 맞춤형 필터를 만드는 핵심입니다.
