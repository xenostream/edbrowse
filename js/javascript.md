...
이 장의 서론에서, 우리는 구문 오류(syntax errors)와 런타임 오류(runtime errors)의 차이에 대해 논의했습니다. 구문 오류는 보통 잡기 쉽습니다. 왜냐하면 인터프리터(웹에서 JavaScript를 사용하는 경우 브라우저)가 즉시 이를 발견하고 코드 실행을 막기 때문입니다.  

런타임 오류는 다루기가 더 어렵습니다. 코드 실행이 시작된 후에만 발견될 수 있고, 문제가 있는 코드 부분이 실제로 실행될 때에만 드러나기 때문입니다. 그러나 여전히 매우 심각하게 다뤄야 합니다. 왜냐하면 런타임 오류가 발생하면 나머지 코드 실행이 중단되어 웹사이트/애플리케이션이 의도대로 작동하지 않게 만들 수 있기 때문입니다.  

처리되지 않은 런타임 오류는 흔히 "uncaught errors" 또는 "uncaught exceptions"라고 불리며, 이는 이를 잡을 수 있는 메커니즘을 가리키는 표현일 가능성이 큽니다. 바로 **try..catch 블록**입니다. 실패할 가능성이 있는 코드를 작성할 때는 항상 try..catch 블록으로 감싸는 것을 고려해야 합니다.  

---

### 예제  
다음 코드는 함수를 호출하지만, 예외가 발생합니다. 여기서는 함수를 선언하지 않았기 때문에 예외가 발생하지만, 실제 상황에서는 함수가 실패하면서 예외를 던질 수도 있습니다:  
...
결과적으로 콘솔에는 다음과 같은 오류가 나타납니다:  

```
Uncaught ReferenceError: AddNumbers is not defined
```  

그리고 어떤 alert도 보이지 않습니다. 왜냐하면 이 처리되지 않은 런타임 오류가 코드 실행을 중단시키기 때문입니다. 이제 try..catch 블록을 사용해 이를 고쳐봅시다.  

---

### try..catch 블록  
try..catch 블록은 두 부분(때로는 세 부분, 이에 대해서는 나중에 설명)을 가집니다.  
- **try 부분**: 오류가 발생할 수 있는 코드를 넣습니다.  
- **catch 부분**: try 부분에서 오류가 발생했을 때 실행되며, 문제를 해결하거나 사용자에게 알리거나, 혹은 아무것도 하지 않을 수 있습니다.  
...
여기서 몇 가지를 주목해야 합니다.  
1. 코드 자체는 여전히 실패합니다. 선언되지 않은 함수를 호출하고 있기 때문입니다. 그러나 이제 콘솔에 오류가 나타나지 않습니다. 왜냐하면 오류가 잡혔기 때문입니다.  
2. `"Done!"` alert가 나타납니다. 이전에는 보이지 않았지만, 이제 try..catch 블록 안에서 오류가 처리되었기 때문에 이후 코드가 정상적으로 실행됩니다.  
3. catch 부분에서 아무것도 하지 않았습니다. 상황에 따라 오류를 수정하려고 시도하거나, 로그를 남기거나, 사용자에게 알리거나, 혹은 단순히 무시할 수도 있습니다.  

---

### 오류 처리하기  
위 예제에서 `catch(error)`의 `(error)`가 무엇인지 궁금할 수 있습니다. 기본적으로 catch 문은 오류를 잡아, 마치 매개변수처럼 catch 블록에 전달합니다. 이름은 자유롭게 정할 수 있으며, JavaScript는 어떤 이름을 사용하든 오류 객체를 전달합니다.  

따라서 위 예제에서는 try 부분에서 발생한 오류가 `error`라는 매개변수로 catch 부분에 전달됩니다. 필요하지 않다면 무시할 수도 있습니다.  
...
여기서는 오류 매개변수의 정보를 사용해 오류를 설명했습니다. 이 정보는 로그를 남기거나, 사용자에게 더 친절한 메시지를 보여주는 등 다양한 방식으로 활용할 수 있습니다.  

---

### 선택적 catch 바인딩 (Optional catch binding)  
반대로, 오류 자체에는 관심이 없고 단순히 무시하고 싶을 때도 있습니다. 이전 예제처럼 여전히 매개변수를 선언해야 했지만, 이제는 그렇지 않아도 됩니다. ES2019 사양부터는 catch 블록에서 `(error)` 부분을 생략할 수 있습니다:  
...
이를 **선택적 catch 바인딩(optional catch binding)**이라고 부릅니다. 현재는 모든 최신 브라우저에서 지원되지만, 여전히 이를 지원하지 않는 오래된 브라우저도 많습니다. 따라서 이 기능을 코드에서 사용할지 여부는 프로젝트의 브라우저 호환성을 고려해야 합니다.  

---

### 요약 (Summary)  
JavaScript에서 오류 처리는 try..catch 블록을 사용하면 훨씬 쉬워집니다. 이는 JavaScript에만 국한되지 않고, C#, PHP, Java 등 많은 현대 프로그래밍 언어에서 약간씩 다른 형태로 존재합니다.  

잠재적으로 오류가 발생할 수 있는 코드를 작성할 때는, 해당 코드 블록을 try..catch로 감싸야 할지 고려해야 합니다. 그러나 코드 전체를 try..catch로 도배해 복잡하게 만들 필요는 없습니다. 다른 프로그래밍 작업과 마찬가지로, 적절한 균형을 찾는 것이 중요합니다.  

이번 글에서는 try..catch 블록의 가장 기본적인 사용법과 오류 처리 방법을 배웠습니다. 하지만 오류 처리에는 더 많은 내용이 있으며, 이는 다음 글에서 다루겠습니다.  
...
이전 글에서 우리는 try..catch 블록을 사용한 오류 처리에 대해 소개했습니다. 이번 글에서는 한 단계 더 깊이 들어가, 오류/예외를 다루기 위한 더 많은 도구들을 살펴보겠습니다.  

---

### try..catch..finally  
먼저, try/catch 구조의 세 번째 부분을 소개하겠습니다: **finally 부분**입니다. 이전 글에서 이야기했듯이, 오류가 발생할 수 있는 코드를 담는 **try 부분**이 있고, try 부분에서 오류가 발생했을 때만 실행되는 **catch 부분**이 있습니다. 여기에 선택적으로 추가할 수 있는 부분이 바로 **finally**입니다.  

finally 부분은 오류/예외 발생 여부와 상관없이, 앞의 두 부분이 끝난 후 반드시 실행되는 코드를 넣는 곳입니다. 일반적으로 finally 부분은 try 부분에서 시도한 작업 이후 정리(clean up)를 위해 사용됩니다. 예를 들어, 데이터베이스 연결을 열거나 파일을 열었는데 문제가 생겼다면, finally 부분에서 연결이나 파일을 닫아주는 식입니다.  
...
이 예제에서는 try 부분에서 오류가 발생하든, 모든 것이 정상적으로 동작하든, 데이터베이스 연결은 반드시 닫힙니다.  

finally 부분의 진정한 힘을 보여주는 예제를 보겠습니다:  
...
여기서 try 부분의 코드는 오류를 던지지 않으므로, 결과가 1일 것이라고 예상할 수 있습니다. 그러나 그렇지 않습니다. `return 1`을 시도하더라도, try..catch..finally 구조의 특성상 finally 부분이 마지막에 실행되며, 함수의 결과를 덮어써서 최종적으로 3을 반환합니다.  

---

### 오류 던지기 (Throwing errors)  
오류 처리는 단순히 오류를 잡는 것만이 아닙니다. 다른 프로그래머가 호출할 코드를 작성한다면, 코드 내부에서 발생할 수 있는 오류를 적절히 처리하고, 이를 코드 소비자에게 올바르게 전달하는 책임도 있습니다.  
...
이 가상의 시나리오에서, AddNumbers() 함수는 정수만 처리하도록 설계되었습니다. 그래서 간단한 검증을 추가했습니다. 정수가 아닌 값(예: 실수나 문자열)이 전달되면, 단순히 0을 반환합니다.  

이 경우, 우리가 함수의 작성자이자 소비자이므로 문제가 없지만, 만약 이 함수가 수백 개의 파일 중 하나에 묻혀 있는 대규모 라이브러리/애플리케이션의 일부라면 어떨까요? 그리고 함수가 단순한 것이 아니라 150줄짜리 복잡한 로직이라면 어떨까요?  

다른 프로그래머가 이 함수를 호출할 때, 매개변수가 정수여야 한다는 사실을 모를 수 있습니다. 정수가 아닌 값을 전달했을 때 단순히 0이 반환되면, 왜 그런 결과가 나왔는지 이해하지 못할 수 있습니다. 이런 경우에는 적절한 오류를 던져, 함수 소비자에게 잘못된 사용법을 알리는 것이 더 낫습니다:  
...
이제 정수가 아닌 값이 전달되면, 적절한 오류 메시지와 함께 오류를 던집니다. 이 메시지는 함수 소비자에게 무엇이 잘못되었는지 설명합니다. 이 예제를 실행하면 alert는 보이지 않고, 브라우저 콘솔에 우리가 정의한 오류가 표시됩니다.  

이제 오류 처리의 책임은 함수 소비자에게 넘어갑니다. 소비자는 오류를 잡아, 어떻게 처리할지 결정해야 합니다. 예를 들어, 사용자에게 상세한 메시지를 보여주거나, 단순히 "문제가 발생했습니다!" 같은 메시지를 보여줄 수 있습니다.  
...
### 요약 (Summary)  
- try..catch 블록의 (선택적인) finally 부분은 try..catch 구조가 끝날 때 반드시 실행되는 코드를 보장합니다. 오류 발생 여부와 상관없이 실행되며, 파일이나 원격 연결 같은 중요한 자원을 해제하는 데 자주 사용됩니다.  
- `throw` 키워드를 사용하면, 우리가 직접 오류를 던져 오류 처리 생태계에 참여할 수 있습니다. 이를 통해 무엇이 잘못되었는지 더 정확히 알릴 수 있습니다.  
- 나아가, 사용자 정의 오류(custom errors)를 만들 수도 있으며, 이는 다음 글에서 다룰 예정입니다.  
...
이 장의 이전 글들에서, 우리는 JavaScript에서 try..catch 및 try..catch..finally 블록을 사용해 오류를 잡는 방법에 대해 이야기했습니다. 또한 우리가 직접 오류를 던질 수도 있다는 점도 다뤘습니다.  

지금까지는 JavaScript 오류의 기반이 되는 일반적인 **Error** 타입만 다뤘지만, 사실 JavaScript에는 더 구체적인 버전의 오류 타입들이 존재합니다. 이는 서로 다른 오류 타입들을 구분하고 싶을 때 특히 중요합니다. 이번 글에서는 이러한 오류 타입들을 구분하는 방법을 다루고, 특정 문제 유형에 맞는 오류를 던질 수 있도록 사용자 정의 오류 타입을 만드는 방법도 살펴보겠습니다.  

---

### 오류 타입 (Error types)  
일반적인 Error 타입은 항상 최소한 문제를 설명하는 이름(name)과 메시지(message)를 제공합니다. 그러나 발생한 오류에 따라, JavaScript는 일반 Error 타입을 기반으로 하지만 더 많은 속성이나 메서드를 가진 구체적인 오류 타입을 사용할 수 있습니다.  

예를 들어, **RangeError**가 있습니다. 이는 여러 상황에서 발생할 수 있는데, 그중 하나는 BigInt를 0으로 나누려고 할 때입니다:  
...
이 코드를 실행하면 콘솔에 다음과 같은 오류가 표시됩니다:  

```
Uncaught RangeError: Division by zero
```  

보시다시피, 이제 단순한 Error가 아니라 RangeError가 발생했습니다. 이 오류를 잡을 때 이를 확인할 수 있으며, 다른 오류들과 구분하는 데 사용할 수도 있습니다.  

---

### 조건부 캐치 (Conditional catching)  
조건부 캐치라는 기법을 사용하면, 발생한 오류의 타입에 따라 다른 동작을 수행할 수 있습니다. 이는 여러 종류의 오류가 발생할 수 있는 복잡한 상황에서 매우 유용합니다.  

오류의 타입은 `name` 속성을 확인하거나, `instanceof` 연산자를 사용해 특정 타입과 비교함으로써 알 수 있습니다. 예를 들어:  
...
이전 글에서, 우리는 오류를 던져 호출자에게 무엇이 잘못되었는지 정확히 알릴 수 있다는 것을 보았습니다. 그러나 때로는 특정 오류 타입만 처리하고, 다른 오류들은 코드/함수 소비자가 처리하도록 하고 싶을 때가 있습니다.  

이런 상황에서는 조건부 캐치와 catch 블록에서 오류를 다시 던지는(re-throw) 기능을 결합할 수 있습니다. 예를 들어:  
...
여기서는 두 BigInt를 나누는 함수를 만들었습니다. 모든 오류를 잡지만, RangeError가 아닌 경우에는 단순히 다시 던집니다. 따라서 이 함수를 호출할 때는 우리가 직접 오류 처리를 해야 합니다.  

다음 예제에서는 반대로 RangeError만 다시 던지도록 바꾸고, 함수를 호출하는 부분에 try..catch 블록을 추가했습니다. 또한 원래 오류 메시지와 함께 우리가 작성한 메시지를 섞어 새로운 Error를 던지는 방법도 보여줍니다:  
...
이 예제를 실행하면, RangeError가 발생했기 때문에 이 오류가 다시 던져지고, 함수 외부의 마지막 catch 블록에서 처리됩니다. 여기서는 함수에서 받은 오류를 우리가 작성한 사용자 정의 메시지와 함께 표시합니다.  

---

### 사용자 정의 오류 던지기 (Throwing custom errors)  
앞선 예제에서 보았듯이, 사용자 정의 메시지를 가진 오류를 던지는 것은 간단합니다. 새로운 Error 객체를 생성하고 메시지를 전달하면 됩니다. 그러나 때로는 이것만으로는 충분하지 않습니다. JavaScript가 RangeError 같은 특정 문제를 위한 특수 오류 타입을 제공하듯이, 우리도 자체적인 오류 타입을 만들 수 있습니다.  

예를 들어, 최소값과 최대값을 받아 더 구체적인 오류 메시지를 제공하는 새로운 RangeError 타입을 만들어 보겠습니다. Error 클래스를 확장하여 새로운 클래스를 정의하면 됩니다:  
...
이 클래스는 세 개의 매개변수를 받습니다: 실제 값(예: 사용자가 입력한 값), 최소값, 최대값. 이 세 값을 사용해 더 구체적인 오류 메시지를 생성하고, `super` 키워드를 사용해 Error 클래스의 생성자에 전달합니다.  
...
이 예제를 실행하고 범위(1~10) 밖의 값을 입력해 보세요. 예를 들어 14를 입력하면, 콘솔에 다음과 같은 사용자 정의 오류 메시지가 표시됩니다:  
...
### 요약 (Summary)  
- JavaScript는 다양한 문제 유형에 대해 여러 종류의 오류 타입을 제공합니다.  
- 필요하다면 직접 사용자 정의 오류 타입을 쉽게 만들 수 있습니다.  
- 이러한 오류 타입(내장 및 사용자 정의)을 사용해 조건부 캐치로 오류를 구분하고, 처리하지 않을 오류는 다시 던져(re-throw) 다른 곳에서 처리하도록 할 수 있습니다.  
...
# Regular Expressions
## Introduction
정규 표현식(Regular Expressions, 흔히 "regex" 또는 "regexp"라고 줄여 부름)은 검색 패턴을 지정하는 일련의 문자입니다. 이를 통해 문자열을 찾아내거나, 단순히 존재 여부를 확인하거나, 다른 곳에서 사용하기 위해 추출하거나, 검색/치환 작업을 수행할 수 있습니다.  

JavaScript는 정규 표현식을 훌륭하게 지원하지만, 정규 표현식은 JavaScript 언어에만 국한된 것이 아닙니다. 사실 정규 표현식은 JavaScript보다 훨씬 오래전에 발명되었으며, 대부분의 프로그래밍 언어에서 기본적으로나 추가 라이브러리를 통해 지원됩니다.  

---

### 정규 표현식 정의하기  
대부분의 프로그래밍 언어는 정규 표현식을 다루기 위한 객체, 클래스 또는 메서드 집합을 제공합니다. 그러나 JavaScript에서는 정규 표현식이 명세의 직접적인 일부입니다. 즉, 특정 표기법을 사용해 코드 안에 직접 작성할 수도 있고, RegExp 객체를 사용할 수도 있습니다.  
...
이것은 매우 단순한 정규 표현식으로, 숫자를 찾을 수 있게 해줍니다. 이를 JavaScript에서 사용하려면, 정규 표현식 리터럴 표기법을 사용해 다음과 같이 정의할 수 있습니다:  

```javascript
let regex = /[0-9]+/;
```  

여기서 특수한 표기법에 주목하세요. 실제 정규 표현식 앞뒤에 슬래시(`/`)가 붙어 있는데, 이는 JavaScript 인터프리터에게 우리가 정규 표현식을 정의하고 있음을 알려줍니다.  

대안으로, RegExp 객체를 사용해 새 객체를 생성할 수도 있습니다:  

```javascript
let regex = new RegExp("[0-9]+");
```  

결과는 동일합니다. 사실 리터럴 표기법은 단순한 문법적 설탕(syntactic sugar)일 뿐이며, 내부적으로는 RegExp 객체를 생성합니다. 따라서 어떤 방식을 사용할지는 전적으로 여러분에게 달려 있습니다. 저는 개인적으로 후자의 방식을 선호하는데, 무슨 일이 일어나는지 더 명확하기 때문입니다. 하지만 온라인에서 볼 수 있는 많은 JavaScript 코드 예제에서는 리터럴 표기법을 사용하므로, 이 방식에도 익숙해져야 합니다.  

---

### 정규 표현식 사용하기  
물론, 단순히 RegExp 객체를 생성하는 것만으로는 아무 일도 일어나지 않습니다. 유용한 메서드 중 하나를 사용해야 합니다. 가장 단순한 메서드 중 하나는 **test()** 메서드입니다. 이 메서드는 문자열에 대해 정규 표현식을 적용했을 때 일치하는 항목이 있는지 여부를 알려줍니다.  
...
물론 이 외에도 훨씬 더 많은 작업을 할 수 있습니다. 하지만 JavaScript의 정규 표현식 기능에 대해서는 다음 글에서 더 자세히 다루겠습니다.  

---

### 요약 (Summary)  
정규 표현식(regex)을 사용하면 검색 패턴을 정의하고 문자열에서 검색 및 검색/치환 작업을 수행할 수 있습니다. 정규 표현식은 거의 작은 프로그래밍 언어와 같아서, 매우 복잡하면서도 극도로 다재다능할 수 있습니다.  

이 튜토리얼의 목적은 정규 표현식의 모든 측면을 가르치는 것이 아니라, JavaScript에서 정규 표현식을 사용해 무엇을 할 수 있는지를 보여주는 것입니다. 따라서 다양한 기능을 설명하기 위해 정규 표현식 예제를 제시하겠지만, 정규 표현식을 완전히 이해하려면 온라인에서 정규 표현식 튜토리얼이나 최소한 치트시트를 찾아보는 것을 권장합니다.  



## Regex flags/options
이전 글에서, 우리는 정규 표현식을 JavaScript와 함께 사용하여 간단한 문자열 검색을 수행하는 방법을 간단히 살펴보았습니다. 아주 단순한 정규 표현식을 사용해 이를 시연했지만, 정규 표현식은 매우 복잡해질 수 있습니다.  

이러한 복잡성의 결과로, 정규 표현식을 사용할 때 몇 가지 옵션을 변경할 수 있으며, 이는 특정 정규 표현식이 JavaScript 인터프리터에 의해 처리되는 방식에 영향을 줍니다. 일반적으로 이러한 옵션을 "설정(settings)"이나 "옵션(options)"이라고 부르지만, 정규 표현식에서는 이를 **플래그(flags)**라고 합니다.  

플래그는 기본적으로 정규 표현식에 지정할 수 있는 일련의 옵션으로, 다양한 상황에서 정규 표현식이 어떻게 반응할지를 제어합니다. 이제 플래그에 대해 빠르게 안내하겠지만, 먼저 그것들이 어떻게 지정되는지 살펴봅시다.  
...
여기서는 매우 단순한 정규 표현식을 지정했습니다(사실 정규 표현식 없이도 할 수 있지만, 지금은 신경 쓰지 마세요!). 이 정규 표현식은 "john"이라는 단어를 찾습니다. 그런 다음 문자열을 테스트했는데, 이 문자열에는 분명히 이 흔한 이름이 포함되어 있습니다. 그러나 결과는 놀랍게도 다음과 같습니다:  

```
Has match: false
```  

앞서 이야기했듯이, JavaScript는 대소문자를 구분하며, 정규 표현식도 기본적으로 대소문자를 구분합니다. 따라서 "john"을 찾으려 했지만 문자열에는 "John"만 있었기 때문에 일치하지 않은 것입니다. 그러나 이 동작은 아주 쉽게 바꿀 수 있습니다. RegExp 객체를 생성할 때 두 번째 매개변수로 하나 이상의 옵션을 플래그로 지정할 수 있기 때문입니다.  

---

### 대소문자 무시 플래그: i  

위 예제의 첫 번째 줄을 아주 조금만 수정하면, 정규 표현식이 대소문자를 완전히 무시하도록 만들 수 있습니다. 그래서 이름이 "ignore case"이고, 플래그로 지정할 때는 단순히 "i"라고 씁니다.  
...
다음은 사용 가능한 플래그의 전체 목록입니다. 일부는 설명이 필요 없을 정도로 명확하지만, 일부는 특정 정규 표현식 연산자의 동작과 직접적으로 관련되기 때문에 조금 더 복잡합니다. 지금은 참고용으로 보시고, 정규 표현식을 더 잘 이해하게 되었을 때 다시 돌아와 보세요. 언젠가는 반드시 필요하게 될 것입니다.  

- **g** - 전역 검색 수행. 첫 번째 일치 항목만 찾는 대신 모든 일치 항목을 찾습니다.  
- **i** - 대소문자 무시 (위에서 설명함)  
- **m** - 여러 줄 검색 수행. `^`와 `$` 연산자의 동작이 이에 맞게 바뀝니다.  
- **s** - 정규 표현식의 `.`(마침표) 연산자가 줄바꿈 문자까지 일치하도록 허용합니다.  
- **u** - 유니코드 지원을 활성화합니다.  
- **y** - "sticky" 검색을 수행하여, 텍스트의 정확한 위치에서만 일치 항목을 찾습니다.  

---

### 여러 플래그 사용하기  

필요하다면 여러 플래그를 아주 쉽게 지정할 수 있습니다. 예를 들어, 대소문자를 구분하지 않고, 여러 줄에서, 전역 검색을 수행해야 한다면, 세 가지 플래그를 모두 결합하면 됩니다:  

```javascript
let regex = new RegExp("john", "gim");
```  

여기서 `"gim"`은 전역(global), 대소문자 무시(ignore case), 여러 줄(multiline)을 의미합니다. 플래그의 순서는 중요하지 않습니다.  

---

### 리터럴 정규 표현식 표기법에서의 플래그  

정규 표현식을 리터럴 표기법으로 작성할 때도 플래그를 지정할 수 있습니다. 이 경우 플래그는 마지막 슬래시 뒤에 붙습니다:  

```javascript
let regex = /john/gim;
```  

이 정규 표현식은 위의 예제와 동일하게, "john"이라는 단어를 전역, 대소문자 무시, 여러 줄 플래그와 함께 검색합니다.  

---

### 요약 (Summary)  

플래그는 JavaScript에서 정규 표현식을 사용하는 메서드에 지정할 수 있는 옵션입니다. 하나 또는 여러 개를 자유롭게 지정할 수 있습니다. 다음 글에서는 JavaScript에서 정규 표현식을 더 깊이 활용하는 방법을 다루면서, 일부 플래그들을 실제로 사용해 보겠습니다.  




## Search/match with Regular expressions
이 시점에서, 우리는 정규 표현식이 무엇인지, 지정한 옵션(플래그)에 따라 어떻게 다르게 동작할 수 있는지, 그리고 문자열에서 일치를 찾기 위해 정규 표현식을 사용할 수 있는 몇 가지 기본 예제를 살펴보았습니다.  

그러나 우리는 아직 JavaScript에서 정규 표현식의 모든 잠재력을 보지 못했습니다. 앞서 언급했듯이, 정규 표현식은 JavaScript 언어에 매우 잘 통합되어 있으며, 이번 글에서 이를 확인할 수 있습니다. 우리는 RegExp 객체에서 제공되는 메서드들을 살펴볼 것이고, 정규 표현식과 직접적으로 관련된 String 객체의 메서드들도 다룰 것입니다.  

---

### RegExp.test()  
이 메서드는 소개 글에서 이미 본 적이 있지만, 완전성을 위해 다시 언급하겠습니다. **test()** 메서드는 특정 정규 표현식을 사용해 문자열에서 일치 항목이 있는지 여부를 단순히 확인합니다. 예:  
...
리터럴 표기법을 선호한다면, 위 예제는 이렇게도 쓸 수 있습니다:  
...
test() 메서드는 매우 단순하지만, 많은 상황에서 유용합니다.  

---

### RegExp.exec()  
여기서부터 더 흥미로워집니다. **exec()** 메서드를 사용하면 문자열 내에서 하나 이상의 일치를 찾을 수 있습니다.  
...
더 자세히 보면, exec()의 결과는 단순 문자열이 아니라, 일치한 문자열과 그 위치 등의 정보를 담은 객체입니다:  
...
여기까지는 단일 일치만 찾았지만, 여러 개가 있을 수도 있습니다. exec()는 한 번에 첫 번째 일치만 반환하지만, 실행 후 RegExp 객체의 **lastIndex** 속성을 업데이트하여 다음 실행 시 그 위치부터 검색합니다. 이를 활용해 루프를 돌리면 모든 일치를 추출할 수 있습니다:  
...
⚠️ 여기서 중요한 점은 **g (global) 플래그**를 반드시 지정해야 exec()로 여러 일치를 얻을 수 있다는 것입니다.  

---

### str.search()  
String 객체의 메서드 중 하나인 **search()**는 가장 기본적인 함수입니다. 첫 번째 일치의 위치를 반환하며, 없으면 -1을 반환합니다:  
...
search()는 첫 번째 일치만 찾고 위치만 반환하기 때문에 자주 쓰이지는 않습니다. 대신 match(), matchAll(), exec() 등을 더 많이 사용합니다.  

---

### str.match()  
**match()** 함수는 exec()와 비슷하지만, String 객체에서 호출합니다. 즉, 문자열에서 호출하고 정규 표현식을 매개변수로 전달합니다:  
...
⚠️ 주의:  
- **g 플래그**가 없으면 첫 번째 일치만 반환됩니다.  
- g 플래그가 있으면 캡처 그룹(capturing groups)은 반환되지 않습니다.  
- 캡처 그룹과 전역 검색을 동시에 원한다면 exec() 또는 matchAll()을 사용해야 합니다.  

---

### str.matchAll()  
**matchAll()**은 비교적 최근(2019년 이후) 추가된 기능입니다. 따라서 구형 브라우저에서는 지원되지 않을 수 있습니다.  

matchAll()은 exec()처럼 모든 일치와 캡처 그룹을 반환하지만, 반복 호출할 필요 없이 한 번에 iterable 객체로 반환합니다:  
...
⚠️ 주의:  
- matchAll()은 **g 플래그가 필수**입니다. 없으면 오류가 발생합니다.  
- 일치가 없으면 NULL이 아니라 **빈 iterable 객체**를 반환합니다.  

---

### str.split()  
마지막으로 **split()** 함수입니다. 문자열을 정규 표현식을 기준으로 분할합니다.  
...
이제 구분자가 `-`이든 `/`이든 모두 처리할 수 있습니다.  

---

### 요약 (Summary)  
이번 글에서는 JavaScript에서 정규 표현식을 활용해 문자열을 검색하는 다양한 메서드를 살펴보았습니다:  

- **RegExp.test()** → 일치 여부 확인  
- **RegExp.exec()** → 일치 항목과 위치 반환, 루프와 함께 사용 가능  
- **str.search()** → 첫 번째 일치 위치 반환  
- **str.match()** → 모든 일치 반환 (단, g 플래그 필요)  
- **str.matchAll()** → 모든 일치 + 캡처 그룹 반환 (g 플래그 필수)  
- **str.split()** → 정규 표현식을 기준으로 문자열 분할  

다음 글에서는 이 강력한 도구로 할 수 있는 더 멋진 기능들을 깊이 탐구할 것입니다.  
...
지금까지 우리는 정규 표현식을 사용하여 검색 패턴의 발생 여부를 확인하거나, 이 검색 패턴을 기반으로 전체 문자열을 추출하는 방법을 살펴보았습니다. 그러나 때로는 검색 패턴의 일부만 추출해야 할 때가 있고, 때로는 둘 이상의 부분을 추출해야 할 때도 있습니다. 이러한 목적을 위해, **캡처 그룹(capturing groups)**이라는 매우 똑똑한 기능이 있습니다.  

이 기법은 정규 표현식 안에 캡처 그룹을 정의함으로써 작동합니다. 그룹은 패턴의 일부를 괄호로 감싸 정의할 수 있습니다. 예를 들어:  

```
[^@]+@(.+)
```  

이 정규 표현식은 이메일 주소에서 `@` 문자 뒤의 도메인 부분을 별도의 그룹으로 캡처하며, JavaScript 인터프리터는 매칭을 수행할 때 이 그룹을 사용할 수 있도록 보장합니다. 사용 예시는 다음과 같습니다:  
...
여기서 약간의 설명이 필요합니다. 우선, 제가 사용한 정규 표현식은 **매우 단순한 이메일 검색 패턴**입니다. 실제 코드에서 사용하기에는 지나치게 단순하므로 절대 사용해서는 안 됩니다. 다만 짧고 간단해서 이 예제와 다음 예제를 설명하기에 적합합니다.  

여기서 String 객체의 **match()** 함수를 사용했습니다. 앞서 이야기했듯이, 이 함수는 발견된 매치들을 배열로 반환합니다. 정규 표현식의 마지막 부분 `(.+)`에 주목하세요.  

`.` 연산자는 어떤 문자든 매치하며, 이를 괄호로 감싸 캡처 그룹을 정의했습니다. 덕분에 결과에서 이 특정 부분을 별도로 접근할 수 있습니다. 배열의 첫 번째 요소(인덱스 0)는 항상 전체 매치이고, 그 뒤의 요소들이 캡처 그룹에서 발견된 값들입니다. 이렇게 해서 이메일 주소의 도메인 부분을 추출할 수 있습니다.  

---

### 이메일의 두 부분 모두 캡처하기  

이제 이메일 주소의 두 부분 모두를 캡처할 수 있습니다. 첫 번째 부분도 괄호로 감싸 캡처 그룹으로 만들면 됩니다:  
...
이제 캡처 그룹을 통해 원하는 특정 부분을 쉽게 얻을 수 있으며, 동시에 전체 매치된 문자열에도 접근할 수 있습니다.  

---

### 이름 있는 그룹 (Named groups)  

캡처 그룹이 두세 개 정도라면, 결과 배열에서 어떤 인덱스에 있는지 기억하는 것이 큰 문제가 되지 않습니다. 그러나 더 복잡한 정규 표현식 안에 많은 캡처 그룹이 있다면, 각 그룹에 이름/레이블을 붙이는 것이 훨씬 편리합니다. 이렇게 하면 나중에 코드를 다시 볼 때 정규 표현식이 무엇을 하는지 이해하기도 훨씬 쉬워집니다.  

다행히도 정규 표현식은 **이름 있는 캡처 그룹(named capturing groups)**을 지원하며, JavaScript에서도 이를 사용할 수 있습니다. 그룹에 이름을 붙이려면, 괄호 시작 직후에 물음표와 꺾쇠 괄호 안에 이름을 추가하면 됩니다:  
...
이제 결과의 **groups** 속성을 통해, 우리가 붙인 이름으로 각 캡처 그룹의 값을 접근할 수 있습니다. 정규 표현식이 복잡해질수록 이 방식이 훨씬 더 이해하기 쉽습니다.  

---

### 요약 (Summary)  

- 캡처 그룹은 정규 표현식의 매우 강력한 기능으로, 패턴을 기반으로 문자열의 여러 부분을 추출할 수 있습니다.  
- 캡처 그룹에 이름을 붙이면 정규 표현식이 더 읽기 쉽고, 나중에 코드를 다시 볼 때 이해하기도 쉬워집니다.  
- 캡처 그룹은 검색/치환 작업에서도 사용할 수 있으며, 이는 다음 글에서 다룰 예정입니다.  




## Search/replace with Regular expressions
지난 여러 글에서, 우리는 정규 표현식을 사용해 검색/매칭 작업을 수행하는 다양한 가능성에 대해 논의했습니다. 이것만으로도 매우 유용하지만, 아직 정규 표현식으로 할 수 있는 가장 강력한 기능 중 하나를 다루지 않았습니다. 바로 **검색/치환(search/replace) 작업**입니다. 즉, 단순히 문자열 안에서 다른 문자열을 찾는 것에 그치지 않고, 매치된 문자열을 다른 문자열로 적극적으로 교체할 수 있습니다.  

---

### replace()와 replaceAll()  
문자열에서 치환 작업을 수행하려면, 내장 String 객체의 메서드인 **replace()** 또는 **replaceAll()**을 사용합니다. 앞서 이야기했듯이, 이 메서드들은 단순 문자열을 인자로 받아 기본적인 검색/치환 작업을 수행할 수도 있고, 정규 표현식을 인자로 받아 정규식 기반의 검색/치환 작업을 수행할 수도 있습니다.  

앞서 언급했듯이, replace() 메서드는 단순 문자열을 인자로 사용할 경우 대소문자를 구분합니다. 따라서 정규 표현식 버전을 사용해 대소문자를 무시하는 방법을 보여드리겠습니다:  
...
위 예제에서 문자열 안의 "world"는 대소문자가 뒤섞여 있지만, 정규 표현식에 **i (ignore case)** 플래그를 사용했기 때문에 "universe"로 교체할 수 있습니다.  

또한, 기본적으로 replace()는 **첫 번째 발생만 교체**합니다. 모든 발생을 교체하려면 **g (global)** 플래그를 지정해야 합니다. 이 경우 replace()나 replaceAll()을 호출해도 차이는 없습니다. 두 메서드 모두 여러 발생을 교체하려면 global 플래그가 필요하기 때문입니다:  
...
### 캡처 그룹 사용하기  
앞선 글에서, 문자열 매칭 시 정규 표현식의 **캡처 그룹(capturing groups)**이 얼마나 강력한지 보았습니다. 이들은 검색/치환 작업에서도 사용할 수 있으며, 많은 상황에서 매우 유용합니다.  

예를 들어, 문자열 안의 숫자에 강조를 추가하는 코드를 작성해 보겠습니다. 숫자가 발견될 때마다 HTML 태그로 감싸 교체하는 방식입니다:  
...
여기서 `$1`은 첫 번째 캡처 그룹의 값을 의미합니다. 정규 표현식에서 하나의 캡처 그룹만 정의했으므로, 이는 문자열에서 매치된 숫자가 됩니다.  

---

### 여러 캡처 그룹  
캡처 그룹을 사용한 검색/치환의 대표적인 예는 이름과 성의 순서를 바꾸는 것입니다. 예를 들어 `"John Doe"`를 `"Doe, John"` 형식으로 바꾸는 경우입니다.  
...
여기서는 각 줄에서 두 단어(이름과 성)를 매치해 각각 캡처 그룹에 넣고, 교체 시 두 번째 그룹(성)을 먼저, 첫 번째 그룹(이름)을 나중에 출력합니다.  

여기서 사용된 **m (multiline)** 플래그는 정규 표현식의 `^`와 `$` 연산자가 각 줄의 시작과 끝을 매치하도록 합니다.  

---

### 이름 있는 캡처 그룹  
캡처 그룹이 많아지면 인덱스로 구분하기 어렵습니다. 이때는 **이름 있는 캡처 그룹(named capturing groups)**을 사용할 수 있습니다.  
...
이제 `$<lastName>`과 `$<firstName>`처럼 이름으로 그룹을 참조할 수 있어, 코드 가독성이 훨씬 좋아집니다.  

---

### 치환 함수 (Replacer functions)  
정규 표현식을 사용한 검색/치환은 매우 강력하지만, **치환 값을 함수로 생성**할 수도 있습니다. 이 방식은 유연성이 극대화됩니다.  

예를 들어, 이름의 대소문자가 제멋대로 입력된 경우를 고치면서 동시에 `"성, 이름"` 형식으로 바꾸고 싶다면:  
...
여기서 **FormalName()** 함수는 세 개의 매개변수를 받습니다: 전체 매치 문자열, 그리고 두 개의 캡처 그룹(firstName, lastName). JavaScript는 자동으로 이 값들을 전달합니다. 함수 내부에서는 이름과 성의 첫 글자를 대문자로, 나머지를 소문자로 변환한 뒤 `"성, 이름"` 형식으로 반환합니다.  

replace() 호출 시 함수 이름만 지정하면, JavaScript가 각 매치마다 이 함수를 호출해 교체 값을 생성합니다.  

---

### 요약 (Summary)  
정규 표현식을 사용한 검색/치환은 매우 강력한 도구입니다. 이번 글에서는 다음을 살펴보았습니다:  
- replace()와 replaceAll()의 기본 동작  
- 캡처 그룹을 활용한 치환  
- 이름 있는 캡처 그룹을 사용해 가독성 향상  
- 치환 함수를 사용해 동적으로 교체 값 생성  

이러한 기능들을 활용하면, 정규 표현식을 이용한 문자열 처리 작업을 훨씬 더 유연하고 강력하게 만들 수 있습니다.  
...
이 튜토리얼의 첫 번째 섹션에서, 우리는 JavaScript의 일반적인 개념들을 소개하기 위해 꽤 많은 시간과 노력을 들였습니다. 이미 설명했듯이, JavaScript는 웹사이트에서 많이 사용되는 프로그래밍 언어이지만, 웹 이외의 곳에서도 사용할 수 있습니다.  

그렇기 때문에, 튜토리얼의 첫 번째 섹션은 웹과 관련된 부분을 배제하고, 프로그래밍 언어로서의 JavaScript의 핵심 개념에 100% 집중했습니다. 그러나 이번 섹션부터는 초점이 완전히 바뀝니다. JavaScript가 수많은 개발자들에 의해 웹사이트를 더 상호작용적이고 사용자 친화적으로 만드는 데 사용된다는 사실을 받아들이고, 여러분이 웹을 위해 JavaScript를 사용하고자 할 때 반드시 알아야 할 중요한 개념들을 다룰 것입니다.  

따라서, 만약 여러분이 이 튜토리얼과 JavaScript 자체에 완전히 처음이라면, 이 섹션으로 넘어오기 전에 첫 번째 섹션으로 돌아가 JavaScript의 핵심 개념을 먼저 학습할 것을 권장합니다. JavaScript가 프로그래밍 언어로서 어떻게 동작하는지 이해한다면, 이제 이 섹션에서 다룰 웹 전용 부분으로 넘어갈 준비가 된 것입니다.  

---

### 호스트 환경 (The host environment)  
JavaScript는 처음에는 웹을 위해 만들어졌지만, 이제는 서버부터 스마트 냉장고, 오븐에 이르기까지 모든 종류의 장치와 플랫폼에서 사용됩니다. JavaScript는 누구나 자신의 장치에 구현할 수 있으며, 이를 통해 개발자들이 잘 알려지고 표준화된 언어로 코드를 작성할 수 있도록 합니다.  

JavaScript가 구현되면, 당연히 이 튜토리얼의 첫 번째 부분에서 다룬 모든 객체와 함수들을 사용할 수 있습니다. 이것이 바로 핵심 구현(core implementation)이기 때문입니다. 그러나 JavaScript를 구현하는 장치나 애플리케이션은 플랫폼 특화 기능을 노출할 수도 있는데, 이를 흔히 **호스트 환경(host environment)**이라고 부릅니다.  

예를 들어, 스마트 오븐이 JavaScript를 구현한다면, 오븐을 켜고 끄거나 온도를 조절하는 기능 등을 노출할 것입니다.  

웹을 위한 JavaScript를 사용할 때, 브라우저는 호스트 환경으로 동작하며, 개발자가 웹 페이지의 콘텐츠뿐만 아니라 브라우저 자체와 상호작용할 수 있도록 많은 추가 기능을 제공합니다. 웹 브라우저는 여러 종류가 있으며, 월드 와이드 웹 초창기에는 각 브라우저마다 JavaScript의 구현이 달라, 모든 브라우저에서 동일하게 동작한다고 보장할 수 없었습니다. 다행히도, 시간이 지나면서 이 부분은 크게 개선되었고, 오늘날에는 대부분의 인기 있는 브라우저에서 거의 동일하게 동작한다고 기대할 수 있습니다.  

따라서, 웹 브라우저에서 JavaScript를 사용할 때 반드시 알아야 할 중요한 개념들이 있는데, 이를 DOM과 BOM이라고 합니다. 여기서는 간단히 소개하고, 이후 글에서 더 깊이 다루겠습니다. 우선 **Window 객체**부터 이야기해 봅시다.  

---

### Window  
웹 브라우저가 JavaScript의 호스트 환경으로 동작할 때, 객체 계층 구조의 최상단에는 **Window 객체**가 있습니다. Window 객체는 웹 브라우저 창 전체를 객체로 표현한 것이라고 생각하면 됩니다. 이 객체를 통해 웹사이트의 콘텐츠뿐만 아니라 브라우저 자체와도 상호작용할 수 있습니다.  

제가 만든 다이어그램은 JavaScript의 기능 계층 구조를 보여줍니다:  

**JavaScript 브라우저 호스트 환경 계층 구조**  

보시다시피, 모든 것은 window 객체에서 시작합니다. 그 아래에는 DOM, BOM이 있고, 그 밑에는 함수, 객체, 배열 등 우리가 이미 다룬 JavaScript의 핵심 기능들이 있습니다.  

Window 객체는 많은 기능을 제공합니다. 사실 우리는 이미 이 튜토리얼에서 JavaScript 기능을 설명하기 위해 일부를 사용했습니다. 예를 들어, **alert()** 메서드는 Window 객체에 존재하며, 브라우저에서 팝업 메시지를 표시할 수 있습니다:  

```javascript
window.alert("Hello, window!");
```  

사실, `"window."` 접두어 없이도 이 함수를 사용할 수 있습니다. 왜냐하면 브라우저의 호스트 환경에서 window는 전역 객체(global object)이기 때문입니다. 이에 대해서는 Window 전용 글에서 더 자세히 다룰 것입니다.  

---

### DOM  
Window 객체 아래에는 **DOM(Document Object Model)**이 있습니다. DOM은 객체를 통해 웹 페이지의 콘텐츠와 상호작용할 수 있도록 합니다. 웹 브라우저에서 JavaScript를 사용하기 시작하면, DOM이 매우 중요하고 끊임없이 사용된다는 것을 곧 깨닫게 될 것입니다.  
...
여기서는 window의 document 객체에 접근해 현재 페이지의 제목(HTML의 TITLE 태그에 선언된 값)을 표시합니다. document 객체에는 훨씬 더 많은 기능이 있으며, 이 튜토리얼의 이후 글에서 여러 편에 걸쳐 다룰 예정입니다.  

---

### BOM  
**BOM(Browser Object Model)**은 브라우저 특화 객체들에 접근할 수 있게 해주며, 브라우저의 특정 측면을 조작할 수 있도록 합니다. 예를 들어, location 객체를 통해 현재 페이지의 URL에 접근할 수 있습니다:  

```javascript
window.alert(window.location.href);
```  

BOM에는 이 외에도 많은 객체들이 있으며, 이 주제는 나중에 별도의 장에서 자세히 다룰 것입니다.  

---

### 요약 (Summary)  
- JavaScript는 처음에는 웹을 위해 만들어졌지만, 오늘날에는 훨씬 더 널리 사용됩니다.  
- 웹을 위한 JavaScript를 사용할 때, 브라우저는 호스트 환경으로 동작하며, 핵심 JavaScript 기능 위에 추가 기능을 제공합니다.  
- 브라우저가 JavaScript 호스트 환경으로 동작할 때, **window 객체**는 전역 객체이며, 이를 통해 DOM(Document Object Model)과 BOM(Browser Object Model)에 접근할 수 있습니다. DOM은 웹 페이지의 콘텐츠에, BOM은 브라우저의 특정 부분에 접근할 수 있게 합니다.  

이 섹션에서는 웹 브라우저를 위한 JavaScript에만 집중하며, 그 모든 중요한 측면들을 깊이 탐구할 것입니다.  




## The SCRIPT tag
웹사이트를 개발할 때, 반드시 알아야 할 세 가지 중요한 기술이 있습니다: **HTML, CSS, JavaScript**. 이들은 중요도 순서대로 나열되어 있으며, HTML이 가장 중요하면서도 가장 이해하고 배우기 쉽습니다. 브라우저에서 JavaScript를 사용하려면 최소한 HTML은 알아야 하고, CSS에 대한 좋은 이해도도 권장됩니다. 따라서 HTML이나 CSS 실력을 복습할 필요가 있다면, 우리의 HTML 튜토리얼과 CSS 튜토리얼을 참고하세요.  

HTML은 웹 페이지의 기초로, 페이지의 구조와 콘텐츠를 지정할 수 있게 해줍니다. CSS는 HTML에서 정의된 요소들에 다양한 스타일을 적용할 수 있게 해주며, JavaScript를 사용하면 콘텐츠(HTML)와 스타일(CSS) 모두를 조작하여 훨씬 더 동적인 웹 페이지를 만들 수 있습니다.  

---

### SCRIPT 태그  
이제, JavaScript를 웹 페이지에 통합하는 방법에 대해 이야기해 봅시다. 웹 페이지는 기본적으로 HTML 태그들의 집합이며, 브라우저는 이를 다양한 요소로 변환합니다. 대부분의 요소는 방문자에게 보이지만, 일부 요소는 브라우저에 대한 지시로만 사용됩니다. 이 후자 범주에 속하는 것이 바로 **SCRIPT 태그**입니다. SCRIPT 태그는 브라우저에게, 그 안의 내용을 처리할 때 스크립트 인터프리터로 전환하라고 알려줍니다.  

SCRIPT 태그는 원래 HTML에 JavaScript를 통합하기 위해 도입되었지만, 이후 VBScript(현재는 사용되지 않음) 같은 다른 언어와 기술도 허용하도록 확장되었습니다. 이 때문에, 브라우저가 어떤 스크립트 코드를 기대해야 하는지 type 속성을 통해 지정할 수 있습니다. 그러나 기본적으로 대부분의 브라우저는 type 속성이 지정되지 않으면 SCRIPT 태그가 JavaScript 코드를 의미한다고 가정합니다.  

---

### 인라인 스크립트 (Inline scripts)  
가장 기본적인 형태에서, SCRIPT 태그는 어떤 속성도 필요하지 않습니다. HTML 코드 어디에나 배치하고, 시작 태그와 종료 태그 사이에 JavaScript를 작성하면 됩니다:  
...
브라우저가 이 스크립트 블록에 도달하면, JavaScript 엔진을 사용해 그 안의 코드를 해석하고 실행합니다.  

---

### 외부 스크립트 (External scripts)  
HTML 문서 안에 JavaScript 코드를 인라인으로 작성하는 것이 합리적일 때도 있지만, 일부 또는 전부를 하나 이상의 외부 JavaScript 파일로 옮겨 참조 방식으로 포함시키는 것이 좋습니다. 여전히 SCRIPT 태그를 사용하지만, 코드 대신 **src 속성**을 통해 파일 이름을 전달합니다:  

```html
<script src="javascript.js"></script>
```  

"javascript.js"라는 이름의 파일이 존재한다면, 브라우저가 이 태그에 도달했을 때 해당 파일이 페이지의 일부인 것처럼 실행됩니다.  

여러 줄 이상의 코드가 있다면, 인라인 스크립트 블록 대신 항상 외부 JavaScript 파일을 사용하는 것이 좋습니다. 그 이유는 다음과 같습니다:  

- HTML 문서를 더 짧고 읽기 쉽게 만듭니다.  
...
### SCRIPT 태그의 위치  
앞서 SCRIPT 태그를 HTML 문서 어디에나 배치할 수 있다고 했지만, 일반적으로 사용하는 위치가 있습니다. 또한 SCRIPT 태그는 브라우저가 도달하는 즉시 해석되고 실행되므로, 위치가 중요합니다.  

대부분 비어 있는 HTML 문서는 다음과 같이 보일 수 있습니다:  
...
여기서 HEAD와 BODY 섹션을 알아보셨기를 바랍니다. 만약 익숙하지 않다면, HTML 튜토리얼을 복습하세요.  
...
두 위치 모두 흔히 사용되며, JavaScript 코드의 위치는 차이를 만듭니다. 예를 들어, 코드가 웹 페이지의 요소와 상호작용해야 한다면, 일반적으로 문서의 맨 아래(위치 #2)에 두어야 합니다. 요소들이 브라우저에 의해 해석되고 렌더링된 후에야 JavaScript 코드가 그들과 상호작용할 수 있기 때문입니다.  

반대로, 페이지 로드의 가장 초기에 JavaScript 코드가 실행되어야 할 때도 있습니다(예: 통계 처리). 이 경우에는 위치 #1을 권장하며, 가능하다면 외부 JavaScript 파일로 두는 것이 좋습니다.  

---

### SCRIPT 속성들  
이것은 HTML 튜토리얼이 아니므로, SCRIPT 태그와 관련된 몇 가지 중요한 속성만 언급하겠습니다. 이미 type 속성과 src 속성은 다루었습니다.  
...
#### async 속성  
앞서 언급했듯이, 브라우저가 SCRIPT 태그를 해석하면 즉시 인라인 스크립트를 실행합니다. SCRIPT 태그가 외부 스크립트 파일을 참조하는 경우, 해당 파일이 완전히 로드되고 처리될 때까지 브라우저는 페이지의 나머지를 해석하지 않습니다. 이는 특히 큰 JavaScript 파일을 참조할 경우 방문자에게 대기 시간을 유발할 수 있습니다.  

async 속성을 사용하는 것은 다른 불리언 HTML 속성과 마찬가지로 간단합니다. 태그에 단어를 추가하기만 하면 됩니다:  

```html
<script src="javascript.js" async></script>
```  

단, async 속성을 사용하면 JavaScript가 페이지의 다른 콘텐츠와 병렬로 다운로드되며, 모든 것이 완전히 로드되기 전에 실행될 수 있습니다. 이 경우 존재하지 않는 요소를 참조하려는 상황이 발생할 수 있습니다.  

---

#### defer 속성  
이 속성은 외부 스크립트 참조(src 속성 사용)에만 해당됩니다. 추가되면, 브라우저는 문서가 완전히 해석된 후, 그러나 DOMContentLoaded 이벤트가 발생하기 전까지는 참조된 JavaScript를 실행하지 않습니다. async 속성과 마찬가지로, defer 속성은 브라우저가 외부 JavaScript 파일을 병렬로 가져오면서 동시에 문서의 나머지를 해석할 수 있게 합니다.  

defer 속성은 로드하려는 JavaScript 코드가 페이지의 다른 요소에 의존할 때 유용합니다. async 속성과 동일하게, SCRIPT 태그에 추가하기만 하면 됩니다:  
...
### 요약 (Summary)  
- async나 defer가 지정되지 않으면, 외부 스크립트 참조는 브라우저가 해당 JavaScript 파일을 로드하고 실행할 때까지 문서 해석을 중단시킵니다. 이는 많은 JavaScript 코드를 로드할 경우 페이지를 느리고 둔하게 만들 수 있습니다.  
- 따라서 원하는 동작에 따라 두 속성 중 하나를 지정하는 것이 일반적입니다:  
