# Javascript Tutorial
반갑습니다! `JavaScript` 튜토리얼에 오신 것을 환영합니다. 

이 튜토리얼은 흥미롭고 인기있는 JavaScript 프로그래밍 언어의 새로운 사용자를 대상으로 설명합니다. 고급 사용자도 특정 장(章)을 읽음으로, JavaScript 지식의 빈틈을 채울 수 있으며, 한 두 가지는 배울 수 있습니다.

각 장은 JavaScript에 처음 입문한 사람에게 맞도록 구성되어 있으며, 이전 장을 읽지 않고도 대부분의 장을 활용할 수 있지만, 첫 장부터 차례대로 따라가는 것이 이 튜토리얼로 가장 많은 것을 얻을 수 있는 방법입니다. 만약, 어떤 장에서 이미 알고있는 정보를 설명하면, 다음 장으로 넘어갑니다.

이 튜토리얼은 계속 새로운 장을 추가할 수 있으니, 정기적으로 다시 방문하시기 바랍니다. 이 튜토리얼로 여러분이 JavaScript를 제대로 시작하도록 돕고, 저자만큼 JavaScript에 흥분을 느끼시길 바랍니다.

또한, 별도의 사전 지식이 없더라도 JavaScript를 배울 수 있습니다. 하지만, 웹사이트를 만들 정도의 JavaScript를 사용하려면, `HTML` 에 대한 기초 지식이 권장됩니다. 만약, `HTML` 도 처음이라면, [HTML5 튜토리얼](http://www.html5-tutorials.org/)을 살펴보시길 권합니다.


<br><br>
# About JavaScript
## What is JavaScript?
`JavaScript` (흔히 단순히 `JS` 라고 불림)는 ECMAScript 명세에 기반한 프로그래밍 언어입니다. 이 언어는 웹 브라우저가 웹페이지를 렌더링한 후 항목을 조작해서 웹을 좀 더 살아있게 만들려는 의도로 만들어졌습니다.

현재는 아주 단순하고 사소할 수 있지만, JavaScript가 1997년에 초기 웹브라우저 중 하나인 Netscape Navigator에 처음 도입될 때는 엄청난 일이었습니다. 그 전까지 웹페이지는 전부 레이아웃과 텍스트에 관한 것이었고, 정적인 콘텐츠를 브라우저에 전달하고, 다른 페이지로 이동하기 전까지 변경할 수 없었습니다. JavaScript는 그 모든 것을 바꾸었고, `HTML` 과 `CSS` 와 함께 오늘날 우리가 알고있는 동적인 웹사이트 형태로 진화하기 시작했습니다.


## JavaScript 언어 특징
JavaScript 언어에 대해 말할 것이 많으며, 그 중 많은 부분은 다른 프로그래밍 언어와 비교할 수 있습니다. 만약,  프로그래밍이 처음이라면, 이런 정보는 지금 당장 아무런 의미가 없을 수 있지만, 나중에는 의미가 있습니다.

- 이름의 절반과 구문 스타일은 Java 프로그래밍 언어에서 가져왔지만, 혼동하진 마시기 바랍니다. 중요한 영역에서 두 언어는 상당히 다릅니다.
- 약한 타입(weakly typed) 언어입니다. 즉, 변수를 특정 타입으로 미리 선언할 필요가 없습니다. 대신, 인터프리터가 사용 방식에 따라 타입을 추측합니다.
- 동적(dynamic)입니다. 예를 들어, 정수(integer) 같은 숫자 변수에 단순히 텍스트를 할당해서 문자열(string) 변수로 바꿀 수 있습니다.
- 프로토타입 기반(prototypal)입니다. 즉, 객체 지향은 클래스가 아니라 프로토타입에 기반합니다. 나중에 좀 더 깊이 다룹니다.
- 함수형(functional)입니다. 함수는 일급(first-class)으로 취급되고, 객체로 간주하며, 속성과 메서드를 가질 수 있습니다.
- 런타임 평가(runtime evaluation)를 제공합니다. JavaScript 코드를 원래 코드인 것처럼 인터프리터가 평가할 수 있다는 것을 의미합니다.
...
JavaScript와 웹브라우저는 밀접하게 연관되어 있습니다. 사실, 당연합니다. 왜냐하면, JavaScript는 원래부터 Netscape Navigator 웹브라우저 백엔드 개발팀에 의해 만들어졌고 웹에서만 사용했기 때문입니다. 이것이 바로 JavaScript 언어를 안전한 언어라고 부르는 이유입니다. 가장 일반적인 구현에서 JavaScript는 기본적으로 파일 시스템이나 그와 유사한 것에 접근할 수 없습니다. 대신, 일종의 샌드박스(sandbox) 같이 작동하고 시스템과 관련없는 좋은 일은 처리할 수 있습니다.

그러나, "JavaScript는 웹브라우저 밖에서 사용할 수 있다!" 는 점이 중요합니다. JavaScript를 실행하려면, JavaScript 엔진 또는 인터프리터를 사용합니다. 이 엔진은 브라우저로 실행하며 실제 인류 대부분이 알고있는 거의 모든 현대적인 웹브라우저에서 실행됩니다. 하지만, 이 엔진이 브라우저 밖에서 실행되는 것을 막을 수 있는 것은 아무것도 없습니다! 사실, 여러 번의 시도가 이루졌습니다.

### Node.js
가장 주목할 예로는 `Node.js` 프로젝트입니다. 2009년 만들어진 이 프로젝트는 기본적으로 JavaScript 엔진을 웹브라우저에서 꺼내고 JavaScript를 웹브라우저 전용 스크립트 언어가 아닌 범용 프로그래밍 언어로 만들기 위한 여러 모듈을 추가했습니다. 이 작업은 JavaScript에 대한 새로운 그리고 훨씬 더 고수준의 관심을 불러 일으켰습니다. 

현재 JavaScript 기술은 웹(클라이언트 측)뿐만 아니라 서버측 스크립트에 사용할 수 있습니다. 그리고, `Node.js` 는 크로스 플랫폼으로 브라우저없이 Windows, Linux, Unix, macOS 전반에 걸쳐 코드를 그대로 실행할 수 있습니다.

### 호스트 환경
이 모든 것은 호스트 환경(host environment)에 대한 설명으로 이어집니다. 호스트 환경은 JavaScript가 실제로 실행되는 곳이며 JavaScript 언어에 존재하지 않는 수많은 기능을 포함합니다. 가장 분명한 예는 바로 웹 브라우저입니다. 웹 브라우저는 수많은 기능을 추가해서 JavaScript 코드가 웹 브라우저와 통신하거나 웹사이트 내용을 조작할 수 있습니다.

또 다른 호스팅 환경은 서버측 기능을 추가한 `Node.js` 입니다. 그러나, JavaScript는 어디서든 실행할 수 있도록 설계됐기 때문에 커피 머신일 수도 있습니다. 커피 머신은 JavaScript 엔진을 쉽게 포함할 수 있고 웹사이트 대신 물리 버튼으로 명령을 입력받고 커피를 만드는 것과 관련된 기능을 처리할 수 있습니다.

### 브라우저와 함께 또는 없이?
보다시피 JavaScript는 확실히 웹 브라우저 밖에서 실행할 수 있습니다. 지금도 그렇고 JavaScript의 인기가 높아짐에 따라 더더욱 그럴 것임은 쉽게 상상할 수 있습니다. 따라서, 이 튜토리얼은 무엇보다 "JavaScript 언어 자체를 배우는 것이 중요하다!" 는 점을 강조하고 싶습니다.

튜토리얼 첫 부분은 언어 자체에만 집중할 것이며 웹 브라우저와 어떻게 상호 작용하는지는 크게 이야기하지 않습니다. 그렇다해서 JavaScript가 웹 브라우저와 상호 작용하는 방식을 완전히 무시하는 것도 어리석은 일입니다. 왜냐하면, 여전히 가장 많은 사용 사례에 속하기 때문입니다. 따라서, 튜토리얼 후반부는 이 주제에 대해 설명하며 JavaScript와 웹 브라우저 사이의 연관성에 대해 좀 더 깊이 파고들겠습니다.

### 순수주의자를 위한 작은 예고
튜토리얼 첫 부분에서 "웹 브라우저는 배제한다!" 하더라도 웹 브라우저와 관련 깊은 몇 가지 함수는 사용할 것이란 점은 예상할 수 있습니다. 대표적인 예는 바로 `alert()` 함수입니다. 이 함수는 웹 브라우저에서 단순한 메시지 박스를 보여줍니다. 이 함수를 사용한 이유는 튜토리얼에서 사용하는 예제를 단순하고 이해하기 쉽게 만들기 때문이며 또한, 웹 브라우저로 튜토리얼을 읽으면서 예제를 직접 실행해서 눈에 보이는 결과를 만들 수 있기 때문입니다.


### 요약
JavaScript는 주로 웹브라우저로 실행하지만 항상 그런 것은 아닙니다. 이 튜토리얼은 JavaScript 언어에 집중해서 언어 자체를 배울 수 있도록 설명합니다. 그리고 나중에 JavaScript가 웹브라우저와 어떻게 상호작용할 수 있는지 살펴봅니다. 그러나, 브라우저와 약간의 상호작용하는 예제는 보여줍니다. 왜냐하면, 이해하기 쉽고 글을 읽으면서 직접 테스트할 수 있기 때문입니다.


## JavaScript libraries
이 튜토리얼은 JavaScript 언어에 대해 설명한 후 웹과 관련된 부분도 설명한다고 방금 약속했지만 시작하기 전에 웹과 관련된 몇 가지 주제를 언급하겠습니다: 그것은 바로 JavaScript 라이브러리입니다.

JavaScript는 확장성 높은 언어로 자신만의 기능을 언어에 추가해서 마치 JavaScript의 기본 기능인 것처럼 사용하는 것은 아주 쉽습니다. JavaScript가 도입된 직후 숙련된 개발자는 이런 사실을 활용해서 자신만의 라이브러리를 만들었습니다. 이런 라이브러리는 자주 사용하는 기능을 추가하거나 기존 기능을 쉽게 사용하도록 만든 것입니다. 즉, 보다 적은 코드로 쉽게 작업할 수 있는 일종의 지름길(shortcut)입니다.

개발자들이 이런 지름길 기능을 추가한 이유 중 하나는 과거 브라우저 전쟁 시절의 다양한 웹 브라우저는 자신만의 JavaScript 엔진을 가졌기 때문입니다. 이는 인기있는 브라우저들 사이에서 대부분의 기능은 동일했지만 차이점도 존재했음을 의미합니다. 기본적으로 방문자에게 동일하게 작동할 것이란 신뢰를 줄 수 없었습니다.

그래서, 초기 라이브러리는 주로 방문자가 어떤 브라우저를 사용하는지 확인한 다음 해당 브라우저가 가진 문제점이나 부족한 점을 우회해서 어떤 상황에도 의도한대로 작동하도록 보장하는 기능 위주였습니다.

대부분의 개발자는 라이브러리를 자신만 사용하거나 일하는 회사에서 사용했지만 일부 라이브러리는 공개했습니다. 주로 다른 개발자에게 중요하다고 여겨지는 기능이기 떄문입니다. 이런 라이브러리 공개는 다른 개발자들의 참여로 보다 많은 기능이 추가되고 기존 기능을 완성할 수 있도록 만들었습니다.

### jQuery
일반적으로 사용하기 시작한 최초의 JavaScript 라이브러리 중 하나는 바로 `jQuery` 입니다. 2006년 처음 출시됐으며 주로 `DOM` (Document Object Model) 항목을 찾거나 조작하는 등의 작업을 훨씬 쉽게 만들었습니다. `DOM` 은 웹사이트의 레이아웃과 콘텐츠를 구성하는 모든 요소를 의미합니다. 또한, `jQuery` 는 모든 주요 브라우저에서 웹 관련 기능이 의도한대로 작동하도록 보장합니다.

그 후 `jQuery` 는 지속적인 업데이트를 진행했고 사용자들이 점점 많은 것을 처리할 수 있도록 진화했습니다. 그 예로는 AJAX 요청, 애니메이션 그리고 수많은 유틸리티 함수가 있습니다. 이는 기본적으로 개발자에게 간단한 작업부터 고급 작업까지 보다 적은 코드로 쉽게 작성할 수 있도록 만들었습니다.

물론, `jQuery` 의 성공은 수많은 경쟁 라이브러리를 탄생시켰습니다. 그 중 하나는 2007년 출시한 `mooTools` 였지만 이는 빙산의 일각에 불과합니다. `jQuery` 가 계속 기능을 늘려갈 동안 일부 개발자는 크기와 복잡성을 최소화한 간결한 대안을 내놓거나 전혀 다른 방식으로 동작하는 라이브러리를 출시했습니다.

### jQuery: 현재와 미래 
오늘날 `jQuery` 는 일부 개발자는 구식이라 생각합니다. 그들은 `jQuery` 가 너무 많은 것을 처리한다고 느끼며 `jQuery` 로 해결한 문제들 중 일부는 JavaScript 발전과 더불어 JavaScript 엔진에 통합된 덕분에 더 이상 문제도 아니라고 생각합니다. 그러나, 위키백과에 따르면 “2022년 8월 기준으로, `jQuery` 는 가장 인기있는 천만개 웹사이트 중 77%에서 사용한다” 고 말합니다.

`jQuery` 는 확실히 대안이 많으며 그 중 하나는 단순히 JavaScript 라이브러리를 사용하지 않고 2006년 `jQuery` 출시 때보다 훨씬 더 나아진 JavaScript 엔진을 사용하는 것입니다. 하지만, 초보자의 경우 `jQuery` 는 훌륭한 출발점일 수 있습니다. 많은 작업을 쉽게 처리해주며 동적인 웹사이트를 만드는 재미있는 작업에 집중할 수 있고 지루하고 번거로운 부분은 `jQuery` 에 맡길 수 있습니다.

### 요약
JavaScript 라이브러리로 동적인 웹사이트를 쉽게 개발할 수 있습니다. 그리고 15년 전보다 현재는 덜 필요하지만 웹 작성에 JavaScript를 사용할 때는 시도해 볼 것을 고려해야 합니다.
...
# Getting started
## 개발 도구
JavaScript는 개발에 필요한 도구로 본다면 시작하기 가장 간단한 프로그래밍 언어 중 하나입니다. 사실, 시작할 때 아무것도 다운로드할 필요가 없습니다!! JavaScript는 어떤 텍스트 편집기라도 작성할 수 있으며 Windows 메모장(또는 Mac/Linux의 이와 동등한 프로그램) 같은 단순한 편집기도 사용 가능합니다. 

코드를 해석하거나 테스트할 때 필요한 것은 오직 웹브라우저 뿐입니다!! 다시 말해 값 비싼 IDE를 구매하거나 복잡한 컴파일러/인터프리터를 다운로드하지 않고 JavaScript를 시작할 수 있습니다.

그렇다해도 개발 경험을 향상시키기 위한 몇 가지 유용한 도구들이 존재합니다. 무엇을 향상할 수 있는지 조금만 이야기 하겠습니다.

## 에디터
앞서 말했듯이 JavaScript 코드를 작성할 때는 메모장 같은 단순한 텍스트 편집기면 충분합니다. 하지만, 실제 코드를 작성하기 위한 전용 에디터를 한 번이라도 사용했다면 효율성을 얼마만큼 향상시킬 수 있는지 잘 알 수 있습니다. 현대적인 코드 에디터/IDE(통합 개발 환경)는 자동완성(AutoComplete)과 IntelliSense 같은 편의 기능으로 보다 빠르고 효율적인 코드를 작성할 수 있도록 도와줍니다. 이런 기능은 JavaScript도 필요합니다.

JavaScript는 웹과 아주 밀접하므로 `HTML` 이나 `CSS` 를 작성할 때 코드/웹 에디터를 사용할지도 모릅니다. 그렇다면, 그 에디터는 아마 JavaScript 언어도 지원할 것이며 JavaScript를 배울 때도 그 에디터를 사용할 것을 권장합니다. 그렇지 않으면 다음과 같은 코드 에디터/IDE 중 하나를 구할 것을 추천합니다.

## 다양한 에디터

### 온라인 에디터
개발할 때는 온라인 에디터가 좋을 수도 있습니다. JavaScript는 웹과 밀접하므로 진짜 에디터와 유사한 기능을 처리하는 온라인 에디터가 많으며 이는 설치할 필요없이 웹 브라우저에서 곧바로 사용할 수 있습니다.

이들은 종종 “playgrounds” 나 “fiddles” 라 불리는데 주로 코드를 테스트하거나 시연하고 공유하도록 설계되어 있습니다. 따라서, 대규모 프로젝트보다 단순한 용도에 적합하지만(일부는 대규모 프로젝트에 사용될 수 있음) 이 튜토리얼의 코드를 테스트하거나 실험하기엔 완벽한 개발 환경입니다.

따라서, 선호하는 에디터가 없고 JavaScript를 배워볼 만한지 확인하려면 온라인 에디터를 추천합니다. 이들은 다양한 수준의 IntelliSense도 제공하며 JavaScript 코드를 효율적으로 작성할 수 있도록 도와주며 코드를 브라우저에서 테스트할 수도 있습니다. 몇 가지 제안은 다음과 같습니다:
  
  - [Playcode.io](https://playcode.io/new)
  - [JSFiddle](https://jsfiddle.net/)
  - [JavaScript compiler](https://onecompiler.com/javascript)

직접 사용한 후 마음에 드는 것으로 선택하시기 바랍니다.

### 단순 텍스트 편집기  
앞서 언급했듯이 운영체제에 기본 포함된 단순한 텍스트 편집기(Windows의 메모장 등)를 사용할 수 있습니다. 그러나, 이 선택은 최소한의 도움만 받으려는 순수 개발주의자를 위한 선택입니다. 대부분의 사람은 IntelliSense, 구문 검사 같은 편의 기능을 제공하는 다른 대안을 추천합니다.

### 웹 에디터
HTML, CSS, JavaScript 같은 웹 관련 기술에 중점을 둔 웹 에디터들이 많습니다. 때때로 HTML 에디터라 부르는데 아마 그렇게 시작했기 때문일 것입니다. 그러나, 이들 대부분은 JavaScript도 잘 지원합니다. 여기서 특정 에디터를 추천하진 않지만 위키로 확인할 수 있는 다양한 에디터 목록이 있습니다.

### Visual Studio Code  
Microsoft에서 개발한 Visual Studio Code는 JavaScript를 포함한 수 많은 언어로 코드를 작성할 수 있는 가장 포괄적인 IDE 중 하나입니다. 매우 강력한 다목적 도구로 처음 시작할 경우 최선의 선택이 아닐 수도 있습니다. 그러나, HTML, CSS, JavaScript 그리고 사실상 다른 어떤 언어까지 모두 처리할 수 있는 완전한 솔루션을 원한다면 VS Code는 확실히 시도할 가치가 있습니다. Windows, Mac, Linux에서 사용할 수 있으며 누구나 다운로드할 수 있습니다.

## 인터프리터/JavaScript 엔진  
특별한 요구 사항이 없다면 웹 브라우저에 내장된 JavaScript 엔진을 사용할 것을 권장합니다. 따라서, 단순 텍스트 편집기로 로컬에서 코드를 작성하고 브라우저로 열든 IDE로 브라우저를 열든 온라인 에디터에서 코드를 작성해서 직접 테스트하든 모두 다 사용할 수 있습니다.

반면, 브라우저 밖에서 JavaScript를 사용하려면 `Node.js` 를 다운로드해야 합니다. 그러나, 이 튜토리얼의 범위를 벗어납니다. 따라서, 여기선 언어 자체만 다룹니다.

## 요약
JavaScript는 시작하기 쉽고 간단합니다. 개발에 필요한 것은 Windows의 메모장이나 Linux/Mac에 기본 제공되는 단순한 텍스트 편집기와 웹브라우저 뿐입니다. 이것만 준비되면 JavaScript 학습을 계속할 수 있습니다.



<br><br>
# Hello, world!
역사적으로 모든 프로그래밍 관련 튜토리얼은 “Hello, world!” 예제로 시작합니다. 이 예제의 목적은 단순히 사용자에게 “Hello, world!” 라는 문장으로 이루어진 메시지를 출력하는 것이며 입력을 받거나 다른 어떤 일도 처리하지 않습니다. 이 예제는 1974년까지 거슬러 올라가는데 JavaScript가 발명되기 최소 20년 전이지만 그럼에도 불구하고 이 튜토리얼 역시 “Hello, World!” 예제로 시작합니다.

JavaScript로 메시지를 출력할 때는 해석하는 JavaScript 엔진에 따라 달라질 수 있습니다. 예를 들어, 웹 브라우저에서 실행하려면 `alert()` 함수로 메시지를 표시합니다:

```javascript
alert("Hello, world!");
```

이 예제를 실행하면 세계적으로 유명한 메시지가 담긴 멋진 팝업 윈도우를 눈으로 볼 수 있습니다. 앞서 이야기했듯이 이 튜토리얼은 웹 브라우저 기술에 집중하진 않지만 만약 이 코드를 HTML 파일에 삽입해서 실행하려면 반드시 `SCRIPT` 태그에 있어야 한다는 점을 언급해야 합니다.
...
## Hello, webpage!
실제(비록 단순하지만) 웹페이지에 JavaScript를 통합하는 다른 예제를 보여드리겠습니다. 이를 통해 어떻게 작동하는지 감을 잡을 수 있습니다:
...
예제를 직접 실행해 보시기 바랍니다. 컴퓨터에 `.html` 파일로 저장한 뒤 브라우저로 열 수 있습니다.

여러 가지 `HTML` 태그는 걱정하지 마시기 바랍니다. 약간의 텍스트와 웹페이지의 기본적인 구조일 뿐입니다. 대신 중간 부분의 스크립트 블록에 집중합니다. 그것으로 JavaScript 코드를 HTML 코드에 통합했고(즉, 웹페이지의 중간에) `document.write()` 함수로 콘텐츠에 텍스트를 출력했습니다.


## Hello, console!
`document.write()` 함수는 웹 브라우저 전용 함수로 `Node.js` 로 실행하면 작동하지 않습니다. 대신 많은 JavaScript 엔진에서 사용하는 `console.log()` 함수를 사용합니다:

```javascript
console.log("Hello, world!");
```

웹 브라우저로 위의 메시지를 읽으려면 개발자 콘솔을 확인해야 합니다. 개발자 콘솔에 접근하는 방법은 사용하는 브라우저에 따라 다르지만 보통 <kbd>F12</kbd> 키로 확인합니다. 만약, 작동하지 않는다면 다양한 브라우저에서 개발자 콘솔에 접근하는 방법을 설명한 글을 참고하는 것이 좋습니다.

## 요약
필수적인 “Hello, world!” 예제를 다뤘으니 이제 본격적으로 JavaScript 언어를 배우기 위해 앞으로 나갈 준비가 됐습니다.
...
만약, `C++, Java, C#` 또는 `PHP` 같은 인기있는 프로그래밍 언어 중 하나라도 경험이 있다면 JavaScript 문법을 쉽게 알아볼 수 있습니다. 왜냐하면, 이 모든 언어는 C 프로그래밍 언어에서 파생했거나 C 언어에서 큰 영감을 받았기 때문입니다. 만약, 이런 언어에 대한 경험이 전혀없거나 심지어 프로그래밍이 처음이면 JavaScript 문법에 대해 간단히 소개하겠습니다.

기본적인 규칙부터 시작합니다. 프로그래밍이 완전 처음이라면 혼란스러울 수 있는 규칙입니다:

## JavaScript는 대소문자를 구분합니다!!
"a 와 A 는 같지 않다!!" 는 것을 의미합니다. 예를 들어 보겠습니다:
...
소문자 a 변수를 선언했지만(변수는 나중에 다룹니다.) 다음 줄에서 대문자 A 변수를 참조합니다. 대소문자가 다르므로 실패합니다.

## Statements (문장)  
기본적으로 문장은 코드 한 줄을 의미합니다:

```javascript
alert("Hello, world!");
```

여기서 "세미콜론 문자로 끝난다!!" 는 것을 알 수 있습니다. 이것은 인터프리터에게 "문장이 끝났음!!" 을 알려줍니다. 문장은 보통 한 줄의 코드로 작성하지만 물리적으로 한 줄에 포함될 필요는 없습니다. 세미콜론으로 끝나는 한 여러 줄로 사용할 수 있습니다:
...
일부 C 계열 프로그래밍 언어는 세미콜론 문자가 필수지만 JavaScript는 그렇지 않습니다. 줄 바꿈에서 문장이 끝났다고 추측하며 많은 경우 제대로 작동합니다. 예를 들어:
...
코드가 복잡해질 경우 인터프리터는 사용 의도를 이해하지 못할 수 있으며 그럴 때는 세미콜론을 반드시 추가해야 합니다. 이런 이유로 JavaScript 코딩 가이드라인은 항상 세미콜론을 사용할 것을 권장합니다. 이렇게 처리하면 중요한 상황에서 세미콜론 문자를 잊지 않을 수 있고 나중에 보다 엄격한 프로그래밍 언어로 전환할 때도 새로운 습관을 배울 필요가 없습니다.


## Blocks (블록)
`if` 문이나 반복문 같은 제어 구조를 사용할 때 어떤 코드 줄을 포함하는지 제어해야 합니다. 기본적으로 JavaScript는 제어 구조에서 "다음 한 줄만 포함한다!" 고 가정합니다. 예를 들어:
...
위의 코드를 실행하면 "두 번째 `alert()` 함수만 실행된다!" 는 것을 알 수 있습니다. 그 이유는 `if()` 문은 결코 참이 될 수 없으므로 그 다음 줄은 실행하지 않습니다. 

그러나, `if` 문에서 여러 줄 코드를 포함하면 코드 블록을 만들어야 합니다. 중괄호 문자로 `if` 문으로 영향받는 코드 줄을 묶습니다:
...
위의 예제는 `if` 문 다음 줄(첫 번째 예제)과 블록(두 번째 예제)을 들여쓰기로 작성했습니다. 필수 사항은 아니지만 대부분의 프로그래머는 코드 가독성을 위해 이렇게 처리합니다. 키보드의 <kbd>Tab</kbd> 키로 한 줄 또는 여러 줄을 들여쓸 수 있습니다. 그리고, 블록에 또 다른 블록이 있다면 적절히 들여씁니다. 블록마다 한 번씩 들여쓰는 방식입니다. 예를 들어:
...
코드 블록은 JavaScript에서 자주 사용합니다. 위의 예제처럼 제어 구조뿐만 아니라 함수를 정의할 때도 사용합니다. 이에 대해서는 나중에 다룹니다.

## Comments(주석)
기본적으로 JavaScript는 작성한 모든 것을 공백을 제외하고 해석하므로 유효한 JavaScript 코드여야 합니다. 그러나, 때때로 자신이나 팀을 위해 주석(설명)을 남길 때도 있습니다. 다행히 JavaScript는 주석을 쉽게 처리할 수 있습니다. 

인터프리터가 두 개의 슬래시 문자(`//`)를 만나면 그 줄 나머지는 해석하지 않고 무시합니다:

```javascript
// This line is a comment
```

문장과 같은 줄에서 사용할 수 있습니다. 두 개의 슬래시 문자 이후는 "인터프리터가 무시한다!" 는 점만 기억합니다:

```javascript
alert("Hello!"); // The rest of this line is a comment
```

여러 줄을 주석으로 연속 사용할 수 있습니다:
...
그러나, 많은 줄에 주석이 필요하고 각 줄마다 두 개의 슬래시 문자를 붙이지 않으려면 여러 줄 주석 구문을 사용합니다. 슬래시 문자 뒤에 별표(`*`) 문자를 붙여 시작합니다. 그 이후는 모두 주석문으로 간주하며 별표 문자 뒤에 슬래시 문자를 붙여 닫을 때까지 계속됩니다. 예를 들어:
...
이 두 가지 주석 처리 방식을 자유롭게 사용할 수 있습니다.

## 요약
이번 섹션은 JavaScript 문법(syntax)에 대한 간단한 소개입니다. 하지만, 튜토리얼 나머지 예제를 이해할 때 도움됩니다. 물론, 배워야 할 문법이 많지만 언어의 다양한 측면을 소개할 때 차차 다룹니다.



<br><br>
# Variables
프로그래밍에서 필수적인 부분은 바로 변수(variable)입니다. 변수를 일종의 저장 공간으로 생각합니다. 숫자, 날짜, 혹은 텍스트 같은 정보를 나중에 사용할 수 있도록 보관할 수 있는 일종의 공간입니다. 이 기능이 없다면 멀리 나가지 못하며 물론 JavaScript도 변수를 사용합니다.

변수는 `let` 키워드로 선언합니다. 혹은 저처럼 구식 JavaScript 개발자라면 `var` 키워드를 사용할 수 있습니다. 둘 다 사용할 수 있지만 약간의 차이가 있으며 이에 대해서는 나중에 설명합니다. 지금은 단지 새로운 버전의 `let` 키워드를 사용합니다.

변수 선언은 `let` 키워드 뒤에 변수 이름(이름은 여러분이 정합니다)으로 구성됩니다. 예를 들어:
...
이것은 단순히 age 라는 이름의 변수가 존재한다는 것만 인터프리터에 알려줍니다. 아직까지 값이 없지만 선언 후에 다음과 같이 값을 할당할 수 있습니다:
...
물론 변수의 값을 안다면 선언과 동시에 값을 할당할 수 있습니다:

```javascript
let age = 42;
```

심지어 여러 개의 변수를 서로 다른 값으로 한 줄로 선언할 수 있습니다. 이때 쉼표 문자로 변수를 구분합니다:

```javascript
let name = "John Doe", age = 42, mail = "john@doe.com";
```

하나 또는 여러 개의 변수를 선언한 후에는 원하는대로 사용하거나 변경할 수 있습니다. 예를 들어:
...
위의 예제에서 두 개의 변수를 선언한 후 사용합니다. name 변수는 단순히 출력하고 age 변수는 값을 변경한 후 출력합니다. 보시다시피 꽤 간단합니다.

<br><br>
## 변수 이름 짓기
앞서 언급했듯이 변수 이름은 자유롭게 정의할 수 있습니다. 하지만, 몇 가지 네이밍 규칙이 있습니다:

- 변수 이름은 문자, 숫자, 그리고 두 개의 특수 문자(`$, _`)만 사용할 수 있습니다.
- 변수 이름은 숫자로 시작할 수 없습니다!!
- 키워드/예약어는 사용할 수 없지만 변수 이름의 일부로 사용할 수 있습니다. 예를 들어, `break` 는 키워드로 변수 이름을 `break` 로 사용할 수 없지만 `lunchBreak` 는 사용할 수 있습니다.

JavaScript에서 복잡한 변수 이름을 지을 때는 `camelCase` 방식을 사용합니다. 첫 단어는 소문자로 시작하고 그 이후 단어들은 대문자로 시작하는 방식입니다. 예를 들어:
...
하지만, 단지 가이드라인일 뿐입니다. 전부 소문자나 대문자를 사용하는 것도 자유입니다. 단, JavaScript는 "대소문자를 구분한다!!" 는 점을 항상 기억하시기 바랍니다. `NAME` 이란 변수를 선언했다면 `name` 으로 참조할 수 없습니다!! 

<br><br>
## 상수 (Constants)
지금까지 변수는 언제든지 원하는 값을 할당하거나 변경할 수 있다는 것을 배웠습니다. 하지만, 때로는 변수를 선언한 즉시 값을 할당하고 그 값을 절대 변경할 수 없도록 처리할 때가 있습니다. 이를 위한 상수(constant)가 있습니다. 상수는 변수와 비슷하지만 `var` 나 `let` 대신 `const` 키워드를 사용합니다:

```javascript
const answerToEverything = 42;
```

여기서 "선언 즉시 값을 상수에 할당했다!" 는 것에 주목합니다. 변수와 달리 값을 할당하지 않고 상수를 선언할 수 없습니다!! 그리고 상수이므로 값을 변경할 수 없습니다. 만약, 상수의 값을 변경하면 인터프리터는 즉시 오류를 발생시킵니다:
...
"값을 변경할 수 없는 변수가 있다!" 는 것은 유용합니다. 왜냐하면, 어떤 것은 변경하면 안되기 때문입니다. 이런 이유로 JavaScript도 자체 상수를 많이 포함합니다. (예: `Math.PI`)
...
## 요약
변수를 일종의 라벨이 붙은 잘 정리된 저장 상자로 생각합니다. 변수는 어떤 정보든 저장할 수 있고 이름을 붙이고 나중에 그 내용에 접근하거나 수정할 수 있습니다. 변수는 프로그래밍에서 중요한 핵심이며 이 튜토리얼의 많은 예제에서 사용하는 것을 자주 볼 수 있습니다. 그리고, 실제 코드를 작성하면 자주 사용합니다.


<br><br>
# Truthy & Falsy
대부분의 프로그래밍 언어에나 수학에서도, "참" (true)과 "거짓" (false)의 개념은 완전히 "이진적" 입니다. 어떤 것이 "참" 이면, 그렇지 않으면 "거짓" 입니다. 그러나, JavaScript에서 "참과 거짓의 개념은 확장" 됩니다. 왜냐하면, 이 언어는 일반적으로 꽤 느슨하고, 다른 프로그래밍 언어들만큼 엄격하지 않기 때문입니다.  

실제로 어떤 것이 완전한 참도 거짓이 아님에도 불구하고, JavaScript는 참 또는 거짓으로 평가할 수 있다는 것을 의미합니다. 그 목적은 일을 보다 쉽고 덜 복잡하게 만드는 것입니다. 하지만, 이것을 제대로 알지 못한다거나 잠시 잊는다면 상황이 조금 혼란스러워질 수도 있습니다.  

여러분께 새로운 "Truthy" 와 "Falsy" 개념을 소개합니다. 이들은 단지 "개념" 으로만 존재합니다. 따라서, JavaScript에서 "Truthy" 나 "Falsy" 라는 키워드를 찾을 수 없으며, "Truthy" 나 "Falsy" 값과 직접 비교할 수도 없습니다. 대신, JavaScript는 어떤 것이 완전히 "참" 인지 "거짓" 인지 검사하기 위한, 두 번째 연산자 집합을 도입했습니다. 이것은 "엄격한 연산자" (strict operators)라고 불리며 예를 들어, "엄격한 동등 연산자" (strict equality operator)는 두 개의 등호 문자 대신, 세 개의 등호 문자로 두 값을 비교합니다.  

JavaScript의 다양한 연산자들 특히, "엄격한" 연산자는 이후의 장에서 훨씬 더 많이 이야기합니다. 하지만, 지금은 단지 무엇이 "Truthy" 또는 "Falsy" 로 간주할 수 있는지만 이야기하겠습니다.  

## Truthy  
JavaScript에서 "Falsy" 로 지정하지 않은 모든 값은 "Truthy" 입니다!! 

JavaScript는 루프나 `if` 문과 같이 불리언 값이 요구되는 문맥에서, 어떤 것을 불리언 값으로 변환하기 위해 "타입 강제 변환" (type coercion)을 사용합니다. 예를 들어, 다음 값은 모두 "Truthy" 이므로 "참" 으로 처리됩니다:  
...
위의 예제를 실행하면, 세 개의 `if` 문 모두 "참으로 평가된다" 는 것을 알 수 있습니다. 그런데, 왜 "hello world" 까지 참일까요? 실제 값을 포함하고 있기 때문에 JavaScript는 참 또는 거짓이 요구되는 문맥에서 이것을 "Truthy" 로 간주하기 때문입니다.  

## Falsy  
일반적으로, `null, undefined`, 또는 비어있는 값은 "Falsy" 로 간주하며, 루프나 `if` 문 같은 불리언 문맥에서 "거짓" 으로 간주합니다. 몇 가지 예는 다음과 같습니다:  
...
## 요약  
JavaScript가 어떤 것을 "참" 또는 "거짓" 으로 판단할 때 예를 들어, 루프나 `if` 문의 조건으로 평가할 때, 필요하면 "타입 강제 변환" 으로 실제 불리언이 아닌 값을 참 또는 거짓으로 바꿉니다. 그 결과를 "Truthy" 와 "Falsy" 라고 부릅니다. 

왜냐하면, 실제 "참" 이나 "거짓" 은 아니지만, JavaScript가 이를 기반으로 결정을 내리기는 충분하기 때문입니다. 이 글의 예제가 이것을 제대로 보여줍니다.  

---

# Control structures
## The IF statement
"코드의 실행 흐름을 제어한다는 것" 은 프로그래밍에서 가장 중요한 처리 개념 중 하나이며, 가장 기본적인 제어 구조는 `if` 문입니다. `if` 문은 다음 줄(들)의 실행 여부를 결정하는 "조건" 을 지정할 수 있습니다. 간단한 예는 다음과 같습니다:  
...
첫 번째 줄이 바로 `if` 문입니다. 즉, 다음 줄의 실행 여부를 결정하는 간단한 "조건" 입니다. 

이 예제를 사용자에게 입력받는 형태로 바꿔서, 실제 작동하는지 테스트합니다:  
...
튜토리얼 앞부분에서 말했듯이, 여러 줄의 코드가 필요할 경우는 반드시 중괄호 문자로 감싸서 "코드 블록" 으로 만들어야 합니다:  
...
`if` 문은 단순히 주어진 "조건이 참으로 간주될 수 있는지 확인" 합니다. 또한, 원하는 만큼 "조건을 추가" 할 수 있습니다. 

`&&` (AND)와 `||` (OR) 연산자로 "조건을 추가" 합니다. (이에 대해서는 나중에 다룹니다.)  
...
즉, 운전면허가 있거나, 혹은 적절한 나이(18세 이상 80세 이하)라면, 위의 코드는 자동차 운전을 허용합니다. 현실적인 예는 아니지만, `if` 문에 "여러 조건" 을 어떻게 사용할 수 있는지 보여주기는 충분합니다.  

### if...else  
`if` 문은 "조건이 참일 때" 만 무언가 실행합니다. 그러나, 조건이 "거짓" 일 경우 다른 동작을 처리할 수 있습니다. 이를 위한 `else` 문이 있으며, 당연히 `if` 문과 함께 작동합니다. 예를 들어:  
...
그리고 다시, 특정 조건(또는 양쪽)에 여러 줄의 코드가 필요하다면, 블록으로 감싸면 됩니다:  
...
필요하면, 이전 예제처럼 조건을 코드 블럭으로 감쌀 수도 있습니다.  

## 요약  
`if` 문은 사용하기 쉽고 이해하기 간단하지만, 그만큼 강력합니다. 프로그래밍 세계에 처음왔다면, `if` 문이 얼마나 흔하게 사용되는지 알면, 놀랄 수도 있습니다. 또한, `if...else` 문을 작성할 때 사용할 수 있는 보다 짧은 "대체 표기법도 있다" 는 것도 알아두시기 바랍니다. 하지만, 연산자에 관한 장에서 다룹니다. 지금은 좀 더 고급 제어 구조로 넘어가겠습니다.  


## The switch statement
`switch` 문은 동일한 변수를 검사하는 일련의 `if` 문과 비슷하지만, 각 값마다 다른 동작을 수행하도록 처리할수 있습니다. 복잡하게 들릴 수 있지만, 전혀 그렇지 않습니다. 다음과 같은 간단한 예를 살펴보겠습니다:  
...
### break 키워드  
각 `case` 끝에 사용한 `break` 키워드에 주목하시기 바랍니다. `switch` 문은 각 `case` 문을 "올바르게 종료" 해야 하며, 이를 위해 `break` 키워드를 사용하거나, 함수일 경우, `return` 키워드로 함수를 완전히 종료해야 합니다. 

만약, 이를 생략하면, 해당 값과 일치한 `case` 아래의 모든 `case` 문으로 계속 흘러갑니다. 위의 예제에서 `break` 키워드를 제거한 후, 다시 실행하면 쉽게 확인할 수 있습니다.  

마지막 `break` 키워드는 생략할 수 있습니다. 왜냐하면, 어차피 `switch` 문은 끝나기 때문입니다. 하지만, 모든 프로그래밍 언어에서 작동하는 것은 아니므로, "일관성" 을 위해 프로그래머는 모든 `case` 문에 항상 `break` 키워드를 포함하는 경향이 있습니다.  

### Case 그룹  
때로는 여러 `case` 문에서 동일한 동작을 수행할 때가 있습니다. 이것도 문제없습니다. 단순히 여러 `case` 문을 서로 쌓아올려 같은 코드를 사용하도록 처리하면 됩니다. 예를 들어:  
...
### default 키워드  
위의 예제는 명백한 결함이 있습니다. 사용자가 정확히 일치할 값을 입력하지 않으면, 아무런 일도 일어나지 않는다는 점입니다. 다른 옵션에 해당되지 않을 때, 실행되는 "대체 동작" (fallback action)이 있다면 좋습니다. 다행히, JavaScript는 `default` 키워드로 처리할 수 있습니다. 

다음은 위의 예제를 수정해서 `default` 키워드를 추가한 버전입니다:  
...
이제 `switch` 문은 모든 상황을 처리할 수 있습니다. `default` 키워드로 대체 동작을 처리할 수 있기 때문입니다.  

### Case 비교  
위의 예제를 실제 상황에 사용하면, 각 `case` 문의 조건과 어떻게 비교하는지 고려해야 합니다. 예를 들어, "yes" 를 검사하지만, 문자열 비교는 항상 "대소문자를 구분" (case-sensitive)합니다. 

따라서, 사용자가 "Yes", "YeS", "YES" 라고 입력하면, 첫 번째 `case` 문은 일치하지 않습니다. 항상 이런 처리를 해야 하며 예를 들어, 비교하기 전에 답변을 모두 "소문자로 변환" 할 수 있습니다:  
...
또 다른 고려 사항은 모든 비교는 "엄격한 동등 비교 연산자(`===`)를 사용한다" 는 점입니다. 즉, `switch` 문의 표현식이 문자열이면, 다른 문자열과 비교할 수 있다는 뜻입니다. 비록, 그 문자열이 숫자를 나타내더라도 말입니다. 예를 들어 보겠습니다:  
...
하지만, 위의 예제는 기대한대로 작동하지 않습니다. 왜일까요? `prompt()` 함수의 결과는 사용자가 숫자를 입력했더라도, "항상 문자열로 인식" 하기 때문입니다. 그런데도 숫자(41, 42)와 비교하고 있습니다. 이를 제대로 처리하려면 예를 들어, `prompt()` 함수의 처리 결과를 "숫자로 변환" 하거나, 문자열로 비교해야 합니다. 따라서, 첫 번째 줄을 바꿔서 `Number()` 함수를 사용하겠습니다:  
...
## 요약  
`switch` 문을 사용하면 하나의 표현식으로 여러 가지 선택지와 비교하거나, 그에 따라 다른 동작을 수행할 수 있습니다. 이것은 일련의 `if...else...if` 문과 비슷하지만, 좀 더 읽기 쉬운 구문 형식입니다. `switch` 문은 "엄격한 동등 비교(===)연산자를 사용한다" 는 점도 기억하시기 바랍니다.  


## Loops: for and while
지금까지 프로그래밍 언어의 중요한 부분인 `if` 문을 학습했습니다. 똑같이 중요한 것이 "반복문" (loop)입니다. 반복문은 동일한 코드 블록을 여러 번 "반복 실행" 할 수 있으며, 반복 횟수를 제어할 수 있는 기능도 제공합니다.  

현재 JavaScript에는 `for` 문과 `while` 문 두 가지 종류의 반복문 구조가 있습니다. 여러 가지 변형도 존재하며, 이번에는 어떻게 작동하고, 어떻게 사용하는지 좀 더 깊이 파헤쳐 보겠습니다.  

### while 문  
가장 간단하게 시작할 수 있는 반복문은 `while` 문입니다. 사용 구문은 매우 단순합니다:  
...
괄호 안의 조건이 "참으로 평가되는 한", 그 뒤의 코드 블록은 계속 "반복" 실행됩니다. 이때, 반복될 코드 블록에 반드시 루프를 종료할 수 있는 무언가의 조건을 추가해야 합니다. 그렇지 않으면, 이른바 "무한 루프" (endless loop)가 만들어지고 코드는 끝나지 않게 됩니다.  
...
위의 코드는 "counter" 변수가 10 보다 크지 않은 동안 실행되며, 각 반복마다 숫자를 출력하고 "counter" 변수를 증가시킵니다. 특히, 마지막 부분이 중요합니다. 만약, 이 변수를 증가시키지 않으면 "counter" 변수는 결코 10 보다 커지지 않으므로, 루프는 끝없이 실행됩니다. 

나중에 살펴 보겠지만, 사실 이 작업은 `for` 문이 좀 더 적합합니다. 하지만, `while` 문이 어떻게 작동하는지 보여주기 위한 단순한 예제로 사용했습니다.  

### do..while 변형  
일반적인 `while` 문은 위의 예제처럼 루프에 들어가기 전에 "조건을 먼저 평가" 합니다. 만약, 조건이 처음부터 참이 아니라면, 루프에 한 번도 들어가지 않을 수 있다는 뜻입니다. 확인하려면 위의 예제에 "counter" 변수의 초기값을 11 로 바꿔 실행하시기 바랍니다.  

`do..while` 문은 이런 처리 순서를 뒤집습니다. 즉, 코드를 "먼저 실행한 뒤에 조건을 평가" 합니다. 따라서, "최소한 한 번은 실행" 되며, 이것은 특정 상황에서 유용할 수 있습니다. 예를 들어:  
...
여기서 `while` 문을 `do..while` 문으로 바꾸고, "counter" 변수의 초기값을 11 로 설정했습니다. 만약, 일반적인 `while` 문이면, 루프의 코드는 결코 실행되지 않습니다. 하지만, `do..while` 문으로 조건을 첫 번째 코드를 실행한 후에 평가하므로, "최소 한 번은 실행" 됩니다.  

### for 문  
`while` 문의 대안은 `for` 문입니다. 좀 더 복잡해 보이지만, 유연하고 자주 사용합니다. `for` 문은 "카운터 변수 선언 → 조건 지정 → 증감식" 을 한 줄에 작성하기 때문에 복잡해 보일 수 있습니다. 예를 들어:  
...
위의 코드는 이전 `while` 문과 동일한 일을 처리합니다. 즉, 1 부터 10 까지 출력합니다. 하지만, 코드는 더 짧습니다. 왜냐하면, 카운터 메커니즘이 `for` 문에 내장되기 때문입니다.  

코드의 첫 줄을 인터프리터에게 다음과 같이 지시합니다:  

- "counter" 변수를 선언하고 1 을 할당합니다.  

- "counter" 변수가 10 이하일 동안, 루프를 실행합니다.  

- 각 반복마다 "counter" 변수를 1 씩 증가시킵니다.  

좀 더 현실적인 사용 예는 다음과 같습니다:  
...
### for 문의 처리 순서  
`for` 문은 코드 순서대로 실행되지 않습니다. 다음과 같이 처리됩니다:  

1. 변수를 선언합니다.  

2. 조건을 검사합니다. (처음부터 조건이 거짓이면, 반복은 한 번도 실행되지 않습니다.)  

3. 조건이 참이면, 루프 코드를 실행합니다.  
...
실행 결과는 2 부터 10 까지의 "짝수" 를 출력합니다.  

### 루프 제어: break 와 continue  
루프를 좀 더 세밀하게 제어할 때가 있습니다. JavaScript는 루프 전용 키워드인 `break` 와 `continue` 키워드가 있습니다.  
...
위의 코드는 1 부터 10 까지 세지만, 홀수일 경우, `continue` 키워드로 건너뛰고 짝수만 출력합니다.  
...
## 요약  
반복문은 동일한 코드를 여러 번 "반복 실행" 할 수 있으며, 각 반복마다 다른 결과를 만들 수 있습니다. `while` 문과 `for` 문 중에서 어떤 것을 사용할지는 상황에 따라 다르지만, 대부분의 경우 둘 다 사용할 수 있으며, 개인적인 취향에 따라 선택합니다.  

## Loops: for..of and for..in
방금 JavaScript의 기본 반복문인 `for` 와 `while` 문에 대해 다뤘습니다. 특히, `for` 문은 매우 유연하며, 어떤 경우에도 사용할 수 있습니다. 그러나, JavaScript 최신 버전에는 `for` 문의 두 가지 "새로운 버전을 추가" 했습니다: 바로 `for..of` 문과 `for..in` 문입니다.  

### for..of 문  
`for..of` 문은 어떤 종류의 `iterable` 객체든, 모두 순회(iterate)할 수 있습니다. 이 주제는 꽤나 고급이기 때문에, 제대로 다루지 않았지만, 미리 `for..of` 문을 알아두면 좋습니다. `iterable` 객체는 나중에 좀 더 이야기합니다.  

지금 알아야 할 것은, JavaScript에는 `iterable` 객체로 구현한 여러 가지 "내장 객체" 들이 있다는 점입니다. 가장 대표적인 것은 `Array` 객체이며, 사실, `String` 객체도 이를 구현하고 있습니다. 

배열, 객체, 문자열은 아직까지 다루지 않았으므로, 지금은 예제를 단순하고 직관적으로 유지하겠습니다. 나중에 이런 특별한 `for` 문법을 복습할 때, 다시 돌아오면 됩니다.  

먼저, 일반적인 `for` 문과 `for..of` 문의 차이점을 보여드리겠습니다. 이전까지 배열은 다음과 같이 순회하면서 처리했습니다:  
...
배열의 `length` 속성으로 배열 항목에 접근할 때, 어디까지 처리할지 범위를 알 수 있습니다. 이 방식은 잘 작동했고, 과거에는 배열을 순회하는 "표준적인 처리 방법" 이였습니다.  

그러나, `for..of` 문으로 이 작업을 훨씬 단순화할 수 있습니다. 같은 작업을 훨씬 더 "간단한 문법" 으로 수행하는 다음과 같은 예제를 보시기 바랍니다:  
...
작동 방식은 매우 간단합니다. "fruit" 변수를 선언하고, (물론 원하는 이름으로 바꿀 수 있습니다.) 각 반복마다 이 변수는 배열의 다음 항목으로 자동으로 업데이트됩니다. 매우 단순하면서도 효과적입니다.  

그리고, `String` 객체도 `iterable` 객체이므로, 다음과 같이 문자열도 쉽게 순회할 수 있습니다:  
...
### for..in 문  
`for..of` 문은 컬렉션(예: 배열, 문자열)을 위한 것이고, `for..in` 문은 객체를 위한 것입니다. JavaScript에서 "객체" 는 기본적으로 "키와 값의 모음" 이므로, 이 속성을 순회할 수 있습니다. 이것은 매우 유용합니다. 

객체에 대해서는 나중에 자세히 다루지만, 지금은 단지, `for..in` 문이 어떻게 작동하는지 간단히 살펴 보겠습니다:  
...
보시다시피, `for..of` 문과 비슷하지만, 각 반복마다 "속성 이름" 을 가져옵니다. 물론, 실제 속성 값을 가져올 수 있습니다:  
...
보시다시피, 사용하기도 쉽고 강력합니다. 객체를 다룬다면 `for..in` 문의 유용함을 금방 알 수 있습니다.  
...
위의 예제는 `continue` 와 `break` 키워드 모두 사용했습니다. "Hello!" 문자열을 순회하면서, "l" 문자를 만나면, `continue` 키워드로 건너뛰고, "!" 문자를 만나면, `break` 키워드로 루프를 종료합니다. 그 외의 경우는 문자를 출력해서, "H", "e", "o" 를 출력합니다.  

## 요약  
`for..of` 와 `for..in` 문은 JavaScript에 비교적 최근에 "추가한 새로운 기능" 으로 특히, "배열과 객체를 순회" 할 때, 좀 더 쉽게 사용할 수 있습니다. 기존 `for` 와 `while` 문에 비해 직접적인 장점이 많은 것은 아니지만, 좀 더 간단하게 작성할 수 있고, 코드가 덜 복잡해 보이도록 만듭니다.  

앞서 언급했듯이, 배열과 객체는 아직까지 다루지 않았지만, 곧 다룰 예정입니다. 그러니, 이 두 가지 특수한 반복문은 기억했다가, 배열과 객체를 다룰 때 활용해 보시기 바랍니다.  

---

# Data types
## Introduction
튜토리얼 시작에서 JavaScript는 "동적 타입 언어" 라는 사실은 이야기했습니다. 이번과 이어지는 장에는 JavaScript를 사용할 때, 접하게 될 다양한 "데이터 타입" 에 대해 설명합니다. 

그런데, 프로그래밍 언어가 동적 타입 시스템을 사용하는데 왜 데이터 타입에 신경써야 할까요?  

사실, JavaScript는 데이터의 타입에 그다지 엄격하지 않지만, 여전히 "타입이 중요한 상황은 존재" 합니다. 예를 들어, "42"(숫자 42를 담은 문자열)를 숫자 2 와 곱하면 어떻게 될까요? 엄격한 타입의 언어라면 이런 상황은 오류를 만들 가능성이 크지만, JavaScript는 전혀 개의치 않습니다.  

### 동적 타입 (Dynamic types)  
"동적 타입" 이 무엇인지, 그리고 어떻게 사용할 수 있는지 조금 이야기하겠습니다. 강타입(strongly typed) 프로그래밍 언어는 변수를 특정 타입(예: 문자열, 정수)으로 선언하며, 한 번 선언한 변수의 타입은 바꿀 수 없습니다. 만약, 다른 타입의 값을 할당하면 컴파일러는 오류를 출력합니다.  

하지만, JavaScript는 타입을 정의할 필요가 없으며, 다른 타입을 쉽게 재할당할 수 있습니다. 예를 들어:  
...
여기서 "answer" 라는 변수를 선언하고 숫자 42 를 할당했습니다. 다음 줄에서 문자열을 재할당했고, 그 값을 출력했습니다. JavaScript는 이런 변경에 전혀 불평하지 않습니다. 변수는 특정 타입을 기대하지 않기 때문에, 원하는 대로 자유롭게 사용할 수 있습니다.  

### 타입은 여전히 중요하다 (Types still matter)  
하지만, 앞서 언급했듯이, "동적 타입" 이라 해서 타입이 존재하지 않는 것은 아닙니다. JavaScript는 내부적으로 많은 처리를 자동으로 해주기 때문에, 타입에 크게 신경쓰지 않아도 됩니다. 

이를 설명하기 위해, 숫자 두 개를 곱하는 예제로 돌아가겠습니다. 둘 중 하나는 숫자를 담고 있는 문자열입니다:  
...
이 경우, JavaScript는 "두 숫자를 곱한다고 추측" 합니다. 따라서, "문자열임에도 불구" 하고 결과는 84 가 됩니다. 하지만, 덧셈을 시도하면 어떻게 될까요?  
...
결과는 44 라고 예상할 수 있지만, "422" (문자열)가 나옵니다. 왜일까요? 이 경우 JavaScript는 여러분이 수학 연산을 수행하는지 전혀 알 수 없기 때문에, 두 변수를 "문자열로 취급" 해서 단순히 이어붙입니다. 

이것은 "동적 타입 언어도 여전히 타입은 중요하다" 는 이유를 잘 보여줍니다. 위의 예제는 경고나 오류를 내지 않지만, 처리 결과는 기대한 것과 다를 수 있습니다.  
### 원시 타입 (Primitive types)  
대부분 프로그래밍 언어는 "원시 타입과 복합/참조 타입" 을 구분합니다. JavaScript도 마찬가지로, 여러 가지 "원시 타입" 과 "객체 타입" 을 구분합니다. 참고로, JavaScript가 지원하는 원시 데이터 타입은 다음과 같습니다:  
...
이번 장은 "원시 타입" 에 관한 것이며, 각각 살펴보겠습니다. 그 후에 내장 객체들과, 복합 타입을 정의하는 방법도 알아보겠습니다.  

## 요약  
JavaScript는 "동적 타입 시스템을 사용" 하므로, 변수를 선언하고 사용할 때, 타입을 별도로 지정할 필요가 없습니다. 그러나, 이번에 살펴봤듯이, 서로 다른 타입의 변수를 함께 사용할 때는 "주의" 해야 합니다.  

다음에는 JavaScript가 내부적으로 사용하는 다양한 데이터 "타입" 을 좀 더 깊이 파헤치고, 어떻게 다룰 수 있는지 알아보겠습니다.  


## Booleans
JavaScript, 그리고 많은 다른 프로그래밍 언어들에서 찾을 수 있는 가장 단순한 타입 중 하나는 바로 `boolean` 타입입니다. 이 타입은 오직 `true` 또는 `false` "두 가지 값" 만 가질 수 있습니다. 또한, 이 두 값은 "키워드" 이므로, 변수를 선언할 때, 사용할 수 없습니다.   
...
또한, `boolean` 값은 JavaScript의 내장 함수에서 반환할 수 있으며, 다양한 연산자(이에 대해서는 나중에 다룹니다)를 사용한 결과로 얻을 수 있습니다. 예를 들어, 두 숫자를 여러 방식으로 비교하면, `boolean` 값을 얻을 수 있습니다:  
...
이것이 바로 `if` 문에서 "조건" 으로 사용할 수 있는 이유입니다. 조건을 검사하고, 결과가 `true` 라면 뒤따른 코드를 실행합니다:  
...
## boolean vs. Boolean  
`boolean` 타입은 JavaScript에서 이른바, "원시 타입" (primitive type)에 속합니다. 이것은 언어의 "기본적이고 필수적인 부분이며, 확장할 수 없다" 는 뜻이기도 합니다.  그러나, JavaScript는 대부분의 원시 타입에  같은 이름을 가지되, "대문자로 시작하는 객체 버전" 도 제공합니다. 따라서, `boolean` (원시 타입)과 `Boolean` (대응되는 객체)을 갖습니다.  

일반적으로 객체 버전은 기본 단순 타입을 다룰 때 도움을 주며, 기능을 확장할 수 있습니다. 그러나, `boolean` 은 너무 기본적이기 때문에(기억하세요, 오직 `true` 와 `false` 만 이해합니다), `Boolean` 객체를 자주 사용하지 않습니다.  

`Boolean` 객체는 다음과 같이 생성합니다:  
...
여기서 초기값 `true` 를 가진 `Boolean` 객체를 생성했습니다. 만약, `false` 를 원한다면, 단순히 이 값을 바꾸거나, 아예 값을 생략할 수 있습니다. 빈 값은 `false` 로 간주합니다:  
...
흥미로운 점은, 다음 두 변수는 모두 `true` 라는 것입니다:  
...
## 요약
JavaScript의 `boolean` 타입은 "원시 타입" 으로, 사용 가능한 값은 오직 두 가지: `true` 또는 `false` 뿐입니다. JavaScript는 `Boolean` 객체도 함께 제공하지만, 일반적으로 거의 사용하지 않습니다.  


## Numbers
숫자를 다룬다는 것은 어떤 프로그래밍 언어든 매우 중요한 부분입니다. 컴퓨터는 내부적으로 정수, 부동소수점 숫자 등 서로 다른 종류의 숫자를 서로 다르게 처리합니다. 그래서, 많은 프로그래밍 언어들은 여러분도 그렇게 사용하도록 강제하지만, JavaScript는 다양한 종류의 "숫자를 자유롭게 섞어서 사용" 할 수 있습니다.  
...
그리고, 부동소수점 숫자를 원한다면, 그것도 문제없습니다. 마찬가지로 쉽게 처리할 수 있습니다. 단순히 정수 부분과 소수 부분을 "마침표 문자로 구분" 하면 됩니다:  

```javascript
let n = 42.123;
```  

이것은 정수와 부동소수점 숫자를 같은 타입처럼 다루며, 수학 연산까지 처리할 수 있다는 뜻이고, 그 덕분에 훨씬 더 간단해집니다. 예를 들어:  
...
큰 숫자를 정의할 때, 코드에서 그 숫자가 얼마나 큰지 한 눈에 보기에 어려울 수 있습니다:  

```javascript
let n1 = 42000000000;
```  

JavaScript는 "숫자 구분자" 라는 "문법적 설탕" (syntactic sugar)을 제공합니다. 즉, 밑줄(`_`) 문자로 "자릿수를 그룹화" 할 수 있습니다:  
...
때때로 실제 "문자열 값을 숫자로 변환" 할 때가 종종 있습니다. 이를 위해, 전역적으로 사용할 수 있는 `parseInt()` 와 `parseFloat()` 두 가지 함수가 있습니다. 이름 그대로, 문자열이 정수 또는 부동소수점 숫자 형태면 실제 "숫자로 변환" 합니다.  
...
만약, 숫자로 변환하지 않았다면, JavaScript는 두 문자열을 이어붙인다고 간주해서 결과는 "402.42" 가 됩니다. 하지만, 원하는 결과는 "42.42" 입니다.  

대안으로, `Number()` 함수를 사용할 수 있습니다. 이 함수는 어떤 타입의 입력이든 "숫자로 변환하려 시도" 합니다:  
...
JavaScript는 숫자를 다루는 데 유연하지만, 어떤 값이 실제 숫자인지 확실히 판단할 때가 있습니다. 이를 위해, JavaScript는 전역적으로 사용할 수 있는 상수인 `NaN` 을 제공합니다. 이것은 숫자 결과를 반환해야 하지만 불가능할 때, 여러 함수에서 반환합니다. 예를 들어:  

```javascript
alert(parseInt("forty two"));
```  

여기서, `parseInt()` 함수는 숫자를 반환해야 하지만, "forty two" 는 유효한 숫자는 아니므로, `NaN` 을 반환합니다. 따라서, 연산 결과가 실제 숫자인지 확인할 때가 있습니다. 이때는 전역적으로 사용할 수 있는 `isNaN()` 함수를 사용합니다. 이 함수는 전달된 값이 "숫자가 아님" 인지 알려줍니다:  
...
### BigInt  
JavaScript의 일반 숫자 타입이 다룰 수 있는 정수의 크기에는 "제한" 이 있습니다. 정수를 다룰 때, 일반적인 `number` 타입은 최소값과 최대값을 가지며, 이것은 `Number.MIN_SAFE_INTEGER` 와 `Number.MAX_SAFE_INTEGER` 상수로 확인할 수 있습니다:  
...
만약, 이보다 더 큰 숫자를 다룬다면, `bigint` 타입을 사용할 수 있습니다. 이것은 `BigInt` 객체 버전도 지원합니다. `BigInt` 타입은 `BigInt()` 메서드를 호출하거나, 숫자 뒤에 `n` 문자를 붙여 지정합니다:  
...
하지만, 대부분의 작업에는 일반적인 `number` 타입만으로 충분합니다. `bigint` 타입은 정말 정말 큰 숫자를 다룰 때만 사용합니다.  

## 요약  
"숫자를 다룬다" 는 것은 거의 모든 프로그래밍에서 매우 중요한 부분이며, 다행히 JavaScript는 쉽게 처리하는 기능을 제공합니다.  내부적으로, 숫자를 선언할 때마다, `Number` 객체를 생성합니다. 이 객체는 숫자와 관련된 많은 기능들이 있으며, 내장 객체에 관한 장에서 특히, `Number` 객체에 관한 장에서 다시 다룹니다.  


## Strings
프로그래밍에서 텍스트 문자열은 보통 단순히 "문자열" (strings)이라 불리며, 단순히 "텍스트 조각" 입니다. 숫자처럼 텍스트를 저장할 수 있는 능력은 프로그래밍에서 매우 중요하며, JavaScript는 문자열을 쉽게 처리하기 위한 여러 가지 좋은 기능을 제공합니다.  

많은 프로그래밍 언어는 텍스트를 저장하기 위한 두 가지 데이터 타입을 제공합니다: 하나는 "단일 문자" 를 저장하기 위한 `char` 타입, 또 다른 하나는 여러 문자를 저장하기 위한 `string` 타입입니다. 그러나, JavaScript에 `char` 타입은 없습니다!! 저장할 문자 개수와 상관없이 `string` 타입만 사용합니다!!  

### 문자열 정의하기 (Defining a string) 
먼저, JavaScript에서 문자열을 정의하는 방법부터 보여드리겠습니다:  

```javascript
let s = "Hello, world!";
```  

숫자와 달리, 문자열은 "따옴표 문자" 로 감싸져 있습니다. 이 경우, 큰따옴표를 사용했지만, 사실 JavaScript는 작은따옴표도 사용할 수 있습니다:  

```javascript
let s = 'Hello, world!';
```  

이 두 문자열은 사용될 때는 완전히 동일하게 처리됩니다. 따라서, 어떤 따옴표를 사용할지는 여러분의 선택일 뿐입니다.  

### 문자열은 기본적으로 문자 배열 (Strings are basically arrays of characters)  
문자열이 정의되면, 배열처럼 "인덱스" 로 개별 문자에 접근할 수 있습니다. 첫 번째 문자의 인덱스는 0, 두 번째 문자의 인덱스는 1, 이런 식입니다. 예를 들어, 위의 문자열에서 "w" 문자에 접근하려면 다음과 같이 사용합니다:  
...
대괄호 문자로 인덱스 7 을 참조하면 "w" 문자를 얻을 수 있습니다. 이것은 "문자열을 배열처럼 다룰 수 있다" 는 뜻이며 따라서, 반복문으로 순회할 수 있습니다:  
...
문자열을 따옴표 문자로 감싸면, JavaScript는 문자열의 시작과 끝을 알 수 있고 코드와 혼동하지 않습니다. 그러나, 문자열에 따옴표 문자를 넣을 때는 주의해야 합니다. 예를 들어:  

```javascript
let s = 'Hello, world, what's up?'; // Syntax error
```  

이 경우, JavaScript는 "what" 뒤의 작은따옴표에서 문자열이 끝났다고 생각하기 때문에 "구문 오류가 발생" 합니다. 이 경우, 두 가지 처리 방법이 있습니다:  
...
문자열의 작은따옴표 앞에 있는 역슬래시(`\`) 문자가 바로 "이스케이프" 입니다. 이것은 해당 문자의 "특별한 의미를 제거" 하고, 단순히 문자열 일부로 취급하도록 만듭니다.  

### 특수 문자 (Special characters)  
방금 살펴본 것처럼, 따옴표 문자를 이스케이프 처리할 수 있습니다. 하지만, 역슬래시 문자는 다른 문자와 조합되면 특별한 의미를 가질 수도 있습니다. 대표적인 예는 바로 "줄바꿈 문자" 인 `\n` 입니다:  
...
문자열 안의 `\n` 은 "줄 바꿈" (개행 문자)으로 변환됩니다. 또 다른 예는, `\t` 로, 이것은 탭(수평 탭)으로 변환됩니다.  

그렇다면, 역슬래시 문자 자체를 문자열에 넣으려면 어떻게 할까요? 이스케이프 문자를 한 번 더 써서 "이스케이프를 이스케이프 처리" 하면 됩니다:  
...
작은따옴표와 큰따옴표 두 가지면 충분할 것 같지만, 2015년 ES6 명세에서 세 번째 문자열 정의 방식을 추가했습니다. 바로 "백틱" (`) 문자를 사용하는 "템플릿 리터럴" 입니다:  

```javascript
let s = `Hello, world!`;
```  

템플릿 리터럴은 단순히 템플릿을 처리하기 위한 것은 아닙니다. 일반 문자열보다 훨씬 더 유연합니다. 예를 들어, 여러 줄에 걸친 문자열을 "있는 그대로 작성" 할 수 있습니다(Raw 문자열):  
...
실행하면, 탭과 공백까지 포함해서 모든 문자 그대로 출력합니다.  

### 플레이스홀더 (Placeholders)  
템플릿 리터럴이라 불리는 이유는  "플레이스홀더" 기능 때문입니다. 일반 문자열에 변수와 텍스트를 혼용하려면 다음과 같이 처리합니다:  
...
이것을 "문자열 보간" (string interpolation)이라 부릅니다. `${ }` 안에 변수를 넣으면, 자동으로 "값으로 대체" 합니다. 복잡한 문자열을 만들 때는 유용합니다.  
...
좀 더 복잡한 경우를 처리하기 위한, "태그드 템플릿" (tagged templates)이란 기능도 있습니다. 이것은 고급 템플릿 작업을 처리하기 위해 존재하며, 대부분 JavaScript 프로그래머는 자주 사용하지 않습니다.  

태그드 템플릿은 "문자열을 해석할 때, 호출될 함수를 지정" 할 수 있으며, 이를 통해, 문자열 생성 방식을 완전하게 제어할 수 있습니다. 사용법은 함수 이름에 "백틱 문자를 붙이는 것" 입니다:  

```javascript
let s = Highlight`${name} is ${age} years old`;
```  

문자열이 실제 해석될 때, 모든 요소는 "Highlight()" 함수로 그대로 전달됩니다. 이 함수로 문자열을 재구성할 수 있습니다. 예를 들어, 플레이스홀더의 값에 `<b>` 태그를 추가하려면 다음과 같이 처리합니다:  
...
여기서 `strings` 는 플레이스홀더가 아닌 문자열의 부분 배열이고, `highlights` 는 플레이스홀더 값의 배열입니다. 이 두 배열로 문자열을 재구성해서 HTML 태그를 추가했습니다.  

## 요약  
이번에는 JavaScript에서 문자열을 정의하는 방법과 다양한 사용 방식을 배웠습니다. 다음 단계는 문자열을 다루고 조작하는 방법을 배우는 것입니다. 다행히 JavaScript에는 내장된 `String` 객체가 있으므로 이를 처리하기 위한 풍부한 기능을 제공합니다.  


## Null & undefined
`Null` 은 프로그래밍 언어에서 흔히 볼 수 있는 개념으로, 많은 프로그래밍 언어에서 사용합니다. 보통 `NULL` 또는 `null` 로 표기하며, 일부 언어는 "nil" 이나 "none" 이라는 이름으로 바꾸지만, 의미는 "아무것도 없음" 으로 모두 동일합니다. 

`Null` 의 개념은 "null 포인터" (null pointer) 또는 "null 참조" (null reference)라고 불리는데, 이것은 기본적으로 아직까지 "값이 없는 무언가" 를 가리키기 때문입니다.  

### null  
JavaScript에서 `null` 은 "원시 타입" (primitive type)으로 간주합니다. 즉, 변수에 `null` 값을 할당할 수 있으며, 변수가 `null` 인지 아닌지 확인할 떄, `null` 과 비교할 수 있습니다. 예를 들어, 나중에 값을 할당할 계획인 변수를 선언할 때, 초기값을 `null` 로 할당할 수 있습니다:  
...
그러나, 변수를 선언만 하고 아무런 값을 할당하지 않으면, 사실 `null` 과 같습니다:  
...
### undefined  
사실 그렇지 않습니다. JavaScript에서 초기값 없이 선언한 변수는, 값으로 `null` 을 할당한 변수와 완전히 같지 않습니다. 많은 프로그래밍 언어는 이 둘을 특별히 구분하지 않으며, 일반적인 비교 연산자(`==`)를 사용하면 차이가 없어 보이기도 합니다. 그러나, JavaScript는 차이가 있습니다!!  

그 이유는 JavaScript에 `undefined` 이라는 개념이 존재하기 때문입니다. 이름 그대로, 아직까지 "정의되지 않은 상태를 표현" 합니다. `null` 과 `undefined` 의 구분은 프로그래밍에 처음 입문한 사람이라면 꽤 어렵게 느껴질 수 있으며, 오히려 "nothing" 타입이 하나뿐인 다른 언어에 익숙한 사람이면 더욱 혼란스러울 수 있습니다.  

먼저, 이 둘이 JavaScript에서 "동일하지 않다" 는 것을 증명하겠습니다. 이를 위해 "엄격한 동등 연산자" (`===`)를 사용합니다. 일반 "동등 연산자" (`==`)는 차이점을 구분하지 못하기 때문입니다. 예를 들어:  
...
보시다시피, 엄격한 동등 연산자를 사용하면, "v2" 변수는 `null` 과 같지 않습니다. 하지만, 일반 동등 연산자를 사용하면, `null` 과 같은 것으로 간주합니다. 대부분 상황에서 이 둘을 특별히 구분할 필요는 거의 없습니다.  

그러나, 구분이 필요할 때는, 전역 `undefined` 속성과 비교할 수 있습니다. 이 속성은 원시 값 `undefined` 을 가집니다. 그런데, 이렇게 처리하면 상황이 좀 더 혼란스러워질 수 있습니다. 왜냐하면, `null` 로 초기화된 변수도, 일반 동등 연산자(`==`)를 사용하면 `undefined` 으로 간주하기 때문입니다:  
...
이쯤 되면 혼란스러울 수도 있습니다. 이것은 JavaScript의 "동적 타입 시스템" 과 "타입 변환을 자동으로 처리" 하는 특성과 관련 있습니다. 이 경우는 오히려 혼란만 더했지만, 다행히 실제 자주 마주치지 않습니다. 대부분의 경우, 어떤 것이 `null` 로 간주될 수 있는지만 알면 충분합니다. 실제 `undefined` 이라도 말입니다.  

## 요약  
프로그래밍 전반에 그리고, JavaScript도 `null` 은 "아무것도 없음" 을 의미하며, 변수에 할당하거나, 비교할 수 있습니다. JavaScript는 여기에 더해 `undefined` 이라는 개념까지 추가했으며, 그 차이점을 설명했습니다. 하지만, 이런 차이가 항상 중요한 것은 아닙니다.  


## Type conversion
앞서 이야기했듯이, JavaScript는 약한 타입(weakly typed)의 동적(dynamic) 언어입니다. 다시 말해, 변수를 선언할 때, 타입을 별도로 지정할 필요가 없습니다. 대신, 변수에 어떤 값을 할당하느냐에 따라 JavaScript가 내부적으로 그것을 적절히 처리합니다.  

이것은 대부분의 경우 "타입에 대해 걱정할 필요가 없다" 는 뜻입니다. 변수를 선언하고 문자열을 할당하면, JavaScript는 문자열로 취급합니다. 나중에 마음이 바뀌면 숫자를 재할당할 수 있고, JavaScript는 다시 숫자로 취급합니다.  

하지만, 그렇다해서 JavaScript가 타입을 전혀 신경쓰지 않는 것은 아닙니다. 내부적으로 "타입은 여전히 매우 중요" 하며, 때로는 중요할 수 있습니다. 이런 상황에는 원시 타입들 사이에 "수동으로 변환을 수행" 해서, 특정 변수를 기대한 변수로 처리할 수 있습니다.  

이번에는 "타입 변환" 에 대해 설명하고, 다양한 타입 간에 변환하는 방법도 살펴보겠습니다.  

### 불리언으로 변환 (Convert to boolean)  
JavaScript는 "Truthy" 와 "Falsy" 장에서 설명한 것처럼 필요할 때, 자동으로 다양한 값을 `true` 또는 `false` (불리언)로 자동 변환합니다. 그러나, 때로는 실제 불리언 값이 필요할 수 있으며, 이를 처리하기 위한 `Boolean()` 함수가 있습니다. 이 함수는 주어진 입력을 불리언(`true` 또는 `false`)으로 변환합니다.  
...
또한, 문자열 같은 보다 덜 명확한 값도 변환할 수 있습니다. 이 경우, JavaScript는 일반적으로 "비어 있는 값" 이나 "정의되지 않은 값" 은 `false` 로 변환하고, 비어있지 않은 값은 `true` 로 변환합니다. 예를 들어:  
...
때로는 값이 "숫자" 임을 확실히 해야할 때가 있습니다. 예를 들어, 브라우저의 `prompt()` 함수로 사용자 입력을 받으면, 입력 값이 숫자라 하더라도, 문자열로 반환됩니다. 이 상태에서 수학 연산을 시도하면 예상치 못한 결과가 나옵니다:  
...
`prompt()` 함수는 항상 "문자열을 반환" 하기 때문에, 두 변수를 더한다면, 실제 수학 연산이 아니라, "문자열 연결" 작업이 일어납니다. 따라서, "2" 를 입력하면 결과는 숫자 42 가 아니라, 문자열 "402" 가 됩니다. 입력을 숫자로만 처리하려면 변환 작업이 필요하며, 이를 처리하기 위해 `Number()` 함수를 사용할 수 있습니다:  
...
`Number()` 함수는 꽤 유연합니다. 문자열뿐만 아니라, 불리언, `undefined`, `null` 같은 값도 처리할 수 있으며, 문자열에 포함된 공백, 탭, 줄바꿈 같은 불필요한 문자도 제거합니다. 숫자로 변환할 수 없을 경우 `NaN` (Not a Number)을 반환합니다. 예를 들어:  
...
문자열로 작업할 상황도 있습니다. 하지만, 전혀 문제되지 않습니다. 대부분의 타입은 `String()` 함수로 쉽게 "문자열로 변환" 할 수 있기 때문입니다.  

`String()` 함수의 처리 결과는 예측하기 쉽습니다. 입력 값의 문자열 표현으로 반환하며, `null`, `undefined` 같은 값도 문자열로 변환합니다. 예를 들어:  
...
마지막 예제는 주목하시기 바랍니다. "숫자를 문자열로 변환" 했기 때문에, 더하기 연산이 아니라, "문자열 연결 작업" 으로 "402" 가 됩니다.  

## 요약  
JavaScript는 타입에 매우 "동적이고 유연" 하기 때문에, 대부분의 경우 타입에 대해 걱정할 필요가 없습니다. 그러나, 특정 타입으로 작업할 상황도 있습니다. 다행히, JavaScript는 원시 데이터 타입들 간의 변환 작업을 처리하기 쉽습니다. 살펴본 것처럼, `Boolean(), Number(), String()` 함수를 사용하면 간단히 원하는 타입으로 변환할 수 있습니다.  

---

# Built-in objects
## Introduction
JavaScript 프로그래밍 언어는 다양한 "내장 객체" 를 기본으로 제공합니다. 그 중 일부는 이전 글에서 간단히 언급했는데 예를 들어, JavaScript에서 숫자를 다루는 방법을 설명할 때, 언급한 `Number` 객체가 있습니다.  

이전 장에는 데이터 타입 즉, "원시 타입" (primitive types)에 대해 설명했습니다. 이 부분에서 JavaScript는 다른 프로그래밍 언어와 조금 다릅니다. 

예를 들어, JavaScript는 숫자 같은 원시 타입을 `Number` 라는 보조 객체와 결합합니다. 다른 프로그래밍 언어는 배열을 원시 데이터 타입으로 간주하지만, JavaScript는 배열은 오직 내장된 "객체" 로 존재합니다.  

실제 모든 "원시 타입" (단, `null` 과 `undefined` 은 제외)은 대응되는 객체의 래퍼(wrapper) 타입을 가진다는 뜻입니다. 이 객체 래퍼는 기본 원시 값을 다루기 위한 유용한 메서드를 제공합니다. 

다음에 보다 자세히 다룰 것이며, 많은 예제로 확인할 수 있습니다. 예를 들어, `String` 객체는 `slice()` 같은 메서드로 기본 문자열 타입을 조작할 수 있는 기능을 제공합니다. 내부적으로 JavaScript는 원시 값을 해당 내장 객체로 "자동으로 감싸주기" 때문에, 단순한 원시 문자열 타입이라도 `String` 객체의 메서드를 그대로 사용할 수 있습니다.  

이 모든 것은 JavaScript가 모두 "자동으로 처리" 하기 때문에, 프로그래머는 차이점을 느끼지 못할 수 있습니다. 하지만, 보다 엄격하고 전통적인 프로그래밍 언어에 익숙한 사람은 JavaScript를 다루는 것이 다소 혼란스럽게 느껴질 수도 있습니다.  

## 요약  
JavaScript에는 "원시 데이터 타입" 과 객체 같은 "복합 타입" 이 존재합니다. 나중에 살펴 보겠지만, "객체" 나 "클래스" 를 정의할 수 있습니다. 하지만, JavaScript는 꽤 많은 내장 객체를 기본적으로 제공하며, 이번 장에서 그 중 일부를 다룹니다. `Array` 객체처럼 절대적으로 필수적인 것뿐만 아니라, `Math` 객체처럼 많은 상황에 유용한 것도 설명합니다.  

단, 이 장에서 "모든 내장 객체" 를 다루지 않습니다. 내장 객체는 꽤 많기 때문에, 가장 "중요하고 흥미로운 것" 에 집중할 것이며, 나머지는 여러분이 직접 탐구해야 합니다. 또한, `RegExp` 객체처럼 옵션과 기능이 많은 내장 객체는 별도의 장에서 설명하므로, 해당 부분에서 찾아보시기 바랍니다.  


## Number
JavaScript는 숫자를 다룰 때 도움주는 `Number` 객체를 제공합니다. 이 객체는 여러 가지 유용한 메서드와 속성이 있으며, 숫자를 처리할 때 관련이 있습니다. 모든 멤버를 다루지 않지만, 유용한 몇 가지는 반드시 이야기해야 합니다.  

앞서, JavaScript 데이터 타입에 관한 장에서 말했듯이, JavaScript에 정의한 "모든 숫자는 동시에 `Number` 객체" 입니다. 따라서,"`Number` 객체를 따로 인스턴스화할 필요는 없다" 는 뜻입니다. JavaScript가 해당 변수가 숫자를 담고 있다는 것을 안다면, `Number` 객체의 메서드를 사용할 수 있습니다.  

### Number 속성 (Number properties)  
먼저, "속성" 부터 시작하겠습니다. `Number` 객체는 숫자와 관련된 값을 알아낼 수 있는 유용한 속성이 있습니다. 예를 들어, JavaScript에서 표현할 수 있는 가장 "큰 값" 과 가장 "작은 값", `NaN` (Not a Number), 그리고 `Infinity` 같은 것입니다. 이들은 특정 값이 예상 범위내에 있는지 확인하는 작업에 사용합니다.  
...
필요하다면, `Number.POSITIVE_INFINITY` 와 `Number.NEGATIVE_INFINITY` 도 참조할 수 있습니다. 이들 모두 "정적 속성" (static properties)이므로, `Number` 객체로 접근합니다.  

### Number 메서드 (Number methods)  
`Number` 객체는 여러 가지 유용한 메서드들이 있습니다. 사실, 튜토리얼 앞부분에서 이미 몇 가지는 살펴 봤습니다. `parseInt()` 와 `parseFloat()` 메소드는 "전역적" 으로 사용 가능하지만, `Number` 객체의 정적 메서드도 존재합니다:  
...
#### Number.isInteger()  
이 메서드는 값이 정수(소수 부분이 없는 수)인지 확인합니다:  
...
이 경우, "자동 변환" 같은 마법은 일어나지 않습니다. 전달된 값은 실제 숫자여야 하며, 단순히 숫자로 변환될 수 있는 값은 안 됩니다:  
...
#### Number.toFixed()  
`toFixed()` 메서드는 고정 소수점 표기법으로 숫자를 "포맷" 합니다:  
...
세계적으로 숫자는 서로 다르게 포맷될 수  있습니다. 예를 들어, 미국은 소수점 구분 문자로 마침표를 사용하지만, 독일과 많은 유럽 국가는 쉼표 문자를 사용합니다. 또한, 어떤 나라는 천 단위 구분 문자로 쉼표를, 다른 나라는 마침표 문자를, 또 다른 나라는 공백 문자를 사용하기도 합니다.  

이렇기 때문에 숫자를 파싱하는 것도 어렵지만, 사용자에게 직관적으로 보이도록 표시하는 것도 어렵습니다. 다행히 `toLocaleString()` 메서드가 도움됩니다. 가장 단순한 형태는, 사용자 로케일(예: 브라우저 설정)에 맞도록 숫자를 보기좋게 포맷합니다:  
...
미국은 "42,000", 독일은 `"42.000"` 형태로 출력합니다. 특정 로케일을 지정할 수 있습니다:  
...
또한, 두 번째 매개변수에 "옵션" 을 지정할 수 있습니다. 이를 통해, 포맷에 대한 모든 부분을 제어할 수 있습니다. 예를 들어:  
...
같은 숫자라 하더라도, 사용한 "옵션" 에 따라 완전히 다르게 "포맷" 된다는 것을 알 수 있습니다.  

## 요약  
JavaScript에서 숫자를 다룰 때, 내장된 `Number` 객체는 매우 유용하며, 언젠가 꼭 필요한 여러 가지 메서드를 제공합니다.  

이번에는 `Number` 객체의 가장 유용한 "속성" 과 "메서드" 몇 가지만 다뤘습니다. 좀 더 깊이 알려면 `Number` 객체 레퍼런스 페이지를 참고하시기 바랍니다.  

## String
이 튜토리얼에서 JavaScript의 원시 문자열 타입에 대해 설명했지만, 좀 더 복잡한 `String` 객체도 있다는 사실도 언급했습니다. 실제로는 `String` 객체를 인스턴스화해서 문자열을 만들 수 있지만, 그럴 필요가 전혀 없습니다. JavaScript는 필요할 때, 원시 문자열을 자동으로 `String` 객체로 감싸주기(Boxing) 때문에, 원시 문자열에 `String` 객체의 메서드를 직접 사용할 수 있습니다.  

이런 점을 염두에 두고, `String` 객체의 가장 흥미로운 "속성" 과 "메서드" 를 살펴보겠습니다.  

### String 속성  
#### string.length  
`length` 속성으로 "문자열의 길이" 를 확인할 수 있습니다:  
...
단, 문자열에 이모지나 복잡한 유니코드 문자(예: 특수 기호, 드문 한자 등)가 포함되면 정확하지 않을 수도 있습니다. 그러나, 일반적인 경우 `length` 속성이 문자열에 포함된 문자의 개수를 정확히 알려줍니다.  

### String 메서드  
`String` 객체에는 유용한 메서드들이 있으며, 그 중 가장 흥미로운 것을 다룹니다.  
...
#### string.indexOf()  
문자열에 특정 문자열의 위치(인덱스)를 반환합니다. 찾지 못할 경우 `-1` 을 반환합니다.  
...
이 경우, 첫 번째 발견 위치를 반환합니다. 두 번째 매개변수로 시작 위치를 지정하면, 반복문으로 모든 위치를 찾을 수 있습니다.  

#### string.lastIndexOf()
`indexOf()` 메소드와 비슷하지만, 마지막 위치를 반환합니다.  
...
## 요약   
보시다시피, JavaScript의 내장 `String` 객체는 유용한 메서드들이 많습니다. 여기서는 가장 중요한 것만 다뤘고, 정규 표현식과 관련된 메서드는 별도의 장에서 다룰 예정입니다.  


## Array - The basics
"배열" (Arrays)은 항목의 "모음" 으로 정의되며, 종종 같은 타입의 항목들로 구성되고, 각 요소는 인덱스 번호로 접근할 수 있습니다. 많은 프로그래밍 언어에서 배열 타입은 컬렉션을 저장하기 위한 가장 "기본적인 데이터 타입" 으로 제공되며, 배열을 기반으로 좀 더 발전한 "컬렉션" 들이 만들어지기도 합니다. 그러나, JavaScript에는 여러 항목을 같은 리스트에 저장할 때, 주로 배열을 사용합니다.  

다행히 JavaScript에는 배열을 다루기 위한 `Array` 라는 내장 객체가 있으며, 좀 더 쉽게 사용하고 자연스럽게 느껴지도록 만드는 약간의 "문법적 설탕" (syntactic sugar)도 제공됩니다. 

이번에는 JavaScript에서 "배열" 이 어떻게 작동하는지, 그리고 배열이 제공하는 모든 가능성에 대해 깊이 살펴보겠습니다.  
...
후자를 일반적으로 자주 사용하므로, 이 방식으로 사용하겠습니다. 

물론, 두 방식 모두 "비어있는 배열을 생성" 합니다. 원한다면 배열을 생성할 때, 몇 가지 항목을 포함시킬 수 있습니다:  
...
### 배열 키 (Array keys)  
JavaScript 배열은 "연관 배열" (associative array)이 아님에 주의하시기 바랍니다. 즉, 배열 항목을 저장할 때  사용하는 "키" 는 항상 0, 1, 2… 같은 "음수가 아닌 정수" 이며, 문자열 같은 다른 값은 사용할 수 없습니다!!  

"연관 배열" 이란, 배열 항목의 "키" 로 어떤 타입의 객체든 사용할 수 있는 것을 말합니다. 즉, 단순한 인덱스 번호 대신 의미있는 "키와 값을 서로 연관" 시킬 수 있습니다. JavaScript 배열은 불가능하지만, JavaScript의 "객체" 를 사용하면 처리할 수 있습니다. 객체를 다루는 장에서 좀 더 설명하겠습니다.  

### 배열 항목 타입 (Array item types)  
타입이 JavaScript만큼 동적이지 않은 언어는, 배열이 어떤 데이터를 담을지에 따라 배열을 별도로 만들어야 합니다. 예를 들어, 문자열 배열, 정수 배열 등으로 나누어야 합니다. 그러나, JavaScript는 배열에 문자열, 숫자, 객체, 함수 등 "무엇이든 모두 담을 수 있으며", 자유롭게 섞어 사용할 수 있습니다:  
...
### 배열 길이 (Array length)  
배열 변수에 현재 몇 개 항목이 있는지 알려면 `length` 속성을 확인합니다:  
...
단, `length` 속성은 배열이 내부적으로 몇 개의 항목을 가질 수 있는지 제어하는 데 사용됩니다. 대부분 자동으로 처리되며, 배열은 필요한 만큼 자동으로 크기를 조정합니다. 하지만, 특정 길이의 배열을 원한다면 다음과 같이 만들 수 있습니다:  
...
실제 항목은 3개 뿐이지만, `length` 속성을 덮어썼기 때문에 6 을 반환합니다. 그러나, 일반적으로 이렇게 처리하지는 않으므로, 대부분 `length` 속성을 있는 그대로 믿어도 됩니다.  

### 항목 접근하기 (Accessing items)  
배열이 정의되면, "인덱스" 로 특정 항목에 접근할 수 있습니다. 프로그래밍은 "항상 0부터 카운트가 시작한다" 는 점을 기억하시기 바랍니다. 따라서, 첫 번째 항목의 인덱스는 0, 두 번째는 인덱스 1 입니다.  
...
배열을 만들 때와 항목에 접근할 때 모두 "대괄호를 사용" 합니다. JavaScript에서 대괄호는 배열과 아주 밀접하게 연결되어 있습니다.  

### 배열 반복하기 (Looping over an array)  
배열은 항목들의 모음이므로, 항목들을 순회한다는 것은 자연스럽습니다. JavaScript에서 제공하는 모든 반복문을 사용할 수 있지만, 특히 `for` 와 `for..of` 루프를 자주 사용합니다.  
...
카운터 변수가 필요 없고, 인덱스를 신경쓰지 않는다면, `for..of` 루프를 사용할 수 있습니다:  
...
#### unshift() / shift()  
`push()` 와 `pop()` 메소드가 배열 끝에서 동작하는 반면, `unshift()` 와 `shift()` 메소드는 배열의 앞쪽에서 동일한 동작을 수행합니다:  
...
배열은 또 다른 배열을 담을 수 있습니다. 이를 통해, "다차원 배열" 을 만들 수 있으며, 표 형태의 데이터를 저장할 때 유용합니다:  
...
배열 안에 배열을, 또 그 안에 배열을 넣는 방식으로 원하는 만큼 다차원으로 확장할 수 있습니다.  

## 요약  
이번에는 JavaScript의 내장 `Array` 객체를 소개하고, 배열 생성, 항목 접근, 항목 추가/제거 방법을 다뤘습니다. 그러나, `Array` 객체는 이 외에도 훨씬 더 많은 기능이 있으며, 다음에 다양한 `Array` 메서드를 좀 더 깊이 탐구합니다.  


## Array methods
이전에, JavaScript 배열에 대한 꽤 철저한 소개는 했습니다: 배열을 어떻게 선언하고 접근하는지, 그리고 항목을 추가하거나, 제거하면서 약간의 조작도 설명했습니다.  하지만, 내장된 `Array` 객체는 훨씬 더 많은 기능을 제공합니다. 이번에는 `Array` 객체에서 가장 유용한 메서드를 살펴보겠습니다. 가장 생소한 메서드는 제외하지만, 전체 목록은 참고 문서를 확인하시기 바랍니다.  

### 메서드 (Methods)  
내장 `Array` 객체는 속성이 단 하나뿐입니다. (`length`) 하지만, 지난 글에서 이미 다루었으므로, 이번에는 메서드에 집중하겠습니다.  

#### Array.at()  
이전 글에서, 인덱스로 배열 항목에 접근하는 방법을 살펴봤습니다:  
...
`at()` 메서드를 사용하면 동일한 작업을 처리할 수 있습니다:  
...
거의 동일하지만, `at()` 메서드는 한 가지 장점이 있습니다. 음수 인덱스를 지정하면 배열 끝에서 거꾸로 접근할 수 있습니다:  
...
#### Array.concat()  
`concat()` 메서드는 한 배열을 다른 배열 끝에 이어붙여 "새로운 배열" 을 만듭니다. 따라서, 기존 배열은 변경되지 않습니다:  
...
#### Array.every()  
`every()` 메서드는 배열의 "모든 항목" 이 주어진 테스트 함수를 통과하는지를 `true` 또는 `false` 로 반환합니다:  
...
#### Array.filter()  
`filter()` 메서드는 `every()` 메소드와 비슷하지만, 조건을 통과한 항목만 따로 모아 "새로운 배열" 을 반환합니다:  
...
#### Array.forEach()  
JavaScript에는 `foreach` 루프는 없지만, `forEach()` 메서드로 동일한 효과를 얻을 수 있습니다:  
...
#### Array.join()  
배열의 항목들을 문자열로 합칩니다. 기본 구분자는 쉼표 문자이며, 원하는 구분자를 직접 지정할 수 있습니다:  
...
#### Array.map()  
`map()` 메서드는 배열의 각 항목에 함수를 적용하고, 수정한 항목을 "새로운 배열" 로 반환합니다. 따라서, 기존 배열은 변경되지 않습니다.  
...
조금 우스운 예제지만, 요점은 분명합니다. `map()` 메서드를 호출하고, 각 요소에 "fruit" 라는 단어를 덧붙이는 함수입니다. 결과적으로 새로운 배열을 생성합니다.  

다시 말하지만, "PostfixFruit()" 함수의 시그니처는 `filter()` 나 `find()` 같은 다른 배열 메서드에서 본 것과 동일합니다. 즉, 함수는 세 개의 매개변수(값, 인덱스, 배열 전체)를 받습니다. 필요한 경우, 함수 내부에 다양한 처리 로직을 구현할 수 있으며, 반환된 값은 새로운 배열에 사용됩니다.  

#### Array.reverse()  
`reverse()` 메서드는 배열 순서를 뒤집습니다. 즉, 마지막 항목이 첫 번째가 되고, 첫 번째 항목이 마지막이 됩니다.  
...
여기서 주의할 점은, `reverse()` 메서드는 새로운 배열을 반환하지 않고 "기존 배열 자체를 변경한다!!" 는 것입니다. 따라서, 원래 순서를 그대로 유지하려면 `reverse()` 메소드를 호출하기 전에 배열을 먼저 복사해야 합니다.  

#### Array.slice()  
`slice()` 메서드는 배열의 "얕은 복사본을 반환" 합니다. 하나 또는 두 개의 매개변수로 포함할 항목의 범위를 지정할 수 있습니다.  
...
#### Array.some()  
앞서, `every()` 메서드로 배열의 "모든 항목" 이 조건을 만족하는지 확인했습니다. `some()` 메서드는 그와 반대로, "하나라도 조건을 만족한다면 `true`" 를 반환합니다.  
...
#### Array.sort()  
`sort()` 메서드는 배열의 값을 "정렬" 합니다. 기본적으로 항목을 문자열로 변환해서 비교하므로, 문자열 배열은 기대대로 동작합니다:  
```javascript
let fruits = ["Apple", "Pineapple", "Banana"];
fruits.sort();
...
## 요약  
보시다시피, `Array` 객체는 유용한 메서드들이 많습니다. 배열은 JavaScript뿐만 아니라, 다른 프로그래밍 언어에도 "필수적" 이고, 매우 강력한 도구이기 때문에, 다양한 기능은 기본적으로 제공됩니다.  

이번에는 `map()`, `reverse()`, `slice()`, `some()`, `sort()` 메소드를 다뤘으며, 이전에 설명한 메서드(`pop()`, `push()` 등)나 너무 복잡하거나, 보기 드문 메서드는 제외했습니다. 전체 목록은 참고 문서를 확인하시기 바랍니다.  


## Date
"날짜" 를 다루는 것은 어떤 프로그래밍 언어를 사용할 때도 매우 "흔한 작업" 에 속하며, 다행히 JavaScript는 내장된 `Date` 객체가 있으므로 도움됩니다. 그러나, 이름에는 약간의 "오해 소지" 가 있습니다. 단순히 "날짜" 만 다루는 것이 아니라, "시간" 까지 함께 다루기 때문입니다. 이번에는 유용한 객체를 좀 더 깊이 살펴보겠습니다.  
...
출력된 날짜는 매우 상세한 현재 날짜와 시간을 보여주며, 타임존까지 포함됩니다. 물론, 이를 변경할 수 있지만, 나중에 다루겠습니다. 우선, `Date` 객체에 전달할 수 있는 매개변수들에 대해 이야기 하겠습니다.  
...
ISO 8601 형식(`YYYY-MM-DDTHH:mm:ss.sssZ`)을 일관되게 사용하지 않으면, 결과는 JavaScript 엔진/구현에 따라 달라질 수 있습니다.  
...
#### Date.get* 메서드: UTC 변형  
모든 `get*` 메서드는 UTC 버전이 있습니다. 예를 들어, `getHours()` 대신 `getUTCHours()` 메소드를 호출하면, UTC 기준의 시간을 얻습니다:  
...
### Date.parse()  
`Date` 객체의 정적 메서드 `parse()` 를 사용하면, 날짜(그리고 선택적으로 시간)가 포함된 문자열을 파싱할 수 있습니다. 날짜 문자열이 성공적으로 파싱되면, 그 결과는 "1970년 1월 1일 00:00:00 UTC" 부터 경과한 밀리초 값이 됩니다. 이 값은 쉽게 `Date` 객체로 변환하거나, 다른 계산에 사용할 수 있습니다.  
...
ISO 8601 형식(`YYYY-MM-DDTHH:mm:ss.sssZ`)을 일관되게 사용하지 않으면, 결과는 JavaScript 엔진/구현에 따라 달라질 수 있습니다. 특정 브라우저는 잘 동작하는 것처럼 보여도, 다른 엔진에는 제대로 동작하지 않을 수 있으며, 심지어 미래에 동작 방식이 바뀔 수도 있습니다.  
...
여기서 `31` 을 지정한 이유는 원하는 날짜가 바로 31일이기 때문입니다. 하지만, 더 큰 수나 음수를 지정할 수 있습니다!!:  
...
12월은 31일까지 있으므로, JavaScript는 남는 날짜를 다음 달로 넘겨서 처리합니다. 따라서, 결과는 다음 해 1월 1일이 됩니다.  
...
첫 번째 매개변수로 로케일을 지정할 수 있습니다. IETF BCP 47 언어 태그로 사용해야 합니다:  
...
### 요약  
보시다시피, `Date` 객체는 JavaScript에서 날짜를 다룰 때, 유용하며 파싱과 포맷팅을 위한 다양한 메서드를 제공합니다.  

하지만, `Java` 나 `C#` 같은 다른 현대적인 언어에 익숙하다면, JavaScript의 `Date` 객체가 조금 부족하다고 느낄 수 있습니다. 이 경우, `Day.js` 나 [date-fns](https://date-fns.org/) 같은 JavaScript 날짜 관련  라이브러리를 사용하는 것도 좋은 방법입니다. 이들은 좀 더 복잡한 사용 사례와 유용한 풍부한 기능을 제공합니다.  


## Math
더하기, 빼기, 곱하기 등 우리가 이미 다룬 기본적인 연산보다, 좀 더 복잡한 "수학 연산" 이 필요할 때, 내장된 `Math` 객체가 큰 도움이 됩니다. 이 객체는 좀 더 무거운 수학 작업을 처리하기 위한 메서드들과 유용한 속성들이 포함되어 있습니다.  

앞서 다룬 여러 내장 객체들과 달리, `Math` 객체에는 "정적 멤버" (static members)만 존재합니다!! 

다시 말해, `Number` 객체처럼 어떤 값에 직접 `Math` 메서드를 호출할 수 없습니다. 대신, `Math` 객체의 속성과 메서드를 참조할 때, 작업할 숫자를 전달해야 합니다. 또한, `Math` 객체를 새로 생성하거나, 함수처럼 호출할 수도 없습니다.  
...
이것은 사용자의 이름(이름과 성)과 나이를 설명하는 일반적인 JavaScript "객체" 입니다. `JSON` 으로 작성하면 다음과 같이 보입니다:  
...
### JSON 메서드  
JavaScript에는 `JSON` 을 다루기 위한 내장 객체가 있으며, 이름도 `JSON` 입니다. 내장 `Math` 객체처럼, `JSON` 객체도 "정적 멤버" 만 가지므로, 단순히 `JSON` 객체의 메서드를 직접 호출하면 됩니다.  
...
이처럼 외부에서 온, `JSON` 데이터를 단순히 `parse()` 메서드 호출만으로 일반적인 JavaScript "객체" 로 바꿀 수 있습니다.  
...
### 요약  
`JSON` 은 "데이터 교환" 을 위해 매우 널리 사용되는 형식입니다. 구문은 JavaScript의 배열과 객체 선언 방식과 거의 동일하지만, `JSON` 은 거의 모든 현대적인 프로그래밍 언어에서 폭넓게 지원됩니다.  
