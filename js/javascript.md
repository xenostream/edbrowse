# JavaScript 튜토리얼
안녕하세요, 흥미롭고 인기있는 프로그래밍 언어 초보자가 주 대상인 `JavaScript` 튜토리얼에 오신 것을 환영합니다!!  고급 사용자도 JavaScript 지식의 공백을 메우기 위해 특정 장을 읽음으로써 한 두 가지를 배울 수도 있습니다.

각 챕터는 JavaScript를 처음 접하는 사람에게 맞도록 구성되며, 이전 챕터를 읽지 않고 챕터 대부분을 활용할 수 있지만, 처음부터 시작하면서 챕터를 진행하면 이 튜토리얼을 최대한 활용할 수 있습니다. 특정 장에서 알고 있는 정보가 포함됐다면, 다음 장으로 넘어가시면 됩니다.
...
JavaScript는 별다른 사전 지식이 없어도 배울 수 있지만, 웹 사이트 구축에 사용하려면 `HTML` 에 대한 기본 지식을 권장합니다. HTML을 처음 접할 경우는 HTML5 튜토리얼 먼저 살펴보는 것이 좋습니다.
...
# 자바스크립트 정보
## 자바스크립트란 무엇입니까?
`JavaScript` (간단히 "JS" 라고도 함)는 ECMAScript에 기반한 "프로그래밍 언어" 입니다. 웹 브라우저가 웹 페이지 요소를 렌더링한 후, 조작할 수 있도록 만들어, 웹 페이지를 보다 생생하게 만들기 위해 만들어졌습니다.

현재는 매우 간단하고 사소하게 들리겠지만, 1997년 최초의 웹 브라우저 중 하나인 `Netscape Navigator` 에 JavaScript가 처음 도입되었을 때는 엄청난 일이었습니다. 그 전까지는 웹 페이지는 레이아웃과 텍스트에 관한 것이며, 다른 페이지로 이동하기 전에는 변경되지 않았고, 브라우저에 "정적 콘텐츠" 로 전달되었습니다. JavaScript는 이 모든 것을 바꾸었고, HTML 및 CSS와 함께 웹사이트는 현재는 잘 알듯이 매우 역동적인 버전으로 진화하기 시작했습니다.


### JavaScript 언어
JavaScript 언어에 대해 많은 것을 말할 수 있으며, 그 중 많은 것은 다른 프로그래밍 언어와 비교할 수도 있습니다. 프로그래밍을 처음 접할 경우, 이런 정보는 아무런 의미가 없겠지만, 나중에는 도움될 것입니다.

- JavaScript는 이름의 절반을 차지하는 Java 프로그래밍 언어의 구문 스타일도 많지만, 중요 영역은 상당히 다릅니다.

- JavaScript는 약한 유형이므로 변수 유형을 선언할 필요가 없습니다. 대신 인터프리터가 사용 방법을 추측합니다.

- JavaScript는 동적입니다 - 숫자 변수(예: 정수)에 텍스트를 할당하면 문자열 변수로 변경할 수 있습니다.

- JavaScript는 프로토타입이므로 객체 지향에 클래스 대신 프로토타입을 사용합니다. 나중에 자세히 살펴보겠습니다.

- JavaScript는 일급 함수를 사용하며 실제로는 객체로 간주되고 속성과 메서드를 가질 수 있습니다.

- JavaScript는 런타임 평가를 제공하므로 JavaScript 코드의 텍스트 문자열을 가져와서 원본 코드인 것처럼 인터프리터가 평가할 수 있습니다.

이것은 모두 JavaScript에 대한 사실 중 일부분 입니다. 부디 이 튜토리얼이 JavaScript가 무엇이며 어떻게 작동하는지에 대한 전체적인 그림을 제공할 수 있기를 바랍니다.


## JavaScript 와 웹 브라우저
많은 사람들에게 JavaScript와 웹 브라우저는 매우 밀접한 관련이 있습니다. JavaScript는 원래 Netscape Navigator 웹 브라우저 백엔드 팀에 의해 만들어졌으며, "웹 전용" 으로 사용했기 때문에 "안전한 언어" 라고 불리는 이유이기도 한데, 가장 일반적인 구현에는 기본적으로 파일 시스템이나 이와 유사한 것에 대한 액세스 기능을 제공하지 않기 때문입니다.

그러나, "JavaScript는 웹 브라우저 외부에서 쉽게 사용할 수 있다" 는 점을 유의하시기 바랍니다. JavaScript는 JavaScript 엔진 또는 인터프리터로  실행됩니다. 이 엔진은 브라우저 내부에서 실행할 수 있으며, 인류에게 알려진 거의 모든 최신 웹 브라우저에서 실행되지만, 이 엔진이 브라우저 외부에서 실행되는 것을 막을 수 있는 것은 아무 것도 없습니다. 사실 이런 시도는 여러 번 시도됐습니다.


### Node.js
가장 주목할만한 예로 `Node.js` 프로젝트가 있습니다. 2009년에 만들어진 이 앱은 기본적으로 웹 브라우저에서 JavaScript 엔진을 제거하고, JavaScript를 웹 브라우저 스크립팅 언어 대신 "다목적 프로그래밍 언어" 로 만드는 모듈을 추가했습니다. 이로 인해, JavaScript에 대한 새롭고 훨씬 더 높은 수준의 관심이 촉발되는데, 이제 웹(클라이언트 측)과 서버 측 스크립트 모두 JavaScript 기술을 사용할 수 있기 때문입니다. 그리고, Node.js 는 "크로스 플랫폼" 이기 때문에 웹 브라우저 없이 Windows, Linux, Unix 및 macOS에서 코드를 실행할 수 있습니다.


### 호스트 환경
이 모든 것은 "호스트 환경" (host environment)에 대한 설명으로 이어집니다. 호스트 환경은 JavaScript가 실행되는 곳이며, 일반적으로 JavaScript 언어에는 찾을 수 없는 기능도 포함됩니다. 가장 분명한 예는 많은 기능을 추가하고 JavaScript 코드가 웹 브라우저와 통신하고 기본 웹 사이트의 콘텐츠를 조작할 수 있도록 만드는 "웹 브라우저" 환경입니다.

서버 측 기능까지 추가한 호스팅 환경이 Node.js 지만, JavaScript는 원래 어디서나 실행할 수 있도록 설계되었기 때문에 커피 머신이 될 수 있습니다. JavaScript 엔진을 쉽게 포함하고 커피 만들기 및 웹 사이트 대신 푸시 버튼에서 지시를 받는 것 같은 기능을 추가할 수 있습니다.


### 브라우저가 있든 없든?
JavaScript는 확실히 웹 브라우저 외부에서 실행할 수 있습니다. 지금까지 사실이며 JavaScript의 인기가 높아짐에 따라 더욱 더 사실이 될 것이라 상상할 수 있습니다. 따라서, 이 튜토리얼에는 무엇보다 "JavaScript 언어" 를 설명할 것임을 아는 것이 중요합니다.

튜토리얼 첫 번째는 실제 언어에 초점을 맞추고, 웹 브라우저와 상호 작용하는 방법은 많이 이야기하지 않습니니다. 이를 염두에 두고, JavaScript가 웹 브라우저와 상호 작용하는 방식을 완전히 무시하는 것도 일반적인 사용 사례이기 때문에 어리석은 일입니다. 따라서, 튜토리얼 후반에는 JavaScript와 웹 브라우저 간의 관계를 좀 더 깊이 파고듭니다.

순수주의자라면 약간의 주의를 기울이시기 바랍니다. 튜토리얼 첫 번째에서 웹 브라우저를 제외하더라도, 브라우저의 기능으로 간주할 수 있는 몇 가지 함수를 사용할 것임을 알 수 있습니다. 예로는 웹 브라우저에 간단한 "메시지 상자를 표시" 하는 `alert()` 함수입니다. 튜토리얼에서 사용하는 예제에서 간단하면서 이해하기 쉽고, 웹 브라우저에서 직접 예제를 실행할 수 있는 결과를 생성하기 때문에 자주 사용합니다.


### 요약
JavaScript는 대부분 웹 브라우저에서 실행되지만 항상 그런 것은 아닙니다. 이 튜토리얼은 먼저 JavaScript 언어에 중점을 두고 언어를 배울 수 있도록 한 다음, 나중에 JavaScript가 웹 브라우저와 상호 작용하는 방법을 살펴보겠습니다. 그러나, 단순히 이해하기 쉽고 본문을 읽으면서 테스트할 수 있기 때문에 브라우저와 상호 작용하는 예를 보여줄 것입니다.



## JavaScript 라이브러리
이 튜토리얼은 먼저 JavaScript 언어에 집중하고 나중에 웹 관련 부분을 설명할 것이라 약속했지만, 시작하기 전에 웹과 관련된 주제인 JavaScript 라이브러리는 언급해야 합니다.

JavaScript는 "확장성" 이 매우 뛰어난 언어이므로 언어에 고유한 기능을 추가해서 JavaScript 언어의 원래 기능인 것처럼 사용하는 것은 매우 쉽습니다. JavaScript가 도입된 후, 숙련된 개발자는 이런 사실을 활용해서 "자체적인 라이브러리를 구축" 하기 시작했습니다. 이런 라이브러리는 많이 사용하는 기능을 추가하거나, 기존 기능을 훨씬 더 쉽게 사용할 수 있도록 만들어, 보다 적은 코드로 작업을 쉽게 수행할 수 있는 일종의 "단축키" 기능입니다.

개발자가 이런 기능을 추가한 주된 이유는 예전 특히, "브라우저 전쟁" 중에는 다양한 웹 브라우저마다 고유한 사용자 정의된 JavaScript 엔진이 있었다는 사실에 근거합니다. 즉, 대부분의 기능은 인기 있는 웹 브라우저에서 동일하게 작동했지만, 사소한 차이점도 있기 때문에, 모든 방문자에게 동일한 방식으로 작동하도록 할 수 없음을 의미합니다.

따라서, 많은 초기 라이브러리는 기본적으로 방문자가 현재 사용 중인 "브라우저를 확인" 한 다음, 이 브라우저에 처리할 수 없는 문제나 단점을 해결해서 무슨 일이 있더라도 모든 것이 의도한 대로 작동하는지 처리하는 기능 위주였습니다.

대부분의 개발자는 라이브러리를 자신이나 자신이 일하는 회사 내부에 보관했지만, 일부 라이브러리는 많은 개발자에게 중요하다고 간주하는 기능을 대중에게 공개했습니다. 이런 라이브러리 공개로 다른 개발자는 기존 라이브러리를 완성하면서 보다 많은 기능을 추가할 수 있었습니다.


### jQuery
일반적으로 사용한 최초의 JavaScript 라이브러리 중 하나는 `jQuery` 였습니다. 2006년에 처음 출시되었으며, 주로 웹 사이트에서 레이아웃과 콘텐츠를 구성하는 모든 요소인 "DOM" (문서 개체 모델)에서 특정 요소를 쉽게 찾거나 조작할 수 있도록 처리했습니다. 또한, 모든 웹 관련 기능이 의도한 대로 모든 주요 브라우저에서 작동하는지도 확인했습니다.

그 후 몇 년 동안, jQuery는 지속적인 업데이트로 최종 사용자가 점점 더 많은 작업을 수행할 수 있도록 개량했습니다. 이에 대한 예로, AJAX 요청, 애니메이션 및 유틸리티 함수가 있으며, 기본적으로 개발자가 단 몇 줄의 코드로 간단하고 고급 작업을 수행할 수 있도록 합니다.

물론, jQuery의 성공은 수 많은 경쟁 라이브러리를 탄생시켰습니다. 그 중 하나는 2007년에 출시된 `mooTools` 였지만, 이것은 빙산의 일각에 불과했습니다. jQuery가 기능을 계속 늘릴 동안, 다른 개발자들은 크기와 복잡성을 최소화한 좀 더 얇은 대안을 출시하거나, 완전히 다른 작업을 수행하는 라이브러리를 출시해야 한다고 주장했습니다.


### jQuery 사용법: 현재와 미래
일부 개발자는 jQuery를 구식으로 간주합니다. 그들은 JavaScript가 너무 많은 일을 처리한다고 말하며, JavaScript 개발과 JavaScript 엔진 통합 덕분에 문제 중 일부는 더 이상 문제되지 않습니다. 그러나, Wikipedia에 따르면 "2022년 8월 현재 가장 인기 있는 1,000만 개 웹사이트 중 77% 가 jQuery를 현재까지도 사용하고 있습니다".

따라서, jQuery에 대한 많은 대안이 존재하며, 그 중 하나는 단순히 JavaScript 라이브러리를 사용하지 않고 jQuery가 출시된 2006년보다 오늘날 훨씬 더 나은 JavaScript 엔진에 의존하는 것입니다. 그러나, 초보자는 jQuery가 좋은 출발점입니다. 지루하고 따분한 부분은 jQuery에 맡기고 역동적인 웹사이트를 만드는 재미있는 작업에 집중할 수 있어서 일이 훨씬 쉬워집니다.


### 요약
JavaScript 라이브러리를 사용하면 동적인 웹 사이트를 쉽게 개발할 수 있으며, 15년 전에는 오늘날보다 확실히 더 필요했지만, 웹용 JavaScript 개발할 때도 시도할 것을 고려해야 합니다.
...
# 시작
## 필요한 도구
JavaScript는 시작할 때 필요한 도구와 관련해서 가장 간단한 프로그래밍 언어 중 하나입니다. 사실, 시작하기 위해 아무 것도 다운로드할 필요가 없습니다: JavaScript는 Windows 메모장(또는 동등한 Mac/Linux 대안)과 같은 간단한 텍스트 편집기에서 작성할 수 있으며, 코드를 해석하고 테스트할 때또 필요한 것은 단지 "웹 브라우저" 뿐입니다. 즉, 값비싼 IDE에 비용을 지불하거나, 복잡한 컴파일러/인터프리터를 다운로드하지 않고 JavaScript 학습을 시작할 수 있습니다.

하지만, 개발 경험이나 효율을 향상시키기 위한 몇 가지 도구가 있습니다. 선택할 수 있는 것에 대해 조금 이야기하겠습니다.


## 편집기
앞서 말했듯이 메모장이나 간단한 텍스트 편집기로 JavaScript를 작성할 수 있지만, 실제로 코드 작성을 위한 편집기를 사용해 본 적이 있다면, 효율성을 향상시킬 수 있다는 것도 잘 알고 계실겁니다! 최신 코드 편집기/IDE(통합 개발 환경)는 자동 완성 및 IntelliSense 같은 기능으로 코드를 보다 빠르고 효율적으로 작성하는 데 많은 도움을 줍니다. 이것은 JavaScript도 해당됩니다.

JavaScript는 웹과 밀접하기 때문에 HTML 또는 CSS를 작성할 때 코드/웹 편집기를 사용하고 있을 수 있습니다. 그렇다면, JavaScript 언어도 지원할 가능성이 높으며, JavaScript를 배울 때도 계속 사용하는 것이 좋습니다. 그렇지 않다면, 코드 편집기 / IDE를 구입하는 것이 좋습니다.

다양한 옵션을 설명하겠습니다.

### 온라인 편집기
간단히 시작하려면 온라인 편집기를 사용할 수 있습니다. JavaScript는 웹과 밀접하기 때문에 "실제" 편집기와 동일한 작업을 수행할 수 있는 온라인 편집기가 많이 있으며, 아무 것도 설치하지 않고도 브라우저에서 직접 사용할 수 있습니다. 

대부분 코드를 테스트, 시연 및 공유하도록 설계되었으며, 대규모 프로젝트에는 적합하지 않기 때문에 종종 "놀이터" 또는 "바이올린" 이라 불립니다. (일부는 이를 위해 사용할 수도 있음) 하지만, 튜토리얼 코드를 테스트하거나 플레이하는 데 적합합니다.

따라서 선호하는 웹 편집기가 아직까지 없고, JavaScript를 배우는 데 시간을 할애하고 싶은 것인지 확인하려면 온라인 편집기를 추천할 수 있습니다. 다양한 수준의 IntelliSense 기능까지 제공하며 JavaScript 코드를 효율적으로 작성할 수 있도록 도와주고 브라우저에서 직접 코드를 테스트할 수 있습니다. 다음은 몇 가지 제안 사항입니다.
...
직접 시도해 보고 어떤 것이 마음에 드는지 확인하시기 바랍니다.

### 간단한 텍스트 편집기
이미 언급했듯이, 운영 체제에 포함된 간단한 텍스트 편집기(Windows용 메모장 등)를 사용할 수 있습니다. 그러나, 이 선택은 가능한 한 최소한의 도움을 받기를 고집하는 순수주의자를 위한 것입니다. 다른 사람들은 IntelliSense, 구문 검사, 문법 강조  같은 기능으로 도움받을 수 있는 다음에 나열한 다른 대안 중 하나를 사용하는 것이 좋습니다.

### 웹 편집기
HTML, CSS 및 JavaScript 같은 웹 관련 기술에 중점을 둔, 몇 가지 훌륭한 웹 편집기가 있습니다. 때때로 "HTML 편집기" 라고 불리는데, 대체적으로 JavaScript도 잘 지원합니다. 특정 편집기를 추천하지 않지만, 위키백과에서 확인할 수 있는 편집기 목록이 있습니다.

### 비주얼 스튜디오 코드
Microsoft에서 개발한 Visual Studio Code는 JavaScript를 포함한 다양한 언어로 코드를 작성하기 위한 가장 포괄적인 IDE 중 하나입니다. 이것은 매우 강력한 다목적 도구이기 때문에, 이제 막 개발을 시작할 경우는 최선의 선택이 아닐 수도 있습니다. 그러나 HTML, CSS, JavaScript 및 기본적으로 다른 언어에 대한 완벽한 솔루션을 원한다면 VS Code를 시도할 가치가 충분합니다. Windows, Mac 및 Linux에서 사용할 수 있으며, 다음에서 다운로드할 수 있습니다.

### 인터프리터/JavaScript 엔진
특정 요구 사항이 없는 한, 언제든지 사용 가능한 웹 브라우저에 포함된 JavaScript 엔진을 사용하는 것이 좋습니다. 따라서, 간단한 텍스트 편집기로 로컬 파일로 코드를 작성한 후, 브라우저로 열거나, IDE를 사용해서 웹 브라우저를 실행하거나, 온라인 편집기 중 하나로 코드를 작성하고 직접 테스트하든, 모두 사용할 수 있습니다.

브라우저 외부에서 JavaScript를 사용하려면 이미 알고 있듯이 Node.js를 다운로드해야 합니다. 하지만, 이 튜토리얼 범위 밖입니다 - 언어에 대해서만 설명합니다.


### 요약
JavaScript는 Windows 메모장이나 Linux 또는 Mac용 내장 편집기 같은 간단한 텍스트 편집기와 웹 브라우저만 있으면 사용할 수 있습니다. 이제 JavaScript 학습 여정을 계속하겠습니다.



## Hello, world!
역사적으로 모든 프로그래밍 관련 튜토리얼은 "Hello, world!" 라는 예제로 시작합니다. 이 예제의 목적은 단순히 "Hello, world!" 라는 문장열 구성된 메시지를 사용자에게 그대로 출력하는 것입니다. JavaScript가 발명되기 최소 20년 전인 1974년까지 거슬러 올라갈 수도 있지만, 그럼에도 불구하고 이 튜토리얼 또한 "Hello, World!" 예제로 시작하겠습니다.

JavaScript로 메시지를 출력할 때는 사용하는 JavaScript 엔진에 따라 메시지를 출력하는 위치가 달라질 수 있습니다. 예를 들어, 웹 브라우저에서 실행한다면, `alert()` 함수로 메시지를 표시할 수 있습니다.

```js
alert("Hello, world!");
```

이 예제를 실행하면 세계적으로 유명한 메시지가 포함된 멋진 팝업 창이 웹 브라우저에 표시됩니다. 앞서 얘기했듯이 이 튜토리얼은 웹 브라우저에 초점을 맞추지 않지만, 다음 코드를 HTML 파일에 포함시킬 경우, 다음과 같이 `SCRIPT` 태그에 넣어야 한다는 점도 언급하고 싶습니다.
...
이를 염두에 두고, JavaScript를 실제 (단순하지만) 웹 페이지에 통합하는 또 다른 예로 작동 방식에 대한 아이디어를 제공하고 싶습니다.
...
위의 예제를 `.html` 파일로 저장한 후, 브라우저로 열어서 실행하시기 바랍니다.

약간의 텍스트가 포함된 웹 페이지의 "기본 구조" 이므로, HTML 태그는 걱정하지 마시기 바랍니다. 대신 중간에 있는 스크립트 블록에 집중하시기 바랍니다. HTML 코드에서 직접 JavaScript 코드를 통합하고(기본적으로 웹 페이지 중간에) `document.write()` 함수로 콘텐츠 사이에 직접 텍스트를 출력했습니다.

`document.write()` 함수는 브라우저 전용 함수이므로 Node.js 로 실행할 경우는 작동하지 않을 수 있습니다. 대신 많은 JavaScript 엔진에서 작동되는 함수인 `console.log()` 를 사용할 수 있습니다.

```js
console.log("Hello, world!");
```

웹 브라우저로 실행할 경우, 출력 메시지를 읽으려면 개발자 콘솔을 열어야 합니다. 액세스하는 방법은 사용하는 브라우저에 따라 다르지만, 주로 <kbd>F12</kbd> 키를 누릅니다. - 만약, 작동하지 않으면 브라우저에서 개발자 콘솔에 액세스하는 방법을 설명하는 문서를 확인하는 것이 좋습니다.


## 요약
필수적인 "Hello, world!" 예제를 처리했으므로, 이제부터 JavaScript 언어에 대해 보다 자세히 알아볼 준비가 되었습니다.
...
# 기본 사항
## 기본 JavaScript 구문
이전에 `C++, Java, C#` 또는 `PHP` 같이 널리 사용되는 프로그래밍 언어에 대한 경험이 있다면, JavaSript 구문은 쉽게 인식할 수 있습니다. 그 이유는 이런 모든 언어는 C 프로그래밍 언어에서 파생되었거나, C 프로그래밍 언어에서 많은 영감을 받았기 때문입니다. 그러나, 이런 언어에 경험이 없거나, 프로그래밍에 대한 경험이 전혀 없다면, JavaScript 구문에 대해 간략히 소개하겠습니다.

프로그래밍을 처음 접할 경우, 혼란스러울 수도 있는 매우 기본적인 규칙부터 시작하겠습니다.

- **JavaScript는 대소문자를 구분** 합니다!!

이것은 "a 가 A 와 동일하지 않다" 는 것을 의미합니다. - 예를 보여 드리겠습니다.
...
여기서는 a (소문자)라는 이름으로 변수(나중에 자세히 설명)를 선언하지만, 다음 줄에서 A (대문자)를 참조합니다. 따라서, 실패합니다.


### 문장
"명령문" (문장)은 기본적으로 코드 줄입니다.

```js
alert("Hello, world!");
```

세미콜론 문자로 끝내는 방법을 주목하시기 바랍니다 - 이것은 인터프리터에게 명령문이 "완료되었음" 을 알립니다. 또한, 명령문은 일반적으로 한 줄 코드지만, 물리적으로 한 줄에 포함될 필요가 없음을 의미합니다. 세미콜론 문자로 끝나는 한, 여러 줄을 자유롭게 사용할 수 있습니다.
...
일부 C 스타일 프로그래밍 언어는 문장의 끝에서 세미콜론 문자를 요구하는 데 매우 엄격하지만, JavaScript는 실제로 그렇지 않습니다 - 줄 바꿈 문자에 도달했을 때, 명령문이 끝나는지 추측하며 이것은는 많은 경우 정상 작동합니다. 예를 들면 다음과 같습니다.
...
그러나 때때로 인터프리터는 코드 줄이 복잡할 경우 의도를 이해하지 못하므로 어쨌든 세미콜론을 추가해야 합니다!! 이러한 이유로 JavaScript 코드 지침은 일반적으로 항상 끝 "세미콜론을 사용할 것을 권장" 합니다. 또한, 나중에 새로운 관습을 배우지 않고도, 엄격한 프로그래밍 언어로 전환할 수 있습니다.


### 블록
IF 문 및 루프 같은 제어 구조를 사용할 경우 포함된 코드 줄을 제어해야 합니다. 기본적으로 JavaScript는 다음 줄만 제어 구조의 일부라고 가정합니다. 
...
위의 코드를 실행하면 두 번째 `alert()` 함수만 실행된다는 것을 알 수 있습니다. 그 이유는 `if()` 문은 참이 되지 않기 때문에, (이미 확인했습니다.) 다음 줄은 절대 실행되지 않습니다. 그러나, `if` 문에서 두 줄 이상의 코드를 포함할 경우는 "블록" 을 만들어야 합니다. 중괄호 문자로 `if` 문의 영향을 받는 코드 줄을 캡슐화합니다.
...
위의 예제에서 `if` 문 뒤의 다음 줄(첫 번째)과 블록 내부(두 번째)를 들여쓴 것을 확힌하시기 바랍니다. 이것은 필수 사항은 아니지만, 코드를 읽기 쉽게 만들기 때문에, 대부분의 프로그래머가 이렇게 처리합니다. 키보드의 <kbd>Tab</kbd> 키를 사용해서 한 줄 또는 여러 줄을 들여쓸 수 있습니다. 그리고, 블록 내부에 블록이 중첩될 경우, 다음과 같이 블록마다 들여쓰기를 적절하게 사용해야 합니다.
...
블록은 JavaScript에서 많이 사용합니다. (예: 위의 예와 같은 제어 구조뿐만 아니라, 함수를 정의할 때도 마찬가지입니다.) 나중에 자세히 설명합니다.


### 코멘트(주석)
기본적으로 공백 문자를 제외하고 JavaScript 코드에 작성한 모든 것은 인터프리터로 "해석" 하므로, 유효한 JavaScript 코드여야 합니다. 하지만, 자신이나 팀을 위해 설명을 남겨야 할 때도 있습니다. 다행히 JavaScript를 사용하면 이 작업은 매우 쉽게 처리할 수 있습니다. 

인터프리터는 두 개의 슬래시 문자를 만나면 나머지 줄을 무시합니다.

```js
// This line is a comment
```

두 개의 슬래시 문자 뒤의 나머지는 인터프리터에 의해 무시된다는 것을 기억하는 한, 명령문과 같은 줄에서 이 작업을 수행할 수도 있습니다.

```js
alert("Hello!");    // The rest of this line is a comment
```

또한, 여러 줄 주석을 이어갈 수 있습니다.
...
그러나, 많은 줄의 주석이 필요하고 각 주석 앞에 두 개의 슬래시 문자를 붙이고 싶지 않을 경우, "여러 줄 주석 구문" 을 사용할 수 있습니다. 

슬래시 문자로 시작한 후, 바로 뒤에 별표(`*`) 문자가 따릅니다. 그 후에는 주석 모드에서 벗어날 때까지 모든 것은 주석으로 간주되지만, 반대로(별표를 먼저 사용하고 슬래시를 앞으로) 사용합니다. 예를 들면 다음과 같습니다.
...
이 두 가지 유형의 주석을 원하는 대로 전환해서 사용합니다.


### 요약
지금까지 JavaScript 구문에 대한 간략한 소개였으며, 튜토리얼의 나머지 부분의 예제를 이해하는 데 도움되기를 바랍니다. 물론 배울 구문이 더 많지만, 언어의 다양한 측면을 소개할 때 이러한 부분을 깊이 파헤칠 것입니다.


---

## 변수
프로그래밍에서 진정 필수적인 부분은 "변수" (variable)입니다. 나중에 사용할 수 있도록, 숫자, 날짜 또는 텍스트 같은 정보를 보관할 수 있는 "저장 공간" 이라 생각하시기 바랍니다. 변수 없이 진행할 수 없으며, 물론 변수는 JavaScript에서도 사용할 수 있습니다.

변수는 `let` 키워드를 사용하거나, 저 같은 구식 JavaScript 개발자라면 `var` 키워드를 사용해서 선언합니다. 둘 다 사용할 수 있지만, 나중에 설명할 작은 차이점도 있습니다.


### 변수 선언 및 사용
변수 선언은 다음과 같이 `let` 키워드와 그 뒤에 변수 이름으로 구성합니다.

```js
let age;
```

이것은 단순히 인터프리터에게 "age" 라는 이름의 변수가 존재한다는 것만 알려줍니다. 아직까지 설정한 값이 없지만, 선언한 후 다음과 같이 값을 할당할 수 있습니다.
...
물론, 변수의 값을 이미 안다면, 다음과 같이 변수를 선언할 때 할당까지 처리할 수 있습니다.

```js
let age = 42;
```

같은 줄에 다른 값을 가진 여러 변수를 동시에 선언할 수도 있습니다 - 단지 쉼표 문자로 구분하면 됩니다.

```js
let name = "John Doe", age = 42, mail = "john@doe.com";
```

하나 이상의 변수를 선언한 후, 원하는 대로 사용하거나 변경할 수 있습니다. 예를 들면 다음과 같습니다.
...
위의 예에서 두 개의 변수를 선언한 다음, 사용합니다. 단순히 이름만 출력하지만, 나이는 변경한 다음 출력합니다. 보시다시피 매우 간단합니다.


### 변수 이름 지정
언급했듯이 변수의 이름을 결정할 수 있습니다. 하지만, 몇 가지 관례적인 규칙이 있습니다.

- 변수 이름은 문자, 숫자 또는 두 개의 특수 문자(`$` - 달러 기호) 및 밑줄(`_`) 문자만 포함될 수 있습니다.

- 변수 이름은 숫자로 시작할 수 없습니다.

- 키워드/예약어를 사용할 수 없지만, 변수 이름의 일부가 될 수는 있습니다. 예를 들어, `break` 는 키워드이므로, 변수를 "break" 라고 부를 수 없지만, "lunchBreak" 라고 정할 수 있습니다.

- JavaScript에서 좀 더 복잡한 변수의 이름을 지정할 때, "camelCasing" 을 일반적으로 사용합니다. 즉, 단어의 첫 번째는 소문자로 시작하고, 나머지 단어는 대문자로 시작합니다. 다음은 이에 대한 몇 가지 예입니다.
...
이것은 단지 "지침" (가이드)일 뿐이며, 모든 소문자 또는 대문자를 포함해서 원하는 모든 것을 자유롭게 사용할 수 있습니다. JavaScript는 "대소문자를 구분" 하므로, "NAME" 이라는 변수를 선언할 경우, "name" 변수로 참조하는 것은 기대하지 마시기 바랍니다.


### 상수
원할 때마다 변수에 값을 할당하고, 원할 때마다 값을 변경할 수 있다는 것을 방금 배웠습니다. 때로는 변수를 선언하고 즉시 값을 할당한 다음, 다른 사람이 값을 변경하지 못하도록 처리할 때가 있습니다. 이를 위해 "상수" (constant)가 있습니다. 변수처럼 보이지만, `var` 또는 `let` 키워드 대신, `const` 키워드를 사용합니다.

```js
const answerToEverything = 42;
```

상수에 즉시 값을 할당합니다 - 변수와 달리 값을 할당하지 않으면 상수를 선언할 수 없습니다!! 그리고, "상수" 이므로 값을 변경할 수 없습니다!! - 

따라서, 인터프리터는 다음과 같이 시도하면 즉시 "오류를 발생" 시킵니다.
...
위의 작업을 수행하면 다음과 같은 오류가 발생합니다.

```js
Uncaught TypeError: Assignment to constant variable.
```

어떤 값은 단순히 변경되서는 안되기 때문에, 다른 사람이 변경할 수 없는 변수를 갖는 것은 정말 유용할 수도 있습니다. 이런 이유로 JavaScript에는 `Math.PI` 같은 내장된 상수가 있습니다.
...
### 요약
"변수" 를 레이블이 달려 있는 멋지게 정렬된 "저장 상자" 로 생각하시기 바랍니다. 변수에 모든 정보를 저장하고 그에 따른 이름을 지정해서, 나중에 변수의 내용에 액세스하거나 수정할 수 있습니다. 변수는 프로그래밍에서 매우 중요한 부분이며, 이 튜토리얼을 통해 수 많은 예제에 변수를 사용하는 것을 볼 수 있으며, 직접 코드를 작성할 때는 더욱 더 그렇습니다.



## 진실과 가짜
대부분 프로그래밍 언어와 수학의 "참과 거짓의 개념" 은 완전히 다릅니다: 어떤 것이 참이고, 그렇지 않으면 거짓입니다. 그러나, JavaScript 언어가 일반적으로 매우 느슨하고, 다른 프로그래밍 언어만큼 엄격하지 않기 때문에 참과 거짓의 개념이 약간 확장됩니다.

실제로 이것은 JavaScript가 완전히 참 또는 거짓이 아니라는 사실에도 불구하고 무언가를 참 또는 거짓으로 평가한다는 것을 의미합니다. 목적은 단순히 일을 좀 더 쉽고 덜 복잡하게 만드는 것이지만, 이에 대해 잘 모르거나 잠시나마 잊어버리면 상황이 약간 혼란스러워질 수도 있습니다.

그래서 새로운 개념인 "Truthy" 와 "Falsy" 를 소개하겠습니다. 단지, 개념으로만 존재합니다 - 따라서, JavaScript 키워드를 찾을 수 없으며, 진실 또는 거짓 값을 확인할 수도 없습니다. 대신, JavaScript는 어떤 것이 완전한 참인지, 완전한 거짓인지 확인하기 위한 두 번째 연산자 세트를 도입했습니다. 이것을 "엄격한 연산자" 라고 부르며 예를 들어, 두 값을 비교할 때, 두 개 대신 세 개의 등호를 사용하는 "엄격한 평등 연산자" 라고 합니다.

다음 장에서 엄격한 연산자를 포함해서 JavaScript의 다양한 "연산자" 에 대해 훨씬 더 자세히 이야기하겠지만, 지금은 "Truthy" 또는 "Falsy" 로 간주할 수 있는 것에 대해 좀 더 자세히 이야기 하겠습니다.


### 진실
JavaScript에서 모든 값은 특별히 "Falsy" 가 아닌 한, "Truthy" 입니다. JavaScript는 루프 또는 `if` 문 같은 컨텍스트에서 사용할 때, 유형을 강제 변환해서 무언가의 "부울 값" 으로 변환합니다. 예를 들어, "Truthy" 이기 때문에, `true` 로 처리되는 값의 예를 고려합니다.
...
위의 예제를 실행하면, 세 개의 `if` 문 모두 `true` 로 평가된다는 것을 알 수 있지만, "hello world" 가 `true` 인 이유는 무엇일까요? 실제 값을 포함하기 때문에, JavaScript는 `true` 또는 `false` 가 필요한 컨텍스트에 사용될 때, 이 사실을 간주합니다.


### 가짜
일반적으로 `null, undefined` 또는 비어 있는 모든 값은 "Falsy" 로 간주하므로, 루프 및 `if` 문 같은 부울 컨텍스트에서 사용될 때, `false` 로 처리됩니다. 다음은 몇 가지 예입니다.
...
위의 샘플을 실행하면, 경고가 트리거되지 않음도 알 수 있습니다. 위의 모든 `if` 문은 이 컨텍스트에서 `false` 로 간주됩니다.



### 요약
예를 들어, 루프 또는 `if` 문의 조건을 평가할 때, JavaScript가 `true` 또는 `false` 가 필요한 경우, 유형 강제 변환으로 실제 부울 값이 아닌 값을 `true` 또는 `false` 로 바꿉니다. 이 결과물을 "Truthy" 와 "Falsy" 라고 하는데, 이것은 실제 참 또는 거짓은 아니지만, 예에서 알 수 있듯이 JavaScript는 이를 기반으로 결정을 내릴 만큼 중요합니다.
...
# 제어 구조
## IF 문
코드 흐름을 제어한다는 것은 프로그래밍에서 가장 중요한 처리 개념 중 하나이며, 가장 기본적인 제어 구조는 아마 `if` 문일 것입니다. 다음 줄을 실행할지 여부에 대한 "조건" 을 지정할 수 있습니다. 다음은 간단한 예입니다.
...
첫 번째 줄은 `if` 문으로, 다음 줄을 실행할지? 여부에 대한 간단한 조건입니다. 다음과 같이 사용자 입력을 받아 처리하도록 변경해서 실제 작동하는지 여부를 테스트할 수 있습니다.
...
이 튜토리얼 앞부분에서 이야기했듯이 조건에 두 줄 이상의 줄이 포함될 경우, 중괄호 문자로 묶어 "코드 블록" 을 만들어야 합니다.
...
### 여러 조건
`if` 문은 제공된 조건이 `true` 로 간주될지 여부를 확인해서 작동합니다. 이렇게 처리하면 원하는 만큼 조건을 추가할 수 있습니다. `&&` (AND)와 `||` (OR) 연산자(나중에 자세히 설명)를 사용해서 조건을 추가합니다.
...
따라서, 운전 면허증이 있거나, 적절한 연령(18세에서 80세 사이)일 경우, 위의 코드를 사용하면 자동차를 운전할 수 있습니다. 이것은 현실적인 예는 아니지만, `if` 문에 여러 조건을 사용하는 방법을 설명하는 역할은 하기를 바랍니다.


### if... else 
현재 `if` 문은 조건이 참인 경우만 무언가를 수행하지만, 참이 아닌 경우 다른 작업을 수행할 수도 있습니다. 이를 위해, `else` 문이 있는데, 이것은 분명히 `if` 문과 함께 작동합니다. 예를 들면 다음과 같습니다.
...
그리고 다시 한 번 말하지만, 조건 중 하나(또는 둘 다)에서 한 줄보다 많은 줄이 필요할 경우, "블록" 으로 묶으면 됩니다.
...
### if...else..if
중첩된 `if` 문으로 고급 논리 구조를 만들 수 있습니다.
...
원하거나 필요한 경우, 이전 예제처럼 조건 주위에 블록을 사용할 수 있습니다.


### 요약
`if` 문은 사용하기 쉽고 이해하기 쉬우면서 매우 강력합니다. 프로그래밍 세계를 처음 접한 사람이라면 프로그래밍이 얼마나 흔한지 깨닫고 놀랄 수도 있습니다. 또한, 다음과 같은 경우를 위한 대안적이고 보다 짧은 표기법도 있다는 것을 알아야 합니다. `else` 문은 연산자를 설명하는 장에서 다시 설명합니다. 지금은 단지 몇 가지 고급 제어 구조로 넘어 가겠습시다.



## switch 문
`switch` 문은 일련의 `if` 문과 같으며, 모두 동일한 변수를 확인하지만, 가능한 각 값에 따라 다른 작업을 처리합니다. 복잡하게 들릴지 모르지만, 실제로는 그렇지 않습니다. 다음과 같은  간단한 예를 살펴보시기 바랍니다.
...
첫 번째 줄에서 질문합니다. 그런 다음, "answer" 변수를 `switch` 문의 "조건" 으로 사용합니다. `switch` 문에서 `case` 키워드로 "answer" 변수(예, 아니오 또는 아마도)를 확인할 수 있는 여러 값을 정의합니다. 각 작업마다 다른 경고 메세지가 발생합니다.


### break 키워드
각 케이스 끝에 사용하는 `break` 키워드를 유의하시기 바랍니다. `switch` 문을 사용하려면 `break` 키워드를 사용하거나, 함수에 사용할 경우 `return` 키워드로 함수를 완전히 종료해서 각 케이스를 올바르게 종료해야 합니다. 이를 생략하면, 값과 일치한 케이스 아래의 모든 케이스를 실행합니다. 위의 예제에서 `break` 키워드를 제거한 후, 예제를 다시 실행해서 테스트할 수 있습니다.

`switch` 문은 무슨 일이 있더라도 끝나기 때문에, 마지막 `break` 키워드를 생략할 수도 있지만, 모든 프로그래밍 언어에 해당하는 것이 아니므로, 일관성을 위해 많은 프로그래머는 모든 경우 항상 `break` 키워드를 사용하는 경향입니다.


### 케이스 그룹
때로는 여러 케이스가 동일한 작업을 수행할 수 있습니다. 동일한 코드를 사용할 수 있도록, `case` 문을 쌓을 수 있습니다. 예를 들면 다음과 같습니다.
...
### default 키워드
위의 예는 명백한 결함이 있습니다: 사용자가 대소문자 중 하나와 일치한 정확한 값을 입력하지 않으면, 아무런 일도 일어나지 않습니다!! 이와 같은 상황에서 다른 옵션이 관련없는 경우 실행되는 대체 작업이 있다면 좋습니다. 다행스럽게 JavaScript를 사용하면 `default` 키워드로 처리할 수 있습니다. 다음은 위의 예제의 수정된 버전입니다.
...
이렇게 처리하면 `default` 키워드의 대체 작업이 있기 때문에 `switch` 문은 모든 상황을 처리할 수 있습니다.


### case 비교
위의 예제가 실제 시나리오에 사용되면, 고려할 또 다른 문제는 각 `case` 를 조건과 비교하는 방법입니다. 예를 들어, "yes" 를 확인하지만, 문자열은 항상 대소문자를 구분하므로, 사용자가 "Yes", "YeS" 또는 "YES" 를 입력하면, 대소문자가 일치하지 않습니다. 예를 들어, 확인하기 전에 입력 값을 소문자로 변환해서 항상 소문자로 처리해야 합니다.
...
고려할 또 다른 문제는 모든 비교가 엄격한 동등 비교 연산자(`===`)를 사용해서 수행된다는 사실입니다. 즉, `switch` 문의 표현식이 문자열일 경우, 이 문자열이 실제로 숫자라는 것을 알더라도 다른 문자열과 비교할 수 있습니다. 예를 들어 설명하겠습니다.
...
이 예제는 예상대로 작동하지 않습니다!! 왜? `prompt()` 함수의 결과는 텍스트 문자열(사용자가 숫자 값을 입력하더라도...)이지만, 숫자(41 또는 42)와 비교하기 때문입니다. 예를 들어, 답을 확인하기 전에 먼저, 숫자로 바꾸거나, 대신 문자열과 비교해서 처리해야 합니다. 예제의 첫 번째 줄을 변경하고 `Number()` 함수를 사용해서 `prompt()` 함수의 결과를 숫자로 변환하겠습니다.
...
### 요약
`switch` 문을 사용하면 표현식을 여러 선택 항목과 비교하고, 이것은 일련의 `if..` 문과 다른 `if` 문이지만, 좀 더 읽기 쉬운 형식입니다. `switch` 문은 엄격한 동등 비교를 사용한다는 점을 기억하시기 바랍니다.



## Loop: for 및 while
방금 프로그래밍 언어의 매우 필수적인 부분인 `if` 문을 배웠습니다. 거의 똑같이 중요한 것이 루프문입니다: 동일한 코드 블록을 여러 번 반복할 수 있는 기능과 반복 횟수를 제어할 수 있는 기능입니다.

JavaScript에는 현재 `for` 루프와 `while` 루프 두 가지 유형의 루프 구조가 있습니다. 둘 다 여러 가지 변형으로 제공되며, 작동 방식과 사용 방법에 대해 자세히 살펴보겠습니다.



### while 루프
시작할 간단한 유형의 루프는 `while` 루프입니다. 구문은 매우 간단합니다.
...
따라서, 괄호 안의 조건이 `true` 로 평가되는 한, 다음 코드 블록이 계속 반복 실행됩니다. 이렇게 처리하려면 인터프리터가 루프를 종료할 수 있도록, 반복되는 블록 내에서 무언가를 처리해야만 합니다 - 그렇지 않으면 코드가 끝나지 않는 소위 "무한 루프" 를 만들게 됩니다!!

`while` 루프의 실제 사용 예는 다음과 같습니다.
...
"counter" 변수가 10보다 크지 않은 한, 실행되며 각 반복마다 숫자를 출력하고 카운터 변수를 증가시킵니다. 특히, 마지막 부분이 중요한데, 그것 없이는 "counter" 변수가 절대 10보다 크지 않고, 루프는 끝없이 실행되기 때문입니다. 또한, 나중에 보겠지만, `for` 루프가 이와 같은 작업에 좀 더 적합하지만, `while` 루프가 어떻게 작동하는지 설명하기 위해 간단하고 직접적인 예제가 필요했을 뿐입니다.


### do... while 변형
위의 예와 같이 일반 `while` 루프를 사용하면, 루프에 들어가기 전에 조건을 먼저 평가합니다. 이것은 실제로 입력하지 않을 수도 있음을 의미합니다 - 조건이 참이 아니라면 코드 블록은 실행되지 않습니다. 이것을 직접 보려면, 위 예제에서 "counter" 변수의 초기 값을 11 로 변경하고 다시 실행합니다!

`do... while` 루프 변형은 코드를 실행하기 "전에" 조건을 평가하는 대신, 루프 코드가 먼저 실행한 후에 평가를 수행합니다. 이렇게 처리하면 일부 시나리오에서 유용할 수도 있는 "최소 한 번의 반복이 보장" 됩니다. 예를 들면, `while` 부분이 어떻게 작업을 수행하는지 처리 순서를 명확히 알 수 있습니다.
...
위의 예는 `while` 루프를 `do... while` 루프 변형으로 사용한 다음, 카운터 변수의 초기 값을 11 로 변경했습니다. 만약, 일반 `while` 루프였다면,  그 안의 코드는 결코 실행되지 않겠지만, `do... while` 루프에서 `false` 로 평가되도록 처리한 조건은 첫 번째 반복 후에 평가하므로, 적어도 한 번의 실행은 보장됩니다.


### for 루프
`while` 루프의 훌륭한 대안은 `for` 루프입니다. 조금 더 복잡해 보이지만, 좀 더 유연하고 자주 사용하기 때문입니다. `for` 루프는 카운터를 선언한 다음, 이 카운터 조건을 선언한 다음, 각 반복에서 카운터에 대한 단계(증가 또는 감소)를 선언해서 작동하며 이 모두를 한 줄로 정의합니다. 예를 들어, 설명하겠습니다.
...
여기서는 첫 번째 `while` 루프로 했던 것과 동일한 작업을 수행합니다. 1부터 10까지 계산합니다. 그러나, 보시다시피 카운터 메커니즘은 기본적으로 `for` 루프에 내장되기 때문에 코드가 덜 필요합니다. 첫 번째 줄에는 인터프리터에게 각 명령을 세미콜론으로 구분해서 다음을 작업을 수행하도록 지시합니다.

- "counter" 라는 변수를 선언하고 값 1 을 할당합니다

- 카운터 변수가 10 보다 작거나 같은 한, 이 루프가 실행된다고 지정합니다

- 각 반복에서 카운터 변수를 1 씩 늘려야 한다고 지정합니다

위의 예는 긴 변수 이름을 사용하고 증가 연산자를 사용하지 않음으로 의도적으로 장황합니다. (나중에 자세히 설명). 보다 실제적인 예는 다음과 같습니다.
...
위의 `for` 루프 예제는 이전의 `while` 루프와 매우 유사합니다. 또한, `for` 루프가 코드에서 암시하는 정확한 순서대로 실행되지 않기 때문입니다. 첫 번째 줄의 세 부분 모두를 먼저 실행한 다음, 루프의 실제 코드가 실행될 것으로 예상할 수 있지만, 그렇지 않습니다!! 

대신 `for` 루프는 다음과 같이 처리됩니다.

- 변수가 선언됩니다.

- 조건이 확인하므로 첫 번째 시도에서 조건이 실패하면 반복은 수행되지 않습니다.

- 조건이 성공하면 루프 코드(이 경우 document.write())가 실행됩니다.

- 루프 마지막 부분인 단계(i++)가 실행됩니다

처음에는 약간 혼란스러울 수 있지만, 곧 익숙해질 테니 걱정하지 마시기 바랍니다!
