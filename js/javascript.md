...
이 튜토리얼은 흥미롭고 인기있는 JavaScript 프로그래밍 언어의 새로운 사용자를 대상으로 합니다. 고급 사용자도 특정 장(章)을 읽음으로써, JavaScript 지식의 빈틈을 채울 수 있으며, 한 두 가지는 배울 수 있습니다.  

각 장은 JavaScript에 처음 입문한 사람에게 잘 맞도록 구성되어 있으며, 이전 장을 읽지 않고도 대부분의 장을 활용할 수 있지만, 첫 장부터 차례대로 장을 따라가는 것이 이 튜토리얼로 가장 많은 것을 얻을 수 있는 방법입니다. 만약, 어떤 장에서 알고있는 정보를 담고 있다면, 다음 장으로 넘어가시면 됩니다.  

이 튜토리얼은 계속해서 새로운 장을 추가할 수 있으니, 정기적으로 다시 방문하시기 바랍니다. 이 튜토리얼로 여러분이 JavaScript를 제대로 시작하도록 돕고, 저자만큼 JavaScript에 흥분을 느끼시길 바랍니다.  

또한, 사전 지식이 없더라도 JavaScript를 확실히 배울 수 있습니다. 하지만, 웹사이트를 만들기 위한 JavaScript를 사용하려면, `HTML` 에 대한 기초 지식이 권장됩니다. 만약, `HTML` 도 처음이라면, [HTML5 튜토리얼](http://www.html5-tutorials.org/)을 살펴보시길 권합니다.  
...
현재는 아주 단순하고 사소할 수 있지만, JavaScript가 1997년에 초기 웹브라우저 중 하나인 Netscape Navigator에 처음 도입되었을 때는 엄청난 일이었습니다. 그 전까지 웹페이지는 전부 "레이아웃" 과 "텍스트" 에 관한 것이었고, 정적인 콘텐츠를 브라우저에 전달하고, 다른 페이지로 이동하기 전까지 변경할 수 없었습니다. JavaScript는 그 모든 것을 바꾸었고, `HTML` 과 `CSS` 와 함께 오늘날 우리가 알고있는 매우 동적인 웹사이트 형태로 진화하기 시작했습니다.  

## JavaScript 언어 특징
JavaScript 언어에 대해 말할 것이 많으며, 그 중 많은 부분은 다른 프로그래밍 언어와 비교할 수 있습니다. 만약,  프로그래밍이 완전 처음이라면, 이런 정보는 지금 당장 아무런 의미가 없을 수 있지만, 나중에는 의미가 있습니다.  

- JavaScript는 이름 절반과 구문 스타일을 Java 프로그래밍 언어에서 가져왔지만, 혼동하진 마시기 바랍니다. 중요한 영역에서 두 언어는 상당히 다릅니다.  

- JavaScript는 약한 타입(weakly typed) 언어입니다. 즉, 변수를 특정 타입으로 미리 선언할 필요가 없습니다. 대신, 인터프리터가 사용 방식에 따라 타입을 추측합니다.  

- JavaScript는 동적(dynamic)입니다. 예를 들어, 정수(integer) 같은 숫자 변수에 단순히 텍스트를 할당해서 문자열(string) 변수로 바꿀 수 있습니다.  
...
- JavaScript는 런타임 평가(runtime evaluation)를 제공합니다. JavaScript 코드를 원래 코드인 것처럼 인터프리터가 평가할 수 있다는 것을 의미합니다.  
...
JavaScript와 웹브라우저는 밀접하게 연관되어 있습니다. 사실, 당연합니다. 왜냐하면, JavaScript는 원래부터 Netscape Navigator 웹브라우저 백엔드 개발팀에 의해 만들어졌고, 웹에서만 사용했기 때문입니다. 이것이 바로 JavaScript 언어를 "안전한 언어" 라 부르는 이유입니다. 가장 일반적인 구현에서 JavaScript는 기본적으로 파일 시스템이나 그와 유사한 것에 접근할 수 없습니다. 대신, 일종의 "샌드박스" 같이 작동하고 시스템과 관련없는 "좋은" 일은 처리할 수 있습니다.  

그러나, "JavaScript는 웹브라우저 밖에서 사용할 수 있다!" 는 점에 주목하는 것이 중요합니다. JavaScript를 실행하려면, JavaScript 엔진, 또는 인터프리터를 사용합니다. 이 엔진은 브라우저에서 실행할 수 있으며, 실제 인류가 알고있는 거의 모든 현대적인 웹브라우저에서 실행됩니다. 하지만, 이 엔진이 브라우저 밖에서 실행되는 것을 막을 수 있는 것은 아무 것도 없습니다! 사실, 여러 번의 시도가 이루졌습니다.  

## Node.js
이와 관련된 가장 주목할만한 예는 바로 `Node.js` 프로젝트입니다. 2009년 만들어진 이 프로젝트는 기본적으로 JavaScript 엔진을 웹브라우저에서 꺼내고, JavaScript를 "웹브라우저 전용 스크립트 언어" 가 아닌, "범용 프로그래밍 언어" 로 만들기 위한 여러 가지 모듈을 추가했습니다. 이것은 JavaScript에 대한 새로운, 그리고 훨씬 고 수준의 관심을 불러 일으켰습니다. 

이제 JavaScript 기술은 웹(클라이언트 측)뿐만 아니라 ,서버 측 스크립트에도 사용할 수 있기 때문입니다. 그리고, `Node.js` 는 "크로스 플랫폼" 이므로, 브라우저없이 Windows, Linux, Unix, macOS 전반에 걸쳐 코드를 "그대로 실행" 할 수 있습니다.  

## 호스트 환경
이 모든 것은 "호스트 환경" (host environment)에 대한 설명으로 이어집니다. 호스트 환경은 JavaScript가 실제 실행되는 곳이며, JavaScript 언어에 존재하지 않는 수 많은 기능을 포함할 수 있습니다. 가장 분명한 예는 "웹 브라우저" 입니다. 웹 브라우저는 수 많은 기능을 추가해서, JavaScript 코드가 웹 브라우저와 통신하거나, 웹사이트 내용을 조작할 수 있도록 처리할 수 있습니다.  

또 다른 호스팅 환경은 서버 측 기능을 추가한 `Node.js` 입니다. 그러나, JavaScript는 원래부터 어디서든 실행할 수 있도록 설계됐기 때문에, 커피 머신일 수도 있습니다. 커피 머신은 JavaScript 엔진을 쉽게 포함할 수 있고, 웹사이트 대신, 버튼으로 명령을 입력받고, 커피를 만드는 것과 관련된 기능을 처리할 수 있습니다.  

## 브라우저와 함께 혹은 없이?
보시다시피, JavaScript는 확실히 웹 브라우저 밖에서 실행할 수 있습니다. 지금도 그렇고, JavaScript의 인기가 점점 더 높아짐에 따라, 더욱 그럴 것임은 쉽게 상상할 수 있습니다. 따라서, 이 튜토리얼은 무엇보다 "JavaScript 언어 자체를 배우는 것이 중요하다" 는 점을 강조하고 싶습니다.  

그래서, 튜토리얼 첫 부분에는 실제 "언어에만 집중" 할 것이며, 웹 브라우저와 어떻게 상호 작용하는지에 대해서는 크게 이야기하지 않습니다. 그렇다해서 JavaScript가 웹 브라우저와 상호 작용하는 방식을 완전히 무시하는 것도 어리석은 일입니다. 왜냐하면, 여전히 가장 많은 사용 사례이기 때문입니다. 따라서, 튜토리얼 후반부에는 이 주제에 대해서도 설명하며, JavaScript와 웹 브라우저 사이의 연관성에 대해 좀 더 깊이 파고들겠습니다.  

## 순수주의자를 위한 작은 예고  
튜토리얼 첫 부분에서 "웹 브라우저는 배제한다" 하더라도, 웹 브라우저와 관련이 깊은 몇 가지 함수는 사용할 것이란 점은 예상할 수 있습니다. 대표적인 예는 바로 `alert()` 함수입니다. 이 함수는 웹 브라우저에서 단순한 메시지 박스를 보여줍니다. 이 함수를 사용한 이유는, 튜토리얼에서 사용하는 예제를 단순하고 이해하기 쉽게 만들기 때문이며, 또한 웹 브라우저로 튜토리얼을 읽어가면서 예제를 직접 실행해서 눈에 보이는 결과를 만들 수 있기 때문입니다.  

## 요약  
JavaScript는 주로 웹브라우저에서 실행되지만, 항상 그런 것은 아닙니다. 이 튜토리얼은 우선 JavaScript 언어에 집중해서 "언어 자체" 를 배울 수 있도록 합니다. 그리고 나중에 JavaScript가 웹브라우저와 어떻게 상호작용할 수 있는지 살펴봅니다. 그러나, 브라우저와 약간의 상호작용하는 예제는 보여줍니다. 왜냐하면, 이해하기 쉽고, 글을 읽으면서 직접 테스트할 수 있기 때문입니다.  


## JavaScript libraries
이 튜토리얼은 JavaScript 언어만 집중 설명한 후, 웹과 관련된 부분도 설명한다고 방금 약속했지만, 시작하기 전에 웹과 관련된 한 가지 주제는 언급하겠습니다: 바로 "JavaScript 라이브러리" 입니다.  

JavaScript는 매우 확장성 높은 언어이므로, 자신만의 기능을 언어에 추가하고, 마치 JavaScript 기본 기능인 것처럼 사용하는 것은 매우 쉽습니다. JavaScript가 도입된 직후, 숙련된 개발자는 이런 사실을 활용해서 자신만의 "라이브러리" 를 만들었습니다. 이런 라이브러리는 자주 사용하는 기능을 추가하거나, 기존 기능을 보다 쉽게 사용할 수 있도록 만든 것입니다. 즉, 보다 적은 코드로 쉽게 작업할 수 있는 일종의 "지름길" 입니다.  

개발자들이 이런 지름길 기능을 추가한 이유 중 하나는, 과거 "브라우저 전쟁" 시절의 다양한 웹 브라우저는 자신만의 JavaScript 엔진을 가졌기 때문입니다. 가장 인기있는 웹 브라우저 사이에서 대부분 기능은 동일했지만, 차이점도 존재했음을 의미합니다. 기본적으로, 방문자에게 동일하게 작동할 것이란 신뢰를 줄 수 없었던 것입니다.  

그래서, 초기 라이브러리는 주로 방문자가 어떤 브라우저를 사용하는지 확인한 다음, 해당 브라우저가 가진 문제점이나 부족한 점을 우회해서, 어떤 상황에서도 의도한대로 작동하도록 보장하는 기능이 주였습니다.  

대부분의 개발자는 라이브러리를 자신만 사용하거나, 자신이 일하는 회사에서 사용했지만, 일부 라이브러리는 공개했습니다. 주로 개발자에게 중요하다고 여겨지는 기능이기 떄문입니다. 이런 라이브러리 공개는 다른 개발자들의 참여로 보다 많은 기능이 추가되고, 기존 기능을 완성할 수 있도록 만들었습니다.  

### jQuery  
일반적으로 사용한 최초의 JavaScript 라이브러리 중 하나가 바로 `jQuery` 입니다. 2006년에 처음 출시됐으며, 주로 `DOM` (Document Object Model)의 항목을 찾거나, 조작하는 등의 작업을 훨씬 쉽게 만들었습니다. `DOM` 은 웹사이트의 레이아웃과 콘텐츠를 구성하는 "모든 요소" 를 의미합니다. 또한, `jQuery` 는 모든 주요 브라우저에서 웹 관련 기능이 의도한대로 작동하도록 보장합니다.  

그 후 `jQuery` 는 지속적인 업데이트를 진행했고, 사용자들이 점점 많은 것을 처리할 수 있도록 진화했습니다. 그 예는 AJAX 요청, 애니메이션, 그리고 수 많은 유틸리티 함수가 있습니다. 이것은 기본적으로 개발자들이 간단한 것부터 고급 작업까지 보다 적은 코드로 쉽게 달성할 수 있도록 만들었습니다.  

물론, `jQuery` 의 성공은 수 많은 경쟁 라이브러리를 탄생시켰습니다. 그 중 하나는 2007년에 출시한 `mooTools` 였지만, 빙산의 일각에 불과했습니다. `jQuery` 가 기능을 계속 늘려갈 동안, 일부 개발자는 크기와 복잡성을 최소화한 간결한 대안을 내놓거나, 완전히 다른 방식으로 동작하는 라이브러리를 출시했습니다.  

### jQuery 사용: 현재와 미래 
오늘날 `jQuery` 는 일부 개발자는 "구식" 이라 생각합니다. 그들은 `jQuery` 가 너무 많은 것을 처리한다고 느끼며, `jQuery` 로 해결한 문제들 중 일부는 JavaScript의 발전과 더불어 JavaScript 엔진에 통합된 덕분에 더 이상 문제도 아니라고 생각합니다. 그러나, 위키백과에 따르면, “2022년 8월 기준으로, `jQuery` 는 가장 인기 있는 천 만개 웹사이트 중 77%에서 사용한다” 고 말합니다.  

`jQuery` 는 확실히 많은 대안이 있으며, 그 중 하나는 단순히 JavaScript 라이브러리를 사용하지 않고, 2006년 `jQuery` 가 출시될 때보다, 훨씬 더 나아진 JavaScript 엔진을 사용하는 것입니다. 하지만, 초보자의 경우 `jQuery` 는 훌륭한 "출발점" 일 수 있습니다. 많은 작업을 쉽게 처리하며, 동적인 웹사이트를 만드는 재미있는 작업에 집중할 수 있고, 지루하고 번거로운 부분은 `jQuery` 에 맡길 수 있습니다.  

### 요약  
JavaScript 라이브러리로 동적인 웹사이트를 쉽게 개발할 수 있습니다. 그리고, 15년 전보다 현재는 덜 필요하지만, 웹 작성에 JavaScript를 사용할 때는 시도해 보는 것을 고려해야 합니다.  

---

# Getting started
## 필요한 개발 도구
JavaScript는 개발에 필요한 도구로 본다면, 시작하기 가장 간단한 프로그래밍 언어 중 하나입니다. 사실, 시작할 때, 아무 것도 다운로드할 필요가 없습니다. JavaScript는 어떤 텍스트 편집기라도 작성할 수 있으며, Windows 메모장(또는 Mac/Linux의 동등한 프로그램) 같은 단순한 편집기도 사용 가능합니다. 

코드를 해석하거나 테스트할 때, 필요한 것은 오직 "웹브라우저" 뿐입니다. 다시 말해, 값 비싼 IDE를 구매하거나, 복잡한 컴파일러/인터프리터를 다운로드하지 않고 JavaScript를 시작할 수 있습니다.  

그렇다해도, 개발 경험을 향상시키기 위한 몇 가지 유용한 도구들이 존재합니다. 무엇을 향상할 수 있는지 조금 이야기하겠습니다.  

## 에디터  
앞서 말했듯이, JavaScript 코드를 작성할 때는 "메모장" 같은 단순한 텍스트 편집기면 충분합니다. 하지만, 실제 코드를 작성하기 위한 "전용 에디터" 를 한 번이라도 사용했다면, 효율성을 얼마나 향상시킬 수 있는지 잘 알 수 있습니다. 현대적인 코드 에디터/IDE(통합 개발 환경)는 자동완성(AutoComplete)과 IntelliSense 같은 편의 기능으로 보다 빠르고 효율적인 코드를 작성할 수 있도록 도와줍니다. 이런 기능은 JavaScript도 필요합니다.  

JavaScript는 웹과 매우 밀접하기 때문에, `HTML` 이나 `CSS` 를 작성할 때, 코드/웹 에디터를 사용하고 있을지 모릅니다. 그렇다면, 그 에디터는 아마 JavaScript도 지원할 것이며, JavaScript를 배울 때도 그 에디터를 사용할 것을 권장합니다. 그렇지 않다면, 다음 코드 에디터/IDE 중 하나를 구하는 것을 추천합니다.  

## 다양한 에디터  

### 온라인 에디터  
개발할 때, 온라인 에디터를 사용하는 것이 좋을 수도 있습니다. JavaScript는 웹과 밀접하기 때문에, “진짜” 에디터와 거의 유사한 기능을 처리하는 온라인 에디터가 많이 존재하며, 별도로 설치할 필요없이, 웹 브라우저에서 곧바로 사용할 수 있습니다.

이들은 종종 “playgrounds” 나 “fiddles” 이라 불리는데, 주로 코드를 테스트하거나, 시연하고, 공유하도록 설계되어 있습니다. 따라서, 대규모 프로젝트보다 단순한 용도에 적합하지만,(일부는 대규모 프로젝트에 사용될 수 있음) 이 튜토리얼의 코드 예제를 테스트하거나 실험하기는 완벽한 개발 환경입니다.  

따라서, 별도로 선호하는 에디터가 없고, JavaScript가 배워볼 만한지 확인하려면, 온라인 에디터를 강력히 추천합니다. 이들은 다양한 수준의 IntelliSense까지 제공하며, JavaScript 코드를 효율적으로 작성할 수 있도록 도와주며, 코드를 브라우저에서 즉시 테스트할 수도 있습니다. 몇 가지 제안은 다음과 같습니다:  
  ...
  직접 한 번 사용해 본 후, 마음에 드는 것 하나를 선택하시기 바랍니다.  

### 단순 텍스트 편집기  
앞서 언급했듯이, 운영체제에 기본적으로 포함된 단순한 텍스트 편집기(Windows의 "메모장" 등)를 사용할 수 있습니다. 그러나, 이 선택은 최소한의 도움만 받으려는 순수 개발주의자를 위한 선택입니다. 대부분 사람은 IntelliSense, 구문 검사 같은 편의 기능을 제공하는 다른 대안을 추천합니다.  

### 웹 에디터  
HTML, CSS, JavaScript 같은 웹 관련 기술에 중점둔 웹 에디터들이 많습니다. 때때로 “HTML 에디터” 라고 불리는데, 아마 그렇게 시작했기 때문일 것입니다. 그러나, 대부분 JavaScript도 잘 지원합니다. 여기서 특정 에디터를 추천하진 않지만, 위키에서 확인할 수 있는 에디터 목록이 있습니다.  

### Visual Studio Code  
Microsoft에서 개발한 Visual Studio Code는 JavaScript를 포함한 수 많은 언어로 코드를 작성할 수 있는 가장 포괄적인 IDE 중 하나입니다. 매우 강력한 다목적 도구이므로, 처음 시작할 경우는 최선의 선택이 아닐 수도 있습니다. 그러나, HTML, CSS, JavaScript 그리고 사실상 다른 어떤 언어까지 처리할 수 있는 완전한 솔루션을 원한다면, VS Code는 확실히 시도해 볼 가치가 있습니다. Windows, Mac, Linux에서 사용할 수 있으며 누구나 다운로드할 수 있습니다.  

## 인터프리터/JavaScript 엔진  
특별한 요구 사항이 없다면, "웹 브라우저의 JavaScript 엔진을 사용하는 것을 권장" 합니다. 따라서, 단순 텍스트 편집기로 로컬에서 코드를 작성하고 브라우저로 열든, IDE를 사용해서 브라우저를 열든, 온라인 에디터에서 코드를 작성해서 직접 테스트하든, 모두 사용할 수 있습니다.  

반면, 브라우저 밖에서 JavaScript를 사용하려면 `Node.js` 를 다운로드해야 합니다. 그러나, 이 튜토리얼의 범위를 벗어납니다. 여기서는 언어 자체만 다룹니다.  

## 요약  
JavaScript는 시작하기 쉽고 간단합니다. 개발에 필요한 것은 Windows의 "메모장" 이나, Linux/Mac에서 기본 제공하는 단순한 텍스트 편집기와 웹브라우저 뿐입니다. 이것만 준비된다면, JavaScript 학습을 계속할 수 있습니다.  

---

# Hello, world!
역사적으로, 모든 프로그래밍 관련 튜토리얼은 반드시 “Hello, world!” 예제로 시작합니다. 이 예제의 목적은 단순히 사용자에게 “Hello, world!” 라는 문장으로 이루어진 "메시지를 출력하는 것" 이며, 입력을 받거나, 다른 어떤 일도 처리하지 않습니다. 이 예제는 1974년까지 거슬러 올라가는데, JavaScript가 발명되기 최소 20년 전이지만, 그럼에도 불구하고 이 튜토리얼 역시 “Hello, World!” 예제로 시작합니다.  

JavaScript로 메시지를 출력할 때는 해석하는 JavaScript 엔진에 따라 달라질 수 있습니다. 예를 들어, 웹 브라우저에서 실행하려면, 다음과 같이 `alert()` 함수로 메시지를 표시할 수 있습니다:  

```javascript
alert("Hello, world!");
```  

이 예제를 실행하면, 세계적으로 유명한 메시지가 담긴 멋진 팝업 윈도우를 눈으로 직접 볼 수 있습니다. 앞서 이야기했듯이, 이 튜토리얼은 웹 브라우저 기술에 집중하진 않지만 만약, 이 코드를 HTML 파일에 삽입해서 실행하려면, 반드시 `SCRIPT` 태그 안에 있어야 한다는 점은 언급해야 합니다.  
...
## Hello, webpage!  
이제 실제(비록 단순하지만) 웹페이지에 JavaScript를 통합하는 또 다른 예제를 보여드리겠습니다. 이를 통해, 어떻게 작동하는지 감을 잡을 수 있습니다:  
...
위의 예제를 직접 실행해 보시기 바랍니다. 컴퓨터에 `.html` 파일로 저장한 뒤, 브라우저로 열어볼 수 있습니다.  

여러 가지 `HTML` 태그에 대해서는 걱정하지 마시기 바랍니다. 약간의 텍스트와 함께 웹페이지의 기본적인 구조일 뿐입니다. 대신, 중간에 있는 "스크립트 블록" 에 집중하시기 바랍니다. 그것으로 JavaScript 코드를 HTML 코드에 통합했고, (즉, 웹페이지의 중간에) `document.write()` 함수로 콘텐츠에 텍스트를 출력했습니다.  

## Hello, console!  
`document.write()` 함수는 "웹 브라우저 전용 함수" 이므로, `Node.js` 로 실행하면 작동하지 않습니다. 대신, 많은 JavaScript 엔진에서 사용하는 `console.log()` 함수를 사용합니다:   

```javascript
console.log("Hello, world!");
```  

웹 브라우저로 실행해서, 위의 메시지를 읽으려면 "개발자 콘솔" 을 확인해야 합니다. 개발자 콘솔에 접근하는 방법은 사용하는 브라우저에 따라 다르지만, 보통 <kbd>F12</kbd> 키를 눌러볼 수 있습니다. 만약, 작동하지 않으면, 다양한 브라우저에서 개발자 콘솔에 접근하는 방법을 설명한 글을 참고하는 것이 좋습니다.  
...
만약, `C++, Java, C#` 또는 `PHP` 같은 매우 인기있는 프로그래밍 언어 중 하나라도 경험이 있다면, JavaScript 문법을 쉽게 알아볼 수 있습니다. 왜냐하면, 이 모든 언어는 "C 프로그래밍 언어에서 파생" 했거나, C 언어에서 큰 영감을 받았기 때문입니다. 만약, 이런 언어에 대한 경험이 전혀없거나, 심지어 프로그래밍이 처음이라면, JavaScript 문법에 대한 간단한 소개를 하겠습니다.  

아주 기본적인 규칙부터 시작합니다. 프로그래밍이 완전 처음이라면 혼란스러울 수도 있는 규칙입니다:  

## JavaScript는 대소문자를 구분합니다!!  
이것은 "`a` 와 `A` 는 같지 않다" 는 것을 의미합니다. 예를 들겠습니다:  
...
여기서 소문자 `a` 변수를 선언했지만,(변수에 대해서는 나중에 다룹니다.) 다음 줄에서 대문자 `A` 변수를 참조하고 있습니다. 같은 대소문자를 사용하지 않기 때문에 실패합니다.  
...
여기서 "세미콜론 문자로 끝난다" 는 것을 알 수 있습니다. 이것은 인터프리터에게 "문장이 끝났음" 을 알려줍니다. 

또한, 문장은 보통 "한 줄 코드" 로 작성하지만, 물리적으로 반드시 한 줄에 포함될 필요는 없습니다. 세미콜론으로 끝나는 한, 여러 줄에 사용할 수 있습니다:  
...
일부 C 계열 프로그래밍 언어는 세미콜론 문자를 반드시 요구하지만, JavaScript는 그렇지 않습니다. 줄 바꿈이 있을 때, 문장이 끝났다고 추측하며 많은 경우 제대로 작동합니다. 예를 들어:  
...
그러나, 코드가 복잡해질 경우, 인터프리터는 사용 의도를 제대로 이해하지 못할 수 있으며, 그럴 때는 세미콜론을 반드시 추가해야 합니다. 이런 이유로 JavaScript 코딩 가이드라인은 "항상 세미콜론을 사용하는 것을 권장" 합니다. 이렇게 처리하면 중요한 상황에서 세미콜론 문자를 잊지 않을 수 있고, 나중에 보다 엄격한 프로그래밍 언어로 전환할 때도 새로운 습관을 배울 필요가 없습니다.  
...
위의 코드를 실행하면, "두 번째 `alert()` 함수만 실행된다" 는 것을 알 수 있습니다. 그 이유는 `if()` 문은 결코 참이 될 수 없기 때문에, 그 다음 줄은 실행하지 않습니다. 

그러나, `if` 문에서 여러 줄 코드를 포함한다면, "코드 블록" 을 만들어야 합니다. 중괄호 문자로 `if` 문으로 영향받는 코드 줄을 묶습니다:  
...
위의 예제는 `if` 문 다음 줄(첫 번째 예제)과 블록 안(두 번째 예제)에 들여쓰기로 작성했습니다. 이것은 필수 사항은 아니지만, 대부분 프로그래머는 "코드 가독성" 을 위해 이렇게 처리합니다. 키보드의 <kbd>Tab</kbd> 키로 한 줄 또는 여러 줄을 "들여쓰기" 할 수 있습니다. 그리고, 블록 안에 또 다른 블록이 있다면, 적절히 들여쓰기합니다. 매 블록마다 한 번씩 들여쓰는 방식입니다. 예를 들어:  
...
## Comments(주석)  
기본적으로, JavaScript는 작성한 모든 것을 "공백을 제외하고 모두 해석" 하므로, 유효한 JavaScript 코드여야 합니다. 그러나, 때때로 자신이나 팀을 위해 주석(설명)을 남길 때도 있습니다. 다행히 JavaScript는 쉽게 처리할 수 있습니다. 

인터프리터가 "두 개의 슬래시 문자" (`//`)를 만나면, 그 줄 나머지를 해석하지 않고 "무시" 합니다:  

```javascript
// This line is a comment
```  

문장과 같은 줄에 사용할 수 있습니다. 두 개의 슬래시 문자 이후는 "인터프리터가 무시한다" 는 점만 기억하시면 됩니다:  

```javascript
alert("Hello!"); // The rest of this line is a comment
```  

여러 줄을 주석으로 연속해서 사용할 수 있습니다:  
...
그러나, 수 많은 줄에 주석이 필요하고, 각 줄마다 두 개의 슬래시 문자를 붙이지 않으려면, "여러 줄 주석 구문" 을 사용할 수 있습니다. 슬래시 문자 뒤에 별표(`*`) 문자를 붙여서 시작합니다. 그 이후는 모두 주석문으로 간주하며, 별표 문자 뒤에 슬래시 문자를 붙여서 닫을 때까지 계속됩니다. 예를 들어:  
...
## 요약  
이번 섹션은 JavaScript 문법(syntax)에 대한 간단한 소개입니다. 하지만, 튜토리얼 나머지 예제를 이해할 때 도움됩니다. 물론, 배워야 할 문법이 더 많지만, 언어의 다양한 측면을 소개할 때, 차차 다룹니다.  

---

# Variables
프로그래밍에 정말 필수적인 부분은 바로 "변수" (variable)입니다. 변수를 일종의 "저장 공간" 이라 생각하시기 바랍니다. 숫자, 날짜, 혹은 텍스트 같은 정보를 나중에 사용할 수 있도록 보관할 수 있는 일종의 "공간" 입니다. 이 기능 없이는 멀리 가지 못하며 물론, JavaScript도 변수를 사용합니다.  

변수는 `let` 키워드로 "선언" 합니다. 혹은, 저처럼 구식 JavaScript 개발자라면 `var` 키워드를 사용할 수 있습니다. 둘 다 사용할 수 있지만, 약간의 차이도 있으며, 이에 대해서는 나중에 설명합니다. 지금은 단지, 새로운 버전인 `let` 키워드를 사용하시기 바랍니다.  
...
물론, 변수가 어떤 값을 가지는지 안다면, "선언" 과 동시에 값을 "할당" 할 수 있습니다:  

```javascript
let age = 42;
```  

심지어 여러 개의 변수를 서로 다른 값으로 한 줄에 선언할 수 있습니다. 이때 쉼표 문자로 변수를 "구분" 하면 됩니다:  
...
위의 예제에서 두 개의 변수를 선언한 후, 사용합니다. "name" 변수는 단순히 출력하고, "age"  변수는 변경한 후, 출력합니다. 보시다시피, 꽤 간단합니다.  

## 변수 이름 짓기  
앞서 언급했듯이, "변수 이름" 은 사용자가 자유롭게 정의할 수 있습니다. 하지만, 몇 가지 "네이밍 규칙" 이 있습니다:  
...
JavaScript에서 복잡한 변수 이름을 지을 때, `camelCase` 방식을 일반적으로 사용합니다. 첫 단어는 소문자로 시작하고, 그 이후 단어들은 대문자로 시작하는 방식입니다. 예를 들어:  
...
하지만, 이것은 단지 "가이드라인" 일 뿐입니다. 전부 소문자나 대문자를 사용하는 것도 "자유" 입니다. 단, JavaScript는 "대소문자를 구분한다" 는 점을 항상 기억하시기 바랍니다. `NAME` 이란 변수를 선언했다면, `name` 으로 참조할 수 없습니다.  

## 상수 (Constants)  
지금까지 변수는 언제든지 원하는 값으로 할당하거나 변경할 수 있다는 것을 배웠습니다. 하지만, 때로는 변수를 선언한 즉시 값을 할당하고, 그 값을 "절대 변경할 수 없도록 처리" 할 때가 있습니다. 이를 위한 "상수" (constant)가 있습니다. 상수는 변수와 비슷하지만, `var` 나 `let` 대신 `const` 키워드를 사용합니다:  

```javascript
const answerToEverything = 42;
```  

"선언 즉시 값을 상수에 할당했다" 는 것에 주목하시기 바랍니다. 변수와 달리 값을 할당하지 않고, 상수를 선언할 수 없습니다. 그리고, 상수이므로 값을 변경할 수 없습니다. 만약, 상수의 값을 변경하면, 인터프리터는 즉시 "오류를 발생" 시킵니다:  
...
"값을 변경할 수 없는 변수가 있다" 는 것은 유용할 수 있습니다. 왜냐하면, 어떤 것은 단순히 변경하면 안되기 때문입니다. 이런 이유로 JavaScript도 상수를 많이 포함합니다. (예: `Math.PI`)  
...
## 요약  
변수를 "라벨이 붙은 잘 정리된 저장 상자" 라고 생각하시기 바랍니다. 변수는 어떤 정보든 저장할 수 있고, 이름을 붙이고 나중에 그 내용에 접근하거나 수정할 수 있습니다. 변수는 프로그래밍에서 매우 중요한 "핵심 부분" 이며, 이 튜토리얼의 많은 예제에서 사용하는 것을 자주 볼 수 있습니다. 그리고, 실제로 코드를 작성하면, 더욱 더 자주 사용할 것입니다.  
