...
*Org 모드* 는 메모, 기사, LaTeX 문서, 책, 웹 페이지, Beamer 슬라이드 데크 등을 작성하기 위한, */일반 텍스트/* 서식 구문입니다!

이 문서는 *Org 모드 9* 용 치트 시트입니다(Org 모드 7 이후 마크업 구문이 일부 변경), [[https://github.com/fniessen/org-html-themes][ReadTheOrg CSS]]를 기반으로 한 *Org 모드 8* 용 요약 자료입니다.

공식 매뉴얼([[http://orgmode.org/org.pdf][문서)]] 전체를 꼼꼼히 읽어볼 것을 강력히 권장하지만, 인내심 없는 분을 위해 간단한 단계를 아래에 정리했습니다.
...
#+begin_note
~#+ALT_TITLE~ 속성을 사용하면 목차 및 기타 페이지 머리글에 표시되는 특정 제목에 대체 제목(예: 더 짧은 제목)을 설정할 수 있습니다.
#+end_note

목차(TOC)를 임의의 위치에 삽입하려면 ~#+TOC: headlines [n]~ 을 사용합니다. 예를 들어:
...
~#+TOC: figures~ 는 HTML 백엔드에 아직 구현되지 않았습니다.

** List of tables

~#+TOC: tables~ 는 HTML 백엔드에 구현되어 있습니다.
...
#+begin_src org
연속된 두 개의 백슬래시(\\)를 입력하면 새로운 단락을 시작하지 않고도
강제로 줄 바꿈을 할 수 있습니다.
#+end_src
...
#+begin_src org
수평선을 표시하려면 대시 문자를 5개 이상 입력합니다. 다음은
수평선 위
-----
와 그 아래에 있는 텍스트입니다.
#+end_src
...
마크업은 여러 줄에 걸쳐 작성할 수 있으나 기본적으로 *2줄을 초과할 수 없습니다!*:
...
Org 모드는 영숫자에 둘러싸인 마커를 강조 마커로 해석하지 않습니다!! 따라서 단어의 일부만 (쉽게) 강조할 수 없습니다!!:
...
XXX 옵션이 지정되면 Org 모드는 타이포그래피적으로 올바른 출력 결과를 생성하며 직선 따옴표를 곡선 따옴표로 ~---~ 를 엠대시로, ~--~ 를 엔대시로, ~...~ 를 생략 부호로 변환합니다.


* Lists

Org 마크업을 사용하면 *글머리 기호* 또는 *번호* 가 붙은 목록을 만들 수 있습니다. 또한 이 두 가지 목록 유형을 자유롭게 조합할 수 있습니다.

** Unordered lists

항목 목록은 글머리 기호로 구분합니다. 마이너스 기호나 플러스 기호로 목록을 작성합니다.

목록은 데이터를 체계적으로 정리하는 데 유용할 뿐만 아니라, 문서를 보기 좋게 만들고 읽기 쉽게 만듭니다.

#+begin_src org
- 여러 줄에 걸쳐 줄바꿈이 포함된 긴 텍스트가 포함된 항목. 줄바꿈 효과를 보여주기 위해
  단어 몇 개를 추가합니다.
- 글머리 기호.
  + 글머리 기호.
    * 글머리 기호.
...
번호 목록은 숫자나 문자로 구분합니다:

#+begin_src org
  1. 아라비아(십진법) 번호 목록 항목. 줄 바꿈을 보여주기 위해
  몇 단어를 추가합니다.
...
라벨이 붙은 여러 줄로 된 정의 목록.

#+begin_src org
- 정의할 첫 번째 항 ::
     첫 번째 항목 정의. 줄 바꿈이 어떻게 되는지 보여주기 위해 몇 단어를 추가하여,
     긴 줄이 있을 때 어떤 현상이 발생하는지 확인합니다.

- 두 번째 항 ::
     *인라인 마크업* 을 사용해서 두 번째 항목을 설명합니다.
...
인접한 목록은 때때로 하나로 합쳐집니다. 새로운 목록으로 시작하려면, *빈 줄과 주석* 을 삽입해서 두 목록 사이에 간격을 둡니다:
...
표는 Org 모드에서 가장 정교한 부분 중 하나입니다. 표는 만들기도 읽기도 아주 쉽습니다.
...
Org 테이블에서 셀은 길이가 한 줄을 넘지 않습니다!! Org는 여러 줄로 이루어진 테이블 셀이란 개념이 없습니다.
...
자동 정렬 기능을 무시하려면 ~<r>~, ~<c>~ 또는 ~<l>~ 을 사용합니다.
...
표의 맨 위에 머리글 행을 추가할 수 있습니다(표의 나머지 부분과 구분하기 위해 가로로 대시 줄을 사용하는 방식).
...
~noindent~ 는 단락 첫 줄로, 표의 들여쓰기를 제거합니다. ~hfill~ 은 표 뒤에 무한 여백을 추가해서 표를 왼쪽으로 밀어냅니다.
...
여기서 ~hfill~ 은 표 앞에 무한 여백을 추가하므로 표를 오른쪽으로 밀어냅니다.
...
R 명령어를 사용해서 CSV 파일의 데이터를 테이블에 채울 수 있습니다.
...
인라인 앵커는 하이퍼텍스트 링크의 대상 위치를 지정하는 데 사용됩니다.
...
그림에 링크를 추가하려면(예: “그림 1 참조”), 다음과 같이 처리합니다:
...
또한 현재 문서의 앵커 또는 다른 문서의 앵커로 연결되는 하이퍼텍스트 링크를 만들 수 있습니다.
...
문서에 다양한 *형식* 으로 *이미지* 를 삽입할 수 있습니다:
...
책 같은 문서는 이미지를 텍스트 내용 오른쪽이나 왼쪽에 정렬하거나 배치합니다.
...
*책 삽화* (그림)로 간주되어 자동으로 라벨이 지정되고 번호가 매겨질 이미지를 정의하려면 XXX를 사용합니다.

* Videos

동영상은 직접 추가할 수 없습니다!!

하지만 다음과 같이 동영상 링크가 포함된 이미지를 추가할 수 있습니다:
...
인라인 작업(“TODO” 주석을 생성하기 위한) 대신, 메모가 게시된 버전에 표시되지 않게 하려면 드로어를 사용합니다. 예를 들어:
...
*줄 바꿈을 유지할 필요없는* 콘텐츠는 ~quote~ 블록을 사용합니다.
...
~verse~ 는 *각 줄 끝에 암시적인 줄바꿈이 적용되며, 들여쓰기* 도 유지됩니다:
...
일반적으로 이메일 메시지의 일부를 인용할 때 사용합니다:
...
비분리 공백을 추가하려면 유니코드 문자 ~00A0~ 을 삽입합니다.
...
#+begin_note
Org는 주석이 단락을 끝내므로 *단락 내 주석* 을 지원하지 않습니다.
하지만 내보내기 스니펫(예: ~@@comment:...@@~ )을 사용하면 인라인 주석을 모방할 수 있습니다.
#+end_note

#+begin_tip
(예를 들어) 내보내기 과정에서 무시하고 싶은 테이블이 있다면, 한 가지 방법은 *주석 블록* 이나 ~:noexport:~ 하위 트리를 사용합니다.
또 다른 방법은 내보내지 않는 *드로어* 를 *사용* 합니다(~#+OPTIONS: d:~ 참조).
#+end_tip

내보내기 시에만 해당 부분을 무시하되, 버퍼 내에서는 활성화 상태로 유지하려면, 적절한 ~org-export-with-drawers~ 값을 설정한 드로어를 사용할 것을 권장합니다. 예를 들어,
...
[[https://github.com/fniessen/org-macros][GitHub]]에서 좀 더 많은 매크로를 찾아볼 수 있습니다.

** Special characters

널리 사용하는 유니코드 문자를 포함하기 위해 대체 기호를 사용합니다(예: ©, 텍스트 문자를 타이포그래피적으로 올바른 엔티티로 변환한 것).
...
#+begin_note
~C-x 8 RET zero width space RET~ 또는 ~C-x 8 RET 200b RET~ 를 입력해서 “폭이 0인 공백” 유니코드 문자를 삽입할 수 있습니다.
#+end_note
...
다음과 같이 코드를 인라인으로 평가할 수 있습니다. 1 + 1 은 src_R{1 + 1} 입니다.
...
~example~ 및 ~src~ 스니펫 모두 ~begin~ 줄 끝에 ~-n~ 스위치를 추가하면 줄 번호를 표시합니다:
...
~+n~ 스위치를 사용하면, 번호가 매겨진 이전 스니펫 번호에서 현재 스니펫으로 이어집니다:
...
문자 그대로 예에서 Org는 ~(ref:name)~ 과 같은 문자열을 레이블로 해석하고, 이를 ~[[(name)]]~ 과 같은 특수 하이퍼링크의 대상(즉, 괄호 하나로 묶인 참조 이름)으로 사용합니다. HTML에서 이런 링크 위에 마우스를 위치하면 해당 코드 줄이 원격으로 강조되는데 이는 꽤 멋진 기능입니다.

또한 ~-r~ 스위치를 추가해서 소스 코드에서 레이블을 제거할 수 있습니다. ~-n~ 스위치를 사용하면 이런 참조에 대한 링크에 코드의 줄 번호가 레이블로 표시되며, 그렇지 않은 경우 링크는 괄호없이 레이블로 사용합니다. 다음은 예제입니다:
...
#+begin_warning
(Org와 MathJax 모두) ~$...$~ 구문을 사용하는 것은 권장되지 않습니다!!

~$~ 는 유효한 통화 기호이기도 하다는 점을 항상 잊지 마시기 바랍니다!
#+end_warning

** Math expressions in display mode

*눈에 띄고 별도의 줄에 중앙 정렬해서* 표시하려는 수학 식의 경우, ~\[...\]~ 를 사용합니다:
...
이 방정식이 틀렸을 수도 있지만, 멋진 방정식이네요!

** Equation numbers

~$...$~ 및 ~\(...\)~ 과 달리, 방정식은 *번호가 매겨진* 방정식으로 생성하며, 여기에 레이블을 추가하고 본문 다른 부분에서 (레이블) 이름으로 해당 방정식을 참조할 수 있습니다. 번호가 매겨지지 않은 수학식 (~$$~, ...) 은 이 기능이 불가능합니다.
...
Org 문서에 원시 HTML을 포함시킬 수 있으며, 내보낼 때 HTML은 그대로 유지됩니다.
...
특히 이미지나 표 같이 Org 모드가 제공하는 것보다 HTML 옵션을 좀 더 세밀하게 제어해야 하는 고급 기능에 유용합니다.

마찬가지로, 자바스크립트(JS)를 삽입하거나 웹 페이지에서 처리할 수 있는 다른 작업들(예: CSS 파일 불러오기 등)도 수행할 수 있습니다.

*** Native DIV blocks

다음과 같이 이름이 지정된 클래스를 생성할 수 있습니다(CSS로 스타일 제어하기 위해):
...
또한 HTML에 대화형 R 플롯 같은 대화형 요소를 추가할 수 있습니다.

마지막으로, 다음 코드로 (내보내기 시) HTML 파일을 포함시킬 수 있습니다:

#+begin_src org
,#+INCLUDE: "file.html" export html
#+end_src

절대 내보낸 HTML 파일을 수정하지 마시기 바랍니다!

** Raw LaTeX

일반 LaTeX을 사용할 수 있습니다. XXX
...
프로그램, 스크립트 또는 명령어의 *실행* 결과물을 문서에 삽입할 수 있으므로, ‘재현 가능한 연구’ 라는 관점에서 작업할 수 있습니다.
...
#+index: Org-mode
  ...
  다중 항목 용어는 별도의 색인 항목을 생성한다는 점에 유의합니다.
