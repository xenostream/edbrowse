...
Emacs가 Org 파일을 처음 열 때의 전역 상태는 showeverything 으로 설정되어 즉, “파일의 모든 내용을 표시” 합니다. 이는 org-startup-folded 변수로 설정할 수 있으며 다음과 같이 ‘overview’, ‘content’, ‘showall’, ‘showeverything’ 또는 ‘show<n>levels’(n = 2..5)에 ‘STARTUP’ 키워드를 추가해서 파일별로 설정할 수 있습니다:
