...
<br><br>
# 1 Introduction
“Org” 는 빠르고 효율적인 “순수 텍스트 시스템” 으로 메모를 작성하거나 할 일 목록을 관리하고 프로젝트 계획을 세울 수 있는 모드(mode)입니다. 또한, 문서 작성 및 출판 시스템이며 리터러시 프로그래밍과 재현 가능한 연구를 위한 소스 코드 작업도 지원합니다.
...
## 1.1 Installation
**중요**: Emacs 배포판에 포함된 Org를 사용할 경우 이번 섹션은 건너뛰고 ‘활성화’ 섹션으로 이동합니다.
...
Git이나 tar 아카이브로 Org를 설치한 경우 로드 정보를 생성하려면 다음과 같은 명령어를 실행합니다.
...
`Org` 는 “아웃라이너” (outliner)입니다. 아웃라인을 사용하면 “문서를 계층 구조로 정리” 할 수 있는데 제게는 이것이 “메모와 생각을 표현하는 가장 좋은 방법” 입니다. 
...
## 2.1 Headlines
“제목(headline)은 개요 트리(outline tree)의 구조를 정의” 합니다. Org에서 제목은 줄 왼쪽에 여백[^1]없이 하나 이상의 별표(제목 수준)와 공백을 붙여서 시작합니다.

[^1]: 헤드라인에 C-a 및 C-e 동작을 설정하려면 `org-special-ctrl-a/e` 변수를 참조합니다.
...
`org-footnote-section` 에서 이름을 따온 제목(기본값은 ‘Footnotes’)은 특수한 것으로 간주한다는 점에 유의합니다. 이런 제목을 가진 하위 트리는 내보내기(export) 함수에 의해 아무런 경고없이 무시됩니다.

일부 사용자들은 “별표가 많아 산만하다!” 고 느낄 수 있으며 제목의 공백 뒤에 별표 하나만 사용하는 개요(clear view)를 선호합니다. 이를 구현하는 설정 방법은 ‘기타’ 장을 참조합니다.


## 2.2 Visibility Cycling
이처럼 아웃라인을 사용하면 버퍼의 텍스트를 숨길 수 있습니다. Org는 `TAB` 및 `S-TAB` (`org-cycle` 및 `org-shifttab`)에 할당된 두 가지 명령어로 버퍼의 텍스트 표시 여부를 변경합니다.
...
접두사 인자(`C-u TAB`)나 `Shift` 키를 누른 상태로 호출하면 전역 순환 기능이 실행됩니다.
...
Emacs가 Org 파일을 처음 열 때의 전역 상태는 `showeverything` 으로 설정되어 즉, “파일의 모든 내용을 표시” 합니다. 이는 `org-startup-folded` 변수로 설정할 수 있으며 다음과 같이 `overview, content, showall, showeverything` 또는 `show<n>levels` (n = 2..5)를 `STARTUP` 키워드에 설정해서 파일별로 설정할 수 있습니다:
...
## 2.3 Motion
다음 명령어나 단축키는 버퍼의 “다른 제목으로 이동” 합니다.
...
현재 제목과 같은 수준의 새로운 제목을 삽입합니다. 만약 커서가 일반 항목에 있을 경우 새로운 항목을 생성합니다(일반 목록 참조). 이 명령을 줄 중간에서 사용하면 줄은 분할되고 나머지 부분은 새로운 heading [^2]이 됩니다.

[^2]: 이처럼 줄을 나누는 것을 원치않으면 `org-M-RET-may-split-line` 변수를 사용자 정의합니다.
