...
서문에 언급했듯이, 이 책에 제시한 예제는 Chrome/Chromium 콘솔에서 테스트되었습니다. Chromium 기반의 다른 브라우저에도 동작할 수 있습니다. 새 탭에서 **<kbd>Ctrl+Shift+J</kbd>** 단축키를 사용해서 콘솔을 엽니다. 일부 변수 이름은 여러 장에 걸쳐 재사용하므로, 오류를 피하기 위해 그런 경우는 다른 탭을 여시기 바랍니다.  
...
## `test()` method
먼저, 문자열이 다른 문자열의 일부인지 여부를 테스트하는 간단한 예제입니다. 보통은 `includes()` 메서드를 사용하고, 문자열을 인자로 전달합니다. 정규 표현식의 경우, 검색 문자열을 `/.../` 구분자 안에 넣어 정의한 RegExp 객체에서 `test()` 메서드를 사용합니다.  

```js
let sentence = 'This is a sample string'

// 'sentence' 가 주어진 문자열 인자를 포함하는지 확인
sentence.includes('is')
// true
sentence.includes('z')
// false

// 'sentence' 가 RegExp 객체로 설명된 패턴과 일치하는지 확인
/is/.test(sentence)
// true
/z/.test(sentence)
// false
...
다음은 조건식에서 `test()` 메서드를 사용하는 몇 가지 예제입니다.  
...
## Flags
정규 표현식의 일부 기능은 소문자로 표시되는 플래그를 전달함으로 활성화됩니다. 플래그는 명령줄 옵션과 유사합니다. 예를 들어 `grep -i` 는 대소문자를 구분하지 않는 매칭 작업을 수행합니다.  
...
아래는 **i 플래그** 예제입니다. **g 플래그** 는 이 장의 `replace()` 메서드 부분에서 설명합니다.  
...
RegExp 객체는 변수에 저장할 수 있습니다. 이것은 코드 명확성을 높이고, 재사용을 가능하게 만드는 장점이 있습니다.  
...
RegExp 객체는 `RegExp()` 생성자를 사용해서 만들 수도 있습니다. 첫 번째 인자는 문자열 또는 RegExp 객체이고, 두 번째 인자는 하나 이상의 플래그를 지정하는 데 사용됩니다.  
...
## `replace()` method
`replace()` 문자열 메서드는 검색 및 치환 작업에 사용됩니다.  
...
문자열은 **불변(immutable)** 이라는 점을 잊는 것이 가장 흔한 실수입니다. 동일한 변수에 변경 사항을 저장하려면, 반드시 결과를 그 변수에 명시적으로 다시 할당해야만 합니다.  
...
// 'word'를 변경하려면 결과를 명시적으로 다시 할당해야 함
...
> [!WARNING]
> ⚠️ **경고**: `test()` 메서드에서 `g` 플래그를 사용하면 추가 기능이 가능해집니다. 예제는 [MDN: test](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/RegExp/test#Examples) 문서를 참고하시기 바랍니다. 그러나, 제 생각에는 `test()` 메소드와 `g` 플래그를 사용하는 습관에 빠지기 쉽고, 원치 않는 결과를 얻을 수도 있습니다. 대신 `match()` 메서드를 사용하고 필요한 로직을 명시적으로 작성하는 것을 권장합니다.
...
| /pat/                         | RegExp 객체                                                                 |
| const p1 = /pat/              | 정규식을 변수에 저장해서 재사용, 명확성 등을 위해 사용                      |
...
이 장에는 RegExp 객체를 정의하고, `test()` 및 `replace()` 메서드와 함께 사용하는 방법을 소개했습니다. 또한, 정규 표현식의 기본 동작을 변경하기 위한 플래그를 사용하는 방법도 배웠습니다. 제시한 예제는 텍스트 처리 개념을 소개하는 데 초점을 맞추었습니다. 다음 장부터 정규 표현식의 문법과 기능을 배우게 될 것입니다.


## Exercises

> [!NOTE]
> 각 장의 연습문제는 해당 장에서 다룬 기능만을 사용해서 풀어 보시기 바랍니다. 일부 연습문제는 이후 장에 제시한 기법으로 좀 더 쉽게 풀 수 있지만, 이 연습문제의 목적은 지금까지 소개한 기능을 탐구하는 것입니다.  
...
1) 주어진 입력 문자열에 two 가 대소문자 구분 없이 포함되어 있는지 확인하세요.  
...
2) 주어진 배열에서 e 를 포함하지 않는 모든 요소를 걸러내세요.  
...
3) 주어진 문자열에서 5 의 첫 번째 발생만 five 로 바꾸세요.  
...
4) 주어진 문자열에서 5 의 모든 발생을 five 로 바꾸세요.  
...
5) 대소문자 구분 없이 note 의 모든 발생을 X 로 바꾸세요.  
...
6) 주어진 여러 줄 입력 문자열에서 문자열 2를 포함하지 않는 모든 줄을 걸러내세요.  
...
> [!NOTE]
> 점 메타문자와 수량자 장의 끝에서 이 문제를 `replace()` 메서드만으로도 풀 수 있습니다.  

7) 주어진 배열에서 a 또는 w 를 포함하는 모든 요소를 걸러내세요.  
...
8) 주어진 배열에서 e 와 n 을 모두 포함하는 모든 요소를 걸러내세요.  
...
9) 주어진 문자열에서 0xA0 을 0x7F 로, 0xC0 을 0x1F 로 바꾸세요.  
...
# Anchors
이 장에는 패턴을 한정하는 방법을 배우게 됩니다. 입력 문자열 어디서나 일치시키는 대신, 제약을 지정할 수 있습니다. 지금은 정규 표현식 기능에 이미 포함된 것을 보게 될 것입니다. 이후 장에는 사용자 지정 규칙을 정의하는 방법도 배우게 됩니다.

이런 제약은 특정 문자와 이스케이프 시퀀스에 특별한 의미를 부여함으로 가능해집니다. 특별한 의미를 가진 문자들은 정규식 용어로 **메타문자** 라고 합니다. 이런 문자들을 문자 그대로 매치할 경우는 `\` 문자로 이스케이프해야 합니다. (메타문자 이스케이프 장에서 설명)


## String anchors
제약은 입력 문자열의 시작이나 끝에서만 일치하도록 **정규식을 한정하는 것** 에 관한 것입니다. 이것은 문자열 메서드 `startsWith()` 와 `endsWith()` 와 유사한 기능을 제공합니다. 먼저, 문자열의 시작으로 매칭을 제한하는 `^` 메타문자입니다.
...
문자열의 시작과 끝 앵커를 모두 결합하면, 전체 문자열로 매칭을 제한할 수 있습니다. 그 효과는 `==` 연산자를 사용해서 문자열을 비교하는 것과 유사합니다.
...
## Line anchors
문자열 입력은 한 줄 또는 여러 줄을 포함할 수 있습니다. 문자 `\r, \n, \u2028`(줄 구분자) 및 `\u2029`(문단 구분자)는 줄 구분자로 간주합니다. `m` 플래그를 사용하면, `^` 및 `$` 앵커 문자는 각각 모든 줄의 시작과 끝에 매칭됩니다.
...
> [!WARNING]
> Windows OS 기반 텍스트 파일을 다룰 경우, `\r\n` 줄 바꿈 시퀀스를 먼저 `\n` 으로 변환해야 할 수 있습니다. 그렇지 않으면 `\r` 및 `\n` 문자 모두 줄 끝 매칭을 얻게 됩니다. 정규식에서 `\r` 을 수량자로 선택적 문자로 만들어 이 문제를 처리할 수도 있습니다. (예제는 그리디 수량자 섹션을 참조하시기 바랍니다.)


## Word anchors
단어 앵커는 세 번째 유형의 제약입니다. 알파벳(대소문자 구분 없음), 숫자 그리고 밑줄 문자(_)는 단어 문자로 간주합니다. 왜 숫자와 밑줄까지 포함하는지, 알파벳만으로는 안 되는지 궁금할 수 있습니다. 이것은 변수와 함수 이름 지정 관례에서 비롯됩니다 — 일반적으로 알파벳, 숫자, 밑줄이 허용됩니다. 그래서 이런 정의는 자연어보다 프로그래밍 언어에 더 초점이 맞춰져 있습니다.

이스케이프 시퀀스 `\b` 는 단어 경계를 나타냅니다. 이것은 단어 시작과 단어 끝 앵커링 모두에 동작합니다. 단어 시작은 단어 앞의 문자가 비단어 문자이거나, 문자가 없는 경우(문자열의 시작)를 의미합니다. 마찬가지로, 단어 끝은 단어 뒤의 문자가 비단어 문자이거나, 문자가 없는 경우(문자열의 끝)를 의미합니다. 이것은 단어 문자 없이 단어 경계 `\b` 를 가질 수 없음을 암시합니다.
...
// 프로그래밍 문장을 더 읽기 쉽게 만들기
// 설명을 위한 예시일 뿐이며, 모든 경우에 동작하지는 않음
> 'output=num1+35*42/num2'.replace(/\b/g, ' ')
< ' output = num1 + 35 * 42 / num2 '
// 문자열 시작/끝의 과도한 공백은 잘라낼 수 있음
// 나중에 trim이 필요 없도록 한정을 추가하는 방법을 배우게 됨
...
단어 경계에는 반대 앵커도 있습니다. `\B` 는 `\b` 가 매치되지 않는 곳이면 어디든 매치됩니다. 이런 이중성은 다른 이스케이프 시퀀스에도 보게 될 것입니다. 부정 논리는 많은 텍스트 처리 상황에 유용합니다. 하지만, 주의해서 사용하시기 바랍니다. 의도하지 않은 것까지 매치할 수도 있습니다.
...
| \B             | \b가 매치되지 않는 곳이라면 어디든 매치                       | 

이 장에는 정규 표현식의 구성 요소를 보기 시작했고, 그것들이 흥미로운 방식으로 사용될 수 있음을 보았습니다. 그러나, 동시에 정규 표현식은 텍스트 처리 영역에서 또 하나의 도구일 뿐입니다. 종종 정규 표현식을 다른 문자열 메서드 및 표현식과 결합함으로써 보다 단순한 해법을 얻을 수도 있습니다. 연습, 경험, 그리고 상상력이 창의적인 해법을 구성하는 데 도움을 줄 것입니다. 이어지는 장들에는 다른 정규식 기능들과 결합된 앵커의 예를 보게 될 것입니다.
...
# Alternation and Grouping
간혹, 입력 문자열이 여러 패턴과 일치하는지 확인할 때가 있습니다. 예를 들어, 어떤 제품의 색상이 green 인지, blue 인지, red 인지 확인하는 경우입니다. 이 장에는 이러한 경우에 **교대(Alternation)** 를 사용하는 방법을 보여줄 것입니다.  

이러한 패턴들은 서로 공통 요소를 가질 수도 있는데, 이럴 때는 **그룹화(Grouping)** 를 사용해서 좀 더 간결한 정규 표현식을 만들 수도 있습니다. 또한, 이 장에는 어떤 교대가 우선권을 갖는지를 결정하는 **우선순위 규칙(Precedence rules)** 에 대해서도 다룰 것입니다.  


## Alternation
조건식이 논리 OR과 결합되면, 조건들 중 하나라도 만족하면 true 로 평가됩니다. 마찬가지로, 정규식에는 `|` 메타문자를 사용해서 여러 패턴을 결합해서 논리 OR을 나타낼 수 있습니다. 입력 문자열에서 대안 패턴들 중 하나라도 발견되면 매칭은 성공합니다. 이러한 대안들은 정규 표현식의 모든 기능을 그대로 사용할 수 있으며 예를 들어, 각자 독립적인 앵커를 가질 수도 있습니다. 몇 가지 예는 다음과 같습니다.
...
> [!NOTE]
> 위의 예제로부터, 많은 교대(alternation)가 필요한 상황이 있을 수 있음을 추론할 수 있습니다. 예제와 세부 사항은 **Dynamically building alternation** 절을 참조하시기 바랍니다.


## Grouping
정규 표현식의 여러 대안들 사이에는 종종 공통된 부분이 있습니다. 그것은 공통된 문자일 수도 있고, 앵커 같은 한정자일 수도 있습니다. 이런 경우는 괄호 메타문자 쌍을 사용해서 그룹화할 수 있습니다. 수학에서 `a(b+c)d = abd+acd` 가 되는 것처럼, 정규 표현식에서는 `a(b|c)d = abd|acd` 가 됩니다.
...
// 공통 문자까지 뽑아낸 경우
// 나중에 빈 교대를 사용하는 대신 더 나은 기법을 배우게 될 것임
> 'par spare part party'.replace(/\bpar(|t)\b/g, 'X')
< 'X spare X party'
```

> [!NOTE]
> 그룹화는 단순히 더 간결한 정규식을 만드는 것 이상의 많은 용도가 있습니다. 이들은 이후 장에서 필요할 때마다 다루어질 것입니다.


## Precedence rules
교대를 사용할 때, 까다로운 상황이 있습니다. 문자열 입력에 매치를 테스트해서 불리언 결과를 얻는 경우는 모호함이 없습니다. 그러나, 문자열 치환 같은 경우는 몇 가지 요인에 따라 달라집니다. 예를 들어, are 또는 spared 를 치환한다고 가정합니다. — 어느 쪽이 우선권을 가져야 할까요? 좀 더 긴 단어 spared 일까요, 아니면 그 안에 포함된 부분 문자열 are 일까요, 아니면 또 다른 기준일까요?

정규식 교대에서 **입력 문자열에서 좀 더 먼저 매치되는 대안이 우선권을 가집니다.**

```js
> let words = 'lion elephant are rope not'

// 주어진 문자열 입력에서 'on' 의 시작 인덱스 < 'ant' 의 인덱스
...
이 경우는 **선언한 순서대로, 즉 왼쪽에서 오른쪽으로** 우선권이 결정됩니다.
...
이번 장은 `|` 메타문자를 사용해서 하나 이상의 대체 매치를 동일한 정규 표현식 안에서 지정하는 것에 관한 것이었습니다. 교대(alternation)들 사이에 공통 부분이 있다면 `(...)` 그룹화를 사용해서 보다 단순화할 수 있습니다. 교대들 중에서 가장 먼저 매치되는 패턴이 우선권을 가지며, 여러 교대가 동일한 시작 위치를 가질 경우, 왼쪽에서 오른쪽 순서가 우선권을 결정합니다. 다음 장에는 우선순위 규칙을 고려해서 문자열 배열로부터 교대 패턴을 구성하는 방법을 배우게 됩니다. 그룹화는 또 다른 다양한 용도를 가지며, 이것은 이후 장에서 다시 다루어질 것입니다.
...
# Escaping metacharacters
정규 표현식 리터럴을 구성하는 데 도움되는 몇 가지 메타문자와 이스케이프 시퀀스를 이미 봤습니다. 정규 표현식 객체에서 구분자로 사용되는 `/` 문자도 있습니다. 이 장에는 그런 문자들의 특별한 의미를 제거하는 방법을 보여줍니다. 또한, 일반 문자열로부터 정규 표현식 리터럴을 만들 때, 이러한 특수 문자를 어떻게 처리해야 하는지도 배우게 될 것입니다.


## Escaping with backslash
메타문자를 문자 그대로 매치하려면 즉, 특별한 의미를 제거하려면, 그 문자 앞에 `\` (백슬래시) 문자를 접두사로 붙입니다. 문자 그대로의 `\` 문자를 나타내려면 `\\` 를 사용합니다.
