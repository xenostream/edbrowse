...
이 플래그는 substitute 명령어에만 적용되며 / 또는 ? 검색에는 적용되지 않습니다. 플래그는 결합할 수도 있습니다. 예시:
...
> [!NOTE]
> 위에서 보듯이 줄 끝 문자를 일치시키려면 특별한 주의가 필요합니다. 따라서 본 장의 예시와 설명은 별도로 언급되지 않는 한 줄 단위로 작업한다고 가정합니다. 나중에 \_가 일치 대상에 줄 끝을 포함시키기 위해 더 많은 곳에서 사용되는 방식을 보게 될 것입니다.


## Greedy Quantifiers
정량자는 리터럴 문자, 점 메타문자, 그룹, 역참조 및 문자 클래스에 적용될 수 있습니다. 기본 예시는 아래에 제시되며, 후속 섹션에서 더 자세히 논의될 것입니다.
...
교대는 여러 용어를 일치시키는 데 도움이 되며, 각 용어는 자체 앵커를 가질 수도 있습니다(각 대안이 정규 표현식 패턴이기 때문입니다). 정규 표현식 대안들 사이에는 종종 공통점이 있습니다. 이러한 경우 괄호 메타문자 쌍을 사용하여 그룹화할 수 있습니다. 수학에서 a(b+c)d = abd+acd와 유사하게, 정규 표현식에서는 a(b|c)d = abd|acd를 얻습니다.

- \| 지정된 패턴 중 하나를 일치시킵니다
    - min\|max min 또는 max를 일치시킵니다
    - one\|two\|three one, two 또는 three를 일치시킵니다
    - \<par\>\|er$ 단어 par 전체 또는 er로 끝나는 줄을 일치시킵니다
- \(패턴\) 패턴을 그룹화하여 양정자를 적용하거나 공통 요소를 추출해 간결한 정규식을 생성하는 등 활용합니다
...
## Backreference
이전 섹션에서 살펴본 그룹화는 캡처 그룹으로도 알려져 있습니다. 이 그룹들에 의해 캡처된 문자열은 나중에 백레퍼런스 \N을 사용하여 참조할 수 있으며, 여기서 N은 원하는 캡처 그룹 번호입니다. 백레퍼런스는 검색 및 대체 섹션 모두에서 사용할 수 있습니다.
...
- s/\(\d\+\)\%(abc\)\+\(\d\+\)/\2:\1/ 12abcabcabc24를 24:12로 대체 (하나 이상의 abc 시퀀스로 구분된 숫자 일치, :로 구분하여 숫자 교환)
...
- \@! 부정 선행 확인
    ...
    - at\(\(go\)\@!.\)* par는 cat,dog,parrot에는 일치하지만 cat,god,parrot에는 일치하지 않음 (즉, go가 사이에 존재하지 않는 한 at 다음에 par가 오는 경우 일치함. 이는 그룹화를 부정하는 예시임)
- \@<! 부정 후방 확인
...
> [!NOTE]
> 후방 패턴 검색 시 바이트 수를 지정할 수도 있습니다. 이는 일치 속도를 크게 향상시킵니다. @와 < 문자 사이에 숫자를 지정해야 합니다. 예를 들어, _\@1<=ice는 ice 바로 앞 1바이트만 역방향 검색합니다. \(cat.*\)\@10<!dog는 dog 바로 앞 10바이트만 역방향 검색하여 주어진 어설션을 확인합니다.


## Automic Grouping
앞서 논의한 바와 같이, 탐욕적 및 비탐욕적 양화자는 양화자가 일치시키는 문자 수를 변경하여 전체 패턴을 충족시키려 시도합니다. 원자 그룹화를 사용하면 패턴이 더 이상 백트래킹되지 않도록 보호할 수 있습니다. 룩어라운드와 유사하게, 접미사로 \@>를 사용해야 합니다. 예를 들어 \(pattern\)\@>와 같이 사용합니다.
...
### Very magic
Vim 정규 표현식의 기본 구문은 ., *, ^, $와 같은 몇 가지 메타문자만 지원합니다. Perl, Python, JavaScript와 같은 프로그래밍 언어에서 정규 표현식 사용법에 익숙하다면, \v를 사용하여 Vim에서도 유사한 구문을 구현할 수 있습니다. 이를 통해 () {}, +, ? 등과 같은 더 많은 메타문자를 \ 메타문자를 접두사로 붙이지 않고도 사용할 수 있습니다. :h magic 문서에서 발췌:
...
## Escape sequences
탭, 캐리지 리턴, 줄바꿈 등과 같은 특정 문자는 이를 표현하기 위한 이스케이프 시퀀스를 가집니다. 또한 모든 문자는 10진수, 8진수, 16진수 형식의 코드 포인트 값을 사용하여 표현할 수 있습니다. \w와 같은 문자 집합 이스케이프 시퀀스와 달리, 이러한 값은 문자 클래스 내부에서도 사용할 수 있습니다. 이스케이프 시퀀스가 검색 패턴과 대체 문자열 부분에서 다르게 동작하는 경우, 아래 설명에서 강조 표시됩니다.
...
> [!NOTE]
> info %\ 시퀀스를 사용하여 대체 문자열에 문자를 삽입하는 기능은 아직 구현되지 않았습니다. 해결 방법으로는 vi.stackexchange: 16진수 문자로 대체하기를 참조하세요.
...
메타문자를 문자 그대로 일치시키려면(문자 클래스 메타문자(예: -) 포함), 즉 특수한 의미를 제거하려면 해당 문자 앞에 \ (백슬래시) 문자를 붙입니다. 문자 그대로의 \ 문자를 표시하려면 \\를 사용합니다. 패턴에 따라 다른 매직 수정자를 사용하여 이스케이프 필요성을 줄일 수도 있습니다. 별도로 명시되지 않는 한 아래 예시에서는 기본 매직성을 가정합니다.
...
## Macro usage steps
다음은 q 명령어 사용법에 대한 개략적인 설명입니다. 실제 예시는 후속 섹션에서 다룰 예정입니다.
...
## Modifying a macro
앞서 언급했듯이, 레지스터는 기록, 삭제 및 복사 명령 간에 공유됩니다. @를 사용하여 매크로를 호출할 때 레지스터 내용은 실행될 명령어 시퀀스로 처리됩니다. 따라서 레지스터 내용을 편집하면 매크로의 동작도 자동으로 업데이트됩니다. 매크로를 수정할 수 있다는 점을 알면 실수를 했을 때도 도움이 됩니다 — 기록을 다시 시작하지 않고 기록을 완료한 후 나중에 업데이트할 수 있습니다.
...
> [!NOTE]
> q 명령어 대신 필요한 텍스트를 직접 입력하여 새 매크로를 생성하려면 Ctrl+v (또는 Ctrl+q 별칭)를 사용하면 Esc나 Enter 같은 키를 삽입하는 데 유용합니다. Ctrl+v를 누른 후 원하는 키를 입력하세요. Esc는 ^[, Enter는 ^M 등으로 변환됩니다.
...
## Example 3
F`r[f`s]()^["*P 매크로를 사용하여 `:h <topic>`을 이 전자책의 해당 온라인 도움말 페이지로 연결되는 하이퍼링크로 대체했습니다. 커서가 백틱(인라인 코드용 마크다운 서식)으로 둘러싸인 :h <topic> 텍스트 부분 내 어딘가에 있다고 가정합니다. 이를 [:h <topic>](링크)로 변경해야 합니다(마크다운 하이퍼링크 서식).
...
## Motion and Filter
서로 인접한 텍스트 부분에 동일한 매크로를 적용해야 하는 경우, 매크로 끝에 다음 텍스트 부분으로 이동하는 동작 명령어를 추가할 수 있습니다. 동작 명령어는 화살표 이동, /를 이용한 검색 등이 될 수 있습니다. 이렇게 하면 카운트 접두사를 사용하여 한 번에 모든 텍스트 부분에 매크로를 적용할 수 있습니다. 이는 텍스트 부분의 수를 쉽게 셀 수 있다는 가정 하에 가능합니다. 예를 들어, 단일 행 정의들을 다중 행으로 변경하려는 이 Python 스니펫을 고려해 보세요:
...
평소처럼 매크로를 녹화하고, 이 줄들을 시각적으로 선택한 후(또는 범위 선택을 사용) 명령줄 모드에서 일반적인 @d 명령으로 매크로를 적용할 수 있습니다. 또는 아래 설명된 대로 카운트 접두사를 사용하여 자동으로 다음 줄로 이동하는 동작을 추가할 수도 있습니다.
...
위에서 논의한 Python 함수 정의들이 서로 인접해 있지 않고 Python 스크립트 파일 어디에서나 발견될 수 있다고 가정해 보겠습니다. 이러한 경우 정규식 필터를 사용하여 해당 줄을 안정적으로 식별할 수 있다면 :g 명령어를 사용할 수 있습니다.
...
- vi.stackexchange: [재귀 매크로를 줄 끝에서 멈추게 하려면?](https://vi.stackexchange.com/q/6426/1616) — 예시 중 하나는 Vimscript 통합 방법을 보여줍니다. 변수, if 제어 구조 등 완전한 프로그래밍 기능을 활용할 수 있습니다.
...
매크로(또는 선호한다면 substitute 명령어)를 사용하여 아래와 같이 수정된 텍스트를 얻으십시오:
