...
<^K>{arg}     이중 문자열 arg 읽기
<^C>
<ESC>         vi에서 삽입 모드 종료, ex에서 재설정
<^M>
<Enter>       vi에서 <줄바꿈> 삽입, ex에서 명령 제출
```
...
이 필수 도구는 vi의 핵심 역할을 수행하며, vi가 명령어와 매크로를 실행하고 필요 시 
순수 명령줄 인터페이스(CLI)로 전환할 수 있습니다.
...
[range] 내부의 공백은 축소됩니다. [cmd] 와 [args] 사이에는 패딩이 하나만 존재할 수 있습니다.
스크립트에서 모호성을 피하기 위해 [cmd] 와 [args] 사이에는 항상 패딩을 사용하는 것이 권장됩니다.

예제:

```
:evi.c     		“:e vi.c” 로 평가됨
:efbc      		“:ef bc” 로 평가됨 (“:e fbc” 아님)
...
# EX EXPANSION
 [args] 내의 `<%>` 는 현재 버퍼 경로명 또는 해당 버퍼 번호가 뒤따를 경우 어떤 버퍼 경로명으로 확장됩니다.
...
:.,>int>p	        			“int” 발견될 때까지 출력
:<int<,.p	        			역순으로 “int”가 발견될 때까지 출력
:>     	 				이전에 설정된 정규식 키워드로 검색
:'d,'ap	        			마크 <d>부터 마크 <a>까지의 줄 출력
:%p     					버퍼의 모든 줄 출력
:$p     					버퍼의 마지막 줄 출력
...
:;5;+10=	        			+10은 5가 아닌 초기 현재 오프셋 기준
:;5;#+10=	        			+10은 5를 기준으로 한 상대값
...
▶ wq / wq \!		파일 또는 파이프 쓰기 후 종료
[범위]wq[경로]
[범위]wq[\!{명령}]
        파일 또는 파이프 작성 후 종료


▶ wq! / wq! \!	파일 또는 파이프 강제 쓰기 후 종료
[범위]wq![경로]
[범위]wq![\!{명령}]
        파일 또는 파이프 강제 작성 후 강제 종료


▶ x				저장하지 않은 변경 저장 후 종료
[범위]x[경로]
        저장되지 않은 변경 사항 작성 후 종료


▶ x!			저장하지 않은 변경 강제 저장 후 종료
[범위]x![경로]
        저장되지 않은 변경 사항 강제 작성 후 강제 종료
...
# EX OPTIONS
Ex 옵션은 전역 변수를 변경하기 위해 설계된 ex 명령어입니다. 값은 부호 있는 정수를 
저장합니다. 값은 문자를 나타낼 수 있습니다.
...
ish	[val=0]
	...
	전제 조건 1:
	        .rc 파일 이름은 셸에 따라 다릅니다(예: Bash의 .bashrc, Zsh의 .zshrc).
	전제 조건 2:
	        환경 변수 $SHELL이 기본 셸을 결정합니다.
	        설정되지 않은 경우 기본값은 /bin/sh입니다.
	전제 조건 3:
	        <!> 명령어가 예상 결과를 반환하려면
...
grp	[val=0]
	...
	예시: “int” 접두사가 붙은 문자 위치
		:grp 2:%f+int(.):grp

hl	[val=1]
        conf.c에 정의된 규칙에 따라 텍스트 강조 표시

hlr	[val=0]
        텍스트를 역방향으로 강조 표시

hll	[val=0]
        파일 유형 hl에 따라 현재 줄 강조

hlp	[val=0]
        파일 유형 hl에 따라 “[]” “()” “{}” 쌍 강조

hlw	[val=0]
        파일 유형 hl에 따라 현재 단어 강조
...
vis	[val=0]
        ...
        예시: ex 모드에서 :e 메시지 비활성화
        :vis 6
        예시: vi 모드에서 :e 메시지 비활성화
        :vis 4
        예시: 원시 ex 모드 활성화
        :vis 3
        예시: 원시 ex 모드 비활성화
...
mpt	[val=0]
        ...
        vi에서 ex 명령을 호출한 후 0 또는 1로 설정하면
        “[계속하려면 아무 키나 누르세요]” 프롬프트를 비활성화합니다.
        mpt가 음수인 경우 프롬프트는 계속 비활성화된 상태로 유지됩니다.

        예시: 프롬프트와 마지막 출력 차단
        :p:p:p:mpt 0
        예시: 프롬프트 차단, 마지막 출력 유지
        :p:p:p:mpt 1

order	[val=1]
        conf.c에 정의된 규칙에 따라 문자 재정렬
...
td	[val=1]
        ...
        2       오직 좌에서 우 방향
        1       dctxs[](conf.c 내) 따름, 기본값은 좌에서 우
...
pr	[val=0]
        ...
        레지스터가 설정되면 ex_print에 전달된 모든 데이터가
        해당 레지스터에 저장됩니다.

        레지스터가 대문자인 경우, 출력된 내용과 정확히 일치하도록
        <줄바꿈> 문자가 추가됩니다.

             레지스터에 값이 할당된 상태에서는 R 또는 :reg 명령으로
	   레지스터 내용을 표시할 수 없습니다(자기 참조 문제).

             예시: :b 명령과 동일하게 현재 버퍼 목록 붙여넣기
             :pr A:ya! A: b:pu A
             예시: :p로 출력된 줄 저장
             :pr A:ya! A:p
...
seq	[val=1]
        ...
        seq가 0일 때, 여러 개의 서로 다른 작업 실행 취소/다시 실행을
        한 번에 처리합니다.

        중요한 작업 시 메모리 사용을 최적화하려면 seq를 음수 값으로 설정해서
        실행 취소/다시 실행 추적을 비활성화할 수 있습니다.

        예시: 단일 단계에서 실행 취소/다시 실행 작업 테스트
        :&ocontrol test:seq 0:&a step1:&a step2:&a step3:seq
        예시: 실행 취소/다시 실행 완전히 비활성화
        :seq -1

[hscroll]	left	[val=0]
        수평 스크롤 제어

        인수 없이 호출 시 현재 문자 오프셋을 사용해서 값 계산.
        hscroll은 다음 공식을 사용해 값을 계산합니다: xcols / 2 * hscroll
...
err	[val=1]
        ...
        예시: 오류 무시 및 침묵 처리
        :err 4
        예시: 첫 번째 오류 발생 시 보고 및 종료
        :err 3
        예시: 오류 상태 보고 시 마지막 명령어만 고려
        :err 1
```
...
./cbuild.sh      		  	한 번 빌드
./cbuild.sh build        	빌드
./cbuild.sh debug        	심볼 포함, 정적 분석, 동적 분석, 최적화 없음으로 
빌드
./cbuild.sh pgobuild       	PGO 빌드는 일부 애플리케이션 특정 작업에서 상당한 
성능 향상을 가져올 수 있음
./cbuild.sh install        vi를 $DESTDIR$PREFIX/bin에 설치
./cbuild.sh fetch        	업스트림 저장소의 커밋 병합
./cbuild.sh bench        	vi.c 파일에서 2000단어 삭제 성능 벤치마크 테스트
```

# PHILOSOPHY
대부분의 텍스트 편집기에서 유연성은 사소하거나 무관한 설계 목표입니다. 넥스트비(Nextvi)는 
편집기가 사용자의 요구에 맞춰 적응하는 유연성을 지향합니다. 이러한 유연성은 기본 
명령어를 강력하게 연결하고, 이를 통해 완전히 다른 기능을 가진 새로운 명령어를 생성할 
수 있도록 함으로써 달성됩니다. 명령어 재사용은 편집기를 간결하게 유지하면서도 코드를 
빠르게 파악할 수 있는 자유를 침해하지 않습니다. 어떤 것을 커스터마이징하고 싶다면, 
핵심 명령어만 사용하거나 더 어려운 작업의 경우 특정 C 코드와 혼합해서 수행할 수 있어야 
합니다. 단순하고 유연한 설계는 장기적으로 어떤 문제에도 직관적인 해결책을 허용하며 
불량하고 일관성 없는 아이디어를 걸러냅니다.
...
# SEE ALSO
새로운 기능은 패치 브랜치에서 제공하는 선택적 패치를 통해 얻을 수 있습니다. 의미 
있는 기여를 하셨고 공개되길 원하신다면, 이메일이나 GitHub 풀 리퀘스트를 통해 패치를 
