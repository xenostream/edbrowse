...
[#]+     
[#]<^M>     
[#]<Enter>    다음 라인으로 [#]라인 이동, 들여쓰기 감지 
[#]-          이전 라인으로 [#]라인 이동, 들여쓰기 감지 
[#]h          좌측 [#]컬럼 이동 
[#]l          우측 [#]컬럼 이동 
[#]f{arg}     현재 라인 내 {arg}문자 [#]번 전방 검색 
[#]F{arg}     현재 라인 내 {arg}문자 [#]번 후방 검색 
[#]t{arg}     현재 라인 내 {arg}문자 [#]번 전방 검색, {arg}문자 이전 
[#]T{arg}     현재 라인 내 {arg}문자 [#]번 후방 검색, {arg}문자 이후 
[#],          이전 <f F t T> 명령 [#]번 후방 검색 
[#];          이전 <f F t T> 명령 [#]번 전방 검색 
[#]E          [#]번 단어 끝 이동, 구두점 무시 
[#]e          [#]번 단어 끝 [#]번 이동 
[#]B          [#]번 이전 단어 시작 이동, 구두점 무시 
[#]b          [#]번 이전 단어 시작 이동 
[#]W          [#]번 다음 단어 시작 이동, 구두점 무시 
[#]w          [#]번 다음 단어 시작 이동 
vw            <E e B b W w> 명령 라인 전용 모드 토글 
[#](          [#]번 이전 문장 경계 이동 
[#])          [#]번 다음 문장 경계 이동 
[#]{          [#]번 이전 <{> 섹션 경계 이동 
[#]}          [#]번 다음 <}> 섹션 경계 이동 
...
$             라인 끝 이동 
[#]|          [#]번 컬럼 이동 
[#]<Space>    [#]번 문자 전방 이동, 멀티라인 감지 
[#]<^H> |    
[#]<Backspace>      [#]번 문자 후방 이동, 멀티라인 감지 
%             <] ) } [ ( {> 문자 쌍 이동
{#}%          {#} 퍼센트 라인 이동 
'{a-z ` ' [] *}     라인 마크 이동 
`{a-z ` ' [] *}     라인 마크 이동, 커서 위치 감지 
gg            버퍼 시작 라인 이동 
[#]G          버퍼 [#]번 끝 라인 이동 
H             화면 최상단 라인 이동 
L             화면 최하단 라인 이동 
...
z.            화면 중단 커서 위치 
z<^M>     
z<Enter>      화면 최상단 커서 위치 
z-            화면 최하단 커서 위치 
[#]<^E>       [#]번 또는 1라인 아래로 스크롤, 커서 위치 고정 
[#]<^Y>       [#]번 또는 1라인  위로 스크롤, 커서 위치 고정 
[#]<^D>       [#]번 또는 절반 화면 아래로 스크롤 
[#]<^U>       [#]번 또는 절반 화면 위로 스크롤 
<^B>          전체 화면 위로 스크롤 
<^F>          전체 화면 아래로 스크롤 
#             전역 및 상대 라인 번호 표시, 1회성 
2#            전역 라인 번호 상시 표시 토글 
4#            상대 라인 번호 상시 표시 토글, 들여쓰기 감지 
8#            상대 라인 번호 상시 표시 토글 
V             <Space Tab Neline> 숨김 문자 표시 토글 
<^C>          <l h e b E B w W> 라인 모션 표시 토글 
{1-5}<^C>     [#]번 라인 모션 표시 전환 
<^V>          라인 모션 표시 순환 
[#]<^R>       [#]번 재실행 
[#]u          [#]번 실행 취소 
<^I>    
<Tab>         커서에서 라인 끝 문자열로 파일 열기 
<^K>          현재 버퍼를 파일 저장, 2회 실행 시 강제 저장 
[#]<^W>{arg}  [#]번 영역 들여쓰기 해제 
[#]<{arg}     [#]번 {arg} 영역 좌측 들여쓰기 
[#]>{arg}     [#]번 {arg} 영역 우측 들여쓰기 
"{arg}{arg1}  {arg} 레지스터 {arg1} 모션 처리 
R             레지스터 및 내용 출력 
[#]&{arg}     [#]번 {arg} 레지스터 매크로 실행, 논블로킹 모드 
[#]@{arg}     [#]번 {arg} 레지스터 매크로 실행, 블로킹 모드 
[#]@@     
[#]&&         [#]번 마지막 실행 레지스터 매크로 실행 
[#].          [#]번 마지막 일반 명령 재실행 
[#]v.         [#]번 다음 라인 마지막 일반 명령 재실행 
[#]Q          [#]번 라인에서 ex 모드 진입, 현재 수평 스크롤 유지 
:             ex 프롬프트 진입 
[#]!{arg}     [#]번 또는 {arg} 영역 ex 프롬프트 파이프 진입 
vv            b-1 히스토리 버퍼 마지막 라인 ex 프롬프트 진입 
[#]vr         %s/ ex 프롬프트 진입, 커서 [#]단어 삽입 
[#]vt[#arg]   .,.+0s/ ex 프롬프트 진입, 커서 [#]단어 삽입, 커서 {#arg} 단어 삽입 
[#]v/         xkwd 프롬프트 진입 regex 키워드 설정, 커서 [#] 단어 삽입 
v;            파이프 ! ex 프롬프트 진입 
[#]vi         %s/ ex 프롬프트 진입, 공백을 탭으로 전환하는 regex 포함, [#] 탭 넓이 
[#]vI         %s/ ex 프롬프트 진입, 탭을 공백으로 전환하는 regex 포함, [#] 탭 넓이 
vo            후행 화이트 스페이스 및 <\r> 라인 종료 문자 제거, DOS/WiNDOW 문서 
<^G>          버퍼 상태 정보 표시, 1회성 
1<^G>         상태시줄 상시 표시 활성화 
2<^G>         상태표시줄 상시 표시 비활성화 
ga            문자 정보 표시, 1회성 
1ga           문자 정보 표시 활성화, 상태표시줄 라인 
2ga           문자 정보 표시 비활성호, 상태표시줄 라인 
[#]gw         [#]컬럼 라인 워드랩 설정, 기본값 80컬럼 
[#]gq         [#]컬럼 버퍼 워드랩 설정, 기본값 80컬럼 
[#]g~{arg}    [#]번 {arg} 영역 문자 대소문자 변경 
[#]gu{arg}    [#]번 {arg} 영역 소문자 변경 
[#]gU{arg}    [#]번 {arg} 영역 대문자 변경 
[#]~          [#]번 전방 문자 대소문자 변경 
i             삽입 모드 진입 
I             라인 시작 삽입 모드 진입, 들여쓰기 감지 
A             라인 끝 삽입 모드 진입 
a             1문자 전방 삽입 모드 진입 
[#]s          [#]문자 삭제 후 삽입 모드 진입 
S             모든 문자 삭제 후 삽입 모드 진입 
o             다음 라인 생성 후 삽입 모드 진입 
O             이전 라인 생성 후 삽입 모드 진입 
[#]c{arg}     [#]번 영역 삭제 후 삽입 모드 진입 
C             커서에서 라인 끝 삭제 후 삽입 모드 진입 
[#]d{arg}     [#]번 {arg} 영역 삭제 
D             커서에서 라인 끝 삭제 
[#]x          [#]번 커서 전방 문자 삭제 
[#]X          [#]번 커서 후방 문자 삭제 
di{arg}       <( ) "> {arg} 쌍 내부 삭제 
ci{arg}       <( ) "> {arg} 쌍 내부 변경 
[#]r{arg}     커서 전방 {arg} [#] 문자 대체 
[#]K          [#]번 라인 분할 
[#]J          [#]번 라인 병합 
[#]y{arg}     [#]번 {arg} 영역 복사 
[#]Y          [#]번 라인 복사 
[#]p          [#]번 기본 레지스터 내용 붙여넣기, 현재 라인 이후 또는 커서 이후 
[#]P          [#]번 기본 레지스터 내용 붙여넣기, 현재 라인 이전 또는 커서 이전 
m{a-z ` ' [] *}     버퍼 내 로컬 마크 설정 
<^T>          글로벌 마크 0 설정, 글로벌 마크는 항상 유효함 
{0 2 4 6 8}<^T>     글로벌 라인 마크 설정 
{1 3 5 7 9}<^T>     글로벌 라인 마크 전환 
[#]<^7>{0-9}     
[#]<^_>{0-9}     
[#]<^/>{0-9}  버퍼 리스트 표시 및 [#]번 버퍼 전환 또는 0-9 표시된 인덱스 번호 
<^^>     
<^6>          이전 버퍼 전환 
[#]<^N>       다음 버퍼 전환, [#] 전방 후방 방향 전환 
\             /fm/ b-2 버퍼 전환 
{#}\          /fm b-2 버퍼 전환 및 디렉토리 리스트 채우기 
vb            b-1 히스토리 버퍼 재귀적으로 현재 라인 ex 프롬프트 종료 시 현재 라인 삽입 
z1            Farsi 대체 키맵 설정 
z2            Russian 대체 키맵 설정 
ze            English 키맵 전환 
zf            대체 키맵 전환 
...
zr            td ex 옵션 -1 설정 
zR            td ex 옵션 -2 설정 
[#]/          Regex 검색 프롬프트, 다음 일치 또는 [#]일치 
[#]?          Regex 검색 프롬프트, 이전 일치 또는 [#]일치 
[#]n     
[#]N          Regex 검색 반복, [이전 이후] 일치 또는 [#]일치 
<^A>          Regex 커서 1 단어 검색, 중앙 아님, [이전 이후] 순환 
*             Regex 검색, 중앙 아님, [이전 이후] 순환 
{#}*     
{#}<^A>       Regex 검색, 커서 [#]단어 키워드 설정 
<^]>          b-2 버퍼 파일시스템 디렉토리 리스트 기반 regex 전방 검색, <^P> 전역 라인 마크 0 설정 복원 
{#}<^]>       파일시스템 regex 전방 검색, 커서 [#]단어 키워드 설정 
[#]<^P>       파일시스템 regex 후방 검색 
<^Z>          vi 일시 정지 
<^L>          터미널 및 화면 전체 영역 강제 다시 그리기 
Z{*}          터미널 정리 후 종료, & 매크로 강제 종료 
Zz            히스토리 명령 수행 및 종료, & 매크로 강제 종료 
```
