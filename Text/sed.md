...
이 스크립트는 지정한 매크로 패키지 파일에서 매크로 정의 부분부터 매크로 정의 끝부분까지 도달하면 작업을 즉시 중지해 처리 성능을 개선합니다.
...
하지만, "Main Menu" 의 "M" 문자는 글상자에 넣지말아야 한다는 것을 깨달았습니다. 왜냐하면, 이 메뉴는 사용자가 별도로 누를 필요가 없기 때문입니다. 
