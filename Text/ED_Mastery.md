...
사용하지 않고서 완전한 운영체제를 작성할 수 있는 사람과 과연 누가 논쟁할 수 있을까요?
...
ed 명령 프롬프트[^3] 상태에서 명령을 실행해볼 것입니다. 먼저 ENTER (↵) 키를 눌러 명령을
실행합니다.

[^3]: ed의 기본 명령 프롬프트는 `null` 문자입니다. 즉, 아무것도 표시하지 않는 것이 프롬프트입니다.
...
**h** 명령은 ed 에게 마지막으로 발생한 오류에 대해 설명해 달라고 요청합니다. 오류 메시지 표시 기능을 끈 상태에서(기본값) ENTER 키를 다시 입력한 다음, 
**h** 명령으로 해당 오류에 대해 설명하도록 요청합니다.
