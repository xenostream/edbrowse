...
또한, `l` 명령은 매우 긴 라인으로 구성된 텍스트를 라인을 일정한 길이로 나눠 (fold) 출력합니다. 72글자를 초과하는 라인은 줄바꿈 처리해 출력합니다. 
...
`라인 번호` (주소) 를 지정해 해당 라인 범위의 그룹에 대체가 발생하도록 처리할 수 있습니다. 따라서,
...
ed를 사용할 때, 의심할 여지없이 알 수 있는 것은 특정 문자들은 대체 명령의 검색 문자열 에서 사용되거나, 검색 명령의 검색 문자열에 사용될 때, 예기치않게 처리된다는 
것을 알 수 있습니다. 이 섹션에서 종종 `메타 문자` 라고 부르는 `특수 문자` 에 대해 설명합니다.
...
바로 이런 경우 `.` 메타 문자를 사용해 처리할 수 있습니다. `\07` 문자 시퀀스는 실제론 단일 문자를 의미하므로 다음과 같이 명령을 실행합니다.
...
이것은 "h" 문자와 "i" 문자 사이에 어떤 문자가 포함되더라도 `.` 문자와 일치되므로 정상적으로 처리할 수 있습니다. 다시 한 번 기억하시기 바랍니다! 
...
마침표 문자는 모든 단일 문자를 의미하므로, 문자 그대로의 마침표 문자를 검색 문자열에 사용하려면 과연 어떻게 처리하는지 궁금할 것입니다. 
...
또한, 메타 문자는 일반 문자와 결합해 사용할 수 있습니다. 만약, `.PP` 문자열로만 구성된 라인을 검색한다면, 다음과 같은 검색 명령을 사용할 수 있습니다.
...
불특정한 숫자의 공백 문자 모두를 단일 공백 문자로 변경하려 합니다. 해당 라인을 모두 다시 입력하기에는 라인이 너무 길고, 눈으로 세면서 공백 문자를 확인할 수도 없습니다. 
과연 어떻게 처리해야 할까요?
...
따라서, `.*` 는 `가능한 한 많은 모든 단일 문자` 를 의미하므로 주의해 처리하지 않는다면 예상보다 훨씬 더 많은 라인을 일치시킵니다. 예를 들어, 다음과 같은 라인에서
...
다음과 같이 첫 번째로 일치된 "x" 문자부터 첫 번째로 일치된 "y" 문자까지의 모든 것을 대체합니다. 

```
tex y text
```

이 예제는 당연히 원하는 것보다 더 많은 것을 대체시킵니다. 
...
이렇듯, `*` 문자에 대한 혼동은 "0" 번이 별표 문자 이전에 나오는 무언가의 적법한 발생 횟수라는 사실과 관련이 있습니다. 예를 들어, 다음과 같은 명령은
...
실제로 동일한 방법으로 한 라인을 여러 라인으로 만들 수 있습니다. 예제로 "very" 단어를 별도의 라인으로 나누고, 그 앞에 roff 프로그램의 서식 요청 명령을 
...
대체 명령을 실행하면 다음과 같이 "very" 라는 문자열 앞에 `.ul` 요청 명령을 추가하고 또한, "very" 문자열 뒤로 나머지 라인 연결해 출력합니다.
...
대체 명령의 검색 문자열에 `\(` 와 `\)` 문자를 사용하면, 해당 부분은 태그로 기억되고 대체 문자열에서 역참조할 수 있습니다. 대체 문자열에서 `\1` 같은 
