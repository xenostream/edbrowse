# Introduction to Sed
sed 사용법, 파일을 자동으로 수정하는 특수 편집기, 파일을 변경하는 프로그램을 작성하려면 sed를 사용해야 합니다.

유닉스 도구 상자에는 진정한 핵심 프로그램이 몇 가지 있습니다. 이 프로그램들은 간단한 작업에도 사용하기 쉬우면서도 복잡한 작업까지 수행할 수 있는 풍부한 명령어 세트를 갖추고 있습니다. 프로그램의 복잡한 잠재력 때문에 간단한 측면을 활용하지 못하는 일이 없기 바랍니다. 저는 간단한 개념부터 시작해서 나중에는 고급 주제도 소개하겠습니다.

이 글을 처음 작성했을 때(1994년), 대부분의 sed 버전은 스크립트 내부에 주석을 넣을 수 없었습니다. ‘#’ 문자로 시작하는 줄은 주석입니다. 최신 버전의 sed는 줄 끝의 주석도 지원할 수 있습니다.

이를 이해하는 한 가지 방법은, 오래된 “클래식” 버전이 GNU, FreeBSD 및 Solaris 버전의 sed의 기반이란 점입니다. 제가 어떠한 환경에서 작업했는지 이해하는 데 도움이 되도록, Sun/Oracle의 sed(1) 매뉴얼 페이지를 소개합니다.


# The Awful Truth about sed
Sed는 궁극의 스트림 편집기입니다. 이상하게 들린다면, 파이프를 통해 흐르는 개울을 상상해 보시기 바랍니다. 음, 파이프 안에 있으면 개울을 볼 수 없습니다. 무언가 흐르는 비유를 시도한 대가입니다. 문학 작품을 원하신다면 제임스 조이스를 읽으시기 바랍니다.

어쨌든 sed는 놀라운 유틸리티입니다. 안타깝게도 대부분의 사람들은 그 진정한 힘을 결코 배우지 못합니다. 언어 자체는 매우 단순하지만, 문서화가 형편없습니다. Solaris의 sed 온라인 매뉴얼 페이지는 총 다섯 페이지인데, 그 중 두 페이지는 발생 가능한 34가지 오류에 대해 설명하고 있습니다. 언어 설명과 동일한 분량을 오류 설명에 할애하는 프로그램은 학습 곡선이 매우 가파릅니다.

걱정하지 마세요! sed를 이해하지 못하는 건 여러분 탓이 아닙니다. 제가 sed를 완전히 설명해 드리겠습니다. 다만, 제가 배운 순서대로 기능을 설명합니다. 저도 한 번에 모든 걸 배운 건 아닙니다. 여러분도 그럴 필요는 없습니다.


# The essential command: s for substitution
Sed는 여러 명령어가 있지만, 대부분의 사용자는 대체 명령어인 s 만 배웁니다. 대체 명령어는 정규 표현식과 일치하는 모든 부분을 새로운 값으로 바꿉니다. 간단한 예로, “old” 파일에서 “day” 를, ‘new’ 파일에서 “night” 로 바꾸는 작업이 있습니다:
...
인자에 따옴표를 붙이지 않은 이유는 이 예제에서는 필요하지 않았기 때문입니다. 따옴표에 관한 이전 튜토리얼을 읽으셨다면 왜 따옴표가 필요 없는지 이해하실 겁니다. 하지만, 따옴표를 사용하는 것을 권장합니다. 명령어에 특수 문자가 포함된 경우 따옴표는 필수적입니다. 확실하지 않을 때 좋은 습관이며, 앞으로는 “모범 사례” 를 강조하기 위해 예제에 따옴표를 사용할 것입니다. 강한 따옴표(단일 따옴표)를 사용하면 다음과 같습니다:

```
sed 's/day/night/' <old >new
```

sed 편집기는 "정확히 지시한 대로 변경한다" 는 점을 강조해야 합니다. 따라서, 다음과 같이 실행했다면

```
echo Sunday | sed 's/day/night/'
```

이것은 sed가 입력에서 “day” 문자열을 찾았기 때문에 “Sunnight” 라는 단어를 출력합니다.

또 다른 중요한 개념은 "sed는 줄 단위로 작동한다" 는 점입니다. 입력 파일이 다음과 같다고 가정해 보겠습니다:
...
각 줄에서 “one” 을 한 번씩 “ONE” 으로 변경했음을 유의하십시오. 첫 번째 줄에는 “one” 이 두 번 등장했지만, 첫 번째 발생만 변경되었습니다. 이는 기본 동작입니다. 다른 동작을 원한다면 사용 가능한 옵션 중 일부를 사용해야 합니다. 나중에 설명하겠습니다.

자, 계속해 보겠습니다.

이 대체 명령어는 네 부분으로 구성됩니다:
...
검색 패턴(Search Pattern)은 왼쪽에 있고, 대체 문자열(Replacement)은 오른쪽에 있습니다.

인용문과 정규 표현식은 이미  다뤘습니다. 이것만으로도 sed의 대체 명령어 학습에 필요한 90%를 차지합니다. 다시 말해, 가장 빈번하게 사용되는 sed 기능의 90%를 이미 다룰 줄 안다는 뜻입니다. 앞으로 sed 전문가가 되려면 알아야 할 몇 가지 세부 사항이 더 있습니다. (이제 겨우 1장을 마쳤습니다. 앞으로 63개 장이 더 남았습니다. :-) 아, 그리고 이 페이지를 북마크해 두는 게 좋을 겁니다.... 혹시나 끝까지 못 할 경우를 대비해서 입니다..


# The slash as a delimiter
s 뒤에 오는 문자는 "구분자" 입니다. 관례적으로 슬래시(/)를 사용하는데, 이는 ed, more, vi 등이 이 기호를 사용하기 때문입니다. 그러나, 원하는 어떤 문자든 사용할 수 있습니다! 

슬래시가 포함된 경로명(예: /usr/local/bin 을 /common/bin 으로 변경)을 바꾸려면 슬래시를 백슬래시(\)로 인용할 수 있습니다:

```
sed 's/\/usr\/local\/bin/\/common\/bin/' <old >new
```

헉... 어떤 사람은 이것을 '피켓 펜스' 라고 부르는데, 보기 흉합니다. 구분자로 슬래시 대신 밑줄 문자를 사용하면 읽기 더 쉽습니다:
...
마음에 드는 걸 하나 골라서 사용합니다. 찾고 있는 문자열에 포함되지 않는다면 뭐든 상관없습니다. 그리고 "구분자는 세 개가 필요하다" 는 걸 기억해야 합니다. “Unterminated `s' command” 오류가 나면 구분자 하나가 빠졌기 때문입니다.


# Using & as the matched string
때로는 패턴을 검색하고 찾은 패턴 주변이나 근처에 괄호 같은 문자를 추가하고 싶을 때가 있습니다. 특정 문자열을 찾는 경우는 이를 쉽게 수행할 수 있습니다:

```
sed 's/abc/(abc)/' <old >new
```

정확히 무엇을 찾을지 모른다면 이 방법은 통하지 않습니다. 찾을 문자열이 무엇인지 모르는데 어떻게 그 문자열을 대체 문자열에 넣을 수 있습니까?

해결책에는 특수 문자 “&” 가 필요합니다. 이것은 발견된 패턴에 해당합니다.

```
sed 's/[a-z]*/(&)/' <old >new
```

대체 문자열에는 “&” 를 원하는 만큼 사용할 수 있습니다. 패턴을 중복할 수도 있습니다. 예를 들어, 줄의 첫 번째 숫자:
...
이 예제를 약간 수정해 보겠습니다. Sed는 첫 번째 문자열을 찾아서 최대한 탐욕적으로 일치시킵니다. 이에 대해서는 나중에 설명하겠습니다. 탐욕적인 일치를 원하지 않는다면(즉, 일치를 제한하려면) 일치 조건에 제약을 설정해야 합니다.

'[0-9]*' 패턴에 대한 첫 번째 일치 항목은 해당 행의 첫 번째 문자입니다. 이것은 "0개 이상의 숫자와 일치" 하기 때문입니다. 따라서, 입력 값이 “abc 123” 이라면 출력값은 변경되지 않습니다. (글자 앞에 공백이 추가된다는 점만 제외하면) 

숫자를 복제하는 좀 더 나은 방법은 숫자만 일치하도록 처리하는 것입니다:
...
문자열 “abc” 는 정규 표현식과 일치하지 않았기 때문에 변경되지 않습니다. 출력에서 “abc” 를 제거하려면 정규 표현식을 확장해서 줄의 나머지 부분과 일치시키고 “(”, “)” 및 “\1” 을 사용해서 정규 표현식의 일부를 명시적으로 제외해야 합니다. 이것은 다음에 설명할 주제입니다.


# Extended Regular Expressions
여기서 간단히 덧붙이자면, 위의 스크립트를 작성하는 다른 방법이 있습니다. “[0-9]*” 패턴은 0개 이상의 숫자를 일치시킵니다. “[0-9][0-9]*” 패턴은 하나 이상의 숫자를 일치시킵니다. 또 다른 방법은 확장 정규 표현식의 “+” 메타 문자를 사용하는 것입니다. “확장 정규 표현식” 을 사용할 때 “+” 는 특수 문자로 처리되므로 “[0-9]+” 패턴을 사용하면 됩니다. 

확장 정규 표현식은 좀 더 강력하지만, “+” 를 일반 문자로 취급하는 sed 스크립트는 작동하지 않을 수 있습니다. 따라서, 명령줄 옵션으로 이런 확장을 명시적으로 활성화해야 합니다.

GNU sed는 “-r” 명령줄 옵션을 사용할 경우 이 기능을 활성화합니다. 따라서, 위 내용은 다음과 같이 작성할 수도 있습니다.
...
Mac OS X와 FreeBSD는 -r 대신 -E 를 사용합니다. 확장 정규 표현식에 대한 자세한 내용은 정규 표현식과 -r 명령줄 인수 설명을 참조합니다.


# Using \1 to keep part of the pattern
정규 표현식에 관한 저의 튜토리얼에서 이미 “\(”,  “\)” 및 “\1” 의 사용법을 설명한 바 있습니다. 복습하자면, 이스케이프된 괄호(즉, 앞에 백슬래시가 붙은 괄호)는 정규 표현식으로 일치시킨 "문자열의 일부를 기억" 합니다. 이를 활용해서 정규 표현식이 일치시킨 문자열의 일부를 제외시킬 수 있습니다. “\1” 은 첫 번째로 기억된 패턴이고, “\2” 는 두 번째로 기억된 패턴입니다. Sed는 최대 아홉 개의 기억된 패턴을 지원합니다.

줄의 첫 단어를 유지하고 나머지 부분은 삭제하려면, 중요한 부분을 괄호로 표시합니다:

```
sed 's/\([a-z]*\).*/\1/'
```

이 부분은 자세히 설명해야겠습니다. 정규 표현식은 탐욕적이며 가능한 한 많이 일치시키려 합니다. “[a-z]*” 패턴은 0개 이상의 소문자를 일치시키며 가능한 한 많은 문자를 일치시키려 합니다. “.*” 은 첫 번째 일치 이후에 오는 0개 이상의 문자를 일치시킵니다. 첫 번째 표현식이 연속된 소문자를 모두 잡아내기 때문에, 두 번째 표현식은 그 외의 모든 것을 일치시킵니다. 따라서, 다음과 같이 입력하면

```
echo abcd123 | sed 's/\([a-z]*\).*/\1/'
```

“abcd” 를 출력하고 숫자를 삭제합니다.

두 단어를 서로 바꾸려면, 두 가지 패턴으로 기억하고 순서를 바꾸면 됩니다:

```
sed 's/\([a-z]*\) \([a-z]*\)/\2 \1/'
```

두 개의 기억된 패턴 사이에 공백이 있음을 유의합니다. 이것은 두 단어가 발견되도록 제한하기 위해 사용합니다. 그러나, 단일 단어가 발견되거나 글자가 없는 행이 발견되면 아무런 효과가 없습니다. 단어가 "최소한" 하나의 글자를 포함하도록 요구하려면 다음과 같이 사용합니다:
...
“\1” 은 반드시 대체 문자열(오른쪽)에 있을 필요는 없습니다. 검색 패턴(왼쪽)에 있을 수도 있습니다. 중복 단어를 제거하려면 다음을 시도해 볼 수 있습니다:

```
sed 's/\([a-z]*\) \1/\1/'
```

중복 단어를 감지하려면 다음과 같이  사용할 수 있습니다.

```
sed -n '/\([a-z][a-z]*\) \1/p'
```

또는 확장된 정규 표현식으로 다음과 같이 사용할 수 있습니다.

```
sed -rn '/([a-z]+) \1/p'                          # GNU sed
sed -En '/([a-z]+) \1/p'                         # Mac OS X
```
이 필터를 사용하면 중복 단어가 포함된 줄을 출력합니다.

숫자 값은 최대 아홉 개(“\1” 부터 “\9” 까지)까지 지정할 수 있습니다. 

줄의 첫 세 글자를 반전시키려면 다음과 같이 사용할 수 있습니다.
...
# Sed Pattern Flags
대체 명령 마지막 구분자 뒤에 추가 플래그를 붙일 수 있습니다. 이전 대체 명령어 끝에 ‘p' 를 사용한 것을 눈치채셨을 겁니다. 또한, ’-n' 옵션도 추가했습니다. 먼저 'p' 와 다른 패턴 플래그에 대해 설명하겠습니다. 이 플래그들은 일치하는 패턴이 발견되었을 때 수행할 작업을 지정할 수 있습니다. 각각을 설명해 드리겠습니다.


## /g - Global replacement
대부분의 유닉스 유틸리티는 파일을 처리할 때 한 줄씩 읽습니다. sed도 기본적으로 마찬가지입니다. 특정 단어를 변경하라고 지시하면, 해당 단어의 첫 번째 발생 위치만 변경합니다. 줄의 첫 번째 단어가 아닌 모든 단어에 변경을 적용하고 싶을 수 있습니다. 

예를 들어, 한 줄의 단어들을 괄호로 묶어 보겠습니다. “won't” 같은 단어와 일치하지 않는 “[A-Za-z]*” 같은 패턴 대신, 공백을 제외한 모든 것과 일치하는 “[^ ]*” 패턴을 사용합니다. 그러나, “*” 는 0개 이상을 의미하므로 이 패턴 역시 모든 것에 일치하게 됩니다. 
...
해결책은, sed에 “g” 플래그를 사용할 때 공백 문자열 일치를 피해야 합니다. 해결 패턴은 “[^ ][^ ]*” 입니다. 다음은 첫 단어에 괄호를 추가합니다:

```
sed 's/[^ ]*/(&)/' <old >new
```

모든 단어에 변경을 적용하려면 마지막 구분자 뒤에 “g” 를 추가하고 다음과 같은 해결 방법을 사용합니다:
...
## Is sed recursive?
Sed는 입력 데이터에서 발견된 패턴에만 작동합니다. 즉, 입력 행이 읽히고 패턴이 일치하면 수정된 출력이 생성되며, 입력 행의 나머지 부분이 스캔됩니다. “s” 명령어는 새로 생성된 출력을 스캔하지 않습니다. 따라서, 다음과 같은 표현에 대해 걱정할 필요가 없습니다:

```
sed 's/loop/loop the loop/g' <old >new
```

이것은 무한 루프를 일으키지 않습니다. 두 번째 “s” 명령어가 실행되면 이전 명령어의 결과를 수정할 수 있습니다. 나중에 여러 명령어를 실행하는 방법을 보여드리겠습니다.



## /1, /2, etc. Specifying which occurrence
플래그가 없으면 첫 번째로 일치하는 항목이 변경됩니다. “g” 플래그를 사용하면 모든 일치 항목이 변경됩니다. 줄에서 첫 번째가 아닌 특정 패턴을 수정하려면 각 패턴을 “\(” 와 “\)” 로 표시하고, “\1” 을 사용해서 첫 번째 패턴을 변경하지 않고 그대로 유지할 수 있습니다. 
...
으악. 좀 더 쉬운 방법이 있습니다. 대체 명령어 뒤에 숫자를 추가해서 특정 패턴만 일치시키도록 지정할 수 있습니다. 예제:
...
Mac OS는 숫자와 g 명령어를 결합하면 작동하지 않는다고 들었습니다. 아마도 FreeBSD 버전의 sed도 이와 마찬가지일 것입니다.

/2 와 \2 를 혼동하지 마시기 바랍니다. /2 는 대체 명령 끝에 사용됩니다. \2 는 대체 필드 내부에서 사용됩니다.

“*” 문자 뒤에 공백이 있음을 유의합니다. 공백이 없으면 sed는 아주 오래 실행됩니다. 

> [!NOTE]
> 참고: 이 버그는 아마 현재는 수정되었을 겁니다. 이것은 숫자 플래그와 “g” 플래그가 동일한 버그를 가졌기 때문입니다. 

다음과 같은 패턴도 사용할 수 있어야 합니다.

```
sed 's/[^ ]*//2' <old >new
```

이것 역시 CPU를 많이 소모합니다. 만약 이 방법이 여러분의 컴퓨터에서 제대로 작동한다면(일부 UNIX 시스템은 작동합니다.), 암호화된 비밀번호를 비밀번호 파일에서 제거할 수 있습니다:

```
sed 's/[^:]*//2' </etc/passwd >/etc/password.new
```

하지만, 이 글을 작성할 당시에는 이 방법이 통하지 않았습니다. “[^:][^:]*” 를 우회 방법으로 사용해도 소용없습니다. 존재하지 않는 비밀번호를 일치시키지 못할 뿐만 아니라, 오히려 세 번째 필드의 사용자 ID를 삭제하기 때문입니다! 대신 보기 흉한 괄호를 사용해야 합니다:

```
sed 's/^\([^:]*\):[^:]:/\1::/'  </etc/passwd >/etc/password.new
```

첫 번째 패턴에 문자를 추가해서 더 이상 빈 패턴과 일치하지 않도록 처리할 수도 있습니다:

```
sed 's/[^:]*:/:/2'  </etc/passwd >/etc/password.new
```

숫자 플래그는 한 자릿수로 제한되지 않습니다. 1부터 512까지 어떤 숫자든 사용 가능합니다. 각 줄의 80번째 문자 뒤에 콜론을 추가하려면 다음과 같이 입력할 수 있습니다:

```
sed 's/./&:/80' <file >new
```

80개의 점(dot)을 사용해 어려운 방법으로 수행할 수도 있습니다:
...
## /p - print
기본적으로 sed는 모든 줄을 출력합니다. 대체 작업을 수행하면 기존 텍스트 대신 새로운 텍스트가 출력됩니다. sed에 선택적 인자 “sed -n” 을 사용하면 기본적으로 새로운 줄을 출력하지 않습니다. 이 옵션과 다른 옵션은 나중에 다시 다루겠습니다. 

“-n” 옵션이 사용될 때 “p” 플래그는 수정된 줄을 출력합니다. 다음은 sed로 grep의 기능을 복제하는 한 가지 방법입니다:
...
세 번째 구분자 뒤에 추가로 지정할 수 있는 플래그가 하나 더 있습니다. 이를 통해 수정된 데이터를 받을 파일을 지정할 수 있습니다. 
...
이 예제는 입력 파일이 수정되지 않았으므로 출력 파일이 필요하지 않습니다. w 와 파일 이름 사이에는 반드시 공백 하나만 있어야 합니다. 

또한, sed 한 인스턴스로 열 개 파일을 동시에 열 수 있습니다. 이를 통해 데이터 스트림을 별도의 파일로 분할할 수 있습니다. 앞서 설명한 예제와 후술할 다중 치환 명령을 결합하면, 첫 번째 숫자의 마지막 자릿수에 따라 파일을 열 개로 분할할 수 있습니다. 이 방법을 사용해서 오류 또는 디버깅 정보를 별도 파일에 기록할 수도 있습니다.


## /I - Ignore Case
GNU는 또 다른 패턴 플래그를 추가했습니다 - /I

이 플래그는 패턴 일치를 대소문자를 구분하지 않게 처리합니다. 이것은 abc, aBc, ABC, AbC 등과 일치합니다:

```
sed -n '/abc/I p' <old >new
```

'/I'  와 ‘p’ (print) 명령어 사이에 공백을 두는 것은 'p' 가 패턴 매칭 과정의 수식어(플래그)가 아니라 패턴 매칭 이후 실행할 명령어임을 강조하기 위함입니다.


## Combining substitution flags
필요한 경우 플래그를 결합할 수 있습니다. 단, “w” 플래그는 반드시 마지막에 위치해야 합니다. 
...
이전에는 대체 명령어를 하나만 사용했습니다. 두 가지 변경 작업이 필요하고 매뉴얼을 읽고 싶지 않다면 여러 sed 명령어를 파이프로 연결할 수 있습니다:

```
sed 's/BEGIN/begin/' <old | sed 's/END/end/' >new
```

이 방법은 하나의 프로세스 대신 두 개의 프로세스를 사용했습니다. sed 전문가는 하나의 프로세스로 해결할 수 있을 때 두 개의 프로세스를 절대 사용하지 않습니다.


## Multiple commands with -e command
여러 명령어를 결합하는 한 가지 방법은 각 명령어 앞에 -e 옵션을 사용하는 것입니다:

```
sed -e 's/a/A/' -e 's/b/B/' <old >new
```

이전 예제는 “-e” 가 필요하지 않습니다. sed는 항상 하나의 명령어가 있어야 한다는 것을 알고 있기 때문입니다. sed에 하나의 인수를 주면, 그것은 반드시 명령어어야 하며, sed는 표준 입력에서 읽은 데이터를 편집합니다.

GNU의 긴 인수 버전은 다음과 같습니다.

```
sed --expression='s/a/A/' --expression='s/b/B/' <old >new
```

또한, Bourne 셸에서 여러 sed 명령줄을 인용하는 방법도 참고합니다.



## Filenames on the command line
원한다면 명령줄에서 파일을 지정할 수 있습니다. sed에 옵션으로 시작하지 않는 인수가 하나 이상 있을 경우, 반드시 파일명이어야 합니다. 

다음 예제는 세 개의 파일에서 “#:”로 시작하지 않는 줄의 개수를 세어 줍니다.

```
sed 's/^#.*//'  f1 f2 f3 | grep -v '^$' | wc -l
```

이것을 조각으로 나누어 설명하겠습니다. 

- sed의 substitute 명령어는 “#” 으로 시작하는 모든 줄을 빈 줄로 바꿉니다. 
- grep은 빈 줄을 걸러내기(삭제하기) 위해 사용되었습니다. 
- wc는 남은 줄 수를 세어줍니다. 

sed는 grep을 불필요하게 만드는 더 많은 명령어가 있습니다. 그리고 grep -c 는 wc -l 을 대체할 수 있습니다. grep의 일부 기능을 어떻게 복제할 수 있는지는 나중에 설명하겠습니다.

물론 마지막 예제는 “-e” 옵션을 사용해서 작성할 수도 있습니다:
...
## sed -n: no printing
“-n” 옵션은 명시적인 출력 요청이 발견되지 않는 한 아무것도 출력하지 않습니다. 출력을 다시 활성화하는 한 가지 방법으로 대체 명령어에 “/p” 플래그를 언급했습니다. 이를 명확히 설명하겠습니다. 다음과 같은 명령어

```
sed  's/PATTERN/&/p' file
```

PATTERN 이 파일에 없으면 cat 프로그램처럼 동작합니다: 즉, 아무것도 변경하지 않습니다. PATTERN 이 파일에 있으면 이를 포함한 각 줄이 두 번 출력됩니다. “-n” 옵션을 추가하면 이 예제는 grep처럼 동작합니다:
...
## Using 'sed /pattern/'
Sed는 명령어 앞에 주소를 지정해서 어떤 줄을 검사하거나 수정할지 지정할 수 있습니다. 지금은 가장 간단한 버전인 /PATTERN/ 주소에 대해서만 설명하겠습니다. 이 옵션을 사용하면 패턴과 일치하는 줄에만 주소 뒤의 명령어가 적용됩니다. 간단히 말해, /p 플래그와 함께 사용하면 일치하는 줄이 두 번 출력됩니다:

```
sed '/PATTERN/p' file
```

물론 PATTERN 은 어떤 정규 표현식이라도 사용 가능합니다.
...
또한, 꼭 그럴 필요는 없지만 패턴과 명령어 뒤에 공백을 넣는 것을 권장합니다. 이렇게 하면 패턴 매칭을 수정하는 플래그와 패턴이 일치한 후 실행할 명령어를 구분하는 데 도움이 됩니다. 따라서, 다음과 같은 스타일을 권장합니다:
...
## sed -f scriptname
sed 명령어가 많다면, 이를 파일로 만들어 사용할 수 있습니다.

```
sed -f sedscript <old >new
```

sedscript 는 다음과 같을 수 있습니다:
...
명령어가 많아서 한 줄에 깔끔하게 들어가지 않을 경우, 백슬래시를 사용해서 줄을 나누어 사용할 수 있습니다:
...
C 셸에서 여러 줄에 걸친 큰 sed 스크립트를 사용할 수 있지만, 인용문이 여러 줄에 걸쳐 계속된다는 점을 C 셸에 알려야 합니다. 이를 위해 각 줄 끝에 백슬래시를 배치합니다:
...
Bourne 셸은 인용문이 여러 줄을 포함할 수 있으므로 이를 더 쉽게 만듭니다:
...
이 스크립트가 “CapVowel” 이란 이름의 파일에 저장되어 실행 가능하면, 간단한 명령어로 사용할 수 있습니다:
...
## Comments
Sed 주석은 첫 번째 비공백 문자가 “#” 인 줄입니다. 많은 시스템에서 sed는 단 하나의 주석만 허용하며, 반드시 스크립트의 첫 번째 줄이어야 합니다. Sun 시스템(이 글을 작성한 1988년 기준)은 스크립트 내 어디에나 여러 주석 줄을 둘 수 있습니다. 현대판 Sed도 이를 지원합니다. 

첫 번째 줄에 정확히 “#n” 이 포함되어 있다면 “-n” 옵션과 동일한 효과를 냅니다: 즉, 기본적으로 출력을 비활성화합니다. 하지만, sed 인터프리터 스크립트는 이 작업을 수행할 수 없습니다. 왜냐하면, 첫 번째 줄은 반드시 “#!/bin/sed -f” 로 시작하기 때문입니다. 제 기억에 “#!/bin/sed -nf” 는 오류를 발생시켰습니다. 이 글을 처음 작성했을 때(2008년)는 제대로 작동했습니다. “#!/bin/sed -fn” 은 작동하지 않습니다. sed가 스크립트 파일명을 “n” 으로 인식하기 때문입니다. 그러나,
...
sed를 호출하는 셸 스크립트에 단어(매개 변수)를 전달하는 것은 UNIX 인용 메커니즘에 대한 제 튜토리얼을 기억한다면 쉽습니다. 복습하자면, 인용을 켜고 끄려면 작은따옴표를 사용합니다. sed를 사용해서 grep을 모방하는 간단한 셸 스크립트는 다음과 같습니다:
...
그러나, 이 스크립트는 미묘한 문제가 있습니다. 인자로 공백이 주어지면 스크립트는 다음과 같은 구문 오류를 발생시킵니다.
...
이 내용이 sedgrep 이란 파일에 저장되어 있다면, 다음과 같이 입력할 수 있습니다.

```
sedgrep '[A-Z][A-Z]' <file
```

이렇게 하면 sed가 grep 명령어처럼 작동할 수 있습니다.



## Using sed in a shell here-is document
sed를 사용해서 사용자에게 매개 변수를 입력하도록 요청한 후 해당 매개 변수로 채워진 파일을 생성할 수 있습니다. 먼저 가짜 값이 포함된 파일을 생성한 다음 sed로 그 가짜 값을 변경할 수도 있습니다. 

좀 더 간단한 방법은 쉘의 “here 문서” 기능을 사용하는 것으로, 이는 셸 스크립트의 일부를 표준 입력인 것처럼 사용합니다:
...
이것은 인위적인 예제임을 인정합니다. “Here 문서” 는 sed를 사용하지 않고도 값을 평가할 수 있습니다. 다음 예제도 동일한 작업을 수행합니다:
...
그러나, “here 문서” 와 sed를 결합하는 것은 일부 복잡한 경우에 유용할 수 있습니다.
...
사용자가 “a b c” 처럼 공백이 포함된 답을 입력하면 구문 오류가 발생합니다. 좀 더 나은 방법은 값 평가를 따옴표로 묶는 것입니다:
...
따옴표에 관한 제 튜토리얼에서 이 내용을 다뤘습니다.



## Multiple commands and order of execution
sed 명령어를 깊이 살펴볼수록 명령어는 복잡해지고 실제 실행 순서가 혼란스러울 수 있습니다. 사실 아주 간단합니다. 각 줄이 읽어들여집니다. 사용자가 지정한 순서대로 각 명령어가 입력 줄에 작업을 수행할 기회를 얻습니다. 대체 작업이 완료된 후, 다음 명령은 동일한 줄(이전 명령에 의해 수정되었을 수 있음)에 작업할 기회를 얻습니다. 

궁금한 점이 있다면, 작은 예제를 만들어 보는 것이 결과를 파악하는 가장 좋은 방법입니다. 복잡한 명령이 작동하지 않으면 좀 더 단순하게 만듭니다. 복잡한 스크립트가 제대로 작동하지 않는다면, 두 개의 작은 스크립트로 나누고 파이프로 연결합니다.



# Addresses and Ranges of Text
단 하나의 명령어만 배웠을 뿐인데도 sed의 강력한 기능을 확인할 수 있습니다. 하지만, 이 명령어는 단순히 grep과 대체 작업을 수행할 뿐입니다. 즉, 대체 명령어는 주변 행을 고려하지 않고 각 행을 개별적으로 처리합니다. 

특정 행으로 작업을 제한할 수 있는 기능은 유용합니다. 유용한 제한 조건은 다음과 같은 것들이 있을 수 있습니다:

- 라인 번호로 지정하기
- 라인 번호 범위로 지정하기
- 특정 패턴을 포함하는 모든 라인
- 파일 시작부터 정규 표현식까지의 모든 라인
- 정규 표현식부터 파일 끝까지의 모든 라인
- 두 정규 표현식 사이의 모든 라인

Sed는 이 모든 것과 그 이상을 처리할 수 있습니다. sed의 모든 명령어는 위의 예제들처럼 주소, 범위 또는 제한 조건을 앞에 붙일 수 있습니다. 제한 조건이나 주소는 명령어 바로 앞에 위치합니다:
...
## Patterns
vi나 more 같은 많은 유닉스 유틸리티는 정규 표현식 검색에 슬래시 문자를 사용합니다. sed도 동일한 규칙을 따르며, 표현식을 슬래시로 종료해야 합니다. 
...
표현식이 백슬래시 문자로 시작하면 다음 문자는 구분자입니다. 슬래시 대신 쉼표를 사용하려면 다음과 같이 사용합니다:

```
sed '\,^#, s/[0-9][0-9]*//'
```

이 기능의 주요 장점은 슬래시 문자 검색입니다. 예를 들어, “/usr/local/bin” 문자열을 검색해서 “/common/all/bin” 으로 변경하고 싶을 때, 백슬래시를 사용해서 슬래시를 이스케이프 처리할 수 있습니다:

```
sed '/\/usr\/local\/bin/ s/\/usr\/local/\/common\/all/'
```

검색 시 슬래시 대신 밑줄을 사용하면 따라하기가 더 쉬울 것입니다. 다음 예제는 검색 명령과 치환 명령 모두에 밑줄을 사용합니다:

```
sed '\_/usr/local/bin_ s_/usr/local_/common/all_'
```

이것이 바로 "sed 스크립트는 난해하다" 는 평판을 얻는 이유를 보여줍니다. 일부러 좀 더 까다롭게 굴어서 “g” 로 시작하는 모든 줄을 찾아서 그 줄의 각 “g” 를 “s” 로 바꾸는 예제를 보여줄 수 있습니다:

```
sed '/^g/s/g/s/g'
```

대체 명령어 뒤에 공백을 추가하고 밑줄을 사용하면 가독성이 크게 향상됩니다:
...
여기서 배울 점이 하나 있습니다: sed 스크립트에는 주석을 아낌없이 사용합니다. 다른 (구형) 운영체제에서 스크립트를 실행하려면 주석을 제거해야 할 수도 있지만, 이제 아주 쉽게 그렇게 처리할 수 있는 sed 스크립트를 작성하는 방법을 알게 됐습니다! 주석은 좋은 것입니다. 작성할 당시엔 스크립트를 완벽히 이해했을지 몰라도, 6개월 후엔 모뎀 잡음처럼 보일 수 있습니다. 이런 비유가 이해가 안 된다면, 8개월 된 아기가 컴퓨터 키보드를 두드리는 모습을 상상해 보시기 바랍니다.



## Ranges by line number
줄 번호 사이에 쉼표를 삽입해서 번호 범위를 지정할 수 있습니다. 첫 100줄로 치환을 제한하려면 다음과 같이 사용할 수 있습니다:

```
sed '1,100 s/A/a/'
```

파일의 줄 수를 정확히 알고 있다면, 그 숫자를 명시적으로 지정해서 파일의 나머지 부분도 치환할 수 있습니다. 이 경우, wc 명령어를 사용해서 파일에 532줄이 있음을 확인했다고 가정해 보겠습니다:

```
sed '101,532 s/A/a/'
```

좀 더 쉬운 방법은 특수 문자 “$” 를 사용하는 것으로, 이것은 파일의 마지막 줄을 의미합니다.

```
sed '101,$ s/A/a/'
```

“$” 기호는 cat -e, vi, ed 같은 유틸리티에서 “마지막” 을 의미하는 관례 중 하나입니다. “cat -e” 명령어에서 여러 파일을 편집할 경우 줄 번호는 누적됩니다. 즉,
...
## Ranges by patterns
범위로 두 개의 정규식을 지정할 수 있습니다. “#” 이 주석 시작을 의미한다면, 키워드를 검색한 후 두 번째 키워드가 나타날 때까지 모든 주석을 제거할 수 있습니다. 이 경우 사용할 두 개의 키워드는 “start” 와 “stop” 입니다.

```
sed '/start/,/stop/ s/#.*//'
```

첫 번째 패턴은 sed가 모든 줄에 대체 명령을 수행하도록 지시하는 플래그를 켭니다. 두 번째 패턴은 이 플래그를 끕니다. “start” 과 ‘stop’ 패턴이 두 번 발생하면 대체 작업은 두 번 모두 수행됩니다. “stop” 패턴이 누락된 경우 플래그는 절대 꺼지지 않으며, 파일 끝까지 모든 줄에 대체 작업이 수행됩니다.

“start” 패턴이 발견되면, 대체 작업은 “start” 가 포함된 동일한 행에서 발생한다는 점을 알아야 합니다. 이것은 스위치를 켭니다. 이 스위치는 행 단위로 작동합니다. 즉, 다음 행이 읽히고 대체 명령이 확인됩니다. “stop” 이 포함되면 스위치는 꺼집니다. 스위치는 단어 단위가 아닌 행 단위로 작동합니다.

줄 번호와 정규 표현식을 결합할 수 있습니다. 다음 예제는 파일 시작부터 키워드 “start” 를 찾을 때까지 주석을 제거합니다:
...
마지막 예제는 “/start/,/stop/” 범위와 중복되는 범위를 가집니다. 두 범위 모두 키워드가 포함된 줄을 대상으로 처리하기 때문입니다. 지정된 패턴이 포함된 줄까지(해당 줄 제외) 명령을 제한하는 방법은 나중에 설명하겠습니다. 이것은 패턴 범위 내에서 패턴 제외 작업에 해당하지만, 먼저 몇 가지 기본 원칙을 좀 더 다루어야 합니다.

다양한 명령어를 논의하기 전에, 일부 명령어는 행 범위로 적용할 수 없다는 점을 설명해야 합니다. 해당 명령어를 언급할 때 알려드리겠습니다. 다음 섹션에는 세 가지 명령어를 설명할 예정인데, 그 중 하나는 범위로 적용할 수 없습니다.



# Delete with d
범위 사용은 혼란스러울 수 있으므로 새로운 스크립트를 시도할 때는 실험이 필요할 수 있습니다. 

유용한 명령어로는 제한 조건에 맞는 모든 줄을 삭제하는 “d” 입니다. 파일의 처음 10줄을 보려면 다음과 같이 사용할 수 있습니다:

```
sed '11,$ d' <file 
```

이것은 head 프로그램과 기능이 유사합니다. 메일 메시지의 헤더(첫 번째 빈 줄까지의 모든 내용)를 잘라내고 싶다면 다음과 같이  사용합니다:

```
sed '1,/^$/ d' <file
```

파일 길이를 알고 있다면 tail 명령어의 기능을 복제할 수 있습니다. wc 는 줄 수를 세고, expr 은 줄 수에서 10을 뺄 수 있습니다. 파일의 마지막 10줄을 확인하는 Bourne 셸 스크립트는 다음과 같을 수 있습니다:
...
삭제 범위는 작업의 시작과 끝을 표시하는 정규 표현식 쌍일 수 있습니다. 또는 단일 정규 표현식일 수도 있습니다. 
...
주석과 빈 줄까지 제거하려면 두 개의 명령어가 필요합니다. 첫 번째 명령어는 줄 끝까지 “#” 기호로 시작하는 모든 문자를 제거하고, 두 번째 명령어는 모든 빈 줄을 삭제합니다:

```
sed -e 's/#.*//' -e '/^$/ d'
```

세 번째는 줄 끝 바로 앞에 있는 모든 공백과 탭을 제거하기 위해 추가되어야 합니다:

```
sed -e 's/#.*//' -e 's/[ ^I]*$//' -e '/^$/ d' 
```

문자 “^I” 는 CTRL-I 또는 탭 문자입니다. 탭을 명시적으로 직접 입력해야 합니다. 위의 작업 순서는 매우 타당한 이유로 정해진 순서임을 유의합니다. 주석은 줄 중간에서 시작될 수 있으며, 그 앞에 공백 문자가 있을 수 있습니다. 따라서, 주석이 먼저 제거되면, 주석 앞에 있던 공백 문자가 남을 수 있습니다. 두 번째 명령어는 모든 후행 공백을 제거해서 현재 공백인 줄을 빈 줄로 변환합니다. 마지막 명령어는 빈 줄을 삭제합니다. 이 세 명령어는 함께 주석, 탭 또는 공백만 포함된 모든 줄을 제거합니다.

다음은 sed가 한 줄에 작동할 때 사용하는 패턴 공간을 보여줍니다. sed가 실제로 사용하는 작업은 다음과 같습니다:

- 입력 행을 패턴 공간에 복사합니다.
- 주소 제한이 참인 경우, 패턴 공간에 첫 번째 sed 명령을 적용합니다.
- 다음 sed 표현식으로 반복하며, 다시 패턴 공간에 연산을 수행합니다.
- 마지막 연산이 수행되면 패턴 공간을 출력하고 입력 파일에서 다음 행을 읽어들입니다.



# Printing with p
또 다른 유용한 명령은 출력 명령인 “p” 입니다. sed가 “-n” 옵션 없이 시작된 경우, “p” 명령은 입력을 복제합니다. 다음과 같은 명령은
...
Sed는 정규 표현식과 일치하는 모든 줄에 대해 print 연산자를 결합해서 grep처럼 동작할 수 있습니다:
...
grep 명령어와 유사하게 동작합니다. grep의 “-v” 옵션은 패턴을 포함하지 않는 모든 줄을 출력합니다. sed는 다음과 같이 이를 수행할 수 있습니다.
...
이전에 “#” 으로 시작하는 주석을 제거하는 방법을 보여드렸습니다. 제거 작업을 “begin” 과 “end” 키워드 사이의 줄로 제한하려면 다음과 같이 사용할 수 있습니다:
...
중괄호는 중첩할 수 있으므로 주소 범위를 결합할 수 있습니다. 이전과 동일한 작업을 수행하되 변경 작업을 처음 100줄로 제한할 수 있습니다:
...
중괄호 앞에 “!” (반전)를 배치할 수 있습니다. 이것은 주소 제한을 반전시켜 두 예약어 사이에 있는 줄을 "제외한" 모든 줄의 주석을 제거합니다:
...
이를 다른 방식으로 작성하려면 중괄호를 사용해서 그룹화할 수 있습니다:
...
중괄호를 사용해서 “INCLUDE” 가 있는 줄을 삭제할 수 있습니다:
...
이 한계를 해결하려면 즉석에서 sed 명령어를 생성하거나 셸 인용문을 사용해서 변수를 sed 스크립트로 전달하는 방법이 있습니다. 예를 들어, cpp처럼 파일을 포함하는 명령어를 만들고 싶지만, 파일 이름이 스크립트의 인수로 전달되도록 하려면 다음과 같은 스크립트를 사용할 수 있습니다:
...
# The '#' Comment Command
sed를 깊이 파고들수록 주석은 명령어를 이해하기 쉽게 해줍니다. 구버전의 sed는 주석으로 한 줄만 허용하며 반드시 첫 번째 줄이어야 합니다. SunOS(및 GNU sed)는 여러 개의 주석을 허용하며, 이 주석들은 반드시 첫 번째 줄일 필요가 없습니다. 마지막 예제는 다음과 같을 수 있습니다:
...
많은 UNIX 유틸리티에 익숙하다면 sed도 유사한 규칙을 사용할 것으로 예상할 수 있습니다: 이전 줄을 “\” 로 끝내면 줄은 이어집니다. 이 명령어들의 구문은 ‘r’ 및 “w” 명령어처럼 까다롭습니다.
...
“i” 명령어로 패턴 앞에 새로운 줄을 삽입할 수 있습니다:
...
Sed는 모든 명령에서 선행 탭과 공백을 무시합니다. 그러나, “a”, “c” 또는 ‘i’ 명령 뒤에 오는 텍스트 시작 부분에 포함된 공백 문자는 무시될 수도 있고 무시되지 않을 수도 있습니다. SunOS는 두 가지 “기능” 모두 사용할 수 있습니다. Berkeley(및 Linux) 스타일의 sed는 /usr/bin 에 있으며, AT&T 버전(System V)은 /usr/5bin/ 에 있습니다.
...
세 명령어 모두 한 줄 이상 추가할 수 있습니다. 각 줄 끝에 “\” 를 붙이면 됩니다.
...
패턴 공간에 대해서는 이전에 한 번 언급한 바 있습니다. 대부분의 명령어는 패턴 공간에서 작동하며, 후속 명령어는 마지막 수정 결과에 적용할 수 있습니다. 앞서 언급한 세 가지 명령어는 파일 읽기 명령어와 마찬가지로 패턴 공간을 우회해서 새로운 줄을 출력 스트림에 추가합니다.



## Address ranges and the above commands
아까 일부 명령어는 줄 범위를 지정할 수 있지만 다른 명령어는 그렇지 않다고 경고한 것을 기억하실 겁니다. 정확히 말하면 “a”, “i”, “r”, ‘q’ 명령어는 “1,100” 이나 “/begin/,/end/” 같은 범위를 지정할 수 없습니다. 문서에는 read 명령어가 범위를 지정할 수 있다고 나와 있지만, 제가 시도했을 때는 오류가 발생했습니다. “c” 또는 변경 명령어는 이를 허용하며, 여러 줄을 하나의 줄로 변경할 수 있습니다:
...
이 작업을 수행해야 한다면 중괄호를 사용할 수 있습니다. 이를 통해 모든 줄에 작업을 수행할 수 있습니다:
...
sed가 시작되면 첫 번째 줄이 패턴 공간에 배치됩니다. 따라서, 다음 줄은 “CD” 입니다. “n”, ‘d’, “p” 명령어의 동작은 다음과 같이 요약할 수 있습니다:
...
# Transform with y
소문자를 대문자로 바꾸려면 26개의 문자 치환을 작성해서 “a” 를 “A” 로 변환하는 등의 작업을 처리할 수 있습니다. Sed는 tr 프로그램처럼 작동하는 명령어가 있습니다. 이를 ‘y’ 명령어라고 합니다. 예를 들어, “a” 부터 “f” 까지의 문자를 대문자로 바꾸려면 다음과 같이 사용합니다:
...
입력 스트림을 수정하지 않습니다. 대신 첫 번째와 두 번째 줄을 결합한 후 출력하고, 세 번째와 네 번째 줄을 결합한 후 출력하는 등의 방식으로 처리합니다. 다만 새로운 “앵커” 문자 “\n” 을 사용할 수 있습니다. 이 문자는 패턴 공간 내에서 여러 줄을 구분하는 줄바꿈 문자(개행 문자)와 일치합니다. 

문자 “#” 으로 끝나는 줄을 검색하고 그 뒤에 다음 줄을 추가하려면 다음과 같이 사용할 수 있습니다:
...
“ONE” 과 “TWO” 가 포함된 두 줄을 검색하고 연속된 두 줄만 출력할 수 있습니다:
...
연속된 두 줄에서 특정 패턴을 검색하거나, 줄 경계에서 분리될 수 있는 연속된 두 단어를 검색할 수 있습니다. 다음 예제는 두 단어가 같은 줄에 있거나, 하나는 줄 끝에 있고 다른 하나는 다음 줄의 시작 부분에 있는 경우를 찾습니다. 발견되면 첫 번째 단어가 삭제됩니다:
...
앞서 “=” 명령어에 대해 설명하면서  이를 이용해서 파일에 줄 번호를 추가하는 방법을 다뤘습니다. 두 번의 sed 호출을 사용해서 이 작업을 수행할 수 있습니다. (한 번으로 가능하지만, 그 방법은 다음 섹션에서 다루겠습니다.) 
...
다루어야 할 한 가지 “위치” 가 더 있습니다: 홀드 버퍼 또는 홀드 공간입니다. 이를 예비 패턴 버퍼로 생각해야 합니다. 나중에 사용할 수 있도록 패턴 공간의 데이터를 “복사” 하거나 “기억” 하는 데 사용할 수 있습니다. 홀드 버퍼를 사용하는 다섯 가지 명령어가 있습니다.
...
이 스크립트는 키워드 주변의 세 줄을 표시하는 데 사용할 수 있습니다. 예를 들어:
...
“H” 명령어는 홀드 버퍼에 여러 줄을 결합할 수 있게 합니다. 이 명령어는 ‘N’ 명령어와 유사하게 동작하며, 줄 사이에 “\n” 을 삽입해서 버퍼에 줄을 추가합니다. 홀드 버퍼에 여러 줄을 저장해 두고, 나중에 특정 패턴이 발견될 때만 출력할 수 있습니다.

예를 들어, 줄의 첫 번째 문자를 공백으로 사용해서 연속 문자로 처리하는 파일을 가정합니다. 

/etc/termcap, /etc/printcap, makefile 및 메일 메시지 파일은 항목의 연속을 나타내기 위해 공백이나 탭을 사용합니다. 특정 단어 앞의 항목을 출력하려면 다음과 같은  스크립트를 사용할 수 있습니다. 실제 탭 문자를 나타내기 위해 “^I” 를 사용합니다:
...
“H” 명려어를 사용해서 컨텍스트 grep을 확장할 수 있습니다. 다음 예제는 패턴 앞의 한 줄 대신 두 줄을 출력합니다. 두 줄로 제한하는 방법은 “s” 명령어로 새 줄 하나만 남기고 나머지 줄을 삭제하는 것입니다. 이를 grep4 라고 부릅니다:
...
이 코드를 수정해서 패턴 주변에 원하는 만큼의 줄을 출력할 수 있습니다. 보시다시피, 홀드 공간에 무엇이 있는지, 패턴 공간에 무엇이 있는지 기억해야 합니다. 동일한 루틴을 작성하는 다른 방법들도 있습니다.



## Get with g or G
홀드 공간과 패턴 공간을 교환하는 대신, “g” 명령어로 홀드 공간을 패턴 공간에 복사할 수 있습니다. 이 명령은 패턴 공간을 삭제합니다. 패턴 공간에 내용을 추가하려면 “G” 명령을 사용합니다. 이 명령은 패턴 공간에 새 줄을 추가하고, 그 뒤에 홀드 공간을 복사합니다.
...
```
이 예제는 “a” 부터 “f” 까지의 글자만 대문자로 변환합니다. 이것은 좁은 열에 스크립트를 출력하기 쉽도록 선택한 방식입니다. 모든 글자를 대문자로 변환하거나 첫 글자, 두 번째 단어 등을 변경하도록 스크립트를 쉽게 수정할 수 있습니다.
...
- 콜론(:)으로 시작하는 텍스트 문자열로 레이블을 지정할 수 있습니다. 
...
# Testing with t
패턴이 발견되면 분기를 실행할 수 있습니다. 대체가 이루어진 경우에만 분기를 실행하고 싶을 수 있습니다. “t 레이블” 명령은 마지막 대체 명령이 패턴 공간을 수정했을 때만 해당 레이블로 분기합니다.
...
가장 안쪽 괄호 묶음만 제거합니다. 각 묶음이나 괄호를 제거하려면 데이터를 스크립트를 통해 네 번이나 통과시켜야 합니다. 다음과 같은 정규 표현식을 사용할 수 있습니다.

```
sed 's/([ ^I()]*)/g'
```

그러나, 이것은 일치하지 않는 괄호 쌍을 삭제합니다. “t” 명령어로 이 문제를 해결할 수 있습니다:
...
복잡한 여러 줄의 sed 스크립트를 디버깅할 때 유용할 수 있습니다.
...
문서화가 제대로 되지 않은 sed 명령어가 하나 더 있습니다. 바로 “;” 명령어입니다. 이 명령어는 한 줄에 여러 sed 명령어를 결합하는 데 사용할 수 있습니다. 앞서 설명한 grep4 스크립트를 주석과 오류 처리 없이 명령어 사이에 세미콜론을 넣어 다음과 같이 작성해 보겠습니다:
...
이 작업을 스크립트에서 이식성 있게 수행하는 가장 쉬운 방법은 tr(1) 명령어를 사용하는 것입니다. 이 명령어는 8진수 표기법을 이해하며, 사용 가능한 변수에 출력할 수 있습니다.
...
'l' 명령어에 대해서는 이미 설명했습니다. ‘l’ 명령어의 기본 줄 너비는 70자입니다. 이 기본값은 ‘-l N’ 옵션을 추가하고 ‘-l’ 뒤에 최대 줄 길이를 숫자로 지정해서 변경할 수 있습니다.
...
또 다른 예제는 각 파일의 줄 수를 출력하려면 'wc -l' 을 사용할 수 있습니다. 이 명령은 각 파일의 줄 수와 파일명을 출력한 후 마지막에 총 줄 수를 출력합니다. 다음은 이와 비슷한 작업을 수행하는 간단한 셸 스크립트로, sed만 사용합니다:
...
이 버전은 원본 파일을 삭제합니다. 저처럼 신중하신 사람이면 원본의 사본을 보관하기 위해 확장자를 함께 지정하는 것을 선호할 수 있습니다:
...
지난 두 버전은 “a.txt” 파일의 원본이 “a.txt.tmp” 라는 이름으로 저장되었습니다. 모든 작업이 예상대로 완료된 후 원본 파일은 삭제할 수 있습니다. 백업 옵션을 고려하시고, 저의 경고를 유념하시기 바랍니다. 확장자가 고유하다면 백업된 원본 파일을 쉽게 삭제할 수 있습니다.

GNU 버전의 sed는 인수 없이 “-i” 옵션을 사용할 수 있습니다. FreeBSD/Mac OS X 버전은 그렇지 않습니다. FreeBSD/Mac OS X 버전은 반드시 확장자를 함께 지정해야 합니다. 백업 없이 인플레이스 편집을 수행하려면 다음과 같이 사용합니다.
...
```
이 방법은 작동하지만, sed가 결과를 버퍼링하기 때문에 버퍼가 가득 차거나 SlowText 프로그램이 종료될 때까지 기다려야만 결과를 확인할 수 있습니다. “-u” 옵션을 사용하면 이런 버퍼링을 제거하고 SlowText가 결과를 출력하는 즉시 확인할 수 있습니다. 이 옵션을 사용하면 가능한 한 빨리 사각형이 출력되는 것을 볼 수 있습니다:
...
GNU 버전의 sed는 버전 4.2.2에서 개행 문자 대신 “NULL” 문자를 사용하는 기능을 추가했습니다. NULL 을 레코드 구분자로 사용하는 파일이 있을 때 유용할 수 있습니다. 일부 GNU 유틸리티는 “find . -print0” 이나 “grep -lZ” 처럼 개행 문자  대신 NULL 을 사용하는 출력을 생성할 수 있습니다. 이 기능은 공백이나 이진 문자를 포함할 수 있는 파일명을 처리할 때 유용합니다.

예를 들어, “find” 를 사용해서 파일을 검색하고 각 파일 이름 끝에 NULL 을 출력하도록 “-print0” 옵션을 사용한 경우, sed를 사용해서 디렉터리 경로명을 삭제할 수 있습니다:
...
그리고 '/usr/local' 은 삭제하고 다른 3개 경로는 그대로 두고 싶다면, 다음과 같은 간단하지만 (잘못된) 명령어를 사용할 수 있습니다:
...
검색하는 문자열이 공백으로 둘러싸여 있다면 이 방법은 잘 작동합니다. 그러나, 문자열이 여러 가능성 있는 문자 중 하나일 수 있는 다른 문자로 둘러싸여 있다면 어떻게 될까요? ‘단어 끝' 을 정의하는 문자 클래스를 만들 수 있습니다. 예를 들어, 문자열이 영숫자와 슬래시로 구성된다면, 문자 클래스는 ’[a-zA-Z0-9/]‘ 또는 좀 더 유연한 ’[[:alnum:]/]' 로 정의할 수 있습니다. 캐럿(^) 기호를 사용해 ‘[^[:alnum:]/]' 처럼 이 문자군을 제외한 모든 문자를 클래스로 정의할 수 있습니다. 

이전의 공백과 달리 문자 클래스를 사용할 경우 해당 문자들을 기억하고 삭제하지 않도록 주의해야 합니다. 따라서, 공백을 ’[^[:alnum:]/]' 로 대체하고 명령어를 다음과 같이 변경할 수 있습니다.
...
이 방법은 항상 작동하지만 우아하지 않고 오류가 발생하기 쉽습니다. 다른 방법도 있지만 이식성이 많이 떨어질 수 있습니다. Solaris 버전의 sed는 단어 경계를 나타내는 앵커로 특수 문자 ‘\<’ 와 ‘\>’ 를 사용했습니다. 따라서, 다음과 같이 사용할 수 있습니다.
