# Chapter 0: Introduction 
 분명히 말하겠습니다: ed(1)는 유닉스 표준 텍스트 편집기입니다. 유닉스의 공동 창시자 데니스 리치(Dennis Ritchie)가 그렇게 선언했습니다. 유리 텔레타이프 없이도 완전한 운영체제를 작성할 수 있는 사람과 누가 논쟁을 벌이겠습니까? [^1]

[^1]: 글쎄요, 유리 텔레타이프는 애들이 계속 “모니터” 라고 부르는 그 장난감 말입니다. 우리 모두 모니터가 오디오 제작에 쓰이는 레퍼런스 스피커라는 걸 알면서도...
...
[^4]: 파일에 빈 줄이 있다는 사실을 기억하는 것은 ed(1)에 걸맞은 사람이 되고자 한다면 반드시 해야 할 일이다.
```
*+2
...
현재 호스트 시스템 관리 문서를 작성 중인데 현재 IP 주소를 확인해야 한다고 가정합니다. 모든 가상 인터페이스가 아닌 em0만 확인하고 싶습니다. `ifconfig(8)` [^5]로 이 정보를 얻습니다.

[^5]: 진짜 유닉스에는 ifconfig와 ed가 기본으로 포함되어 있습니다.
...
[^6]: 유닉스에 정말 필요한 것은 gren(1) 명령어다. grep(1) 이 외로워서...
...
네 개의 일치 항목이 나옵니다.[^7] 잠깐만요! 16, 17, 18행에는 연속된 e가 두 개 이상 있습니다. 무슨 일이죠? 이건 ed가 여러분보다 좀 더 세세한 부분까지 신경 쓴다는 걸 보여줍니다. 16행에는 e가 연속으로 두 쌍 등장하며, 중간 e는 두 쌍에서 공유됩니다. 17행과 18행에는 e가 연속으로 세 쌍 등장하며, 공유된 e까지 포함하면 좀 더 많습니다.

[^7]: 이 단어들은 우리 목록에 예전에 없었나요? 이상하네요. 마치 저자가 이 섹션을 위해 연속된 동일한 글자를 여러 개 필요로 했지만, 이전 예시들을 다시 돌아가서 업데이트하는 수고를 하지 않은 것 같아요.
...
[^8]: ed는 스타트렉에 나오는 원시적인 인공지능이 아닙니다. 재귀나 역설로 그 머리를 터뜨릴 수 없습니다. 각 줄을 단 한 번만 복제한 뒤 다음으로 넘어갑니다.
```
*g/there/t. n
6 there
9 thereafter
...
[^10]: 이것은 전통적으로 “정규 표현식 디버깅”이라고 불립니다.
...
[^11]: 원시 시스템 관리자조차 예쁜 보고서를 원하는 관리자가 있었기 때문이다.
