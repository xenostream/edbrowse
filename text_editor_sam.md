# Sam Text Editor
sam 편집기에서 `명령` 은 한 글자로 된 이름을 가집니다. 디스크에 파일 쓰기와 같은 비편집 명령을 제외한 대부분의 명령은 점 안의 텍스트를 일부 변경하고 변경 결과의 텍스트를 점으로 설정합니다. 
예를 들어 삭제 명령인 d는 점 안의 텍스트를 삭제하여 null 문자열로 바꾸고 그 결과값을 점으로 설정합니다. 변경 명령인 c는 dot을 임의의 구두점 문자(일반적으로 슬래시)로 구분된 텍스트로 대체합니다. 따라서

```
c/Peter/
```

는 점으로 표시된 텍스트를 문자열 Peter로 바꿉니다. 마찬가지로,

```
a/Peter/
```

(추가)는 점 뒤에 문자열을 추가합니다.

```
i/Peter/
```

(삽입)을 점 앞에 삽입합니다. 세 가지 모두 새 텍스트인 Peter에 점을 남겨 둡니다.

개행은 명령 구문의 일부로, 개행 문자는 어휘적으로 명령을 종료하는 역할을 합니다. 그러나 삽입된 텍스트 내에서 개행은 절대로 암시적이지 않습니다. 그러나 여러 줄의 텍스트를 삽입하는 것이 편리한 경우가 많기 때문에 샘에는 이러한 경우를 위한 특별한 구문이 있습니다:
...
한 줄 구문에서 개행 문자는 C와 같은 이스케이프로 지정할 수 있습니다.
