...
일부 검색/대체 명령어는 edbrowse에선 조금 다르게 동작할 수 있습니다. 이것은 정규 표현식 처리 방식이 전통적인 regexp 라이브러리 대신 perl 호환 정규 표현식(pcre) 라이브러리에 의해 해석되어 처리되기 때문입니다. 따라서, 정규 표현식은 `/bin/ed` 편집기에서 사용하는 정규 표현식보다 많은 기능과 많은 파워를 가지고 있습니다. 사용 문법도 약간 다릅니다.

예를 들어, perl은 ed 편집기에서 이스케이프된 괄호 문자를 사용하는 곳에 괄호 문자만 사용합니다. 검색 문자열의 텍스트 일부를 구분하기 위한 용도입니다. (하위 문자열) 그리고 perl은 하위 문자열을 역참조할 때 `$1 ... $9` 를 사용하는 반면, ed 편집기는 `\1 ... \9` 형식으로 사용합니다. 또한, perl은 대소문자를 구분하지 않는 검색을 위한 `i` 접미사와 전통적인 `g` 접미사를 지원합니다. 이런 모든 세부적인 차이점을 여기서 모두 설명할 필요는 없습니다. 

정규 표현식에 대한 전체적인 설명은 perlre 매뉴얼 페이지인 `man perlre` 를 읽어보거나 {pcre 홈페이지} 를 방문하시기 바랍니다. 만약, 그들의 진정한 파워와 유연성에 익숙해지면 두 번 다시 ed 편집기로 돌아가지 않을 수 있습니다.

자, 아주 좋습니다! perlre 매뉴얼 페이지를 모두 읽고 돌아 오셨군요. 하지만, 제가 perl 정규 표현식에 추가로 적용한 **몇 가지 변경 사항** 이 있습니다.

`(` 와 `)` 문자는 거의 항상 리터럴 문자로 사용하므로 perl의 이스케이프된 괄호 문자의 의미를 변경했습니다. 즉, `(` 와 `)` 문자는 이제 리터럴 문자로 매칭되며 `\(` 와 `\)` 는 검색 문자열의 하위 문자열을 구분하는 용도로 사용합니다. 이런 하위 문자열은 대체 문자열에서 `$1` 부터 `$9` 로 역참조합니다. 이와 비슷하게 `|` 문자는 리터럴 `|` 문자를 의미하며, `\|` 는 교차 (병렬 일치) 를 의미합니다. 

또한, `&` 문자의 의미를 변경해서 ed 편집기에서 의미하는 것과 동일하게 검색 문자열 자체를 의미합니다. `$0` 도 검색 문자열 자체를 의미하며 이것은 perl의 기능입니다. 

`^, $, [ ], +, *, ?, {m,n}` 는 perl에 의해 해석됩니다. 그러나, `*` 문자가 첫 번째 문자일 경우 리터럴 별표 문자로 처리합니다. 이것은 이전 문자가 전혀 없기 때문에 사용 의미는 명확합니다. 일부 버전의 ed 편집기는 이 기능을 수행하고 일부 버전은 수행하지 않습니다. 하지만, 이 기능은 매우 편리합니다. 

`*, +` 또는 `?` 문자를 대체할 때 단순한 문자로 별도로 이스케이프할 필요는 없습니다. 이와 마찬가지로 닫는 괄호가 없는 열린 괄호는 리터럴 문자로 처리되며 `{m,n}` 이 문자열 시작 부분에 있다면 이것도 리터럴 문자로 처리됩니다.

이런 정규 표현식에 대한 변경은 perl 전문가는 혼란스러울 수 있지만, 이 편집기를 주로 사용하는 또 다른 사용자들 특히 경험 많은 ed 편집기 사용자는 좀 더 쉽게 사용할 수 있도록 만듭니다. Perl 사용자는 `ebre-` 명령으로 edbrowse에서 수정한 pcre 정규 표현식 기능을 끌 수 있습니다. 

이 경우 edbrowse는 거의 어떤 변경 없이 정규 표현식을 perl에 있는그대로 전달합니다. 예를 들어, 스크립트에 수 많은 괄호 문자를 포함한 복잡한 정규 표현식을 사용할 경우, 해당 스크립트는 `ebre-` 명령으로 시작할 수 있으며 괄호 문자를 별도로 이스케이프할 필요는 없습니다. 

미리 보기, 뒤로 보기, 역참조 및 모든 pcre 기능을 인식 가능한 형식 그대로 사용할 수 있습니다. 처리가 모두 완료되면 `ebre` 명령으로 복원합니다.

다음은 이 프로그램과 `/bin/ed` 의 차이점입니다.

- 줄이 `#` 문자로 시작하면 무시하고 **주석문으로 인식** 합니다. 이 기능을 통해 edbrowse 스크립트에 주석문을 추가하기 쉬워집니다. 하지만, 줄 중간에 포함된 `#` 문자는 특별한 의미를 가지지 않습니다.

- 줄이 `!` 문자로 시작하면 시스템 쉘 명령을 실행합니다. 또한, `!` 문자가 줄 중간에 포함되어 있을 때는 특별한 의미를 가지지 않으며 정규 표현식의 쉘 명령 실행이나 읽기 또는 쓰기 명령의 일부가 아닌 한 그렇습니다. 즉, `!` 문자는 대화형 하위 쉘을 생성합니다. 따라서, `exit` 명령으로 edbrowse로 다시 돌아갈 수 있습니다. 쉘 명령이 완료되면 *ok*  라는 단어를 출력합니다. 이 기능으로 별도의 출력이 없는 명령이 언제 완료되는지 확인할 수 있습니다. 이러한 *ok* 표시 기능을 억제하려면 `db0` 명령을 사용합니다.

  **경고**: 일부 버전의 `system()` 함수는 `$SHELL` 환경 변수 설정에 관계없이 `/bin/sh` 를 강제로 실행합니다. 이것은 개발자가 별도로 제어할 수 없습니다. 쉘 명령이나 플러그인 명령에서 bash 쉘 기능을 사용할 경우, edbrowse가 `/bin/sh` 쉘을 실행하면 제대로 작동하지 않을 수 있습니다. ed 편집기도 이와 동일한 문제가 있습니다. 이것을 테스트하려면 `!echo $RANDOM` 명령을 사용해 보시기 바랍니다. 이것은 **bash 쉘 전용 기능** 입니다. 만약, 아무것도 출력되지 않는다면 현재 edbrowse는 `/bin/sh` 쉘을 실행하고 있다는 뜻입니다. 이것을 확실히 증명하려면 다음과 같이 실행합니다:

```
!/bin/bash -c 'echo $RANDOM'
```

- `cd dirname` 명령으로 특정 디렉토리로 변경할 수 있습니다. 새로 변경된 디렉토리는 항상 출력합니다. `cd` 명령만 입력하면 현재 디렉토리를 확인할 수 있습니다. 하지만, bash 쉘과 달리 edbrowse는 심볼릭 링크로 경로를 추적하지 않습니다. 따라서, `..` 디렉토리는 항상 물리적인 상위 디렉토리를 의미합니다. 문자열에 `$ [ ] * ?` 또는 선행된 `~` 문자가 포함된 경우 환경 변수는 확장되고 글로빙 작업이 수행됩니다. 예를 들어, `cd ~/work` 명령은 홈 디렉토리 아래의 *work* 디렉토리로 이동합니다. 이 명령은 활성 파일 이름을 변경하지 않습니다. 예를 들어, *foo* 파일을 편집한 후 `cd ..` 명령을 실행한 후 파일을 저장하면 *foo* 파일은 상위 디렉토리로 복사됩니다. 이런 작업을 원하지 않을 수 있으므로 디렉토리를 변경할 때는 항상 주의가 필요합니다.

- 기본적으로 `r` 명령은 현재 줄 기준으로 작동하며 ed 편집기 같이 마지막 줄 기준으로 처리하지 않습니다. 직접 `$r` 명령을 사용해서 현재 작업 중인 파일 끝 부분에 파일을 읽을 수 있습니다.

- `w+` 명령은 파일에 텍스트를 **추가** 합니다. 일부 버전의 ed는 이 작업을 처리하기 위해 *W*  (대문자)명령을 사용하거나 *w>*  명령을 사용하지만 40년 동안 `>` 문자는 잘라내기 기능으로 사용하던 표준 문자로 `>` 문자를 텍스트 추가에 사용하는 것은 다소 혼란스러울 수 있습니다. 또한 *w>>*  명령은 너무 복잡하므로 edbrowse는 `w+` 명령을 사용합니다.

- `w/` 명령은 현재 파일 이름의 마지막 구성 요소로 기록합니다. 예를 들어, `this.that.com/foo/bar/package-2.7.7-22.tar.gz` 파일을 다운로드한 후 로컬 파일에 저장할 때 파일 이름 끝 부분을 다시 입력하지 않을 때 유용합니다. 또한, `f/` 명령은 파일 이름의 마지막 구성 요소만 남기고 파일 이름을 변경합니다.

- 파일을 디스크에서 읽거나 쓸 때마다 파일 이름을 저장한 `$var` 변수는 해당 환경 변수로 대체됩니다. 따라서, 환경 변수에 `$adbook` 이 설정되면 `e $adbook` 명령으로 주소록 파일을 언제든지 편집할 수 있습니다. `${foo}bar` 같은 형식도 정상적으로 지원됩니다. 또한, 선행된 `~/` 문자는 `$HOME/` 으로 대체되므로 홈 디렉토리의 파일을 쉽게 편집할 수 있습니다. 또는, `~user` 형식으로 다른 사용자의 홈 디렉토리를 지정할 수 있습니다.

- 쉘 메타 문자는 결과 값이 파일 이름이 하나일 경우만 확장됩니다. 따라서, 파일 이름에 최소 부분만 입력해서 특정 파일을 읽거나 쓸 수 있습니다. 명령줄의 파일에 대해 `$variables` 또는 `*` 문자는 확장되지 않습니다. 이것은 이미 Unix 쉘에 의해 이미 확장됐기 때문입니다. 따라서, `*.c` 명령을 입력하면 현재 디렉토리의 모든 C 소스 파일을 얻을 수 있습니다. 파일 이름에 대한 변수 대체 및 글로빙 작업은 선행된 백틱 문자로 완전히 억제할 수 있습니다. 예를 들어, `*a?` 란 파일을 읽으려면 `r \a?` 명령을 사용합니다.
  개별 쉘 메타 문자를 역슬래시 문자로 이스케이프할 수 있습니다. 예를 들어 `fun\ehouse` 같이 사용할 수 있습니다.
...
- 대문자 *Q* 명령은 에디터를 강제 종료하지 않습니다. 이것은 자주 <kbd>Caps Lock</kbd> 키를 실수로 누르거나 <kbd>Shift + Q</kbd> 키를 실수로 눌렀을 때 중요 변경 사항을 모두 잃어버리는 상황을 방지하기 위함입니다. 이런 일은 어찌보면 불가능할 수 있지만, 실제론 자주 발생하는 문제로 **대문자 Q 명령은 비활성화** 했습니다. 만약, 강제로 종료하려면 `qt` 명령을 실행합니다.

- 대문자 `J` 명령은 줄 사이에 공백 문자를 추가한 후 연결합니다. `J:` 명령은 줄을 콜론 구분자를 추가해서 연결합니다. 다른 문자도 사용할 수 있습니다.

- `0kx` 는 레이블 *x*  를 마크 해제합니다. 임시로 레이블을 설정하고 작업이 모두 완료되면 해제하는 스크립트 파일에서 유용합니다.

- 쉼표 문자 앞뒤에 특정 줄 번호를 지정하지 않으면 `1` 과 `$` 로 가정합니다. 따라서 `,p` 명령으로 파일 전체를 출력하는 작업과 정확히 일치합니다. edbrowse는 일부 ed 버전에서 지원하는 *3;5p* 명령은 인식하지 않습니다. 쉼표 문자는 주소 범위 구분 문자이며 `;p` 명령은 `.,$p` 를 의미합니다.

- `g` 명령은 모든 줄에 작업을 완료할 때까지 계속해서 스윕 (*sweep*)처리합니다. 대부분의 ed 버전은 이 기능을 수행하지 않습니다. 예를 들어, 전제 10줄로 구성된 파일에 `g/[89]/;m0` 명령을 실행하면 라인 8이 일치하면서 라인 8부터 10까지 시작 부분으로 이동됩니다. 바로 이 시점에서 대부분 ed는 작동을 멈추지만, edbrowse는 다시 스윕해서 두 번째 줄 즉, 라인 9가 포함된 블럭을 발견하고 이를 시작 부분으로 이동합니다. 이런 구현 방식은 어떤 것이 더 옳거나 그르다고 판단하기는 어렵습니다.

- 명령은 두 개의 물리적인 줄에 나눠서 입력할 수 없습니다!! ed 편집기 같이 명령 끝 부분에 역슬래시 문자를 넣어도 안 됩니다!! 그러나, perl에선 `\n` 줄 바꿈을 지원하므로 이렇게 처리할 필요가 전혀 없습니다. 예를 들어, *doghouse*  란 단어를 중간 부분에서 두 줄로 나눌려면 다음과 같이 입력합니다:

```
s/doghouse/dog-\nhouse/
```

- 출력 명령은 줄의 처음 500자만 표시합니다. 나머지 줄은 여전히 활성 버퍼에 있으며 대체 명령으로 수정할 수 있지만, 출력 명령으로 출력하려면 줄을 나눠야 합니다. 이렇게 표시할 문자의 수는 `ll` 명령어로 기본값을 변경할 수 있습니다.

- `a+` 명령은 `a` 명령과 유사하지만 마지막에 입력한 줄까지 버퍼에 추가합니다. 이 기능은 텍스트 추가 모드에 있을 때 실수로 추가 모드가 아니란 것을 알았을 경우는 유용합니다.

- 이 프로그램은 `/bin/ed` 보다 공백 문자에 관대하지 않고 매우 엄격합니다. 예를 들어, `57 , 63 p` 명령은 제대로 작동하지 않습니다.

- 정규 표현식으로 검색할 때 ed와 edbrowse 모두 활성 버퍼를 감싸면서 처음으로 돌아갑니다. 따라서, `/./` 명령을 마지막 줄에서 실행하면 첫 줄을 출력합니다. 이런 동작은 `sw` (*search with wrap*) 명령어로 켜거나 끌 수 있습니다. 일부 사용자는 활성 버퍼의 끝 부분에서 검색을 멈추고 처음으로 다시 돌아가지 않는 것을 선호할 수 있습니다.

- 대체 명령의 오른쪽 즉, 대체 문자열에 사용한 `%` 문자는 마지막으로 사용한 대체 문자열로 대체됩니다. 일부 버전의 ed는 이 기능을 지원하며 일부는 지원하지 않습니다.

- `s,` 명령은 `s/, +/,\n` 명령의 단축형입니다. 이것은 구문 경계로 줄을 나누는 데 사용됩니다. 따라서, `s.` 명령은 첫 번째 마침표를 기준으로 줄을 나누는 데 사용됩니다. `s;, s:, s), s"` 명령도 사용할 수 있습니다. `s,3` 은 세 번째 쉼표 문자 이후를 경계로 줄을 나눕니다. 만약, 문장이 *Mr. Flintstone* 으로 시작하면 `s.2` 명령으로 두 줄로 나눌 수 있습니다. 마지막 마침표 문자를 사용하려면 `s.$` 명령을 사용할 수 있습니다.

- 단독으로 `s` 명령만 입력하면 `s//%` 명령을 의미합니다.

- `/xyz/!` 명령은 *xyz* 문자열이 **포함되지 않은** 줄을 검색합니다. ed는 이 기능을 수행하지 않지만 sed는 이 기능을 지원하며 매우 유용합니다. 또한, *xyz*  는 정규 표현식 버퍼에 저장되어 기억됩니다. `!` 명령도 마찬가지입니다. *xyz*  가 없는 다음 줄을 검색하려면 `/` 명령만 입력하고 이전 줄을 검색하려면 `?` 명령만 입력합니다. 하지만, `g/xyz/!` 명령은 `v/xyz/` 명령과 동일하므로 사용할 필요가 없습니다.

- 앞서 언급한대로 검색이나 대체 명령에 사용한 `i` 접미사는 대소문자를 구분하지 않습니다. 이것은 perl과 호환됩니다. `f` 접미사는 잊어버림 (*forget*) 을 의미하며 이 문자열은 검색 및 대체 명령에서 재사용 용도로 버퍼에 저장하지 않습니다. 따라서, 버퍼에 *foo*  란 단어를 검색할 때 한 번이라도 `/foo/` 를 입력했다면 `/` 명령으로 다음 번 *foo*  인스턴스를 계속 찾을 수 있습니다. `/bar/f` 명령은 *bar*  문자열을 검색하지만 *bar* 문자열을 기억하지 않고 잊어버리므로 다음 `/` 명령은 *foo*  문자열을 검색합니다. `$` 접미사는 줄에서 마지막 대체를 수행합니다. `5g` 명령은 정규 표현식의 다섯 번째 인스턴스와 이후의 모든 인스턴스를 대체합니다. 일부 버전의 sed는 이 기능을 지원합니다. 요약하면, 대체 명령에 사용 가능한 접미사는 `i, f, g, $, number, p` 이며 사용 순서는 별 관계없지만 `$` 는 `g` 또는 숫자와 함께 사용할 수 없습니다.

- `g` 명령은 검색 및 대체 작업을 모든 편집 세션에 걸쳐 전역적으로 처리합니다. 이것은 토글 명령으로 `sg` (*searching global*) 명령을 입력하면 문자열 검색 작업은 현재 세션으로 국한됩니다. 모든 파일에 *foo*  의 모든 인스턴스를 검색하고 *bar*  로 변경하려면 `edit *`  명령을 실행한 후, `sg` 명령으로 모든 편집 세션에 대한 대체 작업을 전역적으로 처리하도록 만듭니다. 첫 번째 세션에 *foo*  문자열을 검색하고 *bar*  로 대체합니다. 그 후 `e2` 또는 `e+`  명령으로 다음 편집 세션으로 이동하면 문자열 *foo*  는 모든 세션 영역에 적용되므로 단지 슬래시 문자로 검색할 수 있습니다. 이와 유사하게 `%` 문자로 *bar*  를 참조할 수 있습니다. `sg-` 명령어는 편집기를 로컬 세션 영역에 처리하도록 되돌려 각 파일에서 고유한 검색/대체 문자열을 제공합니다.

- 파일 읽기 또는 쓰기와 편집 세션간 전환과 관련된 오류는 항상 출력됩니다. 일반적인 오류는 물음표 문자로 표시하며 `h` 명령으로 해당 오류에 대한 설명을 읽어야 합니다. 항상 오류 메시지를 출력하려면 대문자 `H` 명령을 실행합니다. 이것은 ed와 동일합니다. edbrowse에 도입된 새로운 명령 대부분은 오류 발생시 물음표를 표시하지만 일부는 오류 메시지를 직접 출력합니다. 이것은 개인적인 취향에 따른 것이며 소프트웨어 개발자의 재량에 따라 달라질 수 있습니다.

- 대부분의 ed 버전에서 `z7` 명령은 `.+6p` 명령을 의미하며 이 명령은 현재 주소 (줄) 를 `+7` 로 만듭니다. 이것은 매우 혼란스러울 수 있으며 마지막으로 출력한 줄이 현재 줄이 되는 경우가 있습니다. 이 프로그램에서 `z7` 명령은 `+,+7p` 를 의미하며 현재 주소는 마지막으로 출력한 줄이 됩니다. 또한 숫자 없는 `z` 명령은 이전에 지정한 숫자 만큼의 줄을 출력합니다. 따라서, 파일을 청크 또는 화면 단위로 읽을 수 있습니다.

후속 섹션에선 ed에 비해 완전하게 새롭게 추가한 기능을 설명합니다. emacs 및 vi 와 유사한 여러 파일 편집, HTML 파일을 탐색하고 입력 양식을 편집하는 등의 기능이 포함됩니다.



# Performance of the Editor
특정 `g` 명령은 실행 성능을 위해 최적화 됩니다. 예를 들어, `r !seq 100000` 명령 다음에 사용한 `g/[678]$/d` 명령을 가정합니다. 이 명령은 6번 줄을 삭제한 후 나머지 줄을 모두 위로 끌어올려 채운 후 다음 7번 줄을 삭제하고 그 다음 8번 줄을 삭제합니다.  이런 알고리즘은 파일 크기에 비해 베타적이며 매우 큰 파일에선 실행할 수 없습니다.
...
이 명령은 웹 브라우저 모드, SQL 모드, 또는 디렉토리 모드에선 최적화되지 않습니다. 단, 디렉토리 모드에서 `g/re/d` 명령은 최적화되어 처리됩니다. 디렉토리 모드에서 실제로 디렉토리 작업 외에는 처리할 일이 없기 때문에 `g/\e.o$/d` 같은 명령은 별 의미가 없고 `g/\e.o$/-d` 등의 명령은 오히려 위험하고 예측할 수 없습니다.

버퍼로 읽는 작업도 최적화되어 있습니다. 예를 들어, `g/re/ r7` 명령은 디버그 레벨 3에서 *mass read*  메세지를 출력합니다. 버퍼 내용 전체를 읽거나 `at` (`@`) 구문을 사용해서 버퍼 일부만 읽어야 합니다. 파일 내용을 여러 번 읽어 추가하려면 먼저 다른 편집 세션에 파일을 읽어들인 다음 위와 같은 구조를 사용하는 것이 효율적입니다. 파일을 여러 번 열 필요가 없으므로 좀 더 효율적입니다.
...
특정 범위에서 `s` (대체) 명령으로 하나의 줄을 여러 줄로 나눌 경우도 최적화됩니다, 예를 들어, `,s/doghouse/dog\nhouse/` 같은 명령입니다. 그러나, `g` 명령은 최적화되지 않습니다. 만약, *xyz* 로 시작하는 줄만 *doghouse*  를 두 줄로 나누려면, `,g/^xyz/ s/doghouse/dog\nhouse/` 명령 대신에 `,s/\(^xyz.*dog\)house/$1\nhouse/` 명령을 사용합니다.

또 다른 명령이 최적화할 필요가 있다면 개발자에게 문의하시기 바랍니다.



# Balancing Braces
대문자 `B` 명령은 프로그래머들이 주로 관심가질 것으로 일반 사용자는 자주 사용하지 않을 가능성이 큽니다. 이 명령은 균형을 이루는 중괄호 (`{..}`), 괄호 (`(...)`), 또는 대괄호 (`[....]`) 문자가 포함된 줄을 찾습니다. 예를 들어, 다음과 같은 코드 조각을 가정합니다.
...
- `else` 줄에 사용한 `B` 명령은 처리하기 모호합니다. 이 경우 `B{` 또는 `B}` 명령을 실행해서 균형의 방향을 명확히 지정해야 합니다.
- `<..>` 를 명시적으로 지정해서 균형을 맞출 수도 있습니다. 예를 들어, 수 많은 줄로 구성된 HTML 태그에 사용할 수 있습니다.
- `B` 명령은 중괄호가 없는 줄이나 균형이 맞지 않는 중괄호가 없는 줄에서 사용할 수 있습니다:
- `B}` 명령을 네 번째 줄에서 실행하면 그 줄을 포함하는 블록의 시작인 세 번째 줄로 이동합니다.
- `B{` 명령을 네 번째 줄에서 실행하면 마지막 줄로 이동합니다.
- 여러 개의 중괄호가 함께 포함된 줄 예를 들어, `}}}` 에서 실행한 `B}1` 명령은 첫 번째로 균형이 맞지 않는 중괄호 문자를 찾고 이것은 `B` 와 `B}3` 명령과 동일합니다.
- edbrowse는 문자열을 건너뛰기 때문에 `c = '{'` 또는 `s = "abc}def"` 같은 문자열에선 이 기능으로 방해하지 않습니다. 그러나, 주석문이나 구문에 포함된 regular expressions (예: Perl 또는 JavaScript에서 볼 수 있는 경우) 은 edbrowse를 잘못된 경로로 이끌 수도 있습니다.




# Context Switch
이 프로그램은 여러 파일을 동시에 편집하고 파일간에 텍스트를 전송할 수 있습니다. 이것은 가상 터미널 (리눅스) 에서 <kbd>alt-f1</kbd> 에서 <kbd>alt-f6</kbd> 까지 세션을 전환하는 방식과 유사합니다. 이 경우 `e1` 에서 `e6` 명령으로 다른 편집 세션으로 전환할 수 있습니다. 참고로, `e 2` 는 파일 이름이 `2' 인 파일을 편집하는 것이며 `e2` (공백 없이) 는 편집 세션 2로 전환하는 명령입니다. 항상 주의하시기 바랍니다.

이와 비슷하게 `r3` 명령으로 세션 3의 내용을 현재 버퍼로 읽어오거나, `w5` 명령으로 현재 버퍼를 세션 5에 쓸 수 있습니다. 후자의 명령은 세션 5가 이미 존재하고 해당 텍스트가 변경되었으나 저장하지 않았을 경우는 경고 메세지를 발생시킵니다. 즉, 세션 5에서 수정한 내용이 손실될 수 있습니다. 이 경우 `h` 명령을 입력하면 "Expecting `w' on session 5" 라는 오류 메세지를 출력합니다.

- `e+` 와 `e-` 는 마치 TV 리모컨의 채널 업다운처럼 다음 및 이전 편집 세션으로 이동합니다.
- `eret` 는 이전 편집 세션으로 돌아가며 이것은 TV 리모컨에서 방금 전에 보던 채널로 돌아가는 버튼과 유사합니다.
- `e/foo` 는 파일 이름 또는 HTML 제목에 "foo" 가 포함된 세션으로 이동합니다. 검색 작업은 대소문자를 구분하지 않으며 `sw` (search wrap) 옵션이 활성화된 경우 검색 작업은 반복됩니다.
- `e?foo` 는 파일 이름 또는 HTML 제목에 "foo" 가 포함된 이전 세션으로 이동합니다. 스크립트는 임시 버퍼를 생성하고 이름을 지정하며 세션 번호를 알지 못해도 이동할 수 있습니다. `enew` 명령으로 새로운 빈 버퍼를 만들고 `f` 명령으로 파일 이름을 지정하고 `M` 명령으로 새로운 세션으로 이동합니다. 현재 버퍼가 저장된지 확인하지 않으려면 `etmp` 명령을 사용합니다.
- `enum` 명령은 현재 줄에 표시된 번호 세션으로 이동합니다. 만약, 이전에 없는 세션일 경우 새로운 세션이 생성됩니다. 줄에 포함된 첫 번째 번호를 선택합니다. 예를 들어, `enum` 명령을 "jj3jj7jj" 란 텍스트에 입력하면 세션 3으로 이동합니다.
- 세션을 종료하면 유효한 다음 편집 세션으로 이동하며 필요시 세션 1로 돌아갑니다. 마지막 세션까지 모두 종료하면 프로그램은 종료합니다.

**경고:** 이 프로그램은 undo 명령과 관련된 제한 사항이 있습니다. 다른 세션으로 전환한 후 다시 돌아오면 마지막 편집 작업을 취소할 수 없습니다. 이것은 쉽게 수정할 수 있을 것 같지만 생각보다 개발 과정이 복잡해서 아직까지 해결하지 못했습니다. 따라서, 편집 세션을 전환하기 전에 모든 것이 정상인지 항상 확인하시기 바랍니다.

- `bflist` (버퍼 목록) 명령으로 현재 열려있는 모든 편집 세션 목록을 살펴볼 수 있습니다. `bflist` 명령은 세션 번호를 보여주며 세션의 버퍼가 웹 페이지를 포함할 경우 제목을 출력하고 그렇지 않으면 파일 이름을 출력합니다. `bflist/foo` 명령으로 파일 이름 또는 제목에 "foo" 가 포함된 각 세션을 나열합니다. 이때 검색 작업은 대소문자를 구분하지 않습니다. `bflist?foo` 명령으로 버퍼 목록을 역순으로 보여줍니다.
- `hist` 명령으로 현재 세션에 열린 파일 또는 웹 페이지의 이력을 볼 수 있습니다. 이것은 웹 페이지를 브라우징할 때 좀 더 일반적이며, 새로운 웹 페이지가 이전 페이지를 스택에 밀어 넣고 `^` 명령으로 스택을 팝(pop)합니다. 로컬 파일을 편집할 때는 덜 일반적입니다. `hist/foo` 명령으로 파일 이름 또는 제목에 "foo" 가 포함된 각 버퍼를 나열할 수 있습니다. 검색 작업은 대소문자를 구분하지 않습니다. `hist?foo` 명령으로 목록을 역순으로 보여줍니다.

다음은 편집 세션간 텍스트 복사 및 붙여넣기 작업에 대한 예제입니다. 현재 사용자는 세션 1에서 foo 파일을 편집하고 bar 파일의 한 단락이 여기에 맞는다는 것을 깨달았습니다. 이것은 다음과 같이 사용할 수 있습니다. 참고로 `<` 로 시작하는 줄은 사용자 입력을 의미하고 `>` 로 시작하는 줄은 프로그램의 응답을 나타냅니다. `#` 기호는 주석으로 일반적으로 줄 중간에는 나타나지 않습니다.
...
다음 예제는 추가적인 전송용 버퍼를 사용하지 않고 동일한 작업을 보다 빠르고 쉽게 수행하는 방법입니다. 이것은 특별한 표기법 'a,'bw1@'c 를 사용해서 파일을 덮어쓰는 대신, a 에서 b 로 지정한 텍스트 블럭을 세션 1의 라인 레이블 c 위치에 곧바로 삽입합니다. @ 기호가 모든 차이점과 기능을 처리합니다. "at" 이란 단어를 스스로 말해보시기 바랍니다: 기존 버퍼 줄에 텍스트를 *at*  위치에 넣으라는 의미입니다. 이것을 "at 문법" 이라 부릅니다. 

레이블이나 라인 번호 또는 `$` 또는 `.` 또는 `+` 또는 `-` 가 `@` 문자 뒤에 필수적으로 위치해야 하고 공백 문자는 있을 수 없습니다. `@/xyz/-3` 같은 복잡한 표현식은 허용되지 않습니다. 단일 공백만으로 의미가 크게 바뀔 수 있습니다. 'a,'bw 1@'c 명령은 a 파일의 블록 텍스트를 1@'c 라는 이름의 파일에 기록하는 데 이것은 원하는 작업이 아닙니다.
...
다음예제는 파일에 쓰는 대신 읽는 방법입니다. 이 방법은 조금 특별한 표기법 r2@'a,'b 를 사용해서 지정한 텍스트를 a 에서 b 까지 세션 1의 현재 위치로 읽어옵니다. @ 기호가 모든 차이점을 만듭니다. "at" 이란 단어를 스스로 생각해보시기 바랍니다: 다른 세션 줄에서 *at* 시작하는 텍스트를 읽으라는 의미입니다. 이것도 "at 문법" 입니다. 레이블이나 라인 번호 또는 $ 또는 . 또는 + 또는 - 또는 ; 또는 , 문자가 @ 문자 뒤에 와야 하며 공백 문자는 있을 수 없습니다. 

r3@, 명령은 r3 명령과 같다는 점에 유의하시기 바랍니다. @/xyz/-3 같은 복잡한 표현식은 허용되지 않습니다. 두 번째 줄 번호가 주어지지 않는다면 단일 줄만 읽어옵니다.
...
스크립트에서 자르기 및 붙여넣기 작업을 수행하길 원하고 현재 세션 1에 있는지 확신이 없거나 세션 2가 사용 가능한지 확신이 없는 경우 모든 작업을 스택내에서 처리할 수 있습니다. 이것은 다음과 같이 처리할 수 있습니다.
...
이 모든 작업을 처리 가능하도록 만드는 명령은 `r-1@'a,'b` 입니다. `-1` 은 상대적인 숫자입니다. `+` 또는 `- n` 은 edbrowse 편집 세션을 참조하는 것이 아니라 현재 스택내의 버퍼를 참조합니다. 이 경우 현재 위치에서 1 아래에 있는 버퍼를 의미합니다. 즉, foo 파일 아래에 있는 bar 파일을 의미합니다. `hist` 명령을 입력하면 현재 세션에 대한 편집 이력을 확인할 수 있습니다. 이것은 웹브라우저의 이력과 같으며 인터넷에 있는 경우 실제 브라우저의 작업 이력입니다. 이 예제에서는 foo 와 bar 를 출력합니다.

때로는 하나의 스택에서 작업하는 것이 쉬울 때가 있고 때로는 별도의 edbrowse 편집 세션 간에 작업하는 것이 쉬울 때가 있습니다. 별도의 세션은 상호작용적인 설정에서 직관적일 수 있지만 스크립트는 현재 편집 세션이나 사용 가능한 편집 세션을 알 수 없으므로 현재 스택에서 작업하는 것이 좀 더 쉬울 수 있습니다.

`e` 명령만 입력하면 현재 세션 번호를 출력합니다. 이것은 `f` 명령이 현재 파일 이름을 출력하는 것과 유사합니다.

`r/foo` 명령은 파일 이름에 "foo" 라는 하위 문자열이 포함된 편집 세션의 내용을 읽습니다. 또는 웹 페이지일 경우 HTML 제목에 "foo" 라는 하위 문자열이 포함된 세션의 내용을 읽습니다. `bflist` 명령을 사용하면 편집 세션의 파일 이름이나 웹페이지 제목을 살펴볼 수 있습니다. `r?foo` 명령은 세션을 거꾸로 탐색해서 읽을 버퍼를 찾습니다. 이것은 이전에 설명한 `e/foo` 및 `e?foo` 명령과 유사합니다. `w/foo` 와 `w?foo` 명령도 이와 같은 방식으로 작동합니다.

`e/` 명령과 달리, `r/` 및 `w/` 는 at 문법을 존중합니다. `r/foo@3` 명령은 "foo" 라는 하위 문자열이 포함된 세션에서 3번 줄을 현재 버퍼로 읽어옵니다. `'a,'bw/foo@7` 명령은 현재 버퍼에서 "foo" 라는 하위 문자열이 포함된 세션으로 줄 범위를 기록합니다. 이런 표기법은 암호처럼 보일 수 있지만 실제론 매우 강력합니다.
...
여기서 `-f`, `-fm`, 및 `-m` 옵션은 여러 가지 방식으로 사용할 수 있지만 지금은 무시해도 됩니다. 이런 옵션은 edbrowse가 메일 수신기 또는 대화형 메일 클라이언트처럼 작동하도록 처리합니다. 이 부분은 나중에 다시 설명합니다.
...
`edbrowse` 편집 세션에서 `help` 명령을 입력하면 일반적인 edbrowse 명령어에 대한 간단한 요약을 살펴볼 수 있습니다.

- `-dn` 옵션은 디버그 레벨을 n (숫자)으로 설정합니다. 여기서 n 은 0 에서 9 사이의 숫자 값입니다. 기본값은 `-d1` 로 파일 크기를 읽거나 기록할 때도 출력합니다. `-d2` 를 선호하는 사람도 있는데 이것은 새로운 웹 페이지로 이동하거나 온라인 입력폼을 제출할 때 URL까지 출력합니다. 개발자가 아니라면 이보다 높은 디버그 레벨을 사용하는 것은 좋지 않습니다. 디버그 레벨은 `dbn` 명령어로 인터랙티브하게 변경할 수 있습니다. (n은 0 에서 9 사이) `db>filename` 명령으로 디버깅의 출력을 특정 파일로 리다이렉트할 수 있고 `db5` 이상에선 이것을 권장합니다. 하지만, 수 많은 출력이 발생할 수 있습니다.

- `-e` 옵션은 오류가 발생했을 때 edbrowse를 종료합니다. 이것은 스크립트에서 주로 사용합니다. 문제 발생 시 나머지 edbrowse 명령어를 실행하지 않도록 처리하기 위해서입니다. `set -e` 명령은 bash 스크립트에서 이와 동일한 효과를 가집니다.

- `-c` 옵션을 사용하면 `.ebrc` 구성 파일에 대한 처리를 억제하고 편집할 수 있습니다. (이 구성 파일은 나중에 다시 설명합니다.) 구성 파일에 구문 오류가 있어서 edbrowse 시작할 때 오류가 표시되면 이 옵션을 사용할 수 있습니다. 이 경우 구성 파일은 자동으로 세션 1에 로드됩니다. 설정을 변경한 후 파일을 저장하고 `config` 명령어으로 구성 파일을 다시 로드할 수 있습니다. 오류가 발생하면 변경하고 저장한 후 `config` 명령을 다시 입력합니다. 오류가 없을 때까지 이 작업을 계속합니다. 구성 파일에 대한 처리는 첫 번째 오류에서 중단하므로 오류 없이 깨끗한 실행 환경은 중요합니다.

- edbrowse는 `-c` 옵션으로 지정한 파일 이름의 구성 파일로 처리합니다. 새로운 세션으로 전환하고 보기, 편집, 저장, 구성 파일을 다시 적용할 수 있습니다.
...
- `-b` 옵션은 명령줄의 파일이나 URL을 브라우징하거나 플러그인이 있을 경우 재생합니다. `-b` 옵션이 없다면 URL만 브라우징됩니다. `edbrowse this.example.com` 은 웹 페이지에서 데이터를 읽고 바이트 단위의 크기를 출력한 후, 해당 페이지를 브라우징하고 렌더링 처리된 텍스트의 크기를 출력합니다. `-b` 옵션을 사용하면 로컬 파일도 동일한 일이 발생하며 예를 들어 컴퓨터에 있는 HTML 파일을 브라우징합니다. 또한, PDF나 MP3 같은 파일을 렌더링하거나 재생할 수 있는 플러그인도 활성화됩니다. 플러그인은 나중에 다시 설명합니다.

- edbrowse에 대한 인수는 편집할 파일 이름입니다. edbrowse는 이 파일을 세션에 읽고 세션 1에서 시작합니다. 만약 인수가 없는 경우 edbrowse는 파일 이름이 없는 세션 1에서 시작합니다. 파일 이름이 빈 문자열 `""` 이면 해당 세션에 빈 버퍼가 생성됩니다. 파일 이름이 URL인 경우 해당 웹 페이지를 인터넷에서 가져와서 브라우징합니다. 마지막으로 인수가 플러스 기호 문자로 시작할 경우 예를 들어 `edbrowse file "+37"` 은 현재 파일에 해당 명령을 실행합니다. 이 예제는 37번 줄을 출력합니다. `'+<func'` 명령으로 edbrowse 함수를 실행할 수 있습니다. 파일, URL, 및 `+` 명령어는 명령줄에 결합해서 사용할 수 있고 순서대로 처리됩니다. 선행된 `+` 명령어는 이전 파일이 아니라 첫 번째 파일에 적용됩니다.

- 이 프로그램이 마음에 들어 시스템 편집기로 사용하려면 다음과 같은 bash alias 를 쉘 설정 파일에 설정합니다.

```
alias e="/usr/bin/edbrowse"
```

이렇게 하면 `e filename` 명령어만 사용해서 새로운 파일을 언제든지 편집할 수 있습니다. 이것은 edbrowse나 쉘 프롬프트 모두 사용 가능합니다.




# Binary Characters
이 프로그램은 파일 이름을 입력할 때도 항상 입력을 이진 코드로 스캔합니다. 따라서, 비-ASCII 문자인 0xbd 를 입력하려면 `~bd` 라는 세 문자 시퀀스를 사용해야 합니다. 이것은 유니코드로 ½ 문자를 의미합니다. UTF-8 코드 시스템을 사용할 경우 ½ 문자를 입력하려면 `~c2~bd` 명령을 입력합니다. 

이와 마찬가지로 `lna` 명령이 활성화된 상태에서 줄을 나열하면 ½ 문자는 `~c2~bd` 로 표시됩니다. 모든 비-ASCII 및 대부분의 제어 문자는 이와 같은 방식으로 입력하고 표시합니다. Tab 과 newline 은 키보드에서 직접 입력합니다. Tab 과 backspace 는 각각 `>` 와 `<` 로 표시됩니다.
...
그리고 출력하면 같은 텍스트로 보이지만 내부엔 벨 문자와 ½ 문자가 포함되어 있습니다. `~x` 는 x 가 16진수 숫자가 아니기 때문에 인코딩하지 않습니다. 16진수 숫자가 뒤에 오더라도 `~` 를 강제로 입력하려면 두 개의 물결표 `~~` 를 사용합니다.

국제화된 텍스트의 경우 특히 사용자의 모국어에 있는 악센트가 있는 문자가 있고 키보드가 적절히 설정된 경우 문서에 악센트 문자를 입력하는 쉬운 방법이 있습니다. 즉, 일반적으로 악센트 문자는 키보드 단축키가 있으므로 UTF-8 코드를 다루는 번거로움을 피할 수 있습니다.

`~u` 를 사용해서 모국어 이외의 유니코드를 직접 입력할 수 있습니다. 유니코드는 16진수 형식입니다. 예를 들어 `~u3b3;` 는 그리스 문자의 감마(γ) 입니다. 세미콜론은 선택 사항입니다. 다른 16진수 숫자가 뒤에 오는 경우만 필요합니다. 예를 들어 `~u3b3;4` 는 감마4(γ4) 를 나타냅니다. `hello~u1f600world` 는 hello 와 world 사이에 웃는 얼굴 이모지 문자(hello😀world)를 삽입합니다. 왜냐하면 `w` 문자는 16진수 숫자로 혼동되지 않기 때문입니다.

정규 표현식은 16진수나 8진수로 입력할 수 있습니다. 이 프로그램은 `~xx` 를 16진수 값으로 변환하고 Perl 정규 표현식은 `\ennn` 을 8진수로 변환합니다. 따라서 다음과 같은 명령어가 파일을 되돌립니다. 첫 번째는 edbrowse가 변환하고 두 번째와 세 번째는 Perl 정규 표현식으로 변환합니다.
