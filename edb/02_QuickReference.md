...
ed 및 edbrowse 명령어를 모두 모아놓은 **명령 목록** (*Quick Reference*) 이 있습니다. 이것은 명령에 대한 빠른 참조 가이드입니다. 이 명령어들은 문서의 나머지 부분을 모두 읽기 전까지는 대부분 제대로 이해되지 않을 수 있습니다.

`(toggle)` 형태로 지정된 명령어 예를 들어, JavaScript 기능을 켜고 끄는 `js` 명령은 해당 기능을 명시적으로 활성화하거나 비활성화하기 위해 `+` 또는 `-` 접미사를 추가할 수 있습니다. 
즉, `js` 는 JavaScript 기능을 토글하고 `js+` 명령은 JavaScript 기능을 활성화하며 `js-` 명령은 JavaScript 기능을 비활성화합니다. `+` 및 `-` 변형은 지원되지 않습니다.

또한, 이런 토글 명령어는 해당 기능에 대한 현재 상태를 화면에 출력합니다, 즉, 켜짐 (*on*) 또는 꺼짐 (*off*) 을 표시합니다. 만약, 도움말 메시지 표시 기능이 활성화되어 있거나 디버그 레벨이 1 이상일 때 
토글 모드를 활성화하거나 비활성화할 때는 도움말 메시지 표시 기능이 켜져 있을 경우에만 해당 메시지가 정상적으로 출력됩니다.
...
만약 ed 편집기를 사용해본 적이 별로 없다면 이 프로그램이 매우 어색하게 느껴질 수 있습니다. 텍스트를 편집할 때 매번 줄 번호를 일일이 지정해 사용해야만 하는 것처럼 보일 수 있습니다. 
그러나, 실제로는 줄 번호가 거의 필요하지 않습니다. 다음은 이러한 줄 번호 지정 작업을 피할 수 있는 몇 가지 팁입니다.

전체 파일을 읽고 싶다면 `,p` 명령을 입력합니다. 물론 매우 큰 문서일 경우라면 화면을 넘겨가면서 표시될 수 있습니다. 이럴 경우, 페이지 단위로 넘길 수 있는 손 쉬운 방법이 있습니다. 
`0z24`  명령으로 첫 번째 줄부터 24줄을 가져온 후, 이후부터는 `z` 명령어만 사용해 그 다음 24줄을 계속 살펴볼 수 있습니다. 자신의 화면 크기에 맞게 `22`, `23` 등으로 출력될 줄 수를 조정할 수 있습니다. 
또 다른 방법은 <kbd>Enter</kbd> 키를 계속 눌러가면서 한 줄씩 진행하는 것입니다.

만약 정규 표현식 (*regular expression*) 에 익숙해다면 독특한 텍스트 조각으로 검색함으로써 문서내의 어떤 부분이든 빠르게 이동할 수 있습니다. 하지만, 이것은 많은 연습 과정을 통해서만 익숙해집니다. 
또한, 검색 문자열이 고유하지 않다면 원하는 단락을 찾기 위해 두 세번 검색해야 할 수 있습니다.

특정 줄을 표시하고 싶다면 해당 줄 번호를 기억하려고 하지마시길 바랍니다. `k` 명령어를 사용해 해당 줄에 표시(*bookmark*) 하면 됩니다. 예를 들어, `ka` 와 `kb`  명령을 사용해 텍스트 블럭의 시작과 끝 부분을 표시(마크)하고, 
`kc` 명령을 사용해 새로운 위치를 표시합니다. 그런 다음, 사용할 이동 명령어는 `\'a,\'bm\'c` 같이 특정 줄 번호 지정 없이 사용할 수 있습니다. 이것은 표준 ed 편집기의 처리 방식이기도 합니다.

웹 페이지내에서 특정 하이퍼링크를 찾으려면 왼쪽 중괄호 문자 (`{`) 를 검색합니다. 텍스트 파일내에 이미 포함된 문자 그대로의 왼쪽 중괄호 문자를 발견할 수도 있겠지만 이것은 매우 드물게 발생합니다. 
특정 하이퍼링크에 접근하려면 `/Next/g` 명령을 입력해 링크로 이동할 수 있습니다. 마찬가지로, `<`  문자를 검색해 입력 필드를 찾을 수 있습니다. 이것은 웹 페이지에 대한 설명 섹션을 읽는다면 자연스럽게 이해될 것 입니다. 
그리고 여러 작업을 스크립트화할 수 있는 기능 또한 edbrowse의 중요한 기능 중 하나입니다.
