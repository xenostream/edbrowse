...
P=~05 : set prompt to control e, encourages some screen readers to talk
...
s/dog/cat/gfip|T{
replace dog or DOG with cat throughout this line, forget dog and cat as search and replacement strings, and print the result
T}
s/a\e(.\e)c/a$1$1c/|duplicate the letter between a and c
...
s/\ebfoo\eb/mc/g|convert foo to Foo, but not foobar
...
w|write the changes you have made to the current file
...
g/\e.o$/d : delete all the .o files
s/x/y : rename the file
.m3 : move this file to the directory in session 3
...
IRC Client
_
 |\fBirc m n domain nickname\fR set up irc chat using sessions m and n with the specified domain and nickname
 |\fBirc m n domain nickname group\fR set up irc chat and join the specified group
 |\fBirc m n domain:port nickname:password group\fR specify a port and a password
 |\fBirc m n domain+:port* nickname:password group\fR show this channel in the output buffer, and use secure sockets
 |\fBshowchan\fR show channels as messages are displayed (toggle)
 |\fBlst\fR show the date and time when this line was received in the output buffer
_
.TE
