...
현재 가장 중요한 모든 C# 개념을 다루는 94개의 문서로 구성된 이 C# 자습서에 오신 것을 환영합니다. 이 튜토리얼은 주로 이 훌륭한 기술의 신규 사용자를 위한 것이며 가능한 한 최대한 활용하기 위해 모든 장을 살펴보는 것이 좋습니다. 각 장은 이전 장을 읽지 않고도 사용할 수 있지만 일부는 이전 장에서 수행된 내용을 참조할 수 있습니다.


## Introduction
이 C# 자습서에 오신 것을 환영합니다. .NET 프레임워크가 도입되면서 Microsoft는 C#(C Sharp로 발음)이라는 새로운 언어를 포함시켰습니다. C#은 다른 여러 언어, 특히 Java의 핵심 개념을 차용하여 간단하고 현대적이며 범용적인 객체 지향 프로그래밍 언어로 설계되었습니다.

C#은 이론적으로 기계어 코드로 컴파일할 수 있지만 실제로는 항상 .NET 프레임워크와 함께 사용됩니다. 따라서 C#으로 작성된 응용 프로그램을 사용하려면 응용 프로그램을 실행하는 컴퓨터에 .NET Framework가 설치되어 있어야 합니다. .NET 프레임워크를 사용하면 다양한 언어를 사용할 수 있지만 C#은 프레임워크와 함께 설계되었기 때문에 .NET 언어라고도 합니다.

C#은 객체 지향 언어이며 전역 변수나 함수를 제공하지 않습니다. 모든 것이 클래스에 래핑되며, System.Object 클래스에서 상속되는 int 및 string과 같은 간단한 유형도 포함됩니다.
...
Visual Studio는 많은 .NET 개발자가 선택하는 전문 IDE(통합 개발 환경)입니다. .NET 프레임워크와 C# 프로그래밍 언어도 만드는 Microsoft에서 만들었으므로 이는 완벽하게 이해됩니다. 역사적으로 VS(Visual Studio의 약자)는 사용 비용이 많이 들었지만 다행스럽게도 Microsoft는 수년 동안 개별 개발자를 위한 무료 버전을 제공해 왔습니다.

이전에는 이 무료 버전이 Visual C# Express, Visual Web Developer 등과 같은 다양한 작업에 대해 별도의 버전으로 제공되었습니다. 그러나 이제 이름은 단순히 Visual Studio Community이며 Express 버전과 마찬가지로 Visual Studio 프로페셔널 버전의 가벼운 버전입니다. 즉, 일부 기능을 놓치게 될 것이지만 Community 에디션에는 가장 고급 기능을 제외한 모든 기능이 포함되어 있으며 이 튜토리얼을 통해 C#을 배우기에 충분할 것입니다.

### Visual Studio 커뮤니티 다운로드
따라서 이 자습서를 시작하려면 visualstudio.com 에서 Visual Studio Community를 다운로드하세요. 다운로드 페이지에 대한 직접 링크는 다음과 같습니다.

[](https://www.visualstudio.com/downloads/)

다운로드하여 설치하자마자 첫 번째 C# 응용 프로그램을 만드는 다음 문서를 진행할 준비가 된 것입니다.

Windows를 사용하지 않습니까?
macOS용 버전도 있으니 위의 링크를 따라 macOS용 Visual Studio Community 버전을 선택하세요!
...
사실, 이 모든 대사는 실제로 아무것도 성취하지 못하거나 적어도 그렇게 보일 수 있습니다. 키보드에서 F5 키를 눌러 응용 프로그램을 실행해 보십시오. 이렇게 하면 Visual Studio가 코드를 컴파일하고 실행하지만 보시다시피 많은 작업을 수행하지 않습니다. 검은색 창이 실행되고 다시 닫히는 것이 보일 수 있습니다. 우리의 응용 프로그램이 아직 아무 작업도 수행하지 않기 때문입니다. 다음 장에서는 이 줄을 통해 모든 것이 무엇인지 확인하지만 지금은 몇 가지 결과를 보고 싶으므로 C#에 대해 모두 알고 있는 척하고 출력을 얻기 위해 몇 줄을 추가해 보겠습니다. { }의 마지막 집합 안에 다음 줄을 추가합니다.
...
다시 한 번 F5 키를 눌러 실행하면 검은색 창이 실제로 남아 있고 세상에 인사말이 표시되는 것을 볼 수 있습니다. 자, 두 줄의 코드를 추가했는데 어떻게 해야 할까요? C# 및 .NET Framework의 좋은 점 중 하나는 이 예제에서 볼 수 있는 많은 코드가 훈련되지 않은 눈에도 의미가 있다는 사실입니다.


첫 번째 줄은 Console 클래스를 사용하여 텍스트 줄을 출력하고 두 번째 줄은 콘솔에서 텍스트 줄을 읽습니다. 읽다? 왜? 실제로 이것은 약간의 트릭인데, 그렇지 않으면 응용 프로그램이 끝나고 다른 사람이 볼 수 있기 전에 출력이 있는 창을 닫을 것이기 때문입니다.


ReadLine 명령은 응용 프로그램에 사용자의 입력을 기다리도록 지시하며, 알 수 있듯이 이제 콘솔 창에서 텍스트를 입력할 수 있습니다. Enter 키를 눌러 닫습니다. 축하합니다, 방금 첫 번째 C# 응용 프로그램을 만들었습니다! 실제로 무슨 일이 일어나고 있는지에 대한 자세한 내용은 다음 장에서 계속 읽으십시오.
...
이전 장에서는 첫 번째 C# 응용 프로그램에서 콘솔에 텍스트 조각을 작성해 보았습니다. 실제 진행 상황을 확인하기 위해 사용한 코드 줄에 대해 자세히 설명하지 않았으므로 이 장에서는 Hello world 예제 코드에 대한 설명입니다. 코드에서 볼 수 있듯이 일부 줄은 비슷해 보이므로 개별 설명을 위해 그룹으로 다시 가져와 보겠습니다. 코드에서 가장 짧고 일반적인 문자인 { 및 }부터 시작하겠습니다. 중괄호라고도 하며 C#에서는 논리적 코드 블록의 시작과 끝을 표시합니다. 중괄호는 C++, Java, JavaScript 및 기타 여러 언어를 포함한 다른 많은 언어에서 사용됩니다. 코드에서 볼 수 있듯이 함께 속하는 여러 줄의 코드를 래핑하는 데 사용됩니다. 이후 예에서는 사용 방법이 더 명확해질 것입니다.
...
using 는 편집기에서 파란색으로 강조 표시된 키워드입니다. using 키워드는 네임스페이스를 가져오고 네임스페이스는 클래스 모음입니다. 클래스는 일종의 기능을 제공하며 Visual Studio와 같은 고급 IDE로 작업할 때 일반적으로 사소한 코드의 일부를 생성합니다. 이 경우 클래스를 만들고 일반적으로 사용되어야 하거나 예상되는 네임스페이스를 가져왔습니다. 이 경우 5개의 네임스페이스를 가져오고 각 네임스페이스에는 유용한 클래스가 많이 포함되어 있습니다. 예를 들어 System 네임스페이스의 일부인 Console 클래스를 사용합니다.

반면에 우리는 아직 System.Linq 네임스페이스를 실제로 사용하지 않으므로 (예: ) 순수주의자라면 이 줄을 제거하도록 선택할 수 있지만 현재로서는 큰 차이가 없습니다.

보시다시피 자체 네임스페이스도 있습니다.

```
namespace ConsoleApp1
```

네임스페이스 ConsoleApp1은 이제 이 애플리케이션의 기본 네임스페이스이며 새 클래스는 기본적으로 이 네임스페이스의 일부가 됩니다. 분명히이이를 변경하고 다른 네임스페이스에 클래스를 만들 수 있습니다. 이 경우 다른 네임스페이스와 마찬가지로 using 문과 함께 애플리케이션에서 사용하려면 이 새 네임스페이스를 가져와야 합니다.

다음으로 클래스를 정의합니다. C#은 진정한 객체 지향 언어이기 때문에 실제로 작업을 수행하는 모든 코드 줄은 클래스 내에 래핑됩니다. 이 경우 클래스는 단순히 Program이라고 합니다.

```
class Program
```

동일한 파일에도 더 많은 클래스를 가질 수 있습니다. 지금은 하나의 클래스만 필요합니다. 클래스에는 여러 변수, 속성 및 메서드가 포함될 수 있으며, 나중에 자세히 설명할 개념입니다. 지금은 현재 클래스에는 하나의 메서드만 포함되어 있고 다른 것은 아무것도 포함되어 있지 않다는 것만 알아야 합니다. 다음과 같이 선언됩니다.

```
static void Main(string[] args)
```

이 줄은 아마도 이 예에서 가장 복잡한 줄일 것이므로 조금 나누어 보겠습니다. 첫 번째 단어는 정적입니다. static 키워드는 클래스를 인스턴스화하지 않고도 이 메서드에 액세스할 수 있어야 한다고 알려주지만, 이에 대해서는 클래스에 대한 장에서 자세히 설명합니다.

다음 키워드는 void이며 이 메서드가 반환해야 하는 내용을 알려줍니다. 예를 들어 정수 또는 텍스트 문자열일 수 있지만 이 경우 메서드가 아무 것도 반환하지 않는 것을 원하지 않습니다(C#은 키워드 void를 사용하여 아무것도 없다는 개념을 표현합니다).

다음 단어는 Main이며 단순히 메서드의 이름입니다. 이 메서드는 소위 애플리케이션의 진입점, 즉 실행될 첫 번째 코드 조각이며 이 예에서는 실행될 유일한 부분입니다.

이제 메서드 이름 뒤에 괄호 집합 내에 인수 집합을 지정할 수 있습니다. 이 예에서 메서드는 args라는 하나의 인수만 사용합니다. 인수의 유형은 문자열 또는 더 정확하게는 문자열 배열이지만 나중에 자세히 설명합니다. 생각해보면 Windows 응용 프로그램은 항상 선택적 인수 집합으로 호출할 수 있기 때문에 이것은 완벽하게 이해됩니다. 이러한 인수는 기본 메서드에 텍스트 문자열로 전달됩니다.

그리고 그게 다야. 이제 첫 번째 C# 응용 프로그램과 콘솔 응용 프로그램을 작동시키는 기본 원칙에 대한 기본적인 이해를 마쳤을 것입니다.
...
## Variables
변수는 창고에 비유할 수 있으며 프로그래머에게 필수적입니다. C#에서 변수는 다음과 같이 선언됩니다.
...
가장 기본적인 버전이지만 변수에는 아직 값이 없습니다. 다음과 같이 나중에 또는 선언하는 동시에 할당할 수 있습니다.

```
<data type> <name> = <value>;
```

이 변수가 현재 작업중인 메서드(예: 클래스 멤버 변수)에 대해 로컬이 아닌 경우 변수에 가시성을 할당할 수 있습니다.
...
가시성 부분은 클래스와 관련이 있으므로 클래스에 대한 장에서 보다 완전한 설명을 찾을 수 있습니다. 실제로 몇 가지를 사용하는 예를 통해 변수 부분에 집중해 보겠습니다.
...
자, 이 중 많은 부분이 이미 설명되었으므로 흥미로운 부분으로 바로 넘어가겠습니다. 우선, 문자열 유형의 몇 가지 변수를 선언합니다. 문자열은 보시다시피 단순히 텍스트를 포함하는데, 이는 바로 값을 제공하기 때문입니다. 다음으로 두 변수를 사용하는 콘솔에 텍스트 줄을 출력합니다. 문자열은 + 문자를 사용하여 다른 부분을 "수집"하여 구성됩니다.

다음으로, 사용자에게 새 이름을 입력하도록 요청한 다음 ReadLine() 메서드를 사용하여 콘솔에서 사용자 입력을 읽고 firstName 변수에 입력합니다. 사용자가 Enter 키를 누르면 새 이름이 변수에 할당되고 다음 줄에서 이름 표시를 다시 출력하여 변경 사항을 표시합니다. 우리는 방금 첫 번째 변수와 변수의 가장 중요한 기능인 런타임에 값을 변경하는 기능을 사용했습니다.

또 다른 흥미로운 예는 수학을 하는 것입니다. 다음은 방금 사용한 것과 동일한 코드를 기반으로 한 것입니다.
...
이것을 Main 메서드에 넣고 사용해 보십시오. 여기서 사용하는 유일한 새로운 "트릭"은 int입니다. Parse() 메서드를 사용합니다. 단순히 문자열을 읽고 정수로 변환합니다. 보시다시피 이 응용 프로그램은 사용자 입력의 유효성을 검사하려는 노력을 기울이지 않으며 숫자가 아닌 것을 입력하면 예외가 발생합니다. 이에 대해서는 나중에 자세히 설명합니다.

### 변수 및 범위
지금까지 우리는 동일한 메서드 내에서 정의되고 사용되는 변수인 지역 변수만 사용했습니다. C#에서 메서드 내부에 정의된 변수는이 메서드 외부에서 사용할 수 없으므로 local이라고 합니다. 다른 프로그래밍 언어에 익숙하다면 더 많은 위치에서 액세스할 수 있는 전역 변수에 대해서도 알고 있을 수 있지만 C#은 전역 변수의 개념을 지원하지 않습니다. 대신 이 클래스의 모든 메서드에서 액세스할 수 있는 클래스에 필드를 정의할 수 있습니다. 예를 들어 이를 보여 드리겠습니다.
...
메서드 내부가 아닌 클래스 범위에 선언된 helloClass 멤버를 주목하십시오 - 이렇게 하면 Main() 메서드와 자체 DoStuff() 메서드 모두에서 액세스할 수 있습니다. Main() 메서드 내부에서 선언되었으므로 이 특정 메서드 내부에서만 사용할 수 있는 helloLocal 변수에는 해당되지 않습니다.


변수가 선언 된 위치를 구별하는 개념을 범위 지정이라고 하며 코드가 너무 많은 곳에서 변경 될 수있는 변수의 엄청난 혼란이되는 것을 방지합니다. 이를 돕는 또 다른 기술은 멤버 가시성(이 경우 private 키워드로 설명됨)이라고 하며, 클래스에 대한 장에서 논의할 것입니다.

### 요약
변수를 사용하면 텍스트 문자열, 숫자 또는 사용자 정의 개체와 같은 다양한 유형의 데이터를 저장할 수 있습니다. 정의된 메서드 내부에서 액세스할 수 있는 지역 변수가 있고, 가시성이 허용하는 경우 클래스의 모든 메서드와 클래스 외부에서도 액세스할 수 있는 클래스 필드가 있습니다.




## Data types
데이터 형식은 C#과 같은 프로그래밍 언어의 모든 곳에서 사용됩니다. 강력한 형식의 언어이기 때문에 변수에 대한 장에서 볼 수 있듯이 변수를 선언할 때마다 사용할 데이터 유형을 컴파일러에 알려야 합니다. 이 장에서는 가장 많이 사용되는 데이터 유형 중 일부와 작동 방식을 살펴보겠습니다.

bool은 가장 간단한 데이터 유형 중 하나입니다. false 또는 true의 2개 값만 포함할 수 있습니다. bool 유형은 if 문과 같은 논리 연산자를 사용할 때 이해하는 것이 중요합니다.

int는 integer의 약자로, 소수 없이 숫자를 저장하기 위한 데이터 유형입니다. 숫자로 작업할 때 int는 가장 일반적으로 사용되는 데이터 유형입니다. 정수는 저장해야 하는 숫자의 크기에 따라 C# 내에서 여러 데이터 유형을 가지고 있습니다.

string은 텍스트, 즉 여러 문자를 저장하는 데 사용됩니다. C#에서 문자열은 변경할 수 없으므로 문자열을 만든 후에는 변경되지 않습니다. 문자열을 변경하는 메서드를 사용할 때 실제 문자열은 변경되지 않고 대신 새 문자열이 반환됩니다.

char는 단일 문자를 저장하는 데 사용됩니다.

float는 소수를 포함할 수 있는 숫자를 저장하는 데 사용되는 데이터 유형 중 하나입니다.

### 요약
이것들은 C#에서 가장 기본적인 데이터 유형일 뿐이며 매우 건조한 주제이고 이에 대해 자세히 읽기 전에 일부 데이터 유형이 작동하는 것을 보고 싶을 수 있기 때문에 이에 대한 매우 기본적인 내용만 말씀드렸습니다. 따라서 변수를 사용하여 다양한 유형의 데이터를 포함하는 다음 기사로 넘어갑니다!

이 자습서의 뒷부분에서는 .NET Framework에 있는 모든 다양한 데이터 형식에 대해 훨씬 더 자세히 살펴보겠습니다.
...
## Code Comments
코드를 작성할 때 입력하는 거의 모든 문자나 단어가 특별한 의미를 갖는다는 사실에 금방 익숙해질 것입니다. 예를 들어 C#에는 class, namespace, public 등과 같은 많은 키워드가 표시됩니다. 또한 컴파일러가 이러한 키워드와 자체 변수 및 메서드를 올바른 방식으로 사용하고 있는지 확인하는 것을 볼 수 있습니다. C#은 상당히 엄격한 언어이며 컴파일러는 모든 것이 제대로 입력되었는지 확인하는 데 도움이 됩니다. 그러나 코드 주석의 개념 덕분에 원하는 것을 작성할 수 있는 단일 가능성이 있습니다.

C#이든 다른 프로그래밍 언어이든 본 코드 중 일부에서 이미 주석을 경험했을 수 있습니다. 하지만 작성 방식은 매우 다양하므로 C# 코드에서 사용할 수 있는 주석 유형을 살펴보겠습니다.
...
그게 다야: 줄에 두 개의 슬래시(//)를 접두사로 붙이면 텍스트는 컴파일러가 확인하고 불평하는 것에서 컴파일러가 완전히 무시하는 것으로 바뀝니다. 이것은 접두사가 붙은 줄에만 적용되지만 기본적으로 한 줄 주석을 사용하여 여러 주석 줄을 만드는 다음 줄에서도 동일한 작업을 자유롭게 수행할 수 있습니다.
...
### 여러 줄 주석
여러 줄의 주석을 작성하려는 경우 C#에서 제공하는 여러 줄 주석 변형을 사용하는 것이 더 합리적일 수 있습니다. 모든 줄에 접두사를 붙일 필요 없이 시작 및 중지 문자 시퀀스를 입력하기만 하면 됩니다.
...
슬래시 별표 (/*)의 시작 시퀀스를 사용하고, 여러 줄에 걸쳐 원하는 것을 쓴 다음 별표 슬래시 (*/)의 끝 시퀀스로 모두 끝냅니다. 이 마커 사이에는 원하는 것을 쓸 수 있습니다.

거의 모든 다른 프로그래밍 관련 주제와 마찬가지로 여러 개의 한 줄 주석을 사용할지 아니면 하나의 여러 줄 주석을 사용할지 여부는 종종 논쟁됩니다. 개인적으로 저는 다양한 상황에 둘 다 사용합니다 - 결국 그것은 모두 당신에게 달려 있습니다!

### 문서 주석
문서 주석(XML 문서 주석이라고도 함)은 일반 주석처럼 보이지만 XML이 포함되어 있습니다. 일반 주석과 마찬가지로 한 줄과 여러 줄의 두 가지 형태로 제공됩니다. 당신은 또한 같은 방식으로 쓰지만 추가 문자를 사용합니다. 따라서 한 줄 XML 문서 주석은 두 개 대신 세 개의 슬래시(///)를 사용하며 여러 줄 변형은 시작 구분 기호에 추가 별표를 추가합니다. 어떻게 생겼는지 봅시다.
...
여기에서 단일 라인과 다중 라인의 두 가지 변형을 모두 볼 수 있습니다. 결과는 동일하지만 첫 번째 변형이 문서 주석에 가장 일반적으로 사용되는 경향이 있습니다.

문서 주석으로 유형과 해당 구성원을 문서화하는 것은 꽤 큰 주제이므로 이후 기사에서 더 자세히 다루겠지만 이제 어떻게 보이는지 알 수 있습니다!

### 코드 주석 및 작업 목록
Visual Studio를 사용하는 경우 실제로 코드 주석을 추적하는 데 도움이 될 수 있습니다. 작업 목록 창(메뉴에서 액세스) 보기 > 작업 목록) 특별하지만 매우 간단한 작업 목록 주석 구문을 사용하는 경우 주석이 나타납니다.
...
따라서 한 줄 주석 바로 뒤에 TODO 또는 HACK이 오면 다음과 같이 Visual Studio의 작업 목록에 나타납니다.
...
그리고 사용 중인 Visual Studio 버전에 따라 다음 주석 토큰의 일부 또는 전부에 응답하는 더 많은 형식이 있습니다.
...
원하는 경우 자신의 토큰을 추가할 수도 있습니다 - 이 문서에 설명된 단계를 따르기만 하면 됩니다.

### 요약
코드 주석은 코드를 문서화하거나 자신이나 잠재적인 동료에게 작동 방식에 대한 단서를 남기는 데 매우 유용합니다. 추가 이점으로, 무언가를 빠르게 테스트해야 할 때 유용합니다 - 줄을 복사하고 원래 줄을 주석 처리하면 이제 어떻게 작동하는지 확인할 수 있습니다. 결과가 만족스럽지 않으면 새 줄을 삭제하고 원래 줄의 주석 처리를 제거하면 시작 위치로 돌아갈 수 있습니다.

그리고 최종 사용자가 귀하의 주석을 기웃거리는 것에 대해 걱정하지 마십시오 - 이미 언급했듯이 컴파일러에 의해 완전히 무시되므로 최종 DLL 또는 EXE 파일에 포함되지 않습니다. 코드 주석은 프로그래밍할 때 개인적인 여유 공간이므로 원하는 방식으로 사용하십시오.
...
## The if statement
모든 프로그래밍 언어에서 가장 중요한 명령문 중 하나는 if 문입니다. 조건부 코드 블록을 설정할 수 있는 것은 소프트웨어 작성의 기본 원칙입니다. C#에서 if 문은 사용하기가 매우 간단합니다. 이미 다른 프로그래밍 언어를 사용한 경우 C#에서 if 문을 바로 사용할 수 있습니다. 어쨌든 어떻게 사용되는지 보려면 계속 읽으십시오. if 문에는 부울 결과, 즉 true 또는 false가 필요합니다. 일부 프로그래밍 언어에서는 여러 데이터 유형을 자동으로 부울로 변환할 수 있지만 C#에서는 결과를 구체적으로 부울로 만들어야 합니다. 예를 들어, if(number) 를 사용할 수는 없지만 나중에 하는 것처럼 숫자를 무언가와 비교하여 참 또는 거짓을 생성할 수 있습니다.

이전 장에서는 변수를 살펴보았으므로 예제 중 하나를 확장하여 조건부 논리를 사용하는 방법을 살펴보겠습니다.
...
입력한 숫자가 0에서 10 사이인지 확인하기 위해 2개의 if 문을 사용하고 if 문의 동반자인 else 키워드를 사용합니다. 그 의미는 영어를 사용하는 모든 사람에게 분명해야 합니다 - if 문의 조건이 충족되지 않는 경우 실행되는 코드에 대한 대안을 제공합니다.

눈치채셨겠지만 { 및 } 문자를 사용하여 코드의 조건부 블록을 정의하지 않습니다. 규칙은 블록에 한 줄의 코드만 포함된 경우 블록 문자가 필요하지 않다는 것입니다. 자, 이것은 단순히 숫자를 확인하는 데 많은 줄처럼 보이지 않습니까? 다음과 같이 더 적은 코드 줄로 수행할 수 있습니다.
...
각 조건을 괄호 집합에 넣은 다음 || 단순히 "또는"을 의미하는 연산자로 숫자가 10보다 크거나 0보다 작은지 확인합니다. 많이 사용하게 될 또 다른 연산자는 &&와 같이 작성된 AND 연산자입니다. 대신 AND 연산자를 사용할 수 있었을까요? 물론 다음과 같이 약간 뒤집습니다.
...
switch 문은 if 문 집합과 같습니다. 각 가능성에 대한 작업과 다른 것이 true로 평가되지 않는 경우를 대비한 선택적 기본 작업이 있는 가능성 목록입니다. 간단한 switch 문은 다음과 같습니다.
...
확인할 식별자는 switch 키워드 뒤에 배치되고 주어진 값에 대해 식별자를 확인하는 case 문 목록이 있습니다. 각 케이스의 끝에 중단 문이 있음을 알 수 있습니다. C#에서는 블록이 끝나기 전에 블록을 떠나기만 하면 됩니다. 함수를 작성하는 경우 break 문 대신 return 문을 사용할 수 있습니다.

이 경우 정수를 사용하지만 문자열 또는 다른 간단한 유형일 수도 있습니다. 또한 여러 케이스에 대해 동일한 작업을 지정할 수 있습니다. 다음 예제에서도 이를 수행하여 사용자로부터 입력 조각을 가져와 switch 문에서 사용합니다.
...
이 예에서는 사용자에게 질문을 하고 예, 아니오 또는 아마도(mamabe)를 입력하도록 제안합니다. 그런 다음 사용자 입력을 읽고 이에 대한 switch 문을 만듭니다. 사용자를 돕기 위해 소문자와 대문자 사이에 차이가 없도록 소문자 문자열과 비교하기 전에 입력을 소문자로 변환합니다.

그래도 사용자는 오타를 만들거나 완전히 다른 것을 작성하려고 할 수 있으며, 이 경우 이 특정 switch 문에 의해 출력이 생성되지 않습니다. 기본 키워드를 입력하세요!
...
case 문 중 true로 평가되지 않은 경우 기본 문(있는 경우)이 실행됩니다. 이전 예제에서 보았듯이 선택 사항입니다.
...
## Loops
소프트웨어를 작성할 때 또 다른 필수 기술은 코드 블록을 X번 반복하는 기능인 루핑입니다. C#에서는 4가지 변형이 있으며 각각을 살펴보겠습니다.

### while 루프
while 루프는 아마도 가장 간단한 것일 것이므로 그것부터 시작하겠습니다. while 루프는 주어진 조건이 참인 한 코드 블록을 실행하기만 하면 됩니다. 작은 예와 몇 가지 추가 설명:
...
코드를 실행해 보십시오. 0에서 4까지의 멋진 숫자 목록을 얻을 수 있습니다. 숫자는 먼저 0으로 정의되고 루프의 코드가 실행될 때마다 1씩 증가합니다. 그런데 코드에 5라고 표시되어 있는데 왜 4까지만 도달합니까? 조건이 true를 반환하려면 숫자가 5보다 작아야 하며, 이 경우 숫자가 5와 같으면 숫자를 출력하는 코드에 도달하지 못함을 의미합니다. while 루프의 조건은 코드 블록에 들어가기 전에 평가되기 때문입니다.

### do 루프
do 루프의 경우 그 반대가 사실이며, 다른 측면에서는 while 루프처럼 작동합니다. do 루프는 루프가 실행된 후 조건을 평가하여 코드 블록이 항상 한 번 이상 실행되도록 합니다.
...
출력은 동일하지만 숫자가 5보다 크면 루프가 종료됩니다.

### for 루프
for 루프는 약간 다릅니다. 정확한 반복 횟수를 알고 있거나 양을 포함하는 변수가 있기 때문에 원하는 반복 횟수를 알고 있을 때 선호됩니다. 다음은 for 루프의 예입니다.
...
이것은 정확히 동일한 출력을 생성하지만 보시다시피 for 루프는 조금 더 컴팩트합니다. 계산을 위해 변수를 초기화하고, 이를 테스트하기 위해 조건문을 설정하고, 카운터를 증가시킵니다(++는 "변수 = 변수 + 1"과 동일함을 의미함)의 3개 부분으로 구성됩니다.

i 변수를 정의하고 0으로 설정하는 첫 번째 부분은 루프가 시작되기 전에 한 번만 실행됩니다. 마지막 2개 부분은 루프의 각 반복에 대해 실행됩니다. 매번 i는 숫자 변수와 비교됩니다 - i가 숫자보다 작으면 루프가 한 번 더 실행됩니다. 그 후, 나는 1씩 늘어납니다.


프로그램을 실행한 다음 숫자 변수를 5보다 크거나 작은 변수로 변경해 보십시오. 루프가 변경 사항에 응답하는 것을 볼 수 있습니다.

### foreach 루프
우리가 살펴볼 마지막 루프는 foreach 루프입니다. 배열 또는 기타 기본 제공 목록 유형과 같은 항목 모음에서 작동합니다. 이 예에서는 ArrayList라는 간단한 목록 중 하나를 사용합니다. 배열과 매우 유사하게 작동하지만 걱정하지 마십시오.
...
자, ArrayList의 인스턴스를 만든 다음 여기에 몇 가지 문자열 항목을 추가합니다. foreach 루프를 사용하여 각 항목을 실행하고 name 변수를 매번 도달한 항목으로 설정합니다. 이렇게 하면 출력할 명명된 변수가 있습니다. 보시다시피, 우리는 name 변수를 문자열 유형으로 선언합니다 – 컬렉션에서 가져올 데이터 유형을 항상 foreach 루프에 알려야 합니다. 다양한 유형의 목록이있는 경우 특정 클래스 대신 객체 클래스를 사용하여 각 항목을 객체로 가져올 수 있습니다.


컬렉션으로 작업할 때 대부분의 경우 foreach 루프를 사용할 가능성이 매우 높은데, 이는 주로 이러한 종류의 작업에 대해 다른 루프보다 간단하기 때문입니다.
