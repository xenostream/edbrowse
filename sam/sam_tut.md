...
`x` 명령은 "pattern" 이 검색된다면, 현재텍스트로 설정하고 "command" 명령을 실행합니다. 
...
이 예제는 **파일의 모든 "Emacs" 항목을 찾아 현재텍스트로 설정한 후, "vi" 로 대체하는 `c/vi/` 명령을 실행합니다.** 라 읽을 수 있습니다. 
...
`x` 명령은 슬래시 문자안의 검색 텍스트를 현재텍스트로 설정하기 때문에, 현재텍스트를 `p` 명령으로 출력한다는 것은 유용하지 않을 수 있습니다. 
...
현재텍스트로 설정해 별도의 주소 지정없이 곧바로 처리할 가능성이 더 큽니다.
...
**조건부 명령은 현재텍스트를 변경하지 않고, 현재텍스트내에 검색 패턴이 포함될 경우만 해당 명령을 실행합니다.** 

또 다른 조건부 `v` 명령은 현재텍스트에 검색 패턴이 **포함되지 않은 경우** 만 해당 명령을 실행합니다. 
