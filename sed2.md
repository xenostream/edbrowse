...
본 문서는 sed를 일반적인 편집기 용도가 아니라, 특정한 작업을 수행하는 `도구` 의 관점에서 자세히 살펴보겠습니다. 결과적으로 모든 명령을 다루는 것은 아니며, ex와 크게 다른 명령 또는 유용하게 활용할 수 있는 특정 명령에 대해서만 다룰 것입니다.
...
첫 번째 형식인 `-e` 옵션을 사용하면 명령행에서 직접 편집 명령을 지정할 수 있습니다. 또한, 동일한 명령행에 여러 개의 `-e` 옵션을 함께 지정할 수 있습니다. 
...
여러 줄을 주소 지정할 때는 `g` 같은 전역 명령을 사용해 처리합니다. 반면, sed는 기본적으로 `모든 줄에 작동` 하므로, 오히려 선택된 줄을 건너뛰게 처리하는 
명령이 필요합니다. 
...
예를 들어, ex와 sed가 다음과 같은 형태의 명령을 해석하는 방식의 차이점을 고려해 보겠습니다:
...
하이픈 문자를 "em dash" 문자로 자동 변환해 출력합니다. 마지막으로 `eval` 명령을 사용해 최종 명령을 생성하고, 파일을 형식화해 출력합니다.
...
같아보일 수도 있겠지만, sed가 이 명령을 읽는 방식에 맞춰 해석하는 것이 또 다른 경우에는 혼란스러울 수 있습니다).
...
서기관이 해석해야 할 다양한 유형의 명령을 먼저 살펴보겠습니다. 
...
둘째로, 이 프로그램은 sed의 “흐름” 에 맞지 않습니다. b (branch) 명령을 사용하여 sed의 기본 동작을 활용하도록 재작성할 수 있습니다. 
...
검색 문자열이 표준 입력에서 새로 읽은 줄에서 발견된다면 그 줄이 출력됩니다. b 명령을 사용하여 목록의 맨 아래 부분으로 강제로 내려가며, sed는 줄을 출력하고 패턴 공간을 지웁니다. 만약 단일 줄에서 패턴을 포함하지 않는다면, 다음 입력 줄이 패턴 공간에 추가됩니다. 이 줄이 검색 문자열과 매칭될 수도 있습니다. 패턴 공간을 보관 공간에 복사한 후, 이전 줄의 줄 바꿈까지 제거하여 테스트합니다. 
...
( *앞서 설명된 내용은 sed 의 모든 명령을 다루지 않았습니다. sed 명령에 대한 전체 명령 목록은 부록 A에서 확인하실 수 있습니다.* )
