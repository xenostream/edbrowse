...
첫 번째 형식인 `-e` 옵션을 사용하면 명령행에서 직접 편집 명령어를 지정할 수 있습니다. 또한, 동일한 명령행에 여러 개의 `-e` 옵션을 함께 지정할 수 있습니다. 두 번째 형식인 `-f` 옵션을 사용하면 편집 명령어가 포함된 스크립트 파일의 이름을 지정합니다. 
본 문서에서는 이 형식으로 sed 편집기를 사용하는 방식을 선호합니다.
...
이 정규표현식 `s/thier/their own/g` 는 `-f` 옵션을 사용한 sed 스크립트에선 제대로 작동하지만, 명령행에서 사용할 경우 "Command garbled" 오류 메시지가 표시됩니다. 
이것은 시스템 쉘이 공백 문자를 인자의 구분자로 해석해 정규표현식을 `s/thier/their` 로 구문 분석해서 나머지를 두 개의 별도의 파일명으로 간주하기 때문입니다. 또한, `s` 명령에 대한 종료 `/` 문자가 없기 때문에 sed는 오류 메세지를 표시하고 종료합니다.
