...
첫 번째 형식인 `-e` 옵션을 사용하면 명령행에서 직접 편집 명령어를 지정할 수 있습니다. 또한, 동일한 명령행에 여러 개의 `-e` 옵션을 함께 지정할 수 있습니다. 
두 번째 형식인 `-f` 옵션을 사용하면 편집 명령어가 포함된 스크립트 파일의 이름을 지정합니다. 본 문서에서는 이 형식으로 sed 편집기를 사용하는 방식을 선호합니다. 
이 마지막 형식은 특히 셸 스크립트에 유용하며 곧 보게 될 것입니다.

또한, 전체 멀티라인으로 구성된 편집 스크립트를 sed에 인자로 제공할 수 있습니다:
...
또는, 다음과 같이 여러 명령어를 동일한 줄에 함께 결합하여 세미콜론 문자로 구분할 수 있습니다:
